...
int yvex_program_device_descriptor(const yvex_program_physical *p, size_t value, unsigned long long rows,
    yvex_backend_tensor_desc *desc, yvex_error *err)
{
    yvex_program_value_layout layout;
    if (!desc) return program_device_refuse(err, YVEX_ERR_INVALID_ARG, "descriptor destination required");
    memset(desc, 0, sizeof(*desc));
    int rc = yvex_program_physical_value_layout(p, value, rows, &layout, err);
    if (rc != YVEX_OK) return rc;
    if (layout.storage_scalar != YVEX_IR_F32 || layout.rank > YVEX_TENSOR_MAX_DIMS)
        return program_device_refuse(err, YVEX_ERR_UNSUPPORTED, "value is not a backend activation tensor");
    *desc = (yvex_backend_tensor_desc){.name = "program-value", .dtype = YVEX_DTYPE_F32,
        .rank = layout.rank, .bytes = layout.bytes};
    memcpy(desc->dims, layout.dims, layout.rank * sizeof(*desc->dims));
    return YVEX_OK;
}
...
static int program_index_storage_open(yvex_program_device *c, unsigned long long limit, yvex_error *err)
{
    for (size_t i = 0u; i < c->summary->value_count; ++i) {
        const yvex_program_physical_value *v = yvex_program_physical_value_at(c->program, i);
        unsigned long long bytes, total;
        yvex_program_value_layout layout;
        if (v->storage == YVEX_IR_NONE || v->type.scalar != YVEX_IR_INDEX || c->index_storage[v->storage]) continue;
        int rc = yvex_program_physical_value_layout(c->program, i, c->capacity, &layout, err);
        if (rc != YVEX_OK) return rc;
        bytes = layout.bytes;
        if (bytes > SIZE_MAX ||
            !yvex_core_u64_add(c->host_bytes, bytes, &total) || (limit && total > limit))
            return program_device_refuse(err, YVEX_ERR_BOUNDS, "compiled index slots exceed host budget");
        c->index_storage[v->storage] = malloc((size_t)bytes);
        if (!c->index_storage[v->storage])
            return program_device_refuse(err, YVEX_ERR_NOMEM, "compiled index slot allocation failed");
        c->host_bytes = total;
    }
    return YVEX_OK;
}
    ...
    const yvex_program_device_kernel *kernels, size_t count, void *context, yvex_error *err)
{
    ...
    c->program = yvex_program_physical_retain(p, err);
                                ...
                                unsigned long long rows)
{
    ...
    yvex_backend_tensor_desc desc;
    unsigned int axis;
    if (yvex_program_device_descriptor(c->program, slot, rows, &desc, NULL) != YVEX_OK ||
        !tensor || tensor->dtype != YVEX_DTYPE_F32 ||
    ...
    const yvex_program_device_argument *args, yvex_device_tensor *const *outputs, size_t count, yvex_error *err)
{
    ...
    for (i = 0u; i < c->summary->input_count; ++i) {
        ...
        if (t->kind == YVEX_IR_STATE) {
            ...
            int rc = yvex_program_physical_value_layout(c->program, i, rows, &layout, err);
            if (rc != YVEX_OK) return rc;
            c->indices[i] = (yvex_program_index_value){.values = args[i].indices,
                .count = layout.elements, .is_written = 1};
    ...
    for (i = 0u; i < c->summary->result_count; ++i) {
        ...
        yvex_device_tensor input_view = c->values[slot];
        ...
        /* A returned input remains an input throughout computation. Binding
         * its destination must not replace the borrowed, already written view. */
        if (slot < c->summary->input_count) c->values[slot] = input_view;
    ...
    for (i = 0u; i < c->summary->value_count; ++i) {
        const yvex_program_physical_value *v = yvex_program_physical_value_at(c->program, i);
        if (v->storage != YVEX_IR_NONE && v->type.scalar == YVEX_IR_INDEX) {
            yvex_program_value_layout layout;
            int rc = yvex_program_physical_value_layout(c->program, i, rows, &layout, err);
            if (rc != YVEX_OK) return rc;
            c->indices[i] = (yvex_program_index_value){.values = c->index_storage[v->storage],
                .output = c->index_storage[v->storage],
                .count = layout.elements};
            if (!c->indices[i].output)
                return program_device_refuse(err, YVEX_ERR_STATE, "compiled index slot is unavailable");
            continue;
        }
    ...
    int (*cancel)(void *), void *cancel_context, yvex_program_device_result *result, yvex_error *err)
{
    ...
    for (i = 0u; rc == YVEX_OK && i < c->summary->step_count; ++i) {
        ...
        for (j = 0u; j < s->operand_count; ++j) {
            ...
            if (v->type.kind == YVEX_IR_TENSOR && v->type.scalar == YVEX_IR_INDEX &&
                !c->indices[s->operands[j]].is_written)
                rc = program_device_refuse(err, YVEX_ERR_STATE, "index operand has not been produced");
        }
        ...
        for (j = 0u; j < s->result_count; ++j) {
            ...
            if (t->kind == YVEX_IR_TENSOR && !(t->scalar == YVEX_IR_INDEX ?
                c->indices[s->results[j]].is_written : c->values[s->results[j]].is_written))
    ...
    for (i = 0u; rc == YVEX_OK && i < c->summary->result_count; ++i) {
        yvex_ir_id slot = yvex_program_physical_result_at(c->program, i);
        if (yvex_program_physical_value_at(c->program, slot)->type.kind == YVEX_IR_STATE) continue;
        if (slot < c->summary->input_count) {
            rc = yvex_backend_tensor_copy(c->backend, outputs[output], &c->values[slot], err);
            if (rc == YVEX_OK && yvex_backend_kind_of(c->backend) != YVEX_BACKEND_KIND_CPU &&
                !yvex_core_u64_add(result->backend.d2d_bytes, c->values[slot].bytes, &result->backend.d2d_bytes))
                rc = program_device_refuse(err, YVEX_ERR_BOUNDS, "returned input transfer accounting overflowed");
        }
        outputs[output++]->is_written = c->values[slot].is_written;
    }
    if (rc == YVEX_OK && cancel && cancel(cancel_context))
        rc = program_device_refuse(err, YVEX_ERR_CANCELLED, "returned inputs cancelled before publication");
    if (rc != YVEX_OK) for (i = 0u; i < output_count; ++i) outputs[i]->is_written = 0;
    return rc;
}
