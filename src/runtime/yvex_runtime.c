...
static int runtime_backend_name_valid(const char *name)
{
    return name && (strcmp(name, "cpu") == 0 || strcmp(name, "cuda") == 0);
}

static int runtime_backend_is_cuda(const char *name)
{
...
{
    fprintf(stdout, "backend: cuda\n");
    fprintf(stdout, "backend_status: unsupported\n");
...
void print_quoted_bytes(const char *data, unsigned long long len)
{
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (ch == '"' || ch == '\\') {
            ...
            fprintf(stdout, "\\x%02x", (unsigned int)ch);
        } else {
            fputc((int)ch, stdout);
        }
    }
                                      ...
                                      int has_selected)
{
    ...
    fprintf(stdout, "token_input_kind: %s\n",
           input ? yvex_token_input_kind_name(input->kind) : "unknown");
    fprintf(stdout, "token_count: %llu\n", input ? input->token_count : 0ull);
    if (input) {
        fprintf(stdout, "selected_token_index: %llu\n", selected_index);
    }
    if (has_selected) {
        fprintf(stdout, "selected_token_id: %u\n", selected_token);
    } else if (input) {
        fprintf(stdout, "selected_token_id: unavailable\n");
    }
    fprintf(stdout, "token_bounds_status: %s\n", bounds_status ? bounds_status : "not-checked");
}
...
static int command_input(int arg_count, char **args)
{
    ...
    if (strcmp(subcommand, "tokens") == 0) {
        ...
        fprintf(stdout, "token_input: tokens\n");
        fprintf(stdout, "model: %s\n", model_arg);
        fprintf(stdout, "resolved_path: %s\n", ref.path ? ref.path : "");
    ...
    if (rc != YVEX_OK) {
        fprintf(stdout, "token_input: prompt\n");
        fprintf(stdout, "model: %s\n", model_arg);
        fprintf(stdout, "resolved_path: %s\n", ref.path ? ref.path : "");
        fprintf(stdout, "model_input_kind: %s\n",
               ref.kind == YVEX_MODEL_REF_ALIAS ? "alias" : "path");
        fprintf(stdout, "token_input_status: fail\n");
        fprintf(stdout, "token_input_kind: prompt-text\n");
        fprintf(stdout, "tokenizer_status: not-checked\n");
        fprintf(stdout, "reason: %s\n", yvex_error_message(&err));
        fprintf(stdout, "token_bounds_status: not-checked\n");
        fprintf(stdout, "prefill_ready: false\n");
        fprintf(stdout, "logits_ready: false\n");
        fprintf(stdout, "generation: unsupported\n");
        fprintf(stdout, "status: token-input-fail\n");
    ...
    if (rc != YVEX_OK) {
        ...
        fprintf(stdout, "token_input_status: fail\n");
        fprintf(stdout, "token_input_kind: prompt-text\n");
        fprintf(stdout, "tokenizer_status: missing\n");
        fprintf(stdout, "reason: tokenizer-metadata-missing\n");
        fprintf(stdout, "token_bounds_status: not-checked\n");
        fprintf(stdout, "prefill_ready: false\n");
        fprintf(stdout, "logits_ready: false\n");
        fprintf(stdout, "generation: unsupported\n");
    ...
    if (yvex_tokenizer_support_of(ctx.tokenizer) != YVEX_TOKENIZER_SUPPORT_FIXTURE_ENCODE_DECODE) {
        ...
        fprintf(stdout, "token_input_status: fail\n");
        fprintf(stdout, "token_input_kind: prompt-text\n");
        fprintf(stdout, "tokenizer_status: unsupported\n");
        fprintf(stdout, "tokenizer_support: %s\n",
    ...
    fprintf(stdout, "model_input_kind: %s\n",
           ref.kind == YVEX_MODEL_REF_ALIAS ? "alias" : "path");
    fprintf(stdout, "tokenizer_status: present\n");
    fprintf(stdout, "tokenizer_support: %s\n",
           yvex_tokenizer_support_name(yvex_tokenizer_support_of(ctx.tokenizer)));
    print_token_input_summary(&input,
                              rc == YVEX_OK ? "pass" : "fail",
...
static int command_engine(int arg_count, char **args)
{
    ...
    if (backend_name && !runtime_backend_name_valid(backend_name)) {
    ...
    if (rc == YVEX_ERR_UNSUPPORTED && runtime_backend_is_cuda(backend_name)) {
    ...
    if (summary.weights_attached) {
        ...
        for (j = 0; j < count; ++j) {
            ...
            fprintf(stdout, "attached_weight_%llu: %s role=%s rank=%u dims=[",
                   j,
            ...
            for (d = 0; d < tensor->rank; ++d) {
                ...
                fprintf(stdout, "%llu", tensor->dims[d]);
            }
            fprintf(stdout, "] dtype=%s bytes=%llu\n",
...
{
    fprintf(stdout, "info: YVEX %s\n", yvex_version_string());
    fprintf(stdout, "runtime: bounded diagnostic generation\n");
    fprintf(stdout, "cli_output: normal\n");
    fprintf(stdout, "full_model_generation: unsupported\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "hint: use --" "audit for full diagnostic fields\n");
}

static void print_info_audit(void)
{
    fprintf(stdout, "name: YVEX\n");
    fprintf(stdout, "version: %s\n", yvex_version_string());
    fprintf(stdout, "language: C\n");
    fprintf(stdout, "interface: CLI-only\n");
    fprintf(stdout, "status: selected tensor materialization, engine weight attachment, fixture graph execution, real selected graph segments, standalone RoPE, attention, matmul, and MLP ops, explicit token input boundary, prefill state foundation, minimal KV binding, minimal KV ownership, bounded decode/logits/sampling diagnostics, and bounded diagnostic generation loop with explicit append accounting\n");
    fprintf(stdout, "library: libyvex.a\n");
    fprintf(stdout, "filesystem: implemented\n");
    fprintf(stdout, "artifact: open/read implemented\n");
    fprintf(stdout, "gguf: metadata/tensor directory parsing implemented\n");
    fprintf(stdout, "model: descriptor-only implemented\n");
    fprintf(stdout, "tokenizer: fixture encode/decode implemented\n");
    fprintf(stdout, "token_input: explicit token boundary implemented\n");
    fprintf(stdout, "prefill_state: segment-summary foundation, bounded layer-backed prefill state, chunked prefill lifecycle, and minimal KV binding implemented\n");
    fprintf(stdout, "prompt: default renderer implemented\n");
    fprintf(stdout, "graph: partial planning, deterministic fixture execution, selected embedding partial execution, selected embedding RMSNorm segment execution, standalone RoPE position op, standalone F32 attention primitive, standalone F32 matmul/projection primitive, and standalone F32 MLP/feed-forward primitive implemented\n");
    fprintf(stdout, "planner: estimate-only implemented\n");
    fprintf(stdout, "backend: CPU reference implemented\n");
    fprintf(stdout, "backend_cuda: tensor movement plus F32/F16 embed, RMSNorm, RoPE position op, F32 attention primitive, F32 matmul/projection primitive, and F32 MLP/feed-forward primitive implemented when CUDA is available\n");
    fprintf(stdout, "weights: selected tensor materialization implemented\n");
    fprintf(stdout, "engine: descriptor open and selected-weight attachment implemented\n");
    fprintf(stdout, "session: lifecycle diagnostics, engine attachment observer, and KV ownership implemented\n");
    fprintf(stdout, "run: accepted-only runtime shell implemented\n");
    fprintf(stdout, "chat: accepted-only REPL shell implemented\n");
    fprintf(stdout, "metrics: runtime collector implemented\n");
    fprintf(stdout, "trace: JSONL writer implemented\n");
    fprintf(stdout, "profile: JSON writer implemented\n");
    fprintf(stdout, "run_artifacts: metrics/trace/profile files implemented\n");
    fprintf(stdout, "source_manifest: provenance JSON writer implemented\n");
    fprintf(stdout, "native_weights: safetensors header inventory implemented\n");
    fprintf(stdout, "gguf_template: contract validator implemented\n");
    fprintf(stdout, "gguf_emit: controlled GGUF writer implemented\n");
    fprintf(stdout, "conversion: open-weight selected tensor bridge implemented\n");
    fprintf(stdout, "model_ref: alias-or-path resolver implemented\n");
    fprintf(stdout, "model_registry: local model alias registry implemented\n");
    fprintf(stdout, "quant_job: external quantization job manifest implemented\n");
    fprintf(stdout, "qtype_support: conversion support matrix implemented\n");
    fprintf(stdout, "weight_mapping: tensor adapter contract implemented\n");
    fprintf(stdout, "quant_policy: manifest validator implemented\n");
    fprintf(stdout, "imatrix: calibration artifact manifest implemented\n");
    fprintf(stdout, "server_binary: yvexd shell implemented\n");
    fprintf(stdout, "server_endpoints: health/metrics/models status implemented\n");
    fprintf(stdout, "server_generation: not implemented\n");
    fprintf(stdout, "kv: minimal session-owned append/read boundary implemented\n");
    fprintf(stdout, "decode: bounded diagnostic state step implemented\n");
    fprintf(stdout, "logits: bounded diagnostic buffer implemented\n");
    fprintf(stdout, "sampling: bounded greedy sampler implemented\n");
    fprintf(stdout, "generation_loop: bounded diagnostic loop implemented\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "inference: not implemented\n");
    fprintf(stdout, "cuda: available when local driver/device probe succeeds\n");
    fprintf(stdout, "server: yvexd status shell implemented\n");
}
...
static int command_plan(int arg_count, char **args)
{
    ...
    rc = open_model_context(args[2], &ctx, &err);
    if (rc != YVEX_OK) {
        return print_yvex_error(&err, exit_for_status(rc));
    }
...
static int command_run(int arg_count, char **args)
{
    ...
    if (rc == YVEX_ERR_UNSUPPORTED && runtime_backend_is_cuda(backend_name)) {
        fprintf(stdout, "run status: backend-unsupported\n");
        fprintf(stdout, "backend: cuda\n");
        fprintf(stdout, "execution_ready: false\n");
        fprintf(stdout, "reason: %s\n", yvex_error_message(&err));
...
static int command_session(int arg_count, char **args)
{
    ...
    rc = runtime_backend_kind_from_name(backend_name, &backend_options.kind, &err);
    if (rc != YVEX_OK) {
        yvex_engine_close(engine);
        yvex_model_ref_clear(&model_ref);
        return print_yvex_error(&err, exit_for_status(rc));
                             ...
                             yvex_error *err)
{
    ...
    case YVEX_SLASH_RESET:
        ...
        fprintf(stdout, "session reset\n");
        fprintf(stdout, "position: 0\n");
        return YVEX_OK;
    ...
    case YVEX_SLASH_UNKNOWN:
        fprintf(stdout, "unknown slash command: %s\n", line ? line : "");
        fprintf(stdout, "type /help\n");
        return YVEX_OK;
...
static int command_chat(int arg_count, char **args)
{
    ...
    fprintf(stdout, "YVEX chat runtime\n");
    fprintf(stdout, "model: %s\n", engine_summary.model_name);
    fprintf(stdout, "backend: %s\n", backend_name);
    fprintf(stdout, "session_state: %s\n", yvex_session_state_name(session_summary.state));
    fprintf(stdout, "generation: unsupported in diagnostic runtime\n");
    ...
    while (!done) {
...

        {
            ...
            fprintf(stdout, "accepted tokens: %llu\n", result.prompt_tokens);
            fprintf(stdout, "position: %llu\n", result.position);
            fprintf(stdout, "assistant: [generation unsupported in diagnostic runtime]\n");
        }
    }
