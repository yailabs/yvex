...
static const char *const graph_status_names[] = {
    "empty", "built", "partial", "unsupported", "invalid"
};
static const char *const op_kind_names[] = {
    "embed", "rms_norm", "matmul", "rope", "attention_prefill",
    "attention_decode", "kv_write", "kv_read", "swiglu",
    "residual_add", "logits", "sampler", "unsupported"
};
static const char *const op_status_names[] = {
    "planned", "missing_input", "unsupported", "invalid_shape"
};
static const char *const value_kind_names[] = {
    "token_ids", "activation", "weight", "kv_cache", "logits",
    "temporary", "unknown"
};
static const char *const residency_names[] = {
    "host", "device", "backend_decides"
};
                       ...
                       unsigned long long selected_count)
{
    ...
    if (!yvex_attention_compute_round(
            context->layer->compute_contract, destination,
        ...
        return 0;
    return yvex_attention_compute_round(
        context->layer->compute_contract, destination,
        context->layer->head_dimension);
}
...
static int reduce_context_validate(attention_reduce_context *context)
{
    ...
    if (!yvex_core_u64_mul(
            context->layer->query_heads, context->layer->head_dimension,
            &query_width) ||
        context->current_kv_stride < context->layer->head_dimension ||
        !yvex_core_u64_add(
            context->token_position, context->token_count - 1ull,
            &last_position))
        return yvex_attention_reject(
            context->failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_DIMENSION,
            NULL, context->layer->layer_index, YVEX_TENSOR_ROLE_UNKNOWN,
            ULLONG_MAX, context->token_position, context->err,
            YVEX_ERR_BOUNDS,
    ...
    yvex_attention_failure *failure, yvex_error *err)
{
    attention_reduce_context context = {
        ...
        trace_topk_positions, trace_topk_stride, scratch, result, failure,
        err, 0.0
    };
    ...
    if (layer->attention_class == YVEX_ATTENTION_CLASS_CSA &&
        context.compressed_total) {
        unsigned long long selected_capacity = yvex_attention_min_u64(
            context.compressed_total, layer->sparse_topk.k);
    ...
    yvex_error *err)
{
    ...
    if (!session || !out_a || !out_b || !attention_values || !out ||
        !token_count || !group_count || !group_input_width || !rank ||
        !hidden_width || output_stride < hidden_width ||
    ...
    if (!yvex_core_u64_mul(group_count, group_input_width,
                           &attention_width) ||
        attention_stride < attention_width ||
        !yvex_core_u64_mul(group_count, rank, &low_stride) ||
    ...
    if (!yvex_attention_scratch_reserve(
            scratch, low_elements, sizeof(float), &low_bytes))
        return yvex_attention_reject(
            failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_SCRATCH, out_a,
            out_a->layer_index, YVEX_TENSOR_ROLE_ATTENTION_OUT_A,
            scratch ? scratch->limit_bytes : 0ull,
            scratch ? (unsigned long long)scratch->live_bytes : 0ull,
            err, YVEX_ERR_BOUNDS,
    ...
    if (!yvex_attention_compute_round(compute_contract, low, low_elements)) {
        rc = yvex_attention_reject(
            failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_NUMERIC, out_a,
            out_a->layer_index, YVEX_TENSOR_ROLE_ATTENTION_OUT_A,
    ...
    rc = yvex_attention_dot_batch(
        session, out_b, 0ull, low, token_count, low_stride,
        low_stride, hidden_width, out, output_stride, &rows,
        scratch, result, failure, err);
    ...
    if (rc == YVEX_OK) {
        ...
        for (token = 0ull; token < token_count; ++token)
            if (!yvex_attention_compute_round(
                    compute_contract, out + token * output_stride,
                    hidden_width)) {
                rc = yvex_attention_reject(
                    failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_NUMERIC,
                    out_b, out_b->layer_index,
                    YVEX_TENSOR_ROLE_ATTENTION_OUT_B, hidden_width,
    ...
    yvex_error *err)
{
    ...
    for (row = 0ull; row < binding->row_count; ++row) {
        rc = yvex_attention_decode_row(session, runtime_binding, row,
                                  ...
                                  binding->row_width, scratch, result, failure,
                                  err);
        if (rc != YVEX_OK) return rc;
    }
    return YVEX_OK;
    ...
    yvex_error *err)
{
    ...
    if (policy->pre_transform ==
        ...
        if (!yvex_attention_scratch_reserve(
                budget, count, sizeof(*scratch), &scratch_bytes))
            return yvex_attention_reject(
                failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_SCRATCH, NULL,
                layer_index, role,
        ...
        rc = yvex_attention_hadamard_cpu(
            values, count, scale, 1, scratch, budget, failure, err);
        if (rc != YVEX_OK) goto cleanup;
    ...
    if (!yvex_attention_scratch_reserve(
            budget, block_width, sizeof(*block_out), &block_out_bytes)) {
        rc = yvex_attention_reject(
            failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_SCRATCH, NULL,
            layer_index, role,
            budget ? budget->limit_bytes : 0ull,
    ...
    if (!yvex_attention_scratch_reserve(
            budget, block_width, sizeof(*codes), &codes_bytes)) {
        rc = yvex_attention_reject(
            failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_SCRATCH, NULL,
            layer_index, role,
            budget ? budget->limit_bytes : 0ull,
...
int yvex_attention_scratch_reserve(
    yvex_attention_scratch_budget *budget,
    unsigned long long count,
    size_t element_size,
    size_t *bytes_out)
{
    ...
    size_t next;
    if (!budget || !bytes_out ||
        !yvex_attention_checked_size(count, element_size, &bytes) ||
        budget->live_bytes > SIZE_MAX - bytes)
        return 0;
    next = budget->live_bytes + bytes;
    if (budget->limit_bytes &&
        (unsigned long long)next > budget->limit_bytes)
        return 0;
...
void yvex_attention_scratch_release(
    yvex_attention_scratch_budget *budget,
    size_t bytes)
{
    if (budget && bytes <= budget->live_bytes) budget->live_bytes -= bytes;
}
...
// Failure: returns a typed refusal or neutral result without partial capability publication.
// Boundary: remains graph-local and cannot promote attention, KV, or generation support.
int yvex_attention_row_geometry(
    const yvex_materialized_tensor_binding *binding,
...
int yvex_attention_payload_account(
    yvex_attention_cpu_result *result,
    ...
    yvex_error *err)
{
    ...
    if (!yvex_attention_scratch_reserve(
            scratch, row_bytes, 1u, &encoded_scratch))
        return yvex_attention_reject(
    ...
    rc = yvex_attention_payload_account(
        result, row_bytes, runtime_binding, failure, err);
    if (rc != YVEX_OK) goto cleanup;
    ...
    yvex_error *err)
{
    ...
    if (output_stride < rows)
        return yvex_attention_reject(
            failure, YVEX_DEEPSEEK_ATTENTION_FAILURE_DIMENSION,
            runtime_binding, binding->layer_index, binding->role,
            rows, output_stride, err, YVEX_ERR_BOUNDS,
    ...
    if (!yvex_attention_scratch_reserve(
        ...
        return yvex_attention_reject(
            ...
            scratch ? (unsigned long long)scratch->live_bytes : 0ull,
            err, YVEX_ERR_BOUNDS,
            "attention dot row scratch budget exceeded");
    }
    encoded = (unsigned char *)malloc((size_t)row_bytes);
    if (!encoded) {
        rc = yvex_attention_reject(
...
{
    return enum_name(graph_status_names,
                     sizeof(graph_status_names) / sizeof(graph_status_names[0]),
                     (int)status);
}
...
const yvex_graph_value_info *yvex_graph_value_at(const yvex_graph *graph,
                                                 unsigned long long index)
{
    if (!graph || index >= graph->value_count) return NULL;
...
const yvex_graph_op_info *yvex_graph_op_at(const yvex_graph *graph,
                                           unsigned long long index)
{
    if (!graph || index >= graph->op_count) return NULL;
...
const yvex_graph_missing_required *yvex_graph_missing_required_at(const yvex_graph *graph,
                                                                  unsigned long long index)
{
    if (!graph || index >= graph->missing_count) return NULL;
...
{
    return enum_name(op_kind_names,
                     sizeof(op_kind_names) / sizeof(op_kind_names[0]),
                     (int)kind);
}
...
{
    return enum_name(op_status_names,
                     sizeof(op_status_names) / sizeof(op_status_names[0]),
                     (int)status);
}
...
{
    return enum_name(value_kind_names,
                     sizeof(value_kind_names) / sizeof(value_kind_names[0]),
                     (int)kind);
}
...
{
    return enum_name(residency_names,
                     sizeof(residency_names) / sizeof(residency_names[0]),
                     (int)residency);
}
