...
void print_graph_guard_report(const yvex_cli_graph_guard_report *report)
{
    ...
    fprintf(stdout, "output_bytes_planned: %llu\n", report->output_bytes_planned);
    fprintf(stdout, "output_bytes_allocated: %llu\n", report->output_bytes_allocated);
    fprintf(stdout, "reference_bytes_planned: %llu\n", report->reference_bytes_planned);
}
                                   ...
                                   unsigned long long count)
{
    ...
    for (i = 0; i < count; ++i) {
        fprintf(stdout, "%s%.9g", i == 0 ? " " : ",", (double)values[i]);
    }
    fprintf(stdout, "\n");
}
...
{
    fprintf(stdout, "attention_ready: false\n");
    fprintf(stdout, "transformer_block_ready: false\n");
...
{
    fprintf(stdout, "attention_primitive_executed: %s\n", primitive_executed ? "true" : "false");
    fprintf(stdout, "qkv_projection_ready: false\n");
    fprintf(stdout, "transformer_block_ready: false\n");
    fprintf(stdout, "full_prefill_ready: false\n");
    fprintf(stdout, "full_transformer_prefill_ready: false\n");
    print_no_generation_readiness_fields();
}
...
{
    fprintf(stdout, "matmul_primitive_executed: %s\n", primitive_executed ? "true" : "false");
    fprintf(stdout, "qkv_projection_ready: false\n");
    fprintf(stdout, "transformer_block_ready: false\n");
    fprintf(stdout, "full_prefill_ready: false\n");
    fprintf(stdout, "full_transformer_prefill_ready: false\n");
...
{
    fprintf(stdout, "mlp_primitive_executed: %s\n", primitive_executed ? "true" : "false");
    fprintf(stdout, "router_logits_ready: false\n");
    fprintf(stdout, "top_k_routing_ready: false\n");
    fprintf(stdout, "transformer_block_ready: false\n");
    fprintf(stdout, "full_prefill_ready: false\n");
    fprintf(stdout, "full_transformer_prefill_ready: false\n");
    print_no_generation_readiness_fields();
...
static void controlled_block_result_print(const yvex_controlled_block_result *result)
{
    ...
    fprintf(stdout, "graph_kind: %s\n", result->graph_kind);
    fprintf(stdout, "block: fixture\n");
    fprintf(stdout, "backend: %s\n", result->backend_name);
    fprintf(stdout, "backend_status: %s\n", result->backend_status);
    fprintf(stdout, "backend_op_status: %s\n", result->backend_op_status);
    fprintf(stdout, "seq_len: %llu\n", result->seq_len);
    fprintf(stdout, "position: %llu\n", result->position);
    fprintf(stdout, "hidden_dim: %llu\n", result->hidden_dim);
    fprintf(stdout, "head_dim: %llu\n", result->head_dim);
    fprintf(stdout, "ffn_dim: %llu\n", result->ffn_dim);
    fprintf(stdout, "causal: %s\n", result->causal ? "true" : "false");
    fprintf(stdout, "gated: %s\n", result->gated ? "true" : "false");
    fprintf(stdout, "op_count: %llu\n", result->op_count);
    fprintf(stdout, "phase: input\n");
    fprintf(stdout, "phase: attn_norm\n");
    fprintf(stdout, "phase: q_projection\n");
    fprintf(stdout, "phase: k_projection\n");
    fprintf(stdout, "phase: v_projection\n");
    fprintf(stdout, "phase: rope\n");
    fprintf(stdout, "phase: attention\n");
    fprintf(stdout, "phase: o_projection\n");
    fprintf(stdout, "phase: residual_attn\n");
    fprintf(stdout, "phase: post_attn_norm\n");
    fprintf(stdout, "phase: mlp\n");
    fprintf(stdout, "phase: residual_mlp\n");
    fprintf(stdout, "phase: output\n");
    fprintf(stdout, "input_bytes: %llu\n", result->input_bytes);
    fprintf(stdout, "qkv_weight_bytes_each: %llu\n", result->qkv_weight_bytes_each);
    fprintf(stdout, "mlp_weight_bytes: %llu\n", result->mlp_weight_bytes);
    fprintf(stdout, "score_scratch_bytes: %llu\n", result->score_scratch_bytes);
    fprintf(stdout, "mlp_intermediate_bytes: %llu\n", result->mlp_intermediate_bytes);
    fprintf(stdout, "scratch_planned_bytes: %llu\n", result->scratch_planned_bytes);
    fprintf(stdout, "backend_allocated_bytes: %llu\n", result->backend_allocated_bytes);
    fprintf(stdout, "output_bytes: %llu\n", result->output_bytes);
    fprintf(stdout, "checksum: %llu\n", result->output_checksum);
    fprintf(stdout, "output_checksum: %llu\n", result->output_checksum);
    fprintf(stdout, "reference_checksum: %llu\n", result->reference_checksum);
    fprintf(stdout, "max_abs_diff: %.9g\n", (double)result->max_abs_diff);
    fprintf(stdout, "phase: cleanup\n");
    ...
    if (strcmp(result->backend_name, "cuda") == 0) {
        fprintf(stdout, "block_cuda_parity: %s\n", result->block_cuda_parity ? "pass" : "fail");
    }
    fprintf(stdout, "sample_count: %llu\n", result->sample_count);
    fprintf(stdout, "output_sample_values:");
    for (i = 0; i < result->sample_count; ++i) {
        fprintf(stdout, "%s%.9g", i == 0 ? " " : ",", (double)result->output_sample_values[i]);
    }
    fprintf(stdout, "\n");
    fprintf(stdout, "reference_sample_values:");
    for (i = 0; i < result->sample_count; ++i) {
        fprintf(stdout, "%s%.9g", i == 0 ? " " : ",", (double)result->reference_sample_values[i]);
    }
    fprintf(stdout, "\n");
    fprintf(stdout, "execution_ready: false\n");
    fprintf(stdout, "graph_execution_ready: false\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "generation: unsupported\n");
}
                                               ...
                                               int gated)
{
    ...
    if (position >= seq_len) {
        fprintf(stderr, "yvex: block fixture position must be less than seq-len\n");
        return 2;
    }
    if (hidden_dim % head_dim != 0ull) {
        fprintf(stderr, "yvex: block fixture hidden-dim must be divisible by head-dim\n");
        return 2;
    }
    if (hidden_dim != head_dim) {
        fprintf(stderr, "yvex: block fixture currently supports one attention head\n");
        return 5;
    }
    if (!gated) {
        fprintf(stderr, "yvex: block fixture requires gated MLP\n");
        return 2;
    }
    if (seq_len > ULLONG_MAX / hidden_dim ||
        ...
        fprintf(stderr, "yvex: block fixture dimension overflow\n");
        return 4;
    }
    ...
    const yvex_controlled_layer_scheduler_result *result)
{
    ...
    fprintf(stdout, "causal: %s\n", result->causal ? "true" : "false");
    fprintf(stdout, "gated: %s\n", result->gated ? "true" : "false");
    fprintf(stdout, "layer_handoff: selected-position-row\n");
    fprintf(stdout, "sequence_rebuild: deterministic-with-previous-position-row\n");
    fprintf(stdout, "op_count_per_layer: %llu\n", result->op_count_per_layer);
    fprintf(stdout, "total_op_count: %llu\n", result->total_op_count);
    fprintf(stdout, "input_bytes_per_layer: %llu\n", result->input_bytes_per_layer);
    fprintf(stdout, "scratch_planned_bytes_per_layer: %llu\n",
           result->scratch_planned_bytes_per_layer);
    fprintf(stdout, "backend_allocated_bytes_total: %llu\n",
           result->backend_allocated_bytes_total);
    fprintf(stdout, "output_bytes: %llu\n", result->output_bytes);
    for (layer = 0; layer < result->layers && layer < 16ull; ++layer) {
        fprintf(stdout, "layer_%llu_checksum: %llu\n", layer,
               result->layer_output_checksums[layer]);
        fprintf(stdout, "layer_%llu_reference_checksum: %llu\n", layer,
               result->layer_reference_checksums[layer]);
        fprintf(stdout, "layer_%llu_max_abs_diff: %.9g\n", layer,
               (double)result->layer_max_abs_diffs[layer]);
    }
    fprintf(stdout, "final_output_checksum: %llu\n", result->final_output_checksum);
    fprintf(stdout, "final_reference_checksum: %llu\n", result->final_reference_checksum);
    fprintf(stdout, "final_max_abs_diff: %.9g\n", (double)result->final_max_abs_diff);
    if (strcmp(result->backend_name, "cuda") == 0) {
        fprintf(stdout, "layers_cuda_parity: %s\n",
               result->layers_cuda_parity ? "pass" : "fail");
        fprintf(stdout, "cuda_reference_max_abs_diff: %.9g\n",
               (double)result->final_max_abs_diff);
    } else {
        fprintf(stdout, "cpu_reference_max_abs_diff: %.9g\n",
               (double)result->final_max_abs_diff);
    }
    fprintf(stdout, "phase: cleanup\n");
    fprintf(stdout, "cleanup_attempted: %s\n", result->cleanup_attempted ? "true" : "false");
...
{
    fprintf(stdout, "status: graph-check\n");
    fprintf(stdout, "backend: %s\n", options->backend_name);
    fprintf(stdout, "suite: %s\n", options->suite_name);
    fprintf(stdout, "layers: %llu\n", options->layers);
    fprintf(stdout, "graph_check_kind: fixture-proof-preset\n");
    fprintf(stdout, "graph_check_scope: primitives-block-layers\n");
    fprintf(stdout, "real_model_execution: false\n");
    fprintf(stdout, "selected_artifact_execution: false\n");
    fprintf(stdout, "prefill_execution: false\n");
}

static void graph_check_print_summary(const yvex_graph_check_result *result)
{
    fprintf(stdout, "stage: rope %s\n", result->rope);
    fprintf(stdout, "stage: attention %s\n", result->attention);
    fprintf(stdout, "stage: matmul %s\n", result->matmul);
    fprintf(stdout, "stage: mlp %s\n", result->mlp);
    fprintf(stdout, "stage: block %s\n", result->block);
    fprintf(stdout, "stage: layers %s\n", result->layers);
    fprintf(stdout, "primitive_checks: %llu\n", result->primitive_checks);
    fprintf(stdout, "block_checks: %llu\n", result->block_checks);
    fprintf(stdout, "layer_checks: %llu\n", result->layer_checks);
    fprintf(stdout, "checks_passed: %llu\n", result->checks_passed);
    fprintf(stdout, "checks_failed: %llu\n", result->checks_failed);
    if (result->failed_stage) {
        fprintf(stdout, "failed_stage: %s\n", result->failed_stage);
        fprintf(stdout, "failed_exit_code: %d\n", result->failed_exit_code);
    }
    fprintf(stdout, "execution_ready: false\n");
    fprintf(stdout, "graph_execution_ready: false\n");
    fprintf(stdout, "prefill_ready: false\n");
...
static int command_graph_check(int arg_count, char **args)
{
    ...
    for (i = 3; i < arg_count; ++i) {
        if (strcmp(args[i], "--backend") == 0) {
            ...
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &options.layers)) {
                                         ...
                                         unsigned long long head_dim)
{
    ...
    if (head_dim == 0) {
        ...
        fprintf(stdout, "op: rope\n");
        fprintf(stdout, "backend: %s\n", backend_name);
        fprintf(stdout, "position: %llu\n", position);
        fprintf(stdout, "head_dim: %llu\n", head_dim);
        fprintf(stdout, "dtype: f32\n");
        print_rope_readiness_fields();
        fprintf(stdout, "reason: rope-head-dim-zero\n");
        fprintf(stdout, "status: graph-op-fail\n");
    ...
    if ((head_dim & 1ull) != 0ull) {
        ...
        fprintf(stdout, "dtype: f32\n");
        print_rope_readiness_fields();
        fprintf(stdout, "reason: rope-head-dim-odd\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_FORMAT);
    }
    if (cli_test_env_enabled("YVEX_TEST_ROPE_BYTE_OVERFLOW") ||
        ...
        fprintf(stdout, "dtype: f32\n");
        print_rope_readiness_fields();
        fprintf(stdout, "reason: byte-count-overflow\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_BOUNDS);
    }
    ...
    if (!yvex_backend_supports(backend, YVEX_BACKEND_CAP_OP_ROPE)) {
        ...
        fprintf(stdout, "dtype: f32\n");
        print_rope_readiness_fields();
        fprintf(stdout, "reason: backend-op-rope-unsupported\n");
        fprintf(stdout, "status: graph-op-fail\n");
        yvex_backend_close(backend);
        free(reference_values);
        free(output_values);
    ...
    if (rc != YVEX_OK) {
        ...
        fprintf(stdout, "dtype: f32\n");
        print_rope_readiness_fields();
        fprintf(stdout, "status: graph-op-failed-cleaned\n");
        yvex_backend_close(backend);
        free(reference_values);
        free(output_values);
        free(input_values);
    ...
    if (cli_test_env_enabled("YVEX_TEST_FAIL_ROPE_AFTER_ALLOC")) {
        ...
        fprintf(stdout, "input_bytes: %llu\n", bytes);
        fprintf(stdout, "output_bytes: %llu\n", bytes);
        fprintf(stdout, "reference_bytes: %llu\n", bytes);
        print_rope_readiness_fields();
        fprintf(stdout, "reason: injected-rope-after-alloc\n");
        fprintf(stdout, "status: graph-op-failed-cleaned\n");
        yvex_backend_tensor_free(backend, out);
        yvex_backend_tensor_free(backend, input);
        yvex_backend_close(backend);
    ...
    fprintf(stdout, "position: %llu\n", position);
    fprintf(stdout, "head_dim: %llu\n", head_dim);
    fprintf(stdout, "rope_base: %.9g\n", (double)rope_base);
    fprintf(stdout, "dtype: f32\n");
    fprintf(stdout, "input_bytes: %llu\n", bytes);
    fprintf(stdout, "output_bytes: %llu\n", bytes);
    fprintf(stdout, "scratch_bytes: 0\n");
    fprintf(stdout, "reference_bytes: %llu\n", bytes);
    fprintf(stdout, "input_checksum: %llu\n", cli_checksum_bytes(input_values, bytes));
    fprintf(stdout, "output_checksum: %llu\n", cli_checksum_bytes(output_values, bytes));
    fprintf(stdout, "reference_checksum: %llu\n", cli_checksum_bytes(reference_values, bytes));
    fprintf(stdout, "max_abs_diff: %.9g\n", (double)max_abs_diff);
    fprintf(stdout, "input_output_max_abs_diff: %.9g\n", (double)input_output_max_abs_diff);
    fprintf(stdout, "position_zero_identity: %s\n",
           position == 0ull && input_output_max_abs_diff <= 0.0000001f ? "true" : "false");
    fprintf(stdout, "position_dependent_output: %s\n",
           position != 0ull && input_output_max_abs_diff > 0.0001f ? "true" : "false");
    fprintf(stdout, "reference_attempted: true\n");
    if (strcmp(backend_name, "cuda") == 0) {
        fprintf(stdout, "rope_cuda_parity: %s\n", exit_code == 0 ? "pass" : "fail");
        fprintf(stdout, "cuda_reference_max_abs_diff: %.9g\n", (double)max_abs_diff);
    } else {
        fprintf(stdout, "cpu_reference_max_abs_diff: %.9g\n", (double)max_abs_diff);
    }
    fprintf(stdout, "output_sample_count: %llu\n", sample_count);
...
{
    fprintf(stdout, "op: attention\n");
    fprintf(stdout, "backend: %s\n", backend_name);
    fprintf(stdout, "dtype: f32\n");
    fprintf(stdout, "seq_len: %llu\n", seq_len);
    fprintf(stdout, "position: %llu\n", position);
    fprintf(stdout, "head_dim: %llu\n", head_dim);
    fprintf(stdout, "scale: %.9g\n", (double)scale);
    fprintf(stdout, "mask: %s\n", causal ? "causal" : "none");
    fprintf(stdout, "query_bytes: %llu\n", query_bytes);
    fprintf(stdout, "key_bytes: %llu\n", key_bytes);
    fprintf(stdout, "value_bytes: %llu\n", value_bytes);
    fprintf(stdout, "input_bytes: %llu\n", input_bytes);
    fprintf(stdout, "score_scratch_bytes: %llu\n", score_scratch_bytes);
    fprintf(stdout, "probability_scratch_bytes: %llu\n", probability_scratch_bytes);
    fprintf(stdout, "scratch_bytes: %llu\n", score_scratch_bytes + probability_scratch_bytes);
    fprintf(stdout, "output_bytes: %llu\n", output_bytes);
    fprintf(stdout, "reference_bytes: %llu\n", reference_bytes);
}
                                              ...
                                              int causal)
{
    ...
    if (position >= seq_len) {
        ...
        print_attention_operation_fields(backend_name, seq_len, position, head_dim, scale,
                                         causal, 0, 0, 0, 0, 0, 0, 0, 0);
        print_attention_readiness_fields(0);
        fprintf(stdout, "reason: position-out-of-range\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_BOUNDS);
    }
    ...
    if (rc != YVEX_OK) {
        ...
        print_attention_operation_fields(backend_name, seq_len, position, head_dim, scale,
                                         ...
                                         reference_bytes);
        print_attention_readiness_fields(0);
        fprintf(stdout, "visible_keys: %llu\n", visible_keys);
        fprintf(stdout, "masked_keys: %llu\n", masked_keys);
        fprintf(stdout, "reason: %s\n", reason);
        fprintf(stdout, "status: graph-op-fail\n");
        exit_code = exit_for_status(rc);
        goto cleanup_host;
    ...
    if (!yvex_backend_supports(backend, YVEX_BACKEND_CAP_OP_ATTENTION)) {
        ...
        fprintf(stdout, "visible_keys: %llu\n", visible_keys);
        fprintf(stdout, "masked_keys: %llu\n", masked_keys);
        fprintf(stdout, "reason: backend-op-attention-unsupported\n");
        fprintf(stdout, "status: graph-op-fail\n");
...
static void print_matmul_operation_fields(const char *backend_name,
                                          ...
                                          unsigned long long reference_bytes)
{
    fprintf(stdout, "op: matmul\n");
    fprintf(stdout, "backend: %s\n", backend_name);
    fprintf(stdout, "dtype: f32\n");
    fprintf(stdout, "m: %llu\n", m);
    fprintf(stdout, "k: %llu\n", k);
    fprintf(stdout, "n: %llu\n", n);
    fprintf(stdout, "projection_shape: %s\n", m == 1ull ? "true" : "false");
    fprintf(stdout, "non_projection_shape: %s\n", m == 1ull ? "false" : "true");
    fprintf(stdout, "input_bytes: %llu\n", input_bytes);
    fprintf(stdout, "weight_bytes: %llu\n", weight_bytes);
    fprintf(stdout, "output_bytes: %llu\n", output_bytes);
    fprintf(stdout, "scratch_bytes: 0\n");
    fprintf(stdout, "reference_bytes: %llu\n", reference_bytes);
}
                                           ...
                                           unsigned long long n)
{
    ...
    if (m == 0 || k == 0 || n == 0) {
        ...
        fprintf(stdout, "reason: matmul-zero-dimension\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_FORMAT);
    }
    ...
    if (!yvex_backend_supports(backend, YVEX_BACKEND_CAP_OP_MATMUL) ||
        ...
        print_matmul_operation_fields(backend_name, m, k, n,
                                      ...
                                      output_bytes, reference_bytes);
        print_matmul_readiness_fields(0);
        fprintf(stdout, "reason: backend-op-matmul-unsupported\n");
        fprintf(stdout, "status: graph-op-fail\n");
        exit_code = exit_for_status(YVEX_ERR_UNSUPPORTED);
        goto cleanup_backend;
    }
    ...
    print_matmul_operation_fields(backend_name, m, k, n,
                                  input_bytes, weight_bytes,
                                  output_bytes, reference_bytes);
    fprintf(stdout, "input_elements: %llu\n", input_elements);
    fprintf(stdout, "weight_elements: %llu\n", weight_elements);
    fprintf(stdout, "output_elements: %llu\n", output_elements);
    fprintf(stdout, "input_total_bytes: %llu\n", total_input_bytes);
    fprintf(stdout, "input_checksum: %llu\n", cli_checksum_bytes(input_values, input_bytes));
    fprintf(stdout, "weight_checksum: %llu\n", cli_checksum_bytes(weight_values, weight_bytes));
    fprintf(stdout, "output_checksum: %llu\n", cli_checksum_bytes(output_values, output_bytes));
    fprintf(stdout, "reference_checksum: %llu\n", cli_checksum_bytes(reference_values, reference_bytes));
    fprintf(stdout, "max_abs_diff: %.9g\n", (double)max_abs_diff);
    fprintf(stdout, "reference_attempted: true\n");
    if (strcmp(backend_name, "cuda") == 0) {
        fprintf(stdout, "matmul_cuda_parity: %s\n", exit_code == 0 ? "pass" : "fail");
        fprintf(stdout, "cuda_reference_max_abs_diff: %.9g\n", (double)max_abs_diff);
    } else {
        fprintf(stdout, "cpu_reference_max_abs_diff: %.9g\n", (double)max_abs_diff);
    }
...
static void print_mlp_operation_fields(const char *backend_name,
                                       ...
                                       unsigned long long reference_bytes)
{
    fprintf(stdout, "op: mlp\n");
    fprintf(stdout, "backend: %s\n", backend_name);
    fprintf(stdout, "dtype: f32\n");
    fprintf(stdout, "batch: %llu\n", batch);
    fprintf(stdout, "hidden_dim: %llu\n", hidden_dim);
    fprintf(stdout, "ffn_dim: %llu\n", ffn_dim);
    fprintf(stdout, "activation: %s\n", activation ? activation : "unknown");
    fprintf(stdout, "gated: %s\n", gated ? "true" : "false");
    fprintf(stdout, "routed_expert_mode: %s\n", routed ? "true" : "false");
    fprintf(stdout, "expert_count: %llu\n", expert_count);
    fprintf(stdout, "expert_id: %llu\n", expert_id);
    fprintf(stdout, "input_bytes: %llu\n", input_bytes);
    fprintf(stdout, "gate_weight_bytes: %llu\n", gate_bytes);
    fprintf(stdout, "up_weight_bytes: %llu\n", up_bytes);
    fprintf(stdout, "down_weight_bytes: %llu\n", down_bytes);
    fprintf(stdout, "intermediate_bytes: %llu\n", intermediate_bytes);
    fprintf(stdout, "output_bytes: %llu\n", output_bytes);
    fprintf(stdout, "reference_bytes: %llu\n", reference_bytes);
}
                                        ...
                                        int routed)
{
    ...
    if (hidden_dim == 0 || ffn_dim == 0) {
        ...
        fprintf(stdout, "reason: mlp-zero-dimension\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_FORMAT);
    }
    if (!gated || !activation || strcmp(activation, "silu") != 0) {
        ...
        print_mlp_operation_fields(backend_name, batch, hidden_dim, ffn_dim,
                                   ...
                                   expert_id, 0, 0, 0, 0, 0, 0, 0);
        print_mlp_readiness_fields(0);
        fprintf(stdout, "reason: unsupported-activation\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_UNSUPPORTED);
    }
    if (routed && (expert_count == 0 || expert_id >= expert_count)) {
        ...
        print_mlp_operation_fields(backend_name, batch, hidden_dim, ffn_dim,
                                   ...
                                   expert_id, 0, 0, 0, 0, 0, 0, 0);
        print_mlp_readiness_fields(0);
        fprintf(stdout, "reason: expert-id-out-of-range\n");
        fprintf(stdout, "status: graph-op-fail\n");
        return exit_for_status(YVEX_ERR_BOUNDS);
    }
    ...
    if (!yvex_backend_supports(backend, YVEX_BACKEND_CAP_OP_MLP) ||
        ...
        print_mlp_operation_fields(backend_name, batch, hidden_dim, ffn_dim,
                                   ...
                                   reference_bytes);
        print_mlp_readiness_fields(0);
        fprintf(stdout, "reason: backend-op-mlp-unsupported\n");
        fprintf(stdout, "status: graph-op-fail\n");
        exit_code = exit_for_status(YVEX_ERR_UNSUPPORTED);
        goto cleanup_backend;
    }
    ...
    print_mlp_operation_fields(backend_name, batch, hidden_dim, ffn_dim,
                               ...
                               reference_bytes);
    fprintf(stdout, "input_elements: %llu\n", input_elements);
    fprintf(stdout, "gate_weight_elements: %llu\n", gate_elements);
    fprintf(stdout, "up_weight_elements: %llu\n", up_elements);
    fprintf(stdout, "down_weight_elements: %llu\n", down_elements);
    fprintf(stdout, "intermediate_elements: %llu\n", intermediate_elements);
    fprintf(stdout, "output_elements: %llu\n", output_elements);
    fprintf(stdout, "weight_total_bytes: %llu\n", total_weight_bytes);
    fprintf(stdout, "input_total_bytes: %llu\n", total_input_bytes);
    fprintf(stdout, "input_checksum: %llu\n", cli_checksum_bytes(input_values, input_bytes));
    fprintf(stdout, "gate_weight_checksum: %llu\n", cli_checksum_bytes(gate_values, gate_bytes));
    fprintf(stdout, "up_weight_checksum: %llu\n", cli_checksum_bytes(up_values, up_bytes));
    fprintf(stdout, "down_weight_checksum: %llu\n", cli_checksum_bytes(down_values, down_bytes));
    fprintf(stdout, "intermediate_checksum: %llu\n", cli_checksum_bytes(intermediate_values, intermediate_bytes));
    fprintf(stdout, "reference_intermediate_checksum: %llu\n", cli_checksum_bytes(reference_intermediate_values, intermediate_bytes));
    fprintf(stdout, "output_checksum: %llu\n", cli_checksum_bytes(output_values, output_bytes));
    fprintf(stdout, "reference_checksum: %llu\n", cli_checksum_bytes(reference_values, reference_bytes));
    fprintf(stdout, "max_abs_diff: %.9g\n", (double)max_abs_diff);
    fprintf(stdout, "reference_attempted: true\n");
    if (strcmp(backend_name, "cuda") == 0) {
        fprintf(stdout, "mlp_cuda_parity: %s\n", exit_code == 0 ? "pass" : "fail");
        fprintf(stdout, "cuda_reference_max_abs_diff: %.9g\n", (double)max_abs_diff);
    } else {
        fprintf(stdout, "cpu_reference_max_abs_diff: %.9g\n", (double)max_abs_diff);
    }
...
static int command_graph(int arg_count, char **args)
{
    yvex_cli_tokenizer_context ctx;
    ...
    if (strcmp(args[2], "check") == 0) {
        return command_graph_check(arg_count, args);
    }
    ...
    for (i = 2; i < arg_count; ++i) {
        if (strcmp(args[i], "--model") == 0) {
            ...
            backend_name = args[++i];
        } else if (strcmp(args[i], "--execute-fixture") == 0) {
            execute_fixture = 1;
        } else if (strcmp(args[i], "--fixture-token") == 0) {
            if (i + 1 >= arg_count || !parse_uint_allow_zero(args[i + 1], &fixture_options.token_id)) {
                fprintf(stderr, "yvex: --fixture-token requires a non-negative integer\n");
                return 2;
            }
            fixture_token_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--execute-partial") == 0) {
            execute_partial = 1;
        } else if (strcmp(args[i], "--execute-segment") == 0) {
            execute_segment = 1;
        } else if (strcmp(args[i], "--execute-op") == 0) {
            execute_op = 1;
        } else if (strcmp(args[i], "--execute-block") == 0) {
            execute_block = 1;
        } else if (strcmp(args[i], "--execute-layers") == 0) {
            execute_layers = 1;
        } else if (strcmp(args[i], "--op") == 0) {
            if (i + 1 >= arg_count) {
                fprintf(stderr, "yvex: --op requires rope, attention, matmul, or mlp\n");
                return 2;
            }
            op_name = args[++i];
        } else if (strcmp(args[i], "--block") == 0) {
            if (i + 1 >= arg_count) {
                fprintf(stderr, "yvex: --block requires fixture\n");
                return 2;
            }
            block_name = args[++i];
            block_name_provided = 1;
        } else if (strcmp(args[i], "--m") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &matmul_m)) {
            ...
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &matmul_k)) {
            ...
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &matmul_n)) {
                fprintf(stderr, "yvex: --n requires a positive integer\n");
                return 2;
            }
            matmul_n_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--hidden-dim") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &mlp_hidden_dim)) {
                fprintf(stderr, "yvex: --hidden-dim requires a positive integer\n");
                return 2;
            }
            mlp_hidden_dim_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--ffn-dim") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &mlp_ffn_dim)) {
                fprintf(stderr, "yvex: --ffn-dim requires a positive integer\n");
                return 2;
            }
            mlp_ffn_dim_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--activation") == 0) {
            if (i + 1 >= arg_count) {
                fprintf(stderr, "yvex: --activation requires silu\n");
                return 2;
            }
            mlp_activation = args[++i];
            mlp_activation_provided = 1;
        } else if (strcmp(args[i], "--gated") == 0) {
            mlp_gated = 1;
        } else if (strcmp(args[i], "--experts") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &mlp_experts)) {
                fprintf(stderr, "yvex: --experts requires a positive integer\n");
                return 2;
            }
            mlp_experts_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--expert-id") == 0) {
            if (i + 1 >= arg_count || !parse_ull_allow_zero(args[i + 1], &mlp_expert_id)) {
            ...
            layers_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--seq-len") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &attention_seq_len)) {
                fprintf(stderr, "yvex: --seq-len requires a positive integer\n");
                return 2;
            }
            attention_seq_len_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--position") == 0) {
            if (i + 1 >= arg_count || !parse_ull_allow_zero(args[i + 1], &rope_position)) {
                fprintf(stderr, "yvex: --position requires a non-negative integer\n");
                return 2;
            }
            rope_position_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--head-dim") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &rope_head_dim)) {
                fprintf(stderr, "yvex: --head-dim requires a positive integer\n");
                return 2;
            }
            rope_head_dim_provided = 1;
            i += 1;
        } else if (strcmp(args[i], "--causal") == 0) {
            attention_causal = 1;
            ...
            if (i + 1 >= arg_count || !parse_uint_allow_zero(args[i + 1], &partial_options.token_id)) {
        ...
        } else if (strcmp(args[i], "--token-index") == 0) {
            if (i + 1 >= arg_count || !parse_uint_allow_zero(args[i + 1], &token_index)) {
    ...
    if (execute_block) {
        if (model_arg) {
            fprintf(stderr, "yvex: --execute-block does not take a model artifact\n");
            return 2;
        }
        if (!block_name_provided) {
            fprintf(stderr, "yvex: --execute-block requires --block fixture\n");
            return 2;
        }
        if (strcmp(block_name, "fixture") != 0) {
            fprintf(stderr, "yvex: unsupported block: %s\n", block_name);
        ...
        if (!attention_seq_len_provided || !rope_position_provided ||
            !mlp_hidden_dim_provided || !rope_head_dim_provided ||
            !mlp_ffn_dim_provided) {
            fprintf(stderr, "yvex: --execute-block requires --seq-len N --position N --hidden-dim N --head-dim N --ffn-dim N\n");
            return 2;
        }
        if (op_name || matmul_m_provided || matmul_k_provided ||
            matmul_n_provided || mlp_activation_provided ||
            ...
            fixture_token_provided || partial_token_provided ||
            token_input_provided || token_index_provided || segment_name) {
            fprintf(stderr, "yvex: --execute-block cannot be combined with model graph, segment, or standalone op options\n");
            return 2;
        }
    ...
    if (execute_fixture || execute_partial || execute_segment) {
        ...
        if (rc != YVEX_OK) {
            ...
            fprintf(stdout, "status: graph-integrity-fail\n");
        ...
        if (rc == YVEX_ERR_UNSUPPORTED && strcmp(backend_name, "cuda") == 0) {
            ...
            print_graph_guard_report(&graph_guard);
            fprintf(stdout, "%s_backend: cuda\n", graph_backend_label_for_mode(execute_fixture,
                                                                       execute_segment));
            fprintf(stdout, "%s_backend_status: unsupported\n",
        ...
        if (execute_fixture) {
            ...
            print_graph_guard_report(&graph_guard);
            fprintf(stdout, "fixture_graph_executed: true\n");
            fprintf(stdout, "fixture_backend: %s\n", fixture_result.backend_name);
            fprintf(stdout, "fixture_op: %s\n", fixture_result.op_name);
            fprintf(stdout, "fixture_weight: %s\n", fixture_result.weight_name);
            fprintf(stdout, "fixture_token_id: %u\n", fixture_result.token_id);
            fprintf(stdout, "fixture_node_count: %llu\n", fixture_result.node_count);
            fprintf(stdout, "fixture_output_count: %llu\n", fixture_result.output_count);
            fprintf(stdout, "fixture_output_bytes: %llu\n", fixture_result.output_bytes);
            fprintf(stdout, "fixture_output_checksum: %llu\n", fixture_result.output_checksum);
            fprintf(stdout, "fixture_output_values:");
            for (i = 0; (unsigned long long)i < fixture_result.output_value_count; ++i) {
                fprintf(stdout, "%s%.9g", i == 0 ? " " : ",", (double)fixture_result.output_values[i]);
            }
            fprintf(stdout, "\n");
            fprintf(stdout, "execution_ready: false\n");
            fprintf(stdout, "graph_execution_ready: false\n");
            fprintf(stdout, "status: fixture-graph-executed\n");
        } else if (execute_segment) {
            graph_guard_apply_segment_result(&graph_guard, &segment_result, 1);
            print_graph_guard_report(&graph_guard);
            fprintf(stdout, "segment_graph_executed: true\n");
            fprintf(stdout, "segment_backend: %s\n", segment_result.backend_name);
            fprintf(stdout, "segment_name: %s\n", segment_result.segment_name);
            fprintf(stdout, "segment_ops: %llu\n", segment_result.segment_ops);
            fprintf(stdout, "segment_op_0: embed\n");
            fprintf(stdout, "segment_op_1: rms_norm\n");
            fprintf(stdout, "partial_token: %u\n", segment_result.token_id);
            fprintf(stdout, "token_tensor: %s\n", segment_result.token_tensor_name);
            fprintf(stdout, "token_tensor_dtype: %s\n", segment_result.token_tensor_dtype);
            fprintf(stdout, "rmsnorm_tensor: %s\n", segment_result.rmsnorm_tensor_name);
            fprintf(stdout, "rmsnorm_tensor_dtype: %s\n", segment_result.rmsnorm_tensor_dtype);
            fprintf(stdout, "hidden_size: %llu\n", segment_result.hidden_size);
            fprintf(stdout, "vocab_size: %llu\n", segment_result.vocab_size);
            fprintf(stdout, "rmsnorm_epsilon_key: %s\n", segment_result.rmsnorm_epsilon_key);
            fprintf(stdout, "rmsnorm_epsilon: %.9g\n", segment_result.rmsnorm_epsilon);
            fprintf(stdout, "segment_memory_plan: explicit\n");
            fprintf(stdout, "segment_intermediate_count: %llu\n", segment_result.segment_intermediate_count);
            fprintf(stdout, "segment_intermediate_bytes: %llu\n", segment_result.segment_intermediate_bytes);
            fprintf(stdout, "segment_output_count: %llu\n", segment_result.segment_output_count);
            fprintf(stdout, "segment_output_bytes: %llu\n", segment_result.segment_output_bytes);
            fprintf(stdout, "segment_scratch_bytes: %llu\n", segment_result.segment_scratch_bytes);
            fprintf(stdout, "segment_reference_bytes: %llu\n", segment_result.segment_reference_bytes);
            fprintf(stdout, "segment_output_checksum: %llu\n", segment_result.output_checksum);
            fprintf(stdout, "segment_reference_checksum: %llu\n", segment_result.reference_checksum);
            fprintf(stdout, "segment_max_abs_diff: %.9g\n", segment_result.max_abs_diff);
            if (strcmp(segment_result.backend_name, "cuda") == 0) {
                fprintf(stdout, "segment_cuda_parity: pass\n");
                fprintf(stdout, "cuda_reference_max_abs_diff: %.9g\n", segment_result.max_abs_diff);
            } else {
                fprintf(stdout, "cpu_reference_max_abs_diff: %.9g\n", segment_result.max_abs_diff);
            }
            fprintf(stdout, "segment_output_sample_count: %llu\n", segment_result.output_value_count);
            fprintf(stdout, "segment_output_sample_values:");
            for (i = 0; (unsigned long long)i < segment_result.output_value_count; ++i) {
                fprintf(stdout, "%s%.9g", i == 0 ? " " : ",", (double)segment_result.output_values[i]);
            }
            fprintf(stdout, "\n");
            fprintf(stdout, "execution_ready: false\n");
            fprintf(stdout, "graph_execution_ready: false\n");
            ...
            fprintf(stdout, "logits_ready: false\n");
            fprintf(stdout, "generation: unsupported\n");
            fprintf(stdout, "status: real-segment-graph-executed\n");
        } else {
            graph_guard_apply_partial_result(&graph_guard, &partial_result, 1);
            print_graph_guard_report(&graph_guard);
            fprintf(stdout, "real_partial_graph_executed: true\n");
            fprintf(stdout, "partial_graph_kind: %s\n", partial_result.segment_name);
            fprintf(stdout, "partial_backend: %s\n", partial_result.backend_name);
            fprintf(stdout, "partial_weight: %s\n", partial_result.weight_name);
            fprintf(stdout, "partial_weight_dtype: %s\n", partial_result.weight_dtype);
            fprintf(stdout, "partial_token: %u\n", partial_result.token_id);
            fprintf(stdout, "partial_node_count: %llu\n", partial_result.node_count);
            fprintf(stdout, "partial_output_dtype: %s\n", partial_result.output_dtype);
            fprintf(stdout, "partial_output_count: %llu\n", partial_result.output_count);
            fprintf(stdout, "partial_output_bytes: %llu\n", partial_result.output_bytes);
            fprintf(stdout, "partial_output_checksum: %llu\n", partial_result.output_checksum);
            fprintf(stdout, "partial_reference_checksum: %llu\n", partial_result.reference_checksum);
            fprintf(stdout, "partial_max_abs_diff: %.9g\n", partial_result.max_abs_diff);
            fprintf(stdout, "partial_output_sample_count: %llu\n", partial_result.output_value_count);
            fprintf(stdout, "partial_output_sample_values:");
            for (i = 0; (unsigned long long)i < partial_result.output_value_count; ++i) {
                fprintf(stdout, "%s%.9g", i == 0 ? " " : ",", (double)partial_result.output_values[i]);
            }
            fprintf(stdout, "\n");
            fprintf(stdout, "execution_ready: false\n");
            fprintf(stdout, "graph_execution_ready: false\n");
            ...
            fprintf(stdout, "logits_ready: false\n");
            fprintf(stdout, "generation: unsupported\n");
            fprintf(stdout, "status: real-partial-graph-executed\n");
        }
...
{
    return command_graph(arg_count, args);
}

void yvex_graph_help(FILE *fp)
{
    fprintf(fp, "usage: " "yvex graph [--model] FILE_OR_ALIAS [--seq N] [--ctx N] [--backend cpu|cuda] [--execute-fixture] [--execute-partial] [--execute-segment --segment embedding-rmsnorm] [--partial-token N] [--tokens IDS --token-index N]\n");
    fprintf(fp, "       yvex graph --backend cpu|cuda --execute-op --op rope --position N --head-dim N\n");
    fprintf(fp, "       yvex graph --backend cpu|cuda --execute-op --op attention --seq-len N --position N --head-dim N [--causal]\n");
