/*
 * primitive.c - graph primitive proof facts.
 *
 * Owner:
 *   src/graph
 *
                                       ...
                                       yvex_error *err)
{
    ...
    rc = yvex_model_ref_identity_validate(&model_ref, &identity_result, err);
    if (rc != YVEX_OK) {
        unsigned int issue;
        const char *status = strcmp(identity_result.identity_status, "missing") == 0
                                 ? "models-identity-missing"
                                 : (strcmp(identity_result.metadata_status, "missing") == 0
                                        ? "models-metadata-missing"
                                        : (strcmp(identity_result.identity_status, "pass") == 0
                                               ? "models-metadata-drift"
                                               : "models-identity-fail"));
        yvex_graph_report_appendf(report, "artifact_identity: check\n");
        yvex_graph_report_appendf(report, "surface: graph\n");
        yvex_graph_report_appendf(report, "alias: %s\n",
                                  model_ref.alias ? model_ref.alias : "");
        yvex_graph_report_appendf(report, "path: %s\n",
                                  model_ref.path ? model_ref.path : "");
        yvex_graph_report_appendf(report, "registered_sha256: %s\n",
                                  model_ref.sha256 && model_ref.sha256[0]
                                      ? model_ref.sha256 : "absent");
        yvex_graph_report_appendf(report, "current_sha256: %s\n",
                                  identity_result.current_sha256[0]
                                      ? identity_result.current_sha256 : "unavailable");
        yvex_graph_report_appendf(report, "registered_file_size: %llu\n",
                                  identity_result.registered_file_size);
        yvex_graph_report_appendf(report, "current_file_size: %llu\n",
                                  identity_result.current_file_size);
        yvex_graph_report_appendf(report, "digest_status: %s\n",
                                  identity_result.digest_status);
        yvex_graph_report_appendf(report, "identity_status: %s\n",
                                  identity_result.identity_status);
        if (identity_result.metadata_status[0]) {
            yvex_graph_report_appendf(report, "metadata_status: %s\n",
                                      identity_result.metadata_status);
            yvex_graph_report_appendf(report, "readiness_status: %s\n",
                                      identity_result.readiness_status);
        }
        for (issue = 0u;
             issue < identity_result.metadata_drift.issue_count;
             ++issue) {
            yvex_graph_report_appendf(
                report, "metadata_issue_%u_code: %s\n", issue,
                identity_result.metadata_drift.issues[issue].code);
        }
        yvex_graph_report_appendf(report, "reason: %s\n",
    ...
    if (token_input_provided) {
        rc = yvex_token_input_parse_explicit(request->tokens_text,
                                             ...
                                             err);
        if (rc == YVEX_OK) {
            rc = yvex_model_context_vocab_size(model_ref.path,
                                                  &token_vocab_size,
                                                  err);
        }
        if (rc == YVEX_OK) {
