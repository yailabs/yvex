/*
 * Execute the canonical compressed-attention plan on CPU or CUDA.
 *
 * Family compilation seals tensor roles, class geometry, recurrence, and numeric policy into the
 * immutable plan. This owner executes that admitted plan without reading a concrete family IR.
 */
    ...
    yvex_error *err)
{
    ...
    if (!yvex_sha256_hex_valid(logical_identity))
        return yvex_attention_reject(
            ...
            "attention execution requires the compiled logical model identity");
    ...
    yvex_attention_failure *failure, yvex_error *err)
{
    ...
    if (out->token_count != options->token_position)
        return yvex_attention_reject(
            ...
            "attention history is not contiguous");
    if (!options->history && options->token_position)
        return yvex_attention_reject(
            failure, YVEX_ATTENTION_FAILURE_HISTORY, NULL,
            layer->layer_index, YVEX_TENSOR_ROLE_UNKNOWN, options->token_position,
            0ull, err, YVEX_ERR_STATE,
            "attention requires complete history after the first token");
    if (options->history) {
                                       ...
                                       yvex_error *err)
{
    ...
    if (!selection_key || !key) {
        yvex_error_set(err, YVEX_ERR_INVALID_ARG,
                       "graph.attention.selection_key",
                       "attention selection requires swa, csa, or hca");
...
typedef struct {
    ...
    const yvex_attention_component_span *committed[YVEX_ATTENTION_COMPONENT_COUNT];
...
static int cpu_chunk_round(cpu_chunk_context *context, float *values, unsigned long long count, const char *stage)
{
    ...
    return cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_NUMERIC, NULL,
                                     ...
                                     unsigned long long count, size_t element_size, const char *reason)
{
    ...
    if (!yvex_attention_scratch_reserve(
        ...
        return cpu_chunk_reject(
            context, YVEX_ATTENTION_FAILURE_SCRATCH, NULL,
...
static int cpu_chunk_rolling_initialize(cpu_chunk_context *context, unsigned int index)
{
    ...
    if (!yvex_attention_rolling_geometry(
        ...
        return cpu_chunk_reject(
            context, YVEX_ATTENTION_FAILURE_DIMENSION, NULL,
...
if (!context->q_a || !context->q_a_norm || !context->q_b || !context->kv ||
    ...
    !context->out_b)
    return cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_MISSING_BINDING, NULL,
        YVEX_TENSOR_ROLE_UNKNOWN, 8ull, 0ull, YVEX_ERR_FORMAT,
        "attention CPU chunk requires complete Q/KV/sink/output bindings");
...
if (context->q_rank != context->q_b->binding->row_width ||
    ...
    return cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_DIMENSION, context->q_b,
        ...
        "attention CPU chunk tensor dimensions do not match plan");
if (!yvex_core_u64_mul(context->token_count, context->hidden_width,
    ...
    return cpu_chunk_reject(
        ...
        "attention CPU chunk scratch geometry overflowed");
if (context->opts->operation_scope == YVEX_ATTENTION_OPERATION_ENVELOPE) {
    ...
    if (!yvex_attention_envelope_scratch_elements(
        ...
        !yvex_core_u64_add(context->scratch_elements, envelope_elements,
                           &context->scratch_elements))
        return cpu_chunk_reject(context, YVEX_ATTENTION_FAILURE_SCRATCH,
...
context->rc = cpu_chunk_scratch_reserve(
    ...
    "attention CPU chunk scratch budget exceeded");
if (context->rc != YVEX_OK) return context->rc;
...
context->rc = yvex_attention_state_transaction_acquire(
    &context->transaction, YVEX_ATTENTION_COMPONENT_ATTENTION_OUTPUT,
    &context->output_span, context->failure, context->err);
if (context->rc != YVEX_OK) return context->rc;
context->rc = yvex_attention_state_transaction_acquire(
    &context->transaction, YVEX_ATTENTION_COMPONENT_RAW_LOCAL_KV,
    &context->raw_kv_span, context->failure, context->err);
if (context->rc != YVEX_OK) return context->rc;
if (context->opts->operation_scope == YVEX_ATTENTION_OPERATION_ENVELOPE) {
    context->rc = yvex_attention_state_transaction_acquire(
        &context->transaction, YVEX_ATTENTION_COMPONENT_ENVELOPE_OUTPUT,
...
if (!context->hidden || !context->q_low || !context->q_norm_weights || !context->query || !context->kv_norm_weights ||
    ...
    context->rc = cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_ALLOCATION, NULL,
        YVEX_TENSOR_ROLE_UNKNOWN, context->scratch.live_bytes, 0ull,
        YVEX_ERR_NOMEM, "failed to allocate attention CPU chunk scratch");
    return context->rc;
}
if (context->opts->operation_scope == YVEX_ATTENTION_OPERATION_ENVELOPE)
    ...
    for (lane = 0ull; lane < context->hidden_width; ++lane) {
        if (!isfinite(source[lane])) {
            context->rc = cpu_chunk_reject(
                ...
                "attention input contains non-finite values");
            return context->rc;
        }
    }
...
context->rc = cpu_chunk_round(
    ...
    "attention input could not enter the BF16 compute contract");
return context->rc;
}
...
context->rc = cpu_chunk_round(
    ...
    "attention Q-A projection could not publish BF16 values");
if (context->rc != YVEX_OK) return context->rc;
if (context->rows != context->q_rank) {
    context->rc = cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_DIMENSION, context->q_a,
        YVEX_TENSOR_ROLE_ATTENTION_Q_A, context->q_rank, context->rows, YVEX_ERR_FORMAT,
        "attention CPU chunk Q-A projection is incomplete");
    return context->rc;
}
...
for (context->token = 0ull; context->token < context->token_count; ++context->token) {
    if (!yvex_attention_rms_norm(context->q_low + context->token * context->q_rank, context->q_rank,
        ...
        context->rc = cpu_chunk_reject(
            context, YVEX_ATTENTION_FAILURE_NUMERIC, context->q_a_norm,
            YVEX_TENSOR_ROLE_ATTENTION_Q_A_NORM, context->q_rank, context->token,
            YVEX_ERR_FORMAT,
            "attention CPU chunk Q norm produced invalid values");
        return context->rc;
    }
    context->rc = cpu_chunk_round(
        context, context->q_low + context->token * context->q_rank,
        context->q_rank,
        "attention Q norm could not publish BF16 values");
    if (context->rc != YVEX_OK) return context->rc;
}
context->rc = yvex_attention_dot_batch(
...
context->rc = cpu_chunk_round(
    ...
    "attention Q-B projection could not publish BF16 values");
if (context->rc != YVEX_OK) return context->rc;
context->rc = yvex_attention_dot_batch(
...
context->rc = cpu_chunk_round(
    ...
    "attention KV projection could not publish BF16 values");
if (context->rc != YVEX_OK) return context->rc;
context->rc = yvex_attention_decode_row(
...
for (context->token = 0ull; context->token < context->token_count; ++context->token) {
    ...
    for (head = 0ull; head < context->layer_plan->query_heads; ++head) {
        if (!yvex_attention_unit_rms_norm(
            ...
            context->rc = cpu_chunk_reject(
                context, YVEX_ATTENTION_FAILURE_NUMERIC, context->q_b,
                YVEX_TENSOR_ROLE_ATTENTION_Q_B, context->layer_plan->head_dimension,
                head, YVEX_ERR_FORMAT,
                "attention query head RMS norm failed");
            return context->rc;
        }
        context->rc = cpu_chunk_round(
            context,
            ...
            "attention query normalization could not publish BF16 values");
        if (context->rc != YVEX_OK) return context->rc;
        if (!yvex_attention_rope_apply(
            ...
            context->rc = cpu_chunk_reject(
                ...
                "attention query RoPE/YaRN application failed");
            return context->rc;
        }
        context->rc = cpu_chunk_round(
            context,
            context->query + context->token * context->query_width +
                head * context->layer_plan->head_dimension,
            context->layer_plan->head_dimension,
            "attention query RoPE could not publish BF16 values");
        if (context->rc != YVEX_OK) return context->rc;
    }
    if (!yvex_attention_rms_norm(
        ...
        context->rc = cpu_chunk_reject(
            context, YVEX_ATTENTION_FAILURE_NUMERIC, context->kv_norm,
            YVEX_TENSOR_ROLE_ATTENTION_KV_NORM, context->kv_width, context->token,
            YVEX_ERR_FORMAT,
            "attention CPU chunk KV norm produced invalid values");
        return context->rc;
    }
    context->rc = cpu_chunk_round(
        context,
        ...
        context->kv_width,
        "attention KV normalization could not publish BF16 values");
    if (context->rc != YVEX_OK) return context->rc;
    if (!yvex_attention_rope_apply(
            (float *)context->raw_kv_span.data + context->token * context->raw_kv_span.stride,
            ...
            context->opts->token_position + context->token, &context->layer_plan->position, 0)) {
        context->rc = cpu_chunk_reject(
            context, YVEX_ATTENTION_FAILURE_NUMERIC, context->kv,
            YVEX_TENSOR_ROLE_ATTENTION_KV, 1ull, context->token, YVEX_ERR_FORMAT,
            "attention KV RoPE/YaRN application failed");
        return context->rc;
    }
    context->rc = cpu_chunk_round(
        context,
        (float *)context->raw_kv_span.data +
            context->token * context->raw_kv_span.stride,
        context->kv_width,
        "attention KV RoPE could not publish BF16 values");
    if (context->rc != YVEX_OK) return context->rc;
...
static int cpu_chunk_emission_prepare(cpu_chunk_context *context, unsigned int index)
{
    ...
    return cpu_chunk_reject(
        ...
        index == CPU_ROLLING_MAIN
            ? "attention compressed-position allocation failed"
            : "attention indexer-position allocation failed");
}
static int cpu_chunk_rolling_prepare(cpu_chunk_context *context, unsigned int index)
{
    ...
    for (item = 0u; item < 4u; ++item) {
        ...
        if (!stage->weights[item])
            return cpu_chunk_reject(
                context, YVEX_ATTENTION_FAILURE_MISSING_BINDING,
    ...
    for (context->token = 0ull; context->token < context->token_count;
        ...
        if (emitted) {
            ...
            if (!yvex_attention_rms_norm(
                    ...
                    context->layer_plan->rms_norm_epsilon))
                return cpu_chunk_reject(
                    context, YVEX_ATTENTION_FAILURE_NUMERIC,
...
static int cpu_chunk_index_query(cpu_chunk_context *context)
{
    ...
    context->index_q_binding = yvex_attention_binding_find(
        ...
        if (!context->index_q_binding || !context->index_weight_binding ||
            ...
            context->rc = cpu_chunk_reject(
                ...
                context->index_q_binding ? context->index_q_binding->binding->row_count
                                : 0ull,
                YVEX_ERR_FORMAT,
                "CSA index query/weight bindings do not match the plan");
            return context->rc;
        }
        ...
        if (!context->index_query || !context->index_weights) {
            context->rc = cpu_chunk_reject(
                context, YVEX_ATTENTION_FAILURE_ALLOCATION,
                context->index_q_binding,
                YVEX_TENSOR_ROLE_INDEXER_ATTENTION_Q_B, context->scratch_term,
                0ull, YVEX_ERR_NOMEM,
                "CSA index query/weight allocation failed");
            return context->rc;
        }
        context->rc = yvex_attention_dot_batch(
        ...
        if (context->rows != context->layer_plan->indexer_heads *
            ...
            context->rc = cpu_chunk_reject(
                ...
                "CSA index query projection is incomplete");
            return context->rc;
        }
        context->rc = cpu_chunk_round(
            ...
            context->token_count * context->layer_plan->indexer_heads *
                context->layer_plan->indexer_head_dimension,
            "index query projection could not publish BF16 values");
        if (context->rc != YVEX_OK) return context->rc;
        context->rc = yvex_attention_dot_batch(
        ...
        if (context->rows != context->layer_plan->indexer_heads) {
            context->rc = cpu_chunk_reject(
                ...
                "CSA index weight projection is incomplete");
            return context->rc;
        }
        context->rc = cpu_chunk_round(
            context, context->index_weights,
            context->token_count * context->layer_plan->indexer_heads,
            "index weight projection could not publish BF16 values");
        if (context->rc != YVEX_OK) return context->rc;
        for (context->token = 0ull; context->token < context->token_count; ++context->token) {
            unsigned long long head;
            for (head = 0ull; head < context->layer_plan->indexer_heads; ++head) {
                ...
                if (!yvex_attention_rope_apply(
                    ...
                    context->rc = cpu_chunk_reject(
                        ...
                        head, YVEX_ERR_FORMAT,
                        "CSA index query RoPE/YaRN failed");
                    return context->rc;
                }
                context->rc = cpu_chunk_round(
                    context, head_query,
                    context->layer_plan->indexer_head_dimension,
                    "index query RoPE could not publish BF16 values");
                if (context->rc != YVEX_OK) return context->rc;
                context->rc = yvex_attention_activation_apply(
...
if ((context->opts->publication || context->opts->trace) &&
    ...
    if (!yvex_core_u64_add(context->history.compressed_entry_count,
        ...
        context->rc = cpu_chunk_reject(
            ...
            context->rolling[CPU_ROLLING_MAIN].emitted,
            YVEX_ERR_BOUNDS,
            "CSA trace top-k extent overflowed");
        return context->rc;
    }
    ...
    if (context->trace_topk_stride) {
        ...
        if (!context->trace_topk_counts || !context->trace_topk_positions) {
            context->rc = cpu_chunk_reject(
                ...
                "CSA trace top-k allocation failed");
            return context->rc;
        }
    }
}
...
for (component = 0u; component < YVEX_ATTENTION_COMPONENT_COUNT;
     ++component)
...
if (!committed[YVEX_ATTENTION_COMPONENT_ATTENTION_OUTPUT] ||
    !committed[YVEX_ATTENTION_COMPONENT_ATTENTION_OUTPUT]->data ||
    !committed[YVEX_ATTENTION_COMPONENT_RAW_LOCAL_KV] ||
    !committed[YVEX_ATTENTION_COMPONENT_RAW_LOCAL_KV]->data ||
    !context->committed_identity) {
    context->rc = cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_STATE_DELTA, NULL,
        YVEX_TENSOR_ROLE_UNKNOWN, 1ull, 0ull, YVEX_ERR_STATE,
        "attention CPU chunk commit did not publish output identity");
    return context->rc;
}
if (context->opts->operation_scope == YVEX_ATTENTION_OPERATION_ENVELOPE &&
    (!committed[YVEX_ATTENTION_COMPONENT_ENVELOPE_OUTPUT] ||
     !committed[YVEX_ATTENTION_COMPONENT_ENVELOPE_OUTPUT]->data))
    return cpu_chunk_reject(
        context, YVEX_ATTENTION_FAILURE_STATE_DELTA, NULL,
        YVEX_TENSOR_ROLE_UNKNOWN, 1ull, 0ull, YVEX_ERR_STATE,
...
if ((context->opts->publication || context->opts->trace) &&
    !yvex_attention_trace_capture(
        ...
        (const float *)committed[
            YVEX_ATTENTION_COMPONENT_RAW_LOCAL_KV]->data,
        committed[YVEX_ATTENTION_COMPONENT_COMPRESSED_MAIN_KV]
            ? (const float *)committed[
                  YVEX_ATTENTION_COMPONENT_COMPRESSED_MAIN_KV]->data
            : NULL,
        context->rolling[CPU_ROLLING_MAIN].emitted,
        committed[YVEX_ATTENTION_COMPONENT_COMPRESSED_MAIN_KV]
            ? committed[
                  YVEX_ATTENTION_COMPONENT_COMPRESSED_MAIN_KV]->stride : 0ull,
        context->rolling[CPU_ROLLING_MAIN].positions,
        committed[YVEX_ATTENTION_COMPONENT_INDEXER_KV]
            ? (const float *)committed[
                  YVEX_ATTENTION_COMPONENT_INDEXER_KV]->data : NULL,
        context->rolling[CPU_ROLLING_INDEXER].emitted,
        committed[YVEX_ATTENTION_COMPONENT_INDEXER_KV]
            ? committed[
                  YVEX_ATTENTION_COMPONENT_INDEXER_KV]->stride : 0ull,
        context->rolling[CPU_ROLLING_INDEXER].positions,
        context->index_query,
        context->layer_plan->indexer_heads *
            context->layer_plan->indexer_head_dimension,
        ...
        (const float *)committed[
            YVEX_ATTENTION_COMPONENT_ATTENTION_OUTPUT]->data,
        context->trace_topk_counts,
        context->trace_topk_positions, context->trace_topk_stride,
        context->rolling[CPU_ROLLING_MAIN].after.present
            ? &context->rolling[CPU_ROLLING_MAIN].after : NULL,
        committed[YVEX_ATTENTION_COMPONENT_MAIN_KV_STATE]
            ? (const float *)committed[
                  YVEX_ATTENTION_COMPONENT_MAIN_KV_STATE]->data : NULL,
        committed[YVEX_ATTENTION_COMPONENT_MAIN_SCORE_STATE]
            ? (const float *)committed[
                  YVEX_ATTENTION_COMPONENT_MAIN_SCORE_STATE]->data : NULL,
        context->rolling[CPU_ROLLING_INDEXER].after.present
            ? &context->rolling[CPU_ROLLING_INDEXER].after : NULL,
        committed[YVEX_ATTENTION_COMPONENT_INDEXER_KV_STATE]
            ? (const float *)committed[
                  YVEX_ATTENTION_COMPONENT_INDEXER_KV_STATE]->data : NULL,
        committed[YVEX_ATTENTION_COMPONENT_INDEXER_SCORE_STATE]
            ? (const float *)committed[
                  YVEX_ATTENTION_COMPONENT_INDEXER_SCORE_STATE]->data : NULL,
    ...
    context->rc = cpu_chunk_reject(
        ...
        "attention execution trace capture failed");
    return context->rc;
}
if (context->opts->publication || context->opts->trace) {
    ...
    if (!yvex_attention_trace_outputs_attach(
            trace, committed[YVEX_ATTENTION_COMPONENT_ATTENTION_OUTPUT],
            committed[YVEX_ATTENTION_COMPONENT_ENVELOPE_OUTPUT]))
        return cpu_chunk_reject(
            context, YVEX_ATTENTION_FAILURE_ALLOCATION, NULL,
            YVEX_TENSOR_ROLE_UNKNOWN, 1ull, 0ull, YVEX_ERR_NOMEM,
...
context->result->kv_projection_checksum =
    yvex_attention_checksum(
        (const float *)committed[
            YVEX_ATTENTION_COMPONENT_RAW_LOCAL_KV]->data,
        committed[
            YVEX_ATTENTION_COMPONENT_RAW_LOCAL_KV]->expected_elements);
...
yvex_attention_result_outputs_publish(
    ...
    committed[YVEX_ATTENTION_COMPONENT_ATTENTION_OUTPUT],
    committed[YVEX_ATTENTION_COMPONENT_ENVELOPE_OUTPUT]);
    ...
    yvex_attention_failure *failure, yvex_error *err)
{
    ...
    if (rc != YVEX_OK &&
        context.transaction.status == YVEX_ATTENTION_TRANSACTION_BEGUN) {
        ...
        if (yvex_attention_state_transaction_abort(
            ...
            rc = yvex_attention_reject(
                context.failure, YVEX_ATTENTION_FAILURE_CLEANUP, NULL,
...
static const yvex_attention_failure_code cuda_failure_map[] = {
    YVEX_ATTENTION_FAILURE_BACKEND, YVEX_ATTENTION_FAILURE_INVALID_ARGUMENT,
    YVEX_ATTENTION_FAILURE_BACKEND, YVEX_ATTENTION_FAILURE_SCRATCH,
    YVEX_ATTENTION_FAILURE_ALLOCATION, YVEX_ATTENTION_FAILURE_BACKEND,
    YVEX_ATTENTION_FAILURE_BACKEND, YVEX_ATTENTION_FAILURE_BACKEND,
    YVEX_ATTENTION_FAILURE_NUMERIC, YVEX_ATTENTION_FAILURE_CANCELLED,
...
if (!context->plan || !context->session ||
    ...
    return yvex_attention_cuda_reject(
        context, YVEX_ATTENTION_FAILURE_INVALID_ARGUMENT, 1ull, 0ull, YVEX_ERR_INVALID_ARG,
...
default:
    return yvex_attention_cuda_reject(
        context, YVEX_ATTENTION_FAILURE_DIMENSION, 1ull,
...
if (!yvex_backend_host_workspace_summary_get(context->backend, &workspace) ||
    ...
    return yvex_attention_cuda_reject(
        context, YVEX_ATTENTION_FAILURE_SCRATCH, 1ull, 0ull,
        YVEX_ERR_STATE,
...
if (!yvex_core_u64_mul(context->token_count, context->trace.hidden_width, &hidden) ||
    ...
    return yvex_attention_cuda_reject(
        context, YVEX_ATTENTION_FAILURE_SCRATCH, ULLONG_MAX,
        context->token_count, YVEX_ERR_BOUNDS,
...
const yvex_graph_execution_api yvex_attention_execution_api = {
