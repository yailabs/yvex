                                ...
                                const char **value)
{
    if (*index + 1 >= arg_count) {
        fprintf(stderr, "accounts %s: %s requires a value\n", command, flag);
        return 2;
    }
    *value = args[++(*index)];
    if (!*value || !(*value)[0] || strchr(*value, '\n') || strchr(*value, '\r')) {
        fprintf(stderr, "accounts %s: %s value is empty or invalid\n", command, flag);
        return 2;
    }
    return 0;
}
    ...
    for (i = start; i < arg_count; ++i) {
        ...
        if (strcmp(args[i], "--" "audit") == 0) {
            ...
            if (!accounts_parse_output_mode(value, &options->output_mode)) {
                fprintf(stderr, "accounts %s: unsupported output mode: %s\n", command, value);
                return 2;
            }
            ...
            fprintf(stderr, "accounts %s: JSON output is unsupported; use --" "output normal|table|audit\n", command);
            return 2;
        } else {
            fprintf(stderr, "accounts %s: unknown option: %s\n", command, args[i]);
            return 2;
        }
    }
    return 0;
...
{
    fprintf(stdout, "%sprovider: %s\n", prefix, obs->provider_name);
    fprintf(stdout, "%scli: %s\n", prefix, obs->cli_path[0] ? obs->cli_path : "missing");
    fprintf(stdout, "%scli_status: %s\n", prefix, obs->cli_status);
    fprintf(stdout, "%sauth_state: %s\n", prefix, obs->auth_state);
    fprintf(stdout, "%saccount_hint: %s\n", prefix, obs->account_hint);
    fprintf(stdout, "%scredential_source: %s\n", prefix, obs->credential_source);
    fprintf(stdout, "%stoken_env_name: %s\n", prefix, obs->token_env_name);
    fprintf(stdout, "%stoken_value_redacted: %s\n", prefix, obs->token_value_redacted ? "true" : "false");
    fprintf(stdout, "%sraw_token_stored_by_yvex: false\n", prefix);
    fprintf(stdout, "%slast_checked_at: %s\n", prefix, obs->last_checked_at);
    if (obs->top_blocker[0]) fprintf(stdout, "%stop_blocker: %s\n", prefix, obs->top_blocker);
    if (obs->next[0]) fprintf(stdout, "%snext: %s\n", prefix, obs->next);
}
                                  ...
                                  yvex_accounts_output_mode mode)
{
    if (mode == YVEX_ACCOUNTS_OUTPUT_TABLE) {
        fprintf(stdout, "PROVIDER      CLI      AUTH               ACCOUNT\n");
        fprintf(stdout, "%-13s %-8s %-18s %s\n",
               obs->provider_name, obs->cli_status, obs->auth_state, obs->account_hint);
        fprintf(stdout, "status: %s\n", obs->status);
        return;
    }
    if (mode == YVEX_ACCOUNTS_OUTPUT_AUDIT) {
        fprintf(stdout, "accounts: %s\n", surface);
        accounts_print_observation_audit("", obs);
        fprintf(stdout, "state_path: %s\n", obs->state_path);
        fprintf(stdout, "boundary: local provider account state only; no tokens stored by YVEX\n");
        fprintf(stdout, "status: %s\n", obs->status);
        return;
    }
    fprintf(stdout, "provider: %s\n", obs->provider_name);
    fprintf(stdout, "cli: %s\n", obs->cli_status);
    fprintf(stdout, "auth_state: %s\n", obs->auth_state);
    fprintf(stdout, "account: %s\n", obs->account_hint);
    if (obs->top_blocker[0]) fprintf(stdout, "top_blocker: %s\n", obs->top_blocker);
    if (obs->next[0]) fprintf(stdout, "next: %s\n", obs->next);
    fprintf(stdout, "boundary: local provider account state only, no tokens stored by YVEX\n");
    fprintf(stdout, "status: %s\n", obs->status);
}
...
static int command_accounts_status(int arg_count, char **args)
{
    ...
    if (options.output_mode == YVEX_ACCOUNTS_OUTPUT_TABLE) {
        ...
        fprintf(stdout, "accounts: status\n");
        accounts_print_observation_audit("provider_0_", &observations[0]);
        accounts_print_observation_audit("provider_1_", &observations[1]);
        fprintf(stdout, "state_path: %s\n", observations[0].state_path);
        fprintf(stdout, "raw_token_stored_by_yvex: false\n");
        fprintf(stdout, "boundary: local provider account state only, no tokens stored by YVEX\n");
    } else {
        fprintf(stdout, "accounts: provider status\n");
        fprintf(stdout, "huggingface: %s cli=%s account=%s\n",
               observations[0].auth_state, observations[0].cli_status, observations[0].account_hint);
        fprintf(stdout, "github: %s cli=%s account=%s\n",
               observations[1].auth_state, observations[1].cli_status, observations[1].account_hint);
        fprintf(stdout, "boundary: local provider account state only, no tokens stored by YVEX\n");
    }
    fprintf(stdout, "status: accounts-status\n");
    return 0;
}
...
static int command_accounts_whoami(int arg_count, char **args)
{
    ...
    if (arg_count < 4) {
        fprintf(stderr, "accounts whoami: PROVIDER is required\n");
        return 2;
    }
    if (!yvex_account_provider_from_name(args[3], &provider)) {
        fprintf(stderr, "accounts whoami: unsupported provider: %s\n", args[3]);
        return 2;
    }
        ...
        if (strcmp(args[i], "--" "audit") == 0) {
            ...
            if (strcmp(flag, "--" "output") == 0) {
                ...
                if (strcmp(value, "https") != 0 && strcmp(value, "ssh") != 0) {
                    fprintf(stderr, "accounts %s: --git-protocol requires https|ssh\n", command);
                    return 2;
                }
...
static int command_accounts_login(int arg_count, char **args)
{
    ...
    if (arg_count < 4) {
        fprintf(stderr, "accounts login: PROVIDER is required\n");
        return 2;
    }
    if (!yvex_account_provider_from_name(args[3], &provider)) {
        fprintf(stderr, "accounts login: unsupported provider: %s\n", args[3]);
        return 2;
    }
...
static int command_accounts_ensure(int arg_count, char **args)
{
    ...
    if (arg_count < 4) {
        fprintf(stderr, "accounts ensure: PROVIDER is required\n");
        return 2;
    }
    if (!yvex_account_provider_from_name(args[3], &provider)) {
        fprintf(stderr, "accounts ensure: unsupported provider: %s\n", args[3]);
        return 2;
    }
    memset(&options, 0, sizeof(options));
    options.output_mode = YVEX_ACCOUNTS_OUTPUT_NORMAL;
    ...
    for (i = 4; i < arg_count; ++i) {
        ...
        if (strcmp(args[i], "--" "audit") == 0) {
            ...
            if (strcmp(flag, "--" "output") == 0) {
                if (!accounts_parse_output_mode(value, &options.output_mode)) {
                    fprintf(stderr, "accounts ensure: unsupported output mode: %s\n", value);
                    return 2;
                }
            } else if (strcmp(flag, "--token-env") == 0) {
                ...
                fprintf(stderr, "accounts ensure: --interactive requires auto|always|never\n");
                return 2;
            }
        } else {
            fprintf(stderr, "accounts ensure: unknown option: %s\n", args[i]);
            return 2;
        }
    }
    yvex_error_clear(&err);
...
static int command_accounts_logout(int arg_count, char **args)
{
    ...
    if (arg_count < 4) {
        fprintf(stderr, "accounts logout: PROVIDER is required\n");
        return 2;
    }
    if (!yvex_account_provider_from_name(args[3], &provider)) {
        fprintf(stderr, "accounts logout: unsupported provider: %s\n", args[3]);
        return 2;
    }
...
{
    fprintf(fp, "usage: " "yvex accounts providers [--" "output normal|table|audit]\n");
    fprintf(fp, "       yvex accounts status [--" "audit | --" "output normal|table|audit]\n");
    fprintf(fp, "       yvex accounts login PROVIDER [options]\n");
    fprintf(fp, "       yvex accounts logout PROVIDER [options]\n");
    fprintf(fp, "       yvex accounts whoami PROVIDER [--" "audit]\n");
    fprintf(fp, "       yvex accounts ensure PROVIDER [--interactive auto|always|never] [--required]\n");
    fprintf(fp, "\nProviders: huggingface|hf, github|gh.\n");
    fprintf(fp, "Hugging Face CLI discovery uses YVEX_HF_CLI, then hf, then legacy huggingface-cli.\n");
    fprintf(fp, "GitHub CLI discovery uses YVEX_GH_CLI, then gh.\n");
    fprintf(fp, "Boundary: local provider account state only; YVEX stores no raw tokens, does not install tools, does not implement OAuth, does not use MCP/YAI, and does not download, materialize, run, generate, evaluate, or benchmark models from this command.\n");
}
...
int yvex_accounts_command(int arg_count, char **args)
{
    ...
    if (arg_count < 3) {
        fprintf(stderr, "yvex: accounts requires providers, status, login, logout, whoami, or ensure\n");
        return 2;
    }
    sub = args[2];
    ...
    fprintf(stderr, "yvex: unknown accounts subcommand: %s\n", sub);
    return 2;
}
