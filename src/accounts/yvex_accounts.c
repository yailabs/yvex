                                             ...
                                             const yvex_account_observation *obs)
{
    ...
    printf("%scli_status: %s\n", prefix, obs->cli_status);
    printf("%sauth_state: %s\n", prefix, obs->auth_state);
    printf("%saccount_hint: %s\n", prefix, obs->account_hint);
    printf("%scredential_source: %s\n", prefix, obs->credential_source);
    printf("%stoken_env_name: %s\n", prefix, obs->token_env_name);
    printf("%stoken_value_redacted: %s\n", prefix, obs->token_value_redacted ? "true" : "false");
    printf("%sraw_token_stored_by_yvex: false\n", prefix);
    printf("%slast_checked_at: %s\n", prefix, obs->last_checked_at);
    if (obs->top_blocker[0]) printf("%stop_blocker: %s\n", prefix, obs->top_blocker);
    if (obs->next[0]) printf("%snext: %s\n", prefix, obs->next);
}
                                  ...
                                  yvex_accounts_output_mode mode)
{
    if (mode == YVEX_ACCOUNTS_OUTPUT_TABLE) {
        printf("PROVIDER      CLI      AUTH               ACCOUNT\n");
        printf("%-13s %-8s %-18s %s\n",
    ...
    if (mode == YVEX_ACCOUNTS_OUTPUT_AUDIT) {
        ...
        printf("state_path: %s\n", obs->state_path);
        printf("boundary: local provider account state only; no tokens stored by YVEX\n");
        printf("status: %s\n", obs->status);
        return;
    }
    printf("provider: %s\n", obs->provider_name);
    ...
    printf("auth_state: %s\n", obs->auth_state);
    printf("account: %s\n", obs->account_hint);
    if (obs->top_blocker[0]) printf("top_blocker: %s\n", obs->top_blocker);
    if (obs->next[0]) printf("next: %s\n", obs->next);
    printf("boundary: local provider account state only, no tokens stored by YVEX\n");
    printf("status: %s\n", obs->status);
}
...
static int command_accounts_providers(int argc, char **argv)
{
    ...
    if (options.output_mode == YVEX_ACCOUNTS_OUTPUT_TABLE) {
        printf("PROVIDER      ALIAS  CLI      AUTH\n");
        ...
        printf("accounts: providers\n");
        accounts_print_observation_audit("provider_0_", &observations[0]);
        accounts_print_observation_audit("provider_1_", &observations[1]);
        printf("boundary: provider discovery only; no login, no token storage, no download\n");
    } else {
        printf("accounts: providers\n");
        printf("provider: huggingface alias=hf cli=%s\n", observations[0].cli_status);
        printf("provider: github alias=gh cli=%s\n", observations[1].cli_status);
        printf("boundary: provider discovery only; no login, no token storage, no download\n");
    }
    printf("status: account-providers\n");
    return 0;
}
...
static int command_accounts_status(int argc, char **argv)
{
    ...
    if (options.output_mode == YVEX_ACCOUNTS_OUTPUT_TABLE) {
        ...
        printf("raw_token_stored_by_yvex: false\n");
        printf("boundary: local provider account state only, no tokens stored by YVEX\n");
    } else {
        printf("accounts: provider status\n");
        printf("huggingface: %s cli=%s account=%s\n",
               observations[0].auth_state, observations[0].cli_status, observations[0].account_hint);
        printf("github: %s cli=%s account=%s\n",
               observations[1].auth_state, observations[1].cli_status, observations[1].account_hint);
        printf("boundary: local provider account state only, no tokens stored by YVEX\n");
    }
    printf("status: accounts-status\n");
    return 0;
}
