/*
 * yvex_artifact_materialize.c - canonical artifact materialization.
 *
 * Owner:
 *   src/artifact
 *
    ...
    yvex_error *err)
{
    ...
    if (local.require_deepseek_map) {
        const yvex_deepseek_gguf_map_summary *summary =
            yvex_deepseek_gguf_map_summary_get(deepseek_map);
    ...
    for (i = 0ull; i < count; ++i) {
        ...
        descriptor = deepseek_map
            ? yvex_deepseek_gguf_map_find_emitted(deepseek_map, tensor->name)
            : NULL;
        ...
        binding->descriptor_index = descriptor
            ? (unsigned long long)(descriptor - yvex_deepseek_gguf_map_at(deepseek_map, 0ull))
