 ...
 */
#include "cuda_deepseek_attention.h"
...
static int cuda_attention_fail(yvex_cuda_deepseek_failure *failure,
                                   ...
                                   yvex_error *err)
{
    ...
    if (!resources || !out || bytes == 0u ||
        ...
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_INVALID_ARGUMENT, stage,
    ...
    if (resources->current_bytes > ULLONG_MAX - (unsigned long long)bytes)
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_BUDGET, stage, ULLONG_MAX,
    ...
    if (resources->budget && next > resources->budget)
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_BUDGET, stage,
    ...
    if (injected && strcmp(injected, "allocation") == 0)
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_ALLOCATION, stage, bytes,
    ...
    if (source) {
        if (injected && strcmp(injected, "copy-input") == 0)
            return cuda_attention_fail(
                failure, YVEX_CUDA_DEEPSEEK_FAILURE_COPY, stage, bytes, 0ull,
                                  ...
                                  yvex_error *err)
{
    ...
    while (resources->count) {
        ...
        int rc = yvex_cuda_temporary_free(
            resources->backend, YVEX_BACKEND_VARIANT_DEEPSEEK_ATTENTION,
                                 ...
                                 yvex_error *err)
{
    ...
    if (injected && strcmp(injected, stage) == 0)
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_LAUNCH, stage, 1ull, 0ull,
            err, YVEX_ERR_BACKEND,
...
static int cuda_attention_matvec(cuda_attention_resources *resources,
                                 const yvex_cuda_deepseek_weight *weight,
...
static int cuda_attention_rope(cuda_attention_resources *resources,
                               ...
                               const yvex_cuda_deepseek_position *position,
                               int inverse,
                               CUdeviceptr status,
                               const char *stage,
                               yvex_cuda_deepseek_failure *failure,
...
static int cuda_attention_activation(
    ...
    unsigned long long vectors,
    unsigned long long width,
    const yvex_cuda_deepseek_activation *policy,
    CUdeviceptr status,
    const char *stage,
    yvex_cuda_deepseek_failure *failure,
    yvex_error *err)
...
static int cuda_attention_validate_job(
    ...
    yvex_error *err)
{
    const yvex_cuda_deepseek_weight_slot required[] = {
        YVEX_CUDA_DEEPSEEK_WEIGHT_Q_A,
    ...
    if (!job || !output || !job->input || !job->hidden_width || !job->q_rank ||
        ...
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_INVALID_ARGUMENT,
    ...
    for (i = 0u; i < sizeof(required) / sizeof(required[0]); ++i) {
        const yvex_cuda_deepseek_weight *weight = &job->weights[required[i]];
    ...
    yvex_error *err)
{
    ...
    yvex_cuda_deepseek_attention_output committed;
    CUdeviceptr weight[YVEX_CUDA_DEEPSEEK_WEIGHT_COUNT];
    ...
    rc = yvex_cuda_require_capability(
        backend, YVEX_BACKEND_VARIANT_DEEPSEEK_ATTENTION,
        "cuda.deepseek_attention.capability", err);
    if (rc != YVEX_OK)
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_CAPABILITY,
    ...
    if (!cuda_attention_checked_mul(job->query_heads, job->head_dimension,
        ...
        !cuda_attention_checked_mul(job->indexer_heads,
                                    ...
                                    &index_query_extent))
        return cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_BUDGET,
...
#define ALLOC_VALUE(target, count, type, source, zero, stage) do {             \
        if (!cuda_attention_checked_bytes((count), sizeof(type), &bytes)) {     \
            rc = cuda_attention_fail(failure,                                  \
                YVEX_CUDA_DEEPSEEK_FAILURE_BUDGET, stage, ULLONG_MAX, count,   \
    ...
    for (slot = 0u; slot < YVEX_CUDA_DEEPSEEK_WEIGHT_COUNT; ++slot) {
        const yvex_cuda_deepseek_weight *host = &job->weights[slot];
    ...
    rc = cuda_attention_matvec(
        &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_Q_A],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_Q_A], 0ull, job->q_rank, input,
        q_low, device_status, "cuda.deepseek_attention.q_a", failure, err);
    if (rc != YVEX_OK) goto cleanup;
    rc = cuda_attention_weighted_norm(
        &resources, q_low, job->q_rank,
        &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_Q_A_NORM],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_Q_A_NORM], job->rms_epsilon,
        device_status, "cuda.deepseek_attention.q_norm", failure, err);
    if (rc != YVEX_OK) goto cleanup;
    rc = cuda_attention_matvec(
        &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_Q_B],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_Q_B], 0ull, query_width, q_low,
        query, device_status, "cuda.deepseek_attention.q_b", failure, err);
    if (rc != YVEX_OK) goto cleanup;
    rc = cuda_attention_matvec(
        &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_KV],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_KV], 0ull, job->kv_width, input,
        raw_kv, device_status, "cuda.deepseek_attention.kv", failure, err);
    if (rc != YVEX_OK) goto cleanup;
    rc = cuda_attention_weighted_norm(
        &resources, raw_kv, job->kv_width,
        &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_KV_NORM],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_KV_NORM], job->rms_epsilon,
    ...
    rc = cuda_attention_decode(
        &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_SINKS],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_SINKS], 0ull, job->query_heads,
        sinks, device_status, "cuda.deepseek_attention.sinks", failure, err);
    if (rc != YVEX_OK) goto cleanup;

    if (job->attention_class != 0u) {
        int emit;
        const yvex_cuda_deepseek_rolling *rolling = &job->main_rolling;
        void *params[17];
        ...
        rc = cuda_attention_matvec(
            &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_KV],
            weight[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_KV], 0ull,
            rolling->state_width, input, main_kv, device_status,
            "cuda.deepseek_attention.main_kv", failure, err);
        if (rc != YVEX_OK) goto cleanup;
        rc = cuda_attention_matvec(
            &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_GATE],
            weight[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_GATE], 0ull,
            rolling->state_width, input, main_score, device_status,
            "cuda.deepseek_attention.main_gate", failure, err);
        if (rc != YVEX_OK) goto cleanup;
        rc = cuda_attention_decode(
            &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_APE],
            weight[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_APE],
        ...
        if (emit) {
            rc = cuda_attention_weighted_norm(
                ...
                &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_NORM],
                weight[YVEX_CUDA_DEEPSEEK_WEIGHT_MAIN_NORM],
                job->rms_epsilon, device_status,
        ...
        if (job->attention_class == 1u) {
            const yvex_cuda_deepseek_rolling *index = &job->indexer_rolling;
            int index_emit;
            ...
            rc = cuda_attention_matvec(
                &resources,
                &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_KV],
                weight[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_KV], 0ull,
                index->state_width, input, index_kv, device_status,
                "cuda.deepseek_attention.index_kv", failure, err);
            if (rc != YVEX_OK) goto cleanup;
            rc = cuda_attention_matvec(
                &resources,
                &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_GATE],
                weight[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_GATE], 0ull,
                index->state_width, input, index_score, device_status,
                "cuda.deepseek_attention.index_gate", failure, err);
            if (rc != YVEX_OK) goto cleanup;
            rc = cuda_attention_decode(
                &resources,
                &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_APE],
                weight[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_APE],
            ...
            if (index_emit) {
                rc = cuda_attention_weighted_norm(
                    ...
                    &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_NORM],
                    weight[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_NORM],
                    job->rms_epsilon, device_status,
            ...
            rc = cuda_attention_matvec(
                &resources,
                &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_Q],
                weight[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_Q], 0ull,
            ...
            rc = cuda_attention_matvec(
                &resources,
                &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_PROJECTION],
                weight[YVEX_CUDA_DEEPSEEK_WEIGHT_INDEX_PROJECTION], 0ull,
    ...
    for (unsigned long long group = 0ull; group < job->output_groups; ++group) {
        ...
        rc = cuda_attention_matvec(
            &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_OUT_A],
            weight[YVEX_CUDA_DEEPSEEK_WEIGHT_OUT_A],
    ...
    rc = cuda_attention_matvec(
        &resources, &job->weights[YVEX_CUDA_DEEPSEEK_WEIGHT_OUT_B],
        weight[YVEX_CUDA_DEEPSEEK_WEIGHT_OUT_B], 0ull, job->hidden_width,
    ...
    if (rc != YVEX_OK) {
        rc = cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_SYNCHRONIZE,
    ...
    if (host_status != 0) {
        rc = cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_NUMERIC,
    ...
    if (copy_failure && strcmp(copy_failure, "copy-output") == 0) {
        rc = cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_COPY,
...
cleanup:
    ...
    if (rc == YVEX_OK && cleanup_rc != YVEX_OK) {
        rc = cuda_attention_fail(
            failure, YVEX_CUDA_DEEPSEEK_FAILURE_CLEANUP,
