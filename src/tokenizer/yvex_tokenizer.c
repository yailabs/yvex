...
static int command_tokenizer(int arg_count, char **args)
{
    ...
    rc = open_tokenizer_context(args[2], &ctx, &err);
    if (rc != YVEX_OK) {
    ...
    if (yvex_tokenizer_chat_template(ctx.tokenizer, &chat_template, &chat_template_len) == YVEX_OK) {
        (void)chat_template;
        fprintf(stdout, "chat_template: present-unsupported\n");
    } else {
        fprintf(stdout, "chat_template: absent\n");
    }
    fprintf(stdout, "status: tokenizer-descriptor\n");
...
static int command_tokenize(int arg_count, char **args)
{
    ...
    fprintf(stdout, "status: tokenized\n");
...
static int command_detokenize(int arg_count, char **args)
{
    ...
    fprintf(stdout, "\n");
    fprintf(stdout, "status: detokenized\n");
...
static int command_prompt(int arg_count, char **args)
{
    ...
    fprintf(stdout, "template: yvex-default\n");
    fprintf(stdout, "chat_template_metadata: %s\n",
           yvex_tokenizer_chat_template(ctx.tokenizer, &chat_template, &chat_template_len) == YVEX_OK
               ? "present-unsupported"
               : "absent");
    fprintf(stdout, "rendered_bytes: %llu\n", rendered.len);
    ...
    fprintf(stdout, "status: rendered\n");
