...
int yvex_paths_default(yvex_paths *out, yvex_error *err)
{
    ...
    env = env_or_null("YVEX_CONFIG_DIR");
    rc = env ? copy_path(out->config_dir, env, err, "yvex_paths_default")
             : path_format(out->config_dir, YVEX_PATH_CAP, err, "yvex_paths_default", "%s/.config/yvex", home);
    if (rc != YVEX_OK) {
        return rc;
    }

    env = env_or_null("YVEX_CACHE_DIR");
    rc = env ? copy_path(out->cache_dir, env, err, "yvex_paths_default")
             : path_format(out->cache_dir, YVEX_PATH_CAP, err, "yvex_paths_default", "%s/.cache/yvex", home);
    if (rc != YVEX_OK) {
        return rc;
    }

    env = env_or_null("YVEX_STATE_DIR");
    rc = env ? copy_path(out->state_dir, env, err, "yvex_paths_default")
             : path_format(out->state_dir, YVEX_PATH_CAP, err, "yvex_paths_default", "%s/.local/state/yvex", home);
    if (rc != YVEX_OK) {
        return rc;
    }

    env = env_or_null("YVEX_DATA_DIR");
    rc = env ? copy_path(out->data_dir, env, err, "yvex_paths_default")
             : path_format(out->data_dir, YVEX_PATH_CAP, err, "yvex_paths_default", "%s/.local/share/yvex", home);
    if (rc != YVEX_OK) {
        return rc;
    }
...
int yvex_paths_project(yvex_paths *out, const char *project_root, yvex_error *err)
{
    ...
    rc = path_format(out->config_dir, YVEX_PATH_CAP, err, "yvex_paths_project", "%s", out->project_dir);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->cache_dir, YVEX_PATH_CAP, err, "yvex_paths_project", "%s/cache", out->project_dir);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->state_dir, YVEX_PATH_CAP, err, "yvex_paths_project", "%s/state", out->project_dir);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->data_dir, YVEX_PATH_CAP, err, "yvex_paths_project", "%s/data", out->project_dir);
    if (rc != YVEX_OK) {
        return rc;
    }
                                     ...
                                     yvex_error *err)
{
    ...
    if (paths->project_dir[0] != '\0') {
        return path_format(out, cap, err, "operator_paths", "%s/operator-paths.conf", paths->project_dir);
    }
                                    ...
                                    yvex_error *err)
{
    ...
    if (paths->project_dir[0] != '\0') {
        return path_format(out, cap, err, "operator_paths", "%s", paths->project_dir);
    }
                              ...
                              yvex_error *err)
{
    ...
    rc = path_format(out->models_root, sizeof(out->models_root), err,
                          "operator_paths", "%s", models_root);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->hf_root, sizeof(out->hf_root), err,
                          "operator_paths", "%s/hf", models_root);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->gguf_root, sizeof(out->gguf_root), err,
                          "operator_paths", "%s/gguf", models_root);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->reports_root, sizeof(out->reports_root), err,
                          "operator_paths", "%s/reports", models_root);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->reference_root, sizeof(out->reference_root), err,
                          "operator_paths", "%s/reference", models_root);
    if (rc != YVEX_OK) {
        return rc;
    }
    rc = path_format(out->registry_root, sizeof(out->registry_root), err,
                                     ...
                                     yvex_error *err)
{
    ...
    *out_found = 0;
    out_root[0] = '\0';
    rc = operator_config_path(paths, out_config_path, config_cap, err);
    if (rc != YVEX_OK) {
        return rc;
    }
                                ...
                                yvex_error *err)
{
    ...
    rc = operator_config_path(paths, config_path, sizeof(config_path), err);
    if (rc != YVEX_OK) {
        return rc;
    }
                                  ...
                                  yvex_error *err)
{
    ...
    rc = operator_config_dir(paths, config_dir, sizeof(config_dir), err);
    if (rc != YVEX_OK) {
        return rc;
    }
...
int yvex_operator_paths_create(const yvex_operator_paths *operator_paths, yvex_error *err)
{
    ...
    rc = path_format(family_dir, sizeof(family_dir), err, "operator_paths", "%s/deepseek", operator_paths->hf_root);
    if (rc != YVEX_OK) return rc;
    rc = path_format(glm_dir, sizeof(glm_dir), err, "operator_paths", "%s/glm", operator_paths->hf_root);
    if (rc != YVEX_OK) return rc;
    rc = path_format(qwen_dir, sizeof(qwen_dir), err, "operator_paths", "%s/qwen", operator_paths->hf_root);
    if (rc != YVEX_OK) return rc;
    ...
    if (rc != YVEX_OK) return rc;
    rc = path_format(gguf_qwen, sizeof(gguf_qwen), err, "operator_paths", "%s/qwen", operator_paths->gguf_root);
    if (rc != YVEX_OK) return rc;
