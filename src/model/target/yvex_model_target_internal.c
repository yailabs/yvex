...
static int output_contract_measure_stats(
    yvex_output_contract_report report,
    yvex_model_target_output_mode mode,
    const void *profile,
    yvex_output_contract_stats *stats)
...
static const char *output_contract_status(
    ...
    int rendered)
{
    if (!rendered) return "fail-render-path";
    ...
    const void *profile)
{
    ...
    rendered = output_contract_measure_stats(report, mode, profile, &stats);
 ...
 *
 * Effects:
 *   frees output buffers and clears the report by value.
 *
 * Failure:
 *   none.
 *
...
void yvex_model_target_internal_report_close(yvex_model_target_report *report)
{
    ...
    free(report->primary_text);
    free(report->diagnostic_text);
    ...
    yvex_error *err)
{
    ...
    report->primary_text = out_text;
    report->diagnostic_text = err_text;
    report->primary_len = out_len;
    report->diagnostic_len = err_len;
 ...
 *
 * Inputs:
 *   request borrows argc/argv and report receives owned output.
 *
 * Effects:
 *   runs existing model-target report construction against output sinks; it
 ...
 *
 * Failure:
 *   returns invalid-arg, allocation, or sink IO errors through err.
 *
 * Boundary:
 ...
 *
 * Inputs:
 *   report receives owned help text.
 *
 * Effects:
 ...
 *
 * Failure:
 *   returns allocation or sink IO errors through err.
 *
 * Boundary:
