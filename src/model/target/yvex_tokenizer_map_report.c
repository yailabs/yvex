...
static int tokenizer_source_present(const yvex_model_target_request *request,
                                    const char *family)
{
    char path[1024];
    int n;
    ...
    if (request->source_path[0]) {
        return tokenizer_is_dir(request->source_path);
    }
    n = snprintf(path, sizeof(path), "%s/hf/%s/%s",
                 request->models_root, family, request->target_id);
    if (n < 0 || (size_t)n >= sizeof(path)) {
        return 0;
    }
    return tokenizer_is_dir(path);
}
