/*
 * report.c - model-target report coordinator.
 *
 * Owner:
 *   src/model/target
 *
...
void yvex_model_target_report_close(yvex_model_target_report *report)
{
    ...
    yvex_model_register_deepseek_v4()->lowering.close(report->deepseek_gguf_map);
    yvex_model_register_deepseek_v4()->coverage.close(report->deepseek_tensor_coverage);
    yvex_model_register_deepseek_v4()->ir.close(report->deepseek_architecture_ir);
    memset(report, 0, sizeof(*report));
}
