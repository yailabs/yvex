...
#include "yvex_operator_render_private.h"
                                           ...
                                           int selected)
{
    ...
    fprintf(stdout, "  registered_format: %s\n", entry->format ? entry->format : "");
    fprintf(stdout, "  registered_architecture: %s\n", entry->architecture ? entry->architecture : "");
    fprintf(stdout, "  registered_tensor_count: %llu\n", entry->tensor_count);
    fprintf(stdout, "  registered_known_tensor_bytes: %llu\n", entry->known_tensor_bytes);
    fprintf(stdout, "  registered_primary_tensor: %s\n", entry->primary_tensor_name ? entry->primary_tensor_name : "");
    fprintf(stdout, "  registered_primary_role: %s\n", entry->primary_tensor_role ? entry->primary_tensor_role : "");
    fprintf(stdout, "  registered_primary_dtype: %s\n", entry->primary_tensor_dtype ? entry->primary_tensor_dtype : "");
    fprintf(stdout, "  registered_primary_rank: %u\n", entry->primary_tensor_rank);
    fprintf(stdout, "  registered_primary_dims: %s\n", entry->primary_tensor_dims ? entry->primary_tensor_dims : "");
    fprintf(stdout, "  registered_primary_bytes: %llu\n", entry->primary_tensor_bytes);
    fprintf(stdout, "  registered_selected_embedding_ready: %s\n",
...
{
    if (!entry) return;
    fprintf(stdout, "%-3s  %-44s  %-10s  %-16s  %7llu  %12llu  %s\n",
           selected ? "*" : "-",
...
static void print_prepare_common(const yvex_cli_models_prepare_options *options,
                                 ...
                                 const char *registry_path)
{
    fprintf(stdout, "models: prepare\n");
    fprintf(stdout, "target_id: %s\n", options->target);
    fprintf(stdout, "models_root_source: %s\n", operator_paths->models_root_source);
    fprintf(stdout, "models_root: %s\n", operator_paths->models_root);
    fprintf(stdout, "source_path: %s\n", source_path);
    fprintf(stdout, "artifact_path: %s\n", artifact_path);
    fprintf(stdout, "source_manifest_path: %s\n", manifest_path);
    fprintf(stdout, "conversion_plan_path: %s\n", plan_path);
    fprintf(stdout, "registry_path: %s\n", registry_path && registry_path[0] ? registry_path : ".yvex/models.local.json");
    fprintf(stdout, "alias: deepseek4-v4-flash-selected-embed\n");
...
static int print_prepare_unsupported(const char *target)
{
    ...
    if (!reason) {
        fprintf(stdout, "reason: unknown model prepare target\n");
        fprintf(stdout, "status: model-prepare-unknown-target\n");
        return 2;
    }
    fprintf(stdout, "reason: %s\n", reason);
    fprintf(stdout, "status: model-prepare-unsupported\n");
...
static int command_models_prepare(int arg_count, char **args)
{
    ...
    if (options.dry_run) {
        ...
        model_print_runtime_generation("not-performed");
        fprintf(stdout, "status: model-prepare-dry-run\n");
        return 0;
    }
    ...
    if (!source_exists) {
        ...
        model_print_runtime_generation("not-performed");
        fprintf(stdout, "reason: source path does not exist\n");
        fprintf(stdout, "status: model-prepare-fail\n");
        return exit_for_status(YVEX_ERR_IO);
    }
    if (artifact_exists && !options.overwrite) {
        model_stage_print("convert-emit", "refused");
        model_print_runtime_generation("not-performed");
        fprintf(stdout, "reason: artifact exists; pass --overwrite to replace it\n");
        fprintf(stdout, "status: model-prepare-refused\n");
    ...
    fprintf(stdout, "artifact_architecture: %s\n", metadata_snapshot.architecture);
    fprintf(stdout, "artifact_tensor_count: %llu\n", metadata_snapshot.entry.tensor_count);
    ...
    model_print_runtime_generation("not-performed");
    fprintf(stdout, "status: model-prepare\n");
    return 0;
}
...
static const char *model_download_auth_mode_name(yvex_model_download_auth_mode mode)
{
    switch (mode) {
...
static int parse_models_download_options_from(int arg_count,
                                              char **args,
                                              int start_index,
                                              yvex_cli_models_download_options *options)
{
        ...
        if (strcmp(args[i], "--models-root") == 0 ||
            ...
            if (strcmp(flag, "--models-root") == 0) {
                ...
                if (strcmp(value, "release-asset") != 0) {
                    fprintf(stderr, "yvex: models download --github-source supports release-asset only\n");
                    return 2;
                }
    ...
    if (strcmp(options->provider, "github") == 0 && !options->repo) {
        fprintf(stderr, "yvex: models download --provider github requires --repo OWNER/NAME\n");
        return 2;
    }
    if (strcmp(options->provider, "github") == 0 && !options->asset) {
        fprintf(stderr, "yvex: models download --provider github requires --asset GLOB\n");
        return 2;
    }
    if (strcmp(options->provider, "github") == 0 && options->target) {
        fprintf(stderr, "yvex: models download catalog targets use Hugging Face provider in this wave\n");
        return 2;
    }
                                        ...
                                        yvex_error *err)
{
    ...
    fprintf(fp, "provider_cli_path: %s\n", report->provider_cli_path);
    fprintf(fp, "provider_cli_status: %s\n", report->provider_cli_status);
    ...
    if (strcmp(report->provider, "github") == 0) {
        fprintf(fp, "command: %s release download", report->provider_cli_path);
        if (options->release && options->release[0]) fprintf(fp, " %s", options->release);
        fprintf(fp, " --repo %s --pattern %s --dir %s --skip-existing",
    ...
    yvex_model_download_progress_mode effective_mode)
{
    ...
    fprintf(stdout, "model-download: start target=%s\n", report->target_id);
    fprintf(stdout, "provider: %s\n", report->provider);
    fprintf(stdout, "repo: %s\n", report->repo_id);
    fprintf(stdout, "source: %s\n", report->local_source_dir);
    fprintf(stdout, "stage: account-provider %s\n", report->stage_account_provider);
    fprintf(stdout, "stage: download running\n");
    fflush(stdout);
}
                                               ...
                                               yvex_model_download_progress_mode effective_mode)
{
    ...
    fprintf(stdout, "tick: elapsed=%s files=%llu partial=%llu safetensors=%llu bytes=%s delta=%s%s largest=%s (%s)\n",
           elapsed_text,
           scan.file_count,
           scan.partial_file_count,
           scan.safetensors_count,
...
static int model_download_run_github(const yvex_cli_models_download_options *options,
                                     const yvex_model_download_report *report,
                                     yvex_error *err)
{
                                        ...
                                        const yvex_model_download_report *report)
{
    ...
    if (strcmp(report->status, "model-download-dry-run") == 0) {
        fprintf(stdout, "model-download: dry-run target=%s\n", report->target_id);
        fprintf(stdout, "family: %s\n", report->family);
        fprintf(stdout, "provider: %s\n", report->provider);
        fprintf(stdout, "repo: %s\n", report->repo_id);
        fprintf(stdout, "source: %s\n", report->local_source_dir);
        fprintf(stdout, "account_provider: %s\n", report->stage_account_provider);
        fprintf(stdout, "action: planned\n");
        fprintf(stdout, "manifest: skipped\n");
        fprintf(stdout, "native_inventory: skipped\n");
        fprintf(stdout, "boundary: no payload downloaded, runtime unsupported\n");
    ...
    if (strcmp(report->status, "model-download-pass") == 0 ||
        ...
        fprintf(stdout, "files: %llu partial=%llu safetensors=%llu bytes=%s\n",
        ...
        fprintf(stdout, "native_inventory: %s\n",
               report->native_inventory_written ? report->native_inventory_path : "skipped");
        fprintf(stdout, "boundary: source tensors only, runtime unsupported\n");
        fprintf(stdout, "status: %s\n", report->status);
        return;
    }
    if (strcmp(report->status, "model-download-interrupted") == 0) {
        fprintf(stdout, "model-download: interrupted target=%s\n", report->target_id);
        fprintf(stdout, "family: %s\n", report->family);
        fprintf(stdout, "repo: %s\n", report->repo_id);
        fprintf(stdout, "source: %s\n", report->local_source_dir);
        fprintf(stdout, "stage: download interrupted\n");
        fprintf(stdout, "signal: %s\n", model_download_signal_name(report->interrupt_signal));
        fprintf(stdout, "child_signal_forwarded: %s\n", report->signal_forwarded ? "true" : "false");
        fprintf(stdout, "child_exit_status: %s\n", report->child_exit_status);
        model_download_format_bytes(bytes_text, sizeof(bytes_text),
                                    report->source_scan.total_regular_file_bytes);
        ...
        fprintf(stdout, "files: %llu partial=%llu safetensors=%llu bytes=%s\n",
               ...
               report->source_scan.safetensors_count,
               bytes_text);
        fprintf(stdout, "largest: %s (%s)\n", largest_name, largest_text);
        fprintf(stdout, "cleanup: preserved-partial-source\n");
        fprintf(stdout, "lock_cleanup: not-attempted\n");
        fprintf(stdout, "partial_source_preserved: %s\n",
               report->partial_source_preserved ? "true" : "false");
        fprintf(stdout, "lock_files_deleted: %s\n", report->lock_files_deleted ? "true" : "false");
        fprintf(stdout, "stdout_log: %s\n", report->stdout_log_path);
        fprintf(stdout, "stderr_log: %s\n", report->stderr_log_path);
        fprintf(stdout, "boundary: partial source files may exist; runtime unsupported\n");
        fprintf(stdout, "status: %s\n", report->status);
        return;
    }

    fprintf(stdout, "model-download: fail target=%s\n", report->target_id);
    fprintf(stdout, "family: %s\n", report->family);
    fprintf(stdout, "provider: %s\n", report->provider);
    fprintf(stdout, "repo: %s\n", report->repo_id);
    fprintf(stdout, "source: %s\n", report->local_source_dir);
    fprintf(stdout, "hf_exit_code: %d\n", report->hf_exit_code);
    fprintf(stdout, "stderr_log: %s\n", report->stderr_log_path);
    if (report->error[0]) fprintf(stdout, "reason: %s\n", report->error);
    fprintf(stdout, "boundary: no runtime, no generation, no benchmark\n");
    fprintf(stdout, "status: %s\n", report->status);
    (void)options;
}
...
static void model_download_print_table(const yvex_model_download_report *report)
{
    ...
    model_download_format_bytes(bytes_text, sizeof(bytes_text),
                                report->source_scan.total_regular_file_bytes);
    fprintf(stdout, "TARGET       PROVIDER     FAMILY  ACCOUNT  STATUS                       FILES  PARTIAL  SAFETENSORS  BYTES\n");
    fprintf(stdout, "%-12s %-11s  %-6s  %-7s  %-27s  %5llu  %7llu  %11llu  %s\n",
           report->target_id,
           report->provider,
           report->family,
           report->stage_account_provider,
...
static void model_download_print_audit(const yvex_cli_models_download_options *options,
                                       const yvex_model_download_report *report)
{
    fprintf(stdout, "models: download\n");
    fprintf(stdout, "status: %s\n", report->status);
    fprintf(stdout, "target_id: %s\n", report->target_id);
    fprintf(stdout, "family: %s\n", report->family);
    fprintf(stdout, "provider: %s\n", report->provider);
    fprintf(stdout, "repo_id: %s\n", report->repo_id);
    fprintf(stdout, "revision: %s\n", report->revision);
    fprintf(stdout, "local_source_dir: %s\n", report->local_source_dir);
    fprintf(stdout, "models_root: %s\n", report->models_root);
    fprintf(stdout, "models_root_source: %s\n", report->models_root_source);
    fprintf(stdout, "provider_cli: %s\n", report->provider_cli_path[0] ? report->provider_cli_path : "missing");
    fprintf(stdout, "provider_cli_source: %s\n", report->provider_cli_source[0] ? report->provider_cli_source : "none");
    fprintf(stdout, "provider_cli_status: %s\n", report->provider_cli_status);
    fprintf(stdout, "account_hint: %s\n", report->account_hint);
    fprintf(stdout, "credential_source: %s\n", report->credential_source);
    fprintf(stdout, "accounts_state_path: %s\n", report->accounts_state_path);
    fprintf(stdout, "auth_mode: %s\n", model_download_auth_mode_name(options->auth_mode));
    fprintf(stdout, "hf_cli: %s\n", report->hf_cli_path[0] ? report->hf_cli_path : "missing");
    fprintf(stdout, "hf_cli_source: %s\n", report->hf_cli_source[0] ? report->hf_cli_source : "none");
    fprintf(stdout, "hf_exit_code: %d\n", report->hf_exit_code);
    fprintf(stdout, "provider_exit_code: %d\n", report->provider_exit_code);
    fprintf(stdout, "auth_state: %s\n", report->auth_state);
    fprintf(stdout, "token_env_name: %s\n", report->token_env_name);
    fprintf(stdout, "token_value_redacted: %s\n",
           strcmp(report->auth_state, "env-token-present") == 0 ? "true" : "false");
    model_download_print_audit_patterns(options);
    fprintf(stdout, "progress_mode: %s\n", model_download_progress_mode_name(options->progress_mode));
    fprintf(stdout, "tick_seconds: %llu\n", options->tick_seconds);
    fprintf(stdout, "tick_count: %llu\n", report->tick_count);
    fprintf(stdout, "stdout_streamed: %s\n", report->stdout_streamed ? "true" : "false");
    fprintf(stdout, "stderr_streamed: %s\n", report->stderr_streamed ? "true" : "false");
    fprintf(stdout, "stdout_bytes: %llu\n", report->stdout_bytes);
    fprintf(stdout, "stderr_bytes: %llu\n", report->stderr_bytes);
    ...
    fprintf(stdout, "provider_pid: %lld\n", (long long)report->provider_pid);
    fprintf(stdout, "provider_process_group: %lld\n", (long long)report->provider_process_group);
    fprintf(stdout, "interrupted: %s\n", report->interrupted ? "true" : "false");
    fprintf(stdout, "interrupt_signal: %s\n", model_download_signal_name(report->interrupt_signal));
    fprintf(stdout, "signal: %s\n", model_download_signal_name(report->interrupt_signal));
    fprintf(stdout, "signal_forwarded: %s\n", report->signal_forwarded ? "true" : "false");
    fprintf(stdout, "child_signal_forwarded: %s\n", report->signal_forwarded ? "true" : "false");
    fprintf(stdout, "child_terminated: %s\n", report->child_terminated ? "true" : "false");
    fprintf(stdout, "child_killed_after_timeout: %s\n",
           report->child_killed_after_timeout ? "true" : "false");
    fprintf(stdout, "child_exit_status: %s\n", report->child_exit_status);
    fprintf(stdout, "orphan_check_performed: %s\n",
           report->orphan_check_performed ? "true" : "false");
    fprintf(stdout, "orphan_check_status: %s\n", report->orphan_check_status);
    fprintf(stdout, "partial_source_preserved: %s\n",
           report->partial_source_preserved ? "true" : "false");
    fprintf(stdout, "lock_cleanup: not-attempted\n");
    fprintf(stdout, "lock_files_deleted: %s\n", report->lock_files_deleted ? "true" : "false");
    fprintf(stdout, "source_file_count: %llu\n", report->source_scan.file_count);
    fprintf(stdout, "file_count: %llu\n", report->source_scan.file_count);
    fprintf(stdout, "safetensors_count: %llu\n", report->source_scan.safetensors_count);
    fprintf(stdout, "config_present: %s\n", report->source_scan.config_present ? "true" : "false");
    fprintf(stdout, "tokenizer_present: %s\n", report->source_scan.tokenizer_present ? "true" : "false");
    fprintf(stdout, "total_regular_file_bytes: %llu\n", report->source_scan.total_regular_file_bytes);
    fprintf(stdout, "largest_file_name: %s\n",
           report->source_scan.largest_file_name[0] ? report->source_scan.largest_file_name : "none");
    fprintf(stdout, "largest_file_bytes: %llu\n", report->source_scan.largest_file_bytes);
    fprintf(stdout, "manifest_path: %s\n", report->manifest_path);
    ...
    fprintf(stdout, "receipt_path: %s\n", report->receipt_path);
    fprintf(stdout, "stdout_log: %s\n", report->stdout_log_path);
    fprintf(stdout, "stderr_log: %s\n", report->stderr_log_path);
    fprintf(stdout, "created_at: %s\n", report->created_at);
    fprintf(stdout, "yvex_version: %s\n", yvex_version_string());
    fprintf(stdout, "upstream_identity_verified: false\n");
    fprintf(stdout, "remote_lookup_performed: false\n");
    fprintf(stdout, "payload_hash_verified: false\n");
    fprintf(stdout, "payload_loaded: false\n");
    fprintf(stdout, "gguf_created: false\n");
    fprintf(stdout, "materialized: false\n");
    fprintf(stdout, "runtime_ready: false\n");
    fprintf(stdout, "generation: unsupported\n");
    fprintf(stdout, "eval: unsupported\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    if (report->top_blocker[0]) fprintf(stdout, "top_blocker: %s\n", report->top_blocker);
    if (report->error[0]) fprintf(stdout, "reason: %s\n", report->error);
}
...
{
    fprintf(stdout, "artifacts\n\n");
    fprintf(stdout, "%-42s  %-9s  %-18s  %-8s  %-8s  %-8s  %s\n",
                                 ...
                                 const yvex_models_artifact_rows *rows)
{
    ...
    if (options->output_mode == YVEX_ARTIFACTS_OUTPUT_AUDIT) {
        fprintf(stdout, "\nmodels_root: %s\n", operator_paths->models_root);
        fprintf(stdout, "gguf_root: %s\n", operator_paths->gguf_root);
        fprintf(stdout, "reports_root: %s\n", operator_paths->reports_root);
        fprintf(stdout, "registry_root: %s\n", operator_paths->registry_root);
        for (i = 0; i < rows->count; ++i) {
            const yvex_models_artifact_row *row = &rows->rows[i];
            fprintf(stdout, "artifact.%u.target_id: %s\n", i, row->target_id);
            fprintf(stdout, "artifact.%u.family: %s\n", i, row->family);
            fprintf(stdout, "artifact.%u.source_status: %s\n", i, row->source_status);
            fprintf(stdout, "artifact.%u.expected_artifact_path: %s\n", i,
                   row->expected_path[0] ? row->expected_path : row->path);
            fprintf(stdout, "artifact.%u.download_registry_path: %s\n", i,
                   row->registry_path[0] ? row->registry_path : "none");
            fprintf(stdout, "artifact.%u.source_manifest_path: %s\n", i,
                   row->source_manifest_path[0] ? row->source_manifest_path : "none");
            fprintf(stdout, "artifact.%u.native_inventory_path: %s\n", i,
                   row->native_inventory_path[0] ? row->native_inventory_path : "none");
            fprintf(stdout, "artifact.%u.tensor_map_path: %s\n", i,
                   row->tensor_map_path[0] ? row->tensor_map_path : "none");
            fprintf(stdout, "artifact.%u.output_head_map_path: %s\n", i,
                   row->output_head_map_path[0] ? row->output_head_map_path : "none");
            fprintf(stdout, "artifact.%u.tokenizer_map_path: %s\n", i,
                   row->tokenizer_map_path[0] ? row->tokenizer_map_path : "none");
            fprintf(stdout, "artifact.%u.top_blocker: %s\n", i, row->top_blocker);
            fprintf(stdout, "artifact.%u.detail: %s\n", i, row->detail);
        }
        fprintf(stdout, "source_payload_loaded: false\n");
        fprintf(stdout, "hash_performed: false\n");
        fprintf(stdout, "runtime_ready: false\n");
        fprintf(stdout, "generation: unsupported\n");
        fprintf(stdout, "benchmark_status: not-measured\n");
    }
                                   ...
                                   const yvex_models_artifact_row *row)
{
    ...
    if (options->output_mode == YVEX_ARTIFACTS_OUTPUT_JSON) {
        fprintf(stdout, "{\"status\":\"artifacts-status\",\"target_id\":");
        artifacts_json_string(row->target_id);
        fprintf(stdout, ",\"family\":");
        artifacts_json_string(row->family);
        fprintf(stdout, ",\"source_status\":");
        artifacts_json_string(row->source_status);
        fprintf(stdout, ",\"artifact_status\":");
        artifacts_json_string(row->artifact_status);
        fprintf(stdout, ",\"expected_artifact_path\":");
    ...
    fprintf(stdout, "artifact: %s\n", row->target_id);
    fprintf(stdout, "family: %s\n", row->family);
    fprintf(stdout, "class: %s\n", row->artifact_class[0] ? row->artifact_class : "unknown-gguf");
    fprintf(stdout, "source: %s\n", row->source_status);
    fprintf(stdout, "artifact_status: %s\n", row->artifact_status);
    fprintf(stdout, "expected: %s\n", row->expected_path[0] ? row->expected_path : row->path);
    fprintf(stdout, "prepare: %s\n", row->prepare_status);
    fprintf(stdout, "top_blocker: %s\n", row->top_blocker);
    if (strcmp(row->prepare_status, "blocked") == 0) {
        fprintf(stdout, "next: %s\n", artifacts_row_next(row));
    }
    fprintf(stdout, "detail: %s\n", row->detail);
    fprintf(stdout, "boundary: artifact discovery only; no runtime/generation\n");
    if (options->output_mode == YVEX_ARTIFACTS_OUTPUT_AUDIT) {
        fprintf(stdout, "models_root: %s\n", operator_paths->models_root);
        fprintf(stdout, "gguf_root: %s\n", operator_paths->gguf_root);
        fprintf(stdout, "artifact_class: %s\n", row->artifact_class);
        fprintf(stdout, "artifact_path: %s\n", row->path[0] ? row->path : "missing");
        fprintf(stdout, "artifact_size: %s\n", size_text);
        fprintf(stdout, "source_manifest_path: %s\n",
               row->source_manifest_path[0] ? row->source_manifest_path : "none");
        fprintf(stdout, "native_inventory_path: %s\n",
               row->native_inventory_path[0] ? row->native_inventory_path : "none");
    ...
    fprintf(stdout, "status: artifacts-status\n");
}

static int command_models_artifacts(int arg_count, char **args)
{
    yvex_cli_models_artifacts_options options;
    yvex_paths paths;
    yvex_operator_paths operator_paths;
    ...
    pid_t provider_pgid)
{
    ...
    if (options && options->output_mode == YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "models: download status\n");
        fprintf(stdout, "target_id: %s\n", report->target_id);
        fprintf(stdout, "family: %s\n", report->family);
        fprintf(stdout, "provider: %s\n", report->provider);
        fprintf(stdout, "repo_id: %s\n", report->repo_id);
        ...
        for (i = 0; i < report->source_scan.lock_count && i < YVEX_MODEL_DOWNLOAD_PATTERN_CAP; ++i) {
            ...
            if (path_join2(abs_lock, sizeof(abs_lock), report->local_source_dir,
                ...
                fprintf(stdout, "lock.%llu.path: %s\n", i, abs_lock);
            }
            fprintf(stdout, "lock.%llu.age_seconds: %llu\n", i, report->source_scan.lock_age_seconds[i]);
        }
        fprintf(stdout, "source_file_count: %llu\n", report->source_scan.file_count);
        fprintf(stdout, "safetensors_count: %llu\n", report->source_scan.safetensors_count);
        fprintf(stdout, "total_regular_file_bytes: %llu\n", report->source_scan.total_regular_file_bytes);
        fprintf(stdout, "largest_file_name: %s\n",
               report->source_scan.largest_file_name[0] ? report->source_scan.largest_file_name : "none");
        fprintf(stdout, "largest_file_bytes: %llu\n", report->source_scan.largest_file_bytes);
        fprintf(stdout, "partial_file_count: %llu\n", report->source_scan.partial_file_count);
        fprintf(stdout, "cache_file_count: %llu\n", report->source_scan.cache_file_count);
        fprintf(stdout, "stdout_log: %s\n", report->stdout_log_path);
        fprintf(stdout, "stderr_log: %s\n", report->stderr_log_path);
        fprintf(stdout, "source_manifest_path: %s\n", report->manifest_path);
        fprintf(stdout, "native_inventory_path: %s\n", report->native_inventory_path);
        fprintf(stdout, "safetensors_header_checked: %s\n", safe_check->checked ? "true" : "false");
        fprintf(stdout, "safetensors_size_status: %s\n", safe_check->status);
        fprintf(stdout, "resume_available: %s\n", resume_available ? "true" : "false");
        fprintf(stdout, "stop_available: %s\n", stop_available ? "true" : "false");
        fprintf(stdout, "runtime_ready: false\n");
        fprintf(stdout, "generation: unsupported\n");
        fprintf(stdout, "benchmark_status: not-measured\n");
        fprintf(stdout, "status: model-download-status\n");
        return;
    }
    ...
    fprintf(stdout, "model-download-status: target=%s\n", report->target_id);
    fprintf(stdout, "provider: %s\n", report->provider);
    fprintf(stdout, "repo: %s\n", report->repo_id);
    fprintf(stdout, "source: %s\n", report->local_source_dir);
    fprintf(stdout, "process: %s\n", process_alive ? "active" : "none");
    fprintf(stdout, "last_status: %s\n",
           last_receipt_present ? (last_receipt_status && last_receipt_status[0]
                                   ? last_receipt_status : "unknown") : "none");
    fprintf(stdout, "locks: %llu\n", report->source_scan.lock_count);
    fprintf(stdout, "files: %llu partial=%llu safetensors=%llu bytes=%s\n",
           report->source_scan.file_count,
           report->source_scan.partial_file_count,
           report->source_scan.safetensors_count,
    ...
    fprintf(stdout, "boundary: source state only, runtime unsupported\n");
    fprintf(stdout, "status: model-download-status\n");
}
...
static int command_models_download_stop(int arg_count, char **args)
{
    ...
    if (pgid <= 0 || !model_download_pgid_alive(pgid)) {
        fprintf(stdout, "model-download-stop: target=%s\n", report.target_id);
        fprintf(stdout, "process: none\n");
        fprintf(stdout, "action: none\n");
        fprintf(stdout, "next: yvex models download status %s --models-root %s\n",
               report.target_id, report.models_root);
        fprintf(stdout, "status: model-download-stop-none\n");
        return 0;
    }
    if (!options.dry_run) {
    ...
    fprintf(stdout, "process_group: %lld\n", (long long)pgid);
    fprintf(stdout, "child_signal_forwarded: %s\n", signaled ? "true" : "false");
    fprintf(stdout, "child_killed_after_timeout: %s\n", killed ? "true" : "false");
    fprintf(stdout, "cleanup: preserved-partial-source\n");
    fprintf(stdout, "lock_cleanup: not-attempted\n");
    fprintf(stdout, "partial_source_preserved: true\n");
    fprintf(stdout, "lock_files_deleted: false\n");
    fprintf(stdout, "boundary: partial source files may exist; runtime unsupported\n");
                                            ...
                                            unsigned long long *deleted_out)
{
    ...
    for (i = 0; i < report->source_scan.lock_count && i < YVEX_MODEL_DOWNLOAD_PATTERN_CAP; ++i) {
        ...
        fprintf(stdout, "delete_candidate.%llu: %s\n",
...
static int command_models_download_cleanup(int arg_count, char **args)
{
    ...
    if (!model_download_source_path_allowed(&operator_paths, report.local_source_dir, &report)) {
        fprintf(stdout, "model-download-cleanup: target=%s\n", report.target_id);
        fprintf(stdout, "top_blocker: %s\n",
        ...
        fprintf(stdout, "deleted: 0\n");
        fprintf(stdout, "status: model-download-cleanup-blocked\n");
        return exit_for_status(YVEX_ERR_UNSUPPORTED);
    }
    yvex_error_clear(&err);
    (void)model_download_scan_source(report.local_source_dir, &report.source_scan, &err);
    ...
    fprintf(stdout, "model-download-cleanup: target=%s\n", report.target_id);
    if (match.count > 0u) {
        fprintf(stdout, "provider_process_alive: true\n");
        fprintf(stdout, "deleted: 0\n");
        fprintf(stdout, "status: model-download-cleanup-blocked\n");
        return exit_for_status(YVEX_ERR_UNSUPPORTED);
    }
    ...
    fprintf(stdout, "stale_locks: %llu\n", report.source_scan.lock_count);
    fprintf(stdout, "cleanup_stale_locks: %s\n", options.cleanup_stale_locks ? "true" : "false");
    fprintf(stdout, "cleanup_failed_partials: %s\n", options.cleanup_failed_partials ? "true" : "false");
    fprintf(stdout, "cleanup_sidecars: %s\n", sidecars_requested ? "true" : "false");
    fprintf(stdout, "cleanup_logs: %s\n", logs_requested ? "true" : "false");
    fprintf(stdout, "cleanup_provider_cache: %s\n", options.cleanup_all_provider_cache ? "true" : "false");
    fprintf(stdout, "delete_candidates: %llu\n", candidates);
    fprintf(stdout, "missing: %llu\n", missing);
    fprintf(stdout, "failed_deletes: %llu\n", failed_deletes);
    fprintf(stdout, "deleted_locks: %llu\n", lock_deleted);
    fprintf(stdout, "deleted_source_entries: %llu\n", source_deleted);
    fprintf(stdout, "deleted_sidecars: %llu\n", sidecar_deleted);
    fprintf(stdout, "deleted_logs: %llu\n", log_deleted);
    fprintf(stdout, "deleted_provider_cache_entries: %llu\n", cache_deleted);
    fprintf(stdout, "deleted: %llu\n", deleted);
    fprintf(stdout, "dry_run: %s\n", options.dry_run ? "true" : "false");
    fprintf(stdout, "yes: %s\n", options.yes ? "true" : "false");
    fprintf(stdout, "boundary: cleanup removes only target download state under models-root; runtime unsupported\n");
    fprintf(stdout, "status: %s\n", cleanup_status);
...
static int command_models_download_execute(int arg_count, char **args, int start_index, int resume_mode)
{
    ...
    if (options.repo) {
        ...
        if (resolved_dynamic) {
            ...
            fprintf(stdout, "reason: unknown models download target\n");
            fprintf(stdout, "status: model-download-unknown-target\n");
            return 2;
        }
    }
...
static int command_models_download(int arg_count, char **args)
{
    if (arg_count >= 4) {
...
static int print_model_check_unsupported(const yvex_cli_models_check_options *options)
{
    ...
    if (options && options->output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "model-check: unsupported target=%s level=%s\n",
               target,
               options->level_name ? options->level_name : "quick");
        ...
        fprintf(stdout, "boundary: report-only refusal, generation unsupported\n");
        fprintf(stdout, "status: model-check-unsupported\n");
    } else {
        fprintf(stdout, "status: model-check-unsupported\n");
        fprintf(stdout, "target_id: %s\n", target);
        if (strcmp(target, "deepseek4-v4-flash-selected-embed-rmsnorm") == 0) {
            fprintf(stdout, "reason: segment check is planned, not implemented by this preset\n");
        } else {
            fprintf(stdout, "reason: source-only target cannot be checked as a YVEX-produced runtime artifact yet\n");
        }
...
static int command_models_check(int arg_count, char **args)
{
    yvex_cli_models_check_options options;
    yvex_cli_model_check_report report;
    yvex_model_ref ref;
...
static int command_models_scan(int arg_count, char **args)
{
    ...
    if (rc != YVEX_OK) return print_yvex_error(&err, exit_for_status(rc));
    fprintf(stdout, "models: scan\n");
    fprintf(stdout, "root: %s\n", root);
    for (i = 0; i < count; ++i) {
        if (i > 0) fprintf(stdout, "\n");
        print_model_registry_scan_entry_cli(&entries[i]);
    }
    fprintf(stdout, "candidates: %llu\n", count);
    fprintf(stdout, "status: models-scan\n");
    yvex_model_registry_scan_free(entries, count);
    return 0;
}
...
static int command_models_add(int arg_count, char **args)
{
    ...
    fprintf(stdout, "registered_selected_embedding_ready: %s\n",
           entry.selected_embedding_ready ? "true" : "false");
    fprintf(stdout, "registered_selected_embedding_hidden_size: %llu\n",
           entry.selected_embedding_hidden_size);
    fprintf(stdout, "registered_selected_embedding_vocab_size: %llu\n",
           entry.selected_embedding_vocab_size);
    fprintf(stdout, "registered_selected_embedding_output_count: %llu\n",
           entry.selected_embedding_output_count);
    fprintf(stdout, "registered_selected_embedding_slice_bytes: %llu\n",
           entry.selected_embedding_slice_bytes);
    fprintf(stdout, "identity_status: recorded\n");
    fprintf(stdout, "status: models-added\n");
    yvex_model_registry_close(registry);
    return 0;
}
...
static int command_models_list(int arg_count, char **args)
{
    ...
    if (output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "MODELS  count=%llu\n\n", count);
        fprintf(stdout, "%-3s  %-44s  %-10s  %-16s  %7s  %12s  %s\n",
               "SEL",
               "ALIAS",
               "FAMILY",
               "CLASS",
        ...
        fprintf(stdout, "status: models-list\n");
        yvex_model_registry_close(registry);
        return 0;
    }
    fprintf(stdout, "models: list\n");
    for (i = 0; i < count; ++i) {
        const yvex_model_registry_entry *entry = yvex_model_registry_at(registry, i);
        int is_selected = selected_alias[0] && entry && strcmp(selected_alias, entry->alias) == 0;
...
static int command_models_use(int arg_count, char **args)
{
    ...
    fprintf(stdout, "models: use\n");
    fprintf(stdout, "selected: %s\n", alias);
    fprintf(stdout, "status: models-selected\n");
    yvex_model_registry_close(registry);
    return 0;
}
...
static int command_models_current(int arg_count, char **args)
{
    ...
    if (rc != YVEX_OK) return print_yvex_error(&err, exit_for_status(rc));
    selected = yvex_model_registry_selected(registry);
    fprintf(stdout, "models: current\n");
    if (selected) {
        if (output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
            fprintf(stdout, "selected: %s\n", selected->alias);
            fprintf(stdout, "path: %s\n", selected->path);
            fprintf(stdout, "execution_ready: %s\n", selected->execution_ready ? "true" : "false");
            fprintf(stdout, "status: models-current\n");
            fprintf(stdout, "hint: use --" "audit for registered digest and tensor fields\n");
            yvex_model_registry_close(registry);
            return 0;
        }
        fprintf(stdout, "selected: %s\n", selected->alias);
        ...
        fprintf(stdout, "selected: none\n");
        fprintf(stdout, "status: models-none\n");
        if (output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
            fprintf(stdout, "hint: use 'yvex models use ALIAS' to select a model\n");
        }
    }
    yvex_model_registry_close(registry);
    return 0;
...
static int command_models_verify(int arg_count, char **args)
{
    ...
    if (output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "verify: %s alias=%s\n", pass ? "pass" : "fail", entry->alias);
        fprintf(stdout, "identity: %s digest: %s metadata: %s\n",
               identity_status, digest_status, metadata_status);
        fprintf(stdout, "artifact: %s tensors=%llu size=%llu\n",
               entry->format ? entry->format : "unknown",
               metadata_checked ? current_metadata.entry.tensor_count : entry->tensor_count,
               identity.file_size);
        fprintf(stdout, "top_blocker: %s\n", pass ? "none" : reason);
        fprintf(stdout, "boundary: identity verified, runtime generation unsupported\n");
        fprintf(stdout, "status: %s\n", status);
    } else {
        fprintf(stdout, "models: verify\n");
        fprintf(stdout, "alias: %s\n", entry->alias);
        fprintf(stdout, "path: %s\n", entry->path);
        fprintf(stdout, "registered_sha256: %s\n", entry->sha256 && entry->sha256[0] ? entry->sha256 : "absent");
...
static int command_models_remove(int arg_count, char **args)
{
    ...
    fprintf(stdout, "models: remove\n");
    fprintf(stdout, "removed: %s\n", alias);
    fprintf(stdout, "status: models-removed\n");
    yvex_model_registry_close(registry);
    return 0;
}
...
static int command_models_inspect(int arg_count, char **args)
{
    ...
    if (output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        ...
        fprintf(stdout, "model: %s\n", entry->alias);
        fprintf(stdout, "family: %s class=%s tensors=%llu size=%llu\n",
               display_family,
               display_class,
               entry->tensor_count,
               entry->file_size);
        fprintf(stdout, "primary: %s %s %s\n",
               entry->primary_tensor_name ? entry->primary_tensor_name : "",
               entry->primary_tensor_dtype ? entry->primary_tensor_dtype : "",
               entry->primary_tensor_dims ? entry->primary_tensor_dims : "");
        fprintf(stdout, "state: %s execution_ready=%s\n",
               entry->support_level ? entry->support_level : "",
               entry->execution_ready ? "true" : "false");
        fprintf(stdout, "boundary: selected-slice only, full-runtime generation unsupported\n");
        fprintf(stdout, "status: models-inspect\n");
        yvex_model_registry_close(registry);
        return 0;
    }
    fprintf(stdout, "models: inspect\n");
    fprintf(stdout, "alias: %s\n", entry->alias);
    fprintf(stdout, "path: %s\n", entry->path);
    fprintf(stdout, "family: %s\n", entry->family);
    fprintf(stdout, "model: %s\n", entry->model);
    ...
    fprintf(stdout, "registered_tensor_count: %llu\n", entry->tensor_count);
    fprintf(stdout, "registered_known_tensor_bytes: %llu\n", entry->known_tensor_bytes);
    fprintf(stdout, "primary_tensor_name: %s\n", entry->primary_tensor_name ? entry->primary_tensor_name : "");
...
static int parse_fullmodel_options(int arg_count,
                                   char **args,
...
static void print_fullmodel_common_boundaries(void)
{
    ...
    fprintf(stdout, "decode_ready: false\n");
    fprintf(stdout, "logits_ready: false\n");
    fprintf(stdout, "sampling_ready: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "full_model_materialization: planned\n");
    fprintf(stdout, "full_runtime_descriptor: planned\n");
    fprintf(stdout, "generation_ready: false\n");
...
static void fullmodel_print_descriptor_collection(const char *name,
                                                  ...
                                                  const char *blocker)
{
    fprintf(stdout, "collection.%s.status: %s\n", name, count > 0ull ? "present" : "missing");
    fprintf(stdout, "collection.%s.tensor_count: %llu\n", name, count);
    fprintf(stdout, "collection.%s.byte_count: %llu\n", name, bytes);
    fprintf(stdout, "collection.%s.required_for_prefill: %s\n", name, required_for_prefill ? "true" : "false");
    fprintf(stdout, "collection.%s.required_for_decode: %s\n", name, required_for_decode ? "true" : "false");
    fprintf(stdout, "collection.%s.required_for_logits: %s\n", name, required_for_logits ? "true" : "false");
    fprintf(stdout, "collection.%s.required_for_generation: %s\n", name, required_for_generation ? "true" : "false");
    fprintf(stdout, "collection.%s.runtime_consumer: %s\n", name, runtime_consumer ? runtime_consumer : "planned");
    fprintf(stdout, "collection.%s.blocker: %s\n", name, blocker && blocker[0] ? blocker : "none");
}
...
static void fullmodel_print_descriptor_phase(unsigned int index,
                                             ...
                                             const char *status)
{
    fprintf(stdout, "descriptor_phase.%u.name: %s\n", index, name ? name : "");
    fprintf(stdout, "descriptor_phase.%u.status: %s\n", index, status ? status : "planned");
}
...
static void fullmodel_print_descriptor_graph_requirements(const yvex_fullmodel_collections *collections)
{
    ...
    fprintf(stdout, "graph_requirements_status: blocked\n");
    fprintf(stdout, "required_graph_ops: embedding-lookup,rmsnorm,q-projection,k-projection,v-projection,rope-position,attention-score,causal-mask,softmax,attention-value-accumulation,o-projection,residual-add,mlp-gate-up-down,activation,moe-router,expert-dispatch,expert-accumulation,final-norm,output-head-projection\n");
    fprintf(stdout, "unsupported_graph_ops: full-transformer-attention,real-layer-scheduler,real-moe-router,real-expert-dispatch,real-output-head-projection\n");
    fprintf(stdout, "required_backend_ops: tensor-read,rmsnorm,matmul,rope,attention,softmax,activation,residual-add,kv-read,kv-write\n");
    fprintf(stdout, "unsupported_backend_ops: full-transformer-runtime-integration,real-attention-backed-kv,real-output-head-logits\n");
    fprintf(stdout, "graph.embedding_lookup: %s\n",
           collections && collections->has_token_embedding ? "planned-real-tensor" : "missing-tensor");
    fprintf(stdout, "graph.rmsnorm: %s\n",
           fullmodel_has_normalization_collection(collections) ? "implemented-selected-segment" : "missing-tensor");
    fprintf(stdout, "graph.q_projection: %s\n", collections && collections->has_attention_q ? "planned" : "missing-tensor");
    fprintf(stdout, "graph.k_projection: %s\n", collections && collections->has_attention_k ? "planned" : "missing-tensor");
    fprintf(stdout, "graph.v_projection: %s\n", collections && collections->has_attention_v ? "planned" : "missing-tensor");
    fprintf(stdout, "graph.rope_position_op: implemented-primitive\n");
    fprintf(stdout, "graph.attention_primitive: implemented-fixture\n");
    fprintf(stdout, "graph.full_transformer_attention: %s\n", has_attention ? "unsupported" : "missing-tensor");
    fprintf(stdout, "graph.o_projection: %s\n", collections && collections->has_attention_out ? "planned" : "missing-tensor");
    fprintf(stdout, "graph.residual_add: planned\n");
    fprintf(stdout, "graph.mlp_primitive: implemented-fixture\n");
    fprintf(stdout, "graph.full_transformer_mlp: %s\n", has_mlp ? "unsupported" : "missing-tensor");
    fprintf(stdout, "graph.moe_router: %s\n",
           collections && collections->has_moe_router ? "planned" : "missing-tensor");
    fprintf(stdout, "graph.expert_dispatch: %s\n",
           collections && collections->has_moe_expert ? "planned" : "missing-tensor");
    fprintf(stdout, "graph.output_head_projection: %s\n",
                                              ...
                                              int selected_target)
{
    ...
    fprintf(stdout, "fullmodel: descriptor\n");
    fprintf(stdout, "status: fullmodel-descriptor\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
    fprintf(stdout, "model_resolved_path: %s\n", ref && ref->path ? ref->path : "");
    ...
    fprintf(stdout, "artifact_identity_status: %s\n", fullmodel_identity_status(ref, artifact_bytes));
    fprintf(stdout, "tensor_inventory_status: pass\n");
    fprintf(stdout, "materialization_plan_status: %s\n", materialization_plan_status);
    fprintf(stdout, "materialization_proof_status: %s\n", materialization_proof_status);
    fprintf(stdout, "runtime_descriptor: report-only\n");
    fprintf(stdout, "runtime_descriptor_status: %s\n", descriptor_status);
    fprintf(stdout, "runtime_descriptor_kind: fullmodel-planning\n");
    fprintf(stdout, "family: %s\n", fullmodel_family_from_arch(arch));
    fprintf(stdout, "architecture: %s\n", yvex_arch_name(arch));
    fprintf(stdout, "model_class: %s\n", selected_target ? "selected-runtime-slice" : "descriptor-only-candidate");
    fprintf(stdout, "full_runtime_model: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "full_model_materialization: %s\n", full_materialization);
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "tensor_count: %llu\n", tensor_count);
    ...
    fprintf(stdout, "tensor_collection_map_status: %s\n", descriptor_status);
    fprintf(stdout, "required_role_coverage: %s\n", descriptor_complete ? "complete" : (role_coverage ? role_coverage : "partial"));
    fprintf(stdout, "missing_required_roles: %s\n", missing_roles ? missing_roles : "unknown");
    fprintf(stdout, "unsupported_required_roles: %s\n", unsupported_roles ? unsupported_roles : "unknown");
    fprintf(stdout, "unknown_role_count: %llu\n", collections ? collections->unknown : 0ull);
    ...
    fprintf(stdout, "prefill_descriptor: unsupported-full-transformer-prefill\n");
    fprintf(stdout, "prefill.requires_embedding: true\n");
    fprintf(stdout, "prefill.requires_attention_qkv: true\n");
    fprintf(stdout, "prefill.requires_real_kv_writes: true\n");
    fprintf(stdout, "prefill.requires_mlp_or_moe: true\n");
    fprintf(stdout, "prefill.requires_layer_scheduler: true\n");
    fprintf(stdout, "prefill.current_status: unsupported\n");
    fprintf(stdout, "prefill.blocker: real transformer prefill not implemented\n");
    fprintf(stdout, "decode_descriptor: unsupported-full-model-decode\n");
    fprintf(stdout, "decode.mode_required: baseline-autoregressive\n");
    fprintf(stdout, "decode.requires_prefill_state: true\n");
    fprintf(stdout, "decode.requires_kv_read: true\n");
    fprintf(stdout, "decode.requires_layer_execution: true\n");
    fprintf(stdout, "decode.current_status: unsupported\n");
    fprintf(stdout, "decode.blocker: full model decode not implemented\n");
    fprintf(stdout, "logits_descriptor: unsupported-real-output-head-logits\n");
    fprintf(stdout, "sampling_descriptor: unsupported-real-vocabulary-sampling\n");

    fprintf(stdout, "residency_requirements_status: planned\n");
    fprintf(stdout, "residency_plan: descriptor-only-no-allocation\n");
    fprintf(stdout, "cpu_resident_required_bytes: %llu\n", cpu_bytes);
    fprintf(stdout, "cuda_resident_required_bytes: %llu\n", cuda_bytes);
    fprintf(stdout, "host_staged_required_bytes: %llu\n", strcmp(backend, "cuda") == 0 ? total_tensor_bytes : 0ull);
    fprintf(stdout, "ssd_staged_required_bytes: planned\n");
    fprintf(stdout, "kv_required_bytes: planned\n");
    fprintf(stdout, "scratch_required_bytes: planned\n");

    fprintf(stdout, "context_requirements_status: planned\n");
    fprintf(stdout, "max_context: metadata-or-unknown\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_policy: planned\n");
    fprintf(stdout, "position_policy: rope-or-family-specific-planned\n");
    fprintf(stdout, "rope_policy: planned\n");

    fprintf(stdout, "kv_requirements_status: unsupported\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    fprintf(stdout, "kv_layers: unknown\n");
    fprintf(stdout, "kv_heads: unknown\n");
    fprintf(stdout, "kv_head_dim: unknown\n");
    fprintf(stdout, "kv_capacity_status: unsupported-full-transformer-kv\n");
    fprintf(stdout, "kv.required: true\n");
    fprintf(stdout, "kv.real_attention_writes: false\n");
    fprintf(stdout, "kv.runtime_status: unsupported\n");
    fprintf(stdout, "kv_write_ready: false\n");
    fprintf(stdout, "kv_read_ready: false\n");

    fprintf(stdout, "logits_requirements_status: unsupported\n");
    fprintf(stdout, "output_head_present: %s\n", collections && collections->has_output_head ? "true" : "false");
    fprintf(stdout, "output_head_tensor: %s\n",
           fullmodel_descriptor_find_tensor(ctx, "output_head")
               ? fullmodel_descriptor_find_tensor(ctx, "output_head")->name
               : "none");
    fprintf(stdout, "output_head_dtype: %s\n",
           fullmodel_descriptor_find_tensor(ctx, "output_head")
               ? yvex_dtype_name(fullmodel_descriptor_find_tensor(ctx, "output_head")->dtype)
               : "unknown");
    fprintf(stdout, "vocab_size: %s\n", collections && collections->has_output_head ? "from-output-head-shape" : "unknown");
    fprintf(stdout, "logits_buffer_required: true\n");
    fprintf(stdout, "real_output_head_logits: false\n");
    fprintf(stdout, "logits_ready: false\n");
    fprintf(stdout, "logits.blocker: real output-head logits runtime unsupported\n");

    fprintf(stdout, "tokenizer_requirements_status: %s\n",
           collections && collections->has_tokenizer_metadata ? "partial" : "blocked");
    fprintf(stdout, "tokenizer_metadata_present: %s\n",
           collections && collections->has_tokenizer_metadata ? "true" : "false");
    fprintf(stdout, "special_token_policy: planned\n");
    fprintf(stdout, "eos_backed_stop: unsupported\n");
    fprintf(stdout, "stop_token_text_matching: unsupported\n");
    fprintf(stdout, "tokenizer_quality_generation: unsupported\n");

    fprintf(stdout, "backend_requirements_status: %s\n", fit.available ? "planned" : "unsupported");
    fprintf(stdout, "backend.cpu.available: true\n");
    fprintf(stdout, "backend.cuda.available: %s\n", yvex_backend_cuda_available() ? "true" : "false");
    fprintf(stdout, "backend.memory_known: %s\n", fit.memory_known ? "true" : "false");
    fprintf(stdout, "backend.required_bytes: %llu\n", fit.required_bytes);
    fprintf(stdout, "backend.fit_status: %s\n", fit.fit_status);
    fprintf(stdout, "backend.fit_reason: %s\n", fit.fit_reason);
    fprintf(stdout, "backend.primitive_rope: implemented-fixture\n");
    fprintf(stdout, "backend.primitive_attention: implemented-fixture\n");
    fprintf(stdout, "backend.primitive_matmul: implemented-fixture\n");
    fprintf(stdout, "backend.primitive_mlp: implemented-fixture\n");
    fprintf(stdout, "backend.full_transformer_integration: unsupported\n");
    fprintf(stdout, "backend_allocation_attempted: false\n");

    fprintf(stdout, "runtime_blockers: %s\n",
           selected_target
    ...
    fprintf(stdout, "descriptor_blockers: %s\n",
           selected_target
    ...
    fprintf(stdout, "decode_ready: false\n");
    fprintf(stdout, "sampling_ready: false\n");
    fprintf(stdout, "cleanup_attempted: false\n");
    fprintf(stdout, "cleanup_status: not-needed\n");
...
static void fullmodel_print_family_runtime_phase(unsigned int index,
                                                 ...
                                                 const char *status)
{
    fprintf(stdout, "family_runtime_phase.%u.name: %s\n", index, name ? name : "");
    fprintf(stdout, "family_runtime_phase.%u.status: %s\n", index, status ? status : "planned");
}
                                                 ...
                                                 int selected_target)
{
    ...
    fprintf(stdout, "family_runtime: report\n");
    if (!supported_family) {
        ...
        fprintf(stdout, "backend: %s\n", backend);
        fprintf(stdout, "family: %s\n", detected);
        fprintf(stdout, "family_detected: %s\n", detected);
        fprintf(stdout, "family_requested: %s\n", requested);
        fprintf(stdout, "family_adapter: unsupported\n");
        fprintf(stdout, "family_adapter_status: unsupported\n");
        fprintf(stdout, "family_runtime_stage: report-only\n");
        fprintf(stdout, "runtime_claim: unsupported\n");
        fprintf(stdout, "generation: unsupported-full-model\n");
        fprintf(stdout, "benchmark_status: not-measured\n");
        fprintf(stdout, "descriptor_status: unavailable\n");
        fprintf(stdout, "descriptor_source: fullmodel-descriptor-facts\n");
        fprintf(stdout, "full_runtime_model: false\n");
        fprintf(stdout, "full_model_execution: unsupported\n");
        fprintf(stdout, "generation_ready: false\n");
        fprintf(stdout, "runtime_execution_ready: false\n");
        fprintf(stdout, "runtime_blockers: unsupported or unknown runtime family adapter\n");
        fprintf(stdout, "next_required_rows: FAMILY.RUNTIME.0,ATTENTION.CLASS.0,CONTEXT.CLASS.0,KV.CACHE.0,MOE.CLASS.0,family-specific-runtime-target\n");
        fprintf(stdout, "cleanup_attempted: false\n");
        fprintf(stdout, "cleanup_status: not-needed\n");
        fullmodel_print_family_runtime_phases("unsupported", "resolve-family");
        fprintf(stdout, "reason: requested family is not supported by family-runtime report\n");
        return 5;
    }

    fprintf(stdout, "status: fullmodel-family-runtime\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
    fprintf(stdout, "model_resolved_path: %s\n", ref && ref->path ? ref->path : "");
    fprintf(stdout, "target_id: %s\n", target_id ? target_id : "path");
    fprintf(stdout, "target_class: %s\n", target_class ? target_class : "candidate-GGUF-path");
    fprintf(stdout, "backend: %s\n", backend);
    fprintf(stdout, "family: deepseek\n");
    fprintf(stdout, "family_detected: %s\n", detected);
    fprintf(stdout, "family_requested: %s\n", requested);
    fprintf(stdout, "family_adapter: deepseek-runtime-report\n");
    fprintf(stdout, "family_adapter_status: %s\n", adapter_status);
    fprintf(stdout, "family_runtime_stage: report-only\n");
    fprintf(stdout, "runtime_claim: unsupported\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");

    fprintf(stdout, "descriptor_status: %s\n", adapter_status);
    fprintf(stdout, "descriptor_source: fullmodel-descriptor-facts\n");
    fprintf(stdout, "full_runtime_model: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "generation_ready: false\n");
    ...
    fprintf(stdout, "role_adapter_status: %s\n", adapter_status);
    fprintf(stdout, "collection_adapter_status: %s\n", adapter_status);
    fprintf(stdout, "attention_rule_status: %s\n", fullmodel_attention_rule_status(collections));
    fprintf(stdout, "attention_rules: %s\n", fullmodel_attention_rule_status(collections));
    fprintf(stdout, "position_rule_status: planned\n");
    fprintf(stdout, "kv_rule_status: blocked\n");
    fprintf(stdout, "moe_rule_status: blocked\n");
    fprintf(stdout, "mlp_rule_status: %s\n", has_mlp ? "blocked-full-transformer-integration" : "blocked-missing-mlp");
    fprintf(stdout, "output_head_rule_status: %s\n", has_output ? "blocked-logits-runtime" : "blocked-missing-output-head");
    fprintf(stdout, "tokenizer_rule_status: %s\n",
           collections && collections->has_tokenizer_metadata ? "partial" : "blocked-missing-tokenizer-metadata");
    fprintf(stdout, "graph_requirement_status: blocked\n");
    fprintf(stdout, "runtime_blocker_status: blocked\n");

    fprintf(stdout, "token_embedding_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "token_embedding"));
    fprintf(stdout, "attention_norm_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "attention_norm"));
    fprintf(stdout, "post_attention_norm_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "post_attention_norm"));
    fprintf(stdout, "final_norm_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "final_norm"));
    fprintf(stdout, "q_projection_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "q_projection"));
    fprintf(stdout, "k_projection_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "k_projection"));
    fprintf(stdout, "v_projection_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "v_projection"));
    fprintf(stdout, "o_projection_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "o_projection"));
    fprintf(stdout, "mlp_gate_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "mlp_gate"));
    fprintf(stdout, "mlp_up_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "mlp_up"));
    fprintf(stdout, "mlp_down_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "mlp_down"));
    fprintf(stdout, "moe_router_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "moe_router"));
    fprintf(stdout, "moe_expert_roles: %s\n",
           collections && collections->has_moe_expert ? "present" : "missing");
    fprintf(stdout, "output_head_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "output_head"));
    fprintf(stdout, "tokenizer_metadata_role: %s\n", fullmodel_role_status_from_tensor(ctx, collections, "tokenizer_metadata"));

    fprintf(stdout, "attention_family: deepseek-family-attention-planned\n");
    fprintf(stdout, "attention_type: unknown-family-specific\n");
    fprintf(stdout, "attention_heads: unknown\n");
    fprintf(stdout, "kv_heads: unknown\n");
    fprintf(stdout, "head_dim: unknown\n");
    fprintf(stdout, "attention_q_required: true\n");
    fprintf(stdout, "attention_k_required: true\n");
    fprintf(stdout, "attention_v_required: true\n");
    fprintf(stdout, "attention_o_required: true\n");
    fprintf(stdout, "rope_required: true\n");
    fprintf(stdout, "rope_status: planned\n");
    fprintf(stdout, "rope_base: unknown\n");
    fprintf(stdout, "rope_scaling: unknown\n");
    fprintf(stdout, "mask_required: true\n");
    fprintf(stdout, "mask_rule: causal-or-family-specific-planned\n");
    fprintf(stdout, "context_policy: planned\n");
    fprintf(stdout, "attention_runtime_ready: false\n");

    fprintf(stdout, "kv_required: true\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    fprintf(stdout, "kv_capacity_status: unsupported-full-transformer-kv\n");
    fprintf(stdout, "kv_write_ready: false\n");
    ...
    fprintf(stdout, "moe_required: family-specific-planned\n");
    fprintf(stdout, "router_required: family-specific-planned\n");
    fprintf(stdout, "router_present: %s\n", collections && collections->has_moe_router ? "true" : "false");
    fprintf(stdout, "moe_router_present: %s\n", collections && collections->has_moe_router ? "true" : "false");
    fprintf(stdout, "expert_tensors_present: %s\n", collections && collections->has_moe_expert ? "true" : "false");
    fprintf(stdout, "moe_expert_count: unknown\n");
    fprintf(stdout, "expert_count: unknown\n");
    fprintf(stdout, "moe_active_expert_count: unknown\n");
    fprintf(stdout, "active_expert_count: unknown\n");
    fprintf(stdout, "moe_shared_experts: unknown\n");
    fprintf(stdout, "shared_expert_status: planned\n");
    fprintf(stdout, "moe_dispatch_ready: false\n");
    fprintf(stdout, "moe_blockers: router logits, top-k routing, expert dispatch, and expert accumulation are not implemented\n");

    fprintf(stdout, "output_head_required: true\n");
    ...
    fprintf(stdout, "logits_projection_ready: false\n");
    fprintf(stdout, "real_output_head_logits: false\n");
    fprintf(stdout, "logits_blockers: real output-head logits runtime unsupported\n");

    fprintf(stdout, "required_graph_ops: embedding-lookup,rmsnorm,q-projection,k-projection,v-projection,rope-position,attention-score,causal-mask,softmax,attention-value-accumulation,o-projection,residual-add,mlp,moe-router,expert-dispatch,final-norm,output-head-projection\n");
    fprintf(stdout, "implemented_graph_primitives: rope,attention-fixture,matmul,mlp-fixture,controlled-block,controlled-layers,selected-embedding,selected-rmsnorm-segment\n");
    fprintf(stdout, "unsupported_graph_ops: full-attention-from-model-tensors,full-transformer-block-from-model-tensors,full-layer-stack,real-moe-router,real-expert-dispatch,real-output-head-projection\n");
    fprintf(stdout, "graph.rope_primitive: implemented\n");
    fprintf(stdout, "graph.attention_primitive: implemented-fixture\n");
    fprintf(stdout, "graph.matmul_primitive: implemented\n");
    fprintf(stdout, "graph.mlp_primitive: implemented-fixture\n");
    fprintf(stdout, "graph.full_attention_from_model_tensors: unsupported\n");
    fprintf(stdout, "graph.full_transformer_block_from_model_tensors: unsupported\n");
    fprintf(stdout, "graph.full_layer_stack: unsupported\n");
    fprintf(stdout, "graph.full_transformer_attention: %s\n", has_attention ? "unsupported" : "missing-tensor");
    fprintf(stdout, "full_transformer_graph_ready: false\n");

    fprintf(stdout, "backend_requirements_status: %s\n", fit.available ? "planned" : "unsupported");
    fprintf(stdout, "backend_available: %s\n", fit.available ? "true" : "false");
    fprintf(stdout, "backend_memory_known: %s\n", fit.memory_known ? "true" : "false");
    fprintf(stdout, "backend_required_bytes: %llu\n", fit.required_bytes);
    fprintf(stdout, "backend_fit_status: %s\n", fit.fit_status);
    fprintf(stdout, "backend_allocation_attempted: false\n");
    ...
    fprintf(stdout, "next_required_rows: ATTENTION.CLASS.0,CONTEXT.CLASS.0,KV.CACHE.0,MOE.CLASS.0,FAMILY.RUNTIME.DeepSeek.detail,real-transformer-prefill,real-decode,real-output-head-logits,real-vocabulary-sampling,GEN.DEEPSEEK.0\n");
    fprintf(stdout, "cleanup_attempted: false\n");
    fprintf(stdout, "cleanup_status: not-needed\n");
...
static void fullmodel_print_materialize_phase(unsigned int index,
                                              ...
                                              const char *status)
{
    fprintf(stdout, "materialize_phase.%u.name: %s\n", index, name ? name : "");
    fprintf(stdout, "materialize_phase.%u.status: %s\n", index, status ? status : "planned");
}
...
static void fullmodel_print_materialize_report(const yvex_fullmodel_materialize_report *report)
{
    ...
    if (options && options->output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "fullmodel-materialize: %s model=%s backend=%s\n",
        ...
        fprintf(stdout, "bytes: materialized=%llu required=%llu\n",
        ...
        fprintf(stdout, "boundary: bounded proof/refusal only, no full model execution\n");
        fprintf(stdout, "status: %s\n", report && report->status ? report->status : "fullmodel-materialize-fail");
        return;
    }

    fprintf(stdout, "fullmodel: materialize\n");
    fprintf(stdout, "status: %s\n", report && report->status ? report->status : "fullmodel-materialize-fail");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
...
static void fullmodel_print_plan_normal(const yvex_cli_fullmodel_options *options,
                                        ...
                                        const char *top_blocker)
{
    fprintf(stdout, "materialization-plan: %s model=%s backend=%s\n",
           status ? status : "blocked",
           options && options->model ? options->model : "",
           options && options->backend ? options->backend : "cpu");
    fprintf(stdout, "residency: %s\n", options && options->residency ? options->residency : "resident");
    fprintf(stdout, "class: %s\n", target_class ? target_class : "unknown");
    fprintf(stdout, "fit: %s\n", fit ? fit : "unknown");
    fprintf(stdout, "top_blocker: %s\n", top_blocker ? top_blocker : "full-runtime candidate artifact required");
    fprintf(stdout, "boundary: plan-only, no materialization\n");
    fprintf(stdout, "status: fullmodel-materialization-plan\n");
}
...
static void fullmodel_print_descriptor_normal(const yvex_cli_fullmodel_options *options,
                                              ...
                                              const char *missing_roles)
{
    fprintf(stdout, "report: fullmodel-descriptor\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
    fprintf(stdout, "backend: %s target=%s class=%s\n",
           options && options->backend ? options->backend : "cpu",
           target_id ? target_id : "path",
           target_class ? target_class : "unknown");
    fprintf(stdout, "role_coverage: %s\n", role_coverage ? role_coverage : "partial");
    ...
    fprintf(stdout, "boundary: descriptor report-only, no runtime execution\n");
    fprintf(stdout, "status: fullmodel-descriptor\n");
}
...
static void fullmodel_print_family_runtime_normal(const yvex_cli_fullmodel_options *options,
                                                  ...
                                                  const char *missing_roles)
{
    fprintf(stdout, "report: family-runtime\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
    fprintf(stdout, "family: %s backend=%s\n",
    ...
    fprintf(stdout, "boundary: report-only, no runtime execution\n");
}

static int print_fullmodel_source_only_report(const char *target,
                                              const char *backend)
{
    fprintf(stdout, "fullmodel: report\n");
    fprintf(stdout, "status: fullmodel-report-unsupported\n");
    fprintf(stdout, "model: %s\n", target ? target : "");
    fprintf(stdout, "model_resolved_path: source-only-target\n");
    fprintf(stdout, "target_id: %s\n", target ? target : "");
    fprintf(stdout, "target_class: official-source-huge-model\n");
    fprintf(stdout, "source_artifact_class: official safetensors\n");
    fprintf(stdout, "target_artifact_class: future YVEX-produced GGUF\n");
    fprintf(stdout, "artifact_exists: false\n");
    fprintf(stdout, "artifact_bytes: 0\n");
    fprintf(stdout, "artifact_identity_status: not-applicable\n");
    fprintf(stdout, "tensor_count: 0\n");
    fprintf(stdout, "tensor_inventory_status: not-performed-source-only-target\n");
    fprintf(stdout, "metadata_status: not-performed\n");
    fprintf(stdout, "architecture: glm\n");
    fprintf(stdout, "family: glm\n");
    fprintf(stdout, "model_class: huge-MoE-source-target\n");
    fprintf(stdout, "fullmodel_inventory: unsupported-source-only\n");
    fprintf(stdout, "qtype_summary: none\n");
    fprintf(stdout, "dtype_summary: none\n");
    fprintf(stdout, "total_tensor_bytes: 0\n");
    fprintf(stdout, "estimated_cpu_resident_bytes: unknown\n");
    fprintf(stdout, "estimated_cuda_resident_bytes: unknown\n");
    fprintf(stdout, "estimated_kv_bytes: planned\n");
    fprintf(stdout, "estimated_scratch_bytes: planned\n");
    fprintf(stdout, "estimated_total_runtime_bytes: unknown\n");
    fprintf(stdout, "backend: %s\n", backend ? backend : "cpu");
    fprintf(stdout, "backend_placement_status: not-performed\n");
    fprintf(stdout, "cpu_placement: unsupported-source-only\n");
    fprintf(stdout, "cuda_placement: unsupported-source-only\n");
    fprintf(stdout, "cuda_available: %s\n", yvex_backend_cuda_available() ? "true" : "false");
    fprintf(stdout, "cuda_memory_status: unavailable\n");
    fprintf(stdout, "residency_plan: future-YVEX-produced-artifact-required\n");
    fprintf(stdout, "tensor_collections_status: not-performed\n");
    fprintf(stdout, "collection_detected: no\n");
    fprintf(stdout, "collection_supported: false\n");
    fprintf(stdout, "runtime_consumer: unsupported\n");
    fprintf(stdout, "embedding_tensors: 0\n");
    fprintf(stdout, "normalization_tensors: 0\n");
    fprintf(stdout, "attention_tensors: 0\n");
    fprintf(stdout, "kv_cache_requirements: planned\n");
    fprintf(stdout, "mlp_tensors: 0\n");
    fprintf(stdout, "moe_tensors: 0\n");
    fprintf(stdout, "output_tensors: 0\n");
    fprintf(stdout, "tokenizer_tensors: 0\n");
    fprintf(stdout, "required_role_coverage: none\n");
    fprintf(stdout, "missing_required_roles: YVEX-produced-GGUF-artifact\n");
    fprintf(stdout, "unsupported_required_roles: GLM-runtime-family-mapping,GLM-YVEX-produced-GGUF-emission,GLM-runtime-execution\n");
    fprintf(stdout, "runtime_blockers: source-only target; YVEX-produced GGUF emission planned; full model runtime unsupported\n");
                                            ...
                                            const char *target)
{
    ...
    fprintf(stdout, "fullmodel: materialization-plan\n");
    fprintf(stdout, "status: fullmodel-materialization-plan-unsupported\n");
    ...
    fprintf(stdout, "backend: %s\n", backend);
    fprintf(stdout, "residency: %s\n", residency);
    fprintf(stdout, "plan_status: unsupported\n");
    fprintf(stdout, "materialization_plan_ready: false\n");
    fprintf(stdout, "materialization_attempted: false\n");
    fprintf(stdout, "full_materialization_proof: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "plan_id: fullmodel-materialization:%s:%s:%s\n",
           target ? target : "source-only-target", backend, residency);
    fprintf(stdout, "plan_kind: full-model-materialization\n");
    fprintf(stdout, "plan_source: source-target-without-YVEX-GGUF\n");
    fprintf(stdout, "plan_backend: %s\n", backend);
    fprintf(stdout, "plan_residency: %s\n", residency);
    fprintf(stdout, "plan_tensor_count: 0\n");
    fprintf(stdout, "plan_tensor_bytes: 0\n");
    fprintf(stdout, "plan_collection_count: 0\n");
    fprintf(stdout, "plan_phase_count: 1\n");
    fprintf(stdout, "plan_blocker_count: 1\n");
    fprintf(stdout, "plan_cleanup_required: false\n");
    fprintf(stdout, "plan_cleanup_phases: none\n");
    fprintf(stdout, "backend_available: unknown\n");
    fprintf(stdout, "backend_memory_known: false\n");
    fprintf(stdout, "backend_memory_total_bytes: unknown\n");
    fprintf(stdout, "backend_memory_available_bytes: unknown\n");
    fprintf(stdout, "backend_required_bytes: 0\n");
    fprintf(stdout, "backend_fit_status: unsupported\n");
    fprintf(stdout, "backend_fit_reason: source-only target has no YVEX-produced GGUF tensor inventory\n");
    fprintf(stdout, "backend_allocation_attempted: false\n");
    ...
    (void)fullmodel_print_blocker(0u, "artifact", "fatal",
                                  ...
                                  1, 1);
    fprintf(stdout, "cleanup_plan_required: false\n");
    fprintf(stdout, "cleanup_plan_phases: none\n");
    fprintf(stdout, "cleanup_idempotent_required: true\n");
    fprintf(stdout, "cleanup_failure_policy: preserve-failure-report\n");
    fprintf(stdout, "next_required_row: FULLMODEL.2\n");
    fprintf(stdout, "proof_ready_for_fullmodel_2: false\n");
    fprintf(stdout, "fullmodel_2_blockers: YVEX-produced GGUF artifact missing; full tensor inventory unavailable\n");
    return 5;
}
                                                  ...
                                                  const char *target)
{
    ...
    fprintf(stdout, "fullmodel: descriptor\n");
    fprintf(stdout, "status: fullmodel-descriptor-unsupported\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : target ? target : "");
    fprintf(stdout, "model_resolved_path: source-only-target\n");
    fprintf(stdout, "target_id: %s\n", target ? target : "");
    fprintf(stdout, "target_class: official-source-huge-model\n");
    ...
    fprintf(stdout, "artifact_identity_status: not-applicable\n");
    fprintf(stdout, "tensor_inventory_status: not-performed-source-only-target\n");
    fprintf(stdout, "materialization_plan_status: unsupported-source-only\n");
    fprintf(stdout, "materialization_proof_status: unsupported-source-only\n");
    fprintf(stdout, "runtime_descriptor: report-only\n");
    fprintf(stdout, "runtime_descriptor_status: unsupported\n");
    fprintf(stdout, "runtime_descriptor_kind: fullmodel-planning\n");
    fprintf(stdout, "family: glm\n");
    fprintf(stdout, "architecture: glm\n");
    fprintf(stdout, "model_class: huge-MoE-source-target\n");
    fprintf(stdout, "full_runtime_model: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "full_model_materialization: unsupported-source-only\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "tensor_role_map_status: not-performed\n");
    fprintf(stdout, "tensor_collection_map_status: not-performed\n");
    fprintf(stdout, "required_role_coverage: none\n");
    fprintf(stdout, "missing_required_roles: YVEX-produced-GGUF-artifact\n");
    fprintf(stdout, "unsupported_required_roles: GLM-runtime-family-mapping,GLM-YVEX-produced-GGUF-emission,GLM-runtime-execution\n");
    fprintf(stdout, "unknown_role_count: 0\n");
    fprintf(stdout, "embedding_descriptor: not-performed-source-only\n");
    fprintf(stdout, "normalization_descriptor: not-performed-source-only\n");
    fprintf(stdout, "attention_descriptor: not-performed-source-only\n");
    fprintf(stdout, "mlp_descriptor: not-performed-source-only\n");
    fprintf(stdout, "moe_descriptor: not-performed-source-only\n");
    fprintf(stdout, "output_descriptor: not-performed-source-only\n");
    fprintf(stdout, "tokenizer_descriptor: not-performed-source-only\n");
    fprintf(stdout, "kv_descriptor: unsupported-source-only\n");
    fprintf(stdout, "prefill_descriptor: unsupported-source-only\n");
    fprintf(stdout, "decode_descriptor: unsupported-source-only\n");
    fprintf(stdout, "logits_descriptor: unsupported-source-only\n");
    fprintf(stdout, "sampling_descriptor: unsupported-source-only\n");
    fprintf(stdout, "graph_requirements_status: unsupported-source-only\n");
    fprintf(stdout, "required_graph_ops: planned-after-YVEX-produced-GGUF\n");
    fprintf(stdout, "unsupported_graph_ops: GLM-full-transformer-runtime\n");
    fprintf(stdout, "required_backend_ops: planned-after-YVEX-produced-GGUF\n");
    fprintf(stdout, "unsupported_backend_ops: GLM-runtime-execution\n");
    fprintf(stdout, "residency_requirements_status: unsupported-source-only\n");
    fprintf(stdout, "residency_plan: future-YVEX-produced-artifact-required\n");
    fprintf(stdout, "cpu_resident_required_bytes: unknown\n");
    fprintf(stdout, "cuda_resident_required_bytes: unknown\n");
    fprintf(stdout, "host_staged_required_bytes: unknown\n");
    fprintf(stdout, "ssd_staged_required_bytes: planned\n");
    fprintf(stdout, "kv_required_bytes: planned\n");
    fprintf(stdout, "scratch_required_bytes: planned\n");
    fprintf(stdout, "context_requirements_status: planned\n");
    fprintf(stdout, "max_context: unknown\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_policy: planned\n");
    fprintf(stdout, "position_policy: planned\n");
    fprintf(stdout, "rope_policy: planned\n");
    fprintf(stdout, "kv_requirements_status: unsupported-source-only\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    fprintf(stdout, "kv_layers: unknown\n");
    fprintf(stdout, "kv_heads: unknown\n");
    fprintf(stdout, "kv_head_dim: unknown\n");
    fprintf(stdout, "kv_capacity_status: unsupported-source-only\n");
    fprintf(stdout, "kv_write_ready: false\n");
    fprintf(stdout, "kv_read_ready: false\n");
    fprintf(stdout, "logits_requirements_status: unsupported-source-only\n");
    fprintf(stdout, "output_head_present: false\n");
    fprintf(stdout, "output_head_dtype: unknown\n");
    fprintf(stdout, "vocab_size: unknown\n");
    fprintf(stdout, "logits_buffer_required: true\n");
    fprintf(stdout, "real_output_head_logits: false\n");
    fprintf(stdout, "logits_ready: false\n");
    fprintf(stdout, "runtime_blockers: source-only target; YVEX-produced GGUF emission planned; GLM runtime unsupported\n");
    fprintf(stdout, "descriptor_blockers: source-only target has no YVEX-produced GGUF tensor inventory\n");
    fprintf(stdout, "prefill_ready: false\n");
    fprintf(stdout, "decode_ready: false\n");
    fprintf(stdout, "sampling_ready: false\n");
    fprintf(stdout, "cleanup_attempted: false\n");
                                                      ...
                                                      const char *target)
{
    ...
    fprintf(stdout, "family_runtime: report\n");
    fprintf(stdout, "status: fullmodel-family-runtime-unsupported\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : target ? target : "");
    fprintf(stdout, "model_resolved_path: source-only-target\n");
    fprintf(stdout, "target_id: %s\n", target ? target : "");
    fprintf(stdout, "target_class: official-source-huge-model\n");
    fprintf(stdout, "backend: %s\n", backend);
    fprintf(stdout, "family: glm\n");
    fprintf(stdout, "family_detected: glm\n");
    fprintf(stdout, "family_requested: %s\n", requested);
    fprintf(stdout, "family_adapter: unsupported-source-only\n");
    fprintf(stdout, "family_adapter_status: unsupported\n");
    fprintf(stdout, "family_runtime_stage: report-only\n");
    fprintf(stdout, "runtime_claim: unsupported\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "descriptor_status: unsupported-source-only\n");
    fprintf(stdout, "descriptor_source: not-performed-source-only-target\n");
    fprintf(stdout, "full_runtime_model: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "role_adapter_status: not-performed\n");
    fprintf(stdout, "collection_adapter_status: not-performed\n");
    fprintf(stdout, "attention_rule_status: unsupported-source-only\n");
    fprintf(stdout, "position_rule_status: unsupported-source-only\n");
    fprintf(stdout, "kv_rule_status: unsupported-source-only\n");
    fprintf(stdout, "moe_rule_status: unsupported-source-only\n");
    fprintf(stdout, "mlp_rule_status: unsupported-source-only\n");
    fprintf(stdout, "output_head_rule_status: unsupported-source-only\n");
    fprintf(stdout, "tokenizer_rule_status: unsupported-source-only\n");
    fprintf(stdout, "graph_requirement_status: unsupported-source-only\n");
    fprintf(stdout, "runtime_blocker_status: blocked\n");
    fprintf(stdout, "tensor_inventory_status: not-performed-source-only-target\n");
    fprintf(stdout, "token_embedding_role: not-performed-source-only\n");
    fprintf(stdout, "attention_norm_role: not-performed-source-only\n");
    fprintf(stdout, "q_projection_role: not-performed-source-only\n");
    fprintf(stdout, "k_projection_role: not-performed-source-only\n");
    fprintf(stdout, "v_projection_role: not-performed-source-only\n");
    fprintf(stdout, "o_projection_role: not-performed-source-only\n");
    fprintf(stdout, "output_head_role: not-performed-source-only\n");
    fprintf(stdout, "tokenizer_metadata_role: not-performed-source-only\n");
    fprintf(stdout, "attention_family: glm-family-planned\n");
    fprintf(stdout, "attention_type: unknown-source-only\n");
    fprintf(stdout, "attention_runtime_ready: false\n");
    fprintf(stdout, "kv_required: true\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    ...
    fprintf(stdout, "kv_write_ready: false\n");
    fprintf(stdout, "kv_read_ready: false\n");
    fprintf(stdout, "moe_required: true\n");
    fprintf(stdout, "router_required: true\n");
    fprintf(stdout, "router_present: false\n");
    fprintf(stdout, "expert_tensors_present: false\n");
    fprintf(stdout, "moe_expert_count: unknown\n");
    fprintf(stdout, "moe_active_expert_count: unknown\n");
    fprintf(stdout, "moe_shared_experts: unknown\n");
    fprintf(stdout, "moe_dispatch_ready: false\n");
    fprintf(stdout, "output_head_required: true\n");
    fprintf(stdout, "output_head_present: false\n");
    fprintf(stdout, "output_head_tensor: none\n");
    fprintf(stdout, "vocab_size: unknown\n");
    fprintf(stdout, "logits_projection_ready: false\n");
    fprintf(stdout, "real_output_head_logits: false\n");
    fprintf(stdout, "required_graph_ops: planned-after-YVEX-produced-GGUF\n");
    fprintf(stdout, "implemented_graph_primitives: none-for-source-only-target\n");
    fprintf(stdout, "unsupported_graph_ops: GLM-full-transformer-runtime\n");
    fprintf(stdout, "graph.full_attention_from_model_tensors: unsupported\n");
    fprintf(stdout, "graph.full_transformer_block_from_model_tensors: unsupported\n");
    fprintf(stdout, "graph.full_layer_stack: unsupported\n");
    ...
    fprintf(stdout, "sampling_ready: false\n");
    fprintf(stdout, "runtime_execution_ready: false\n");
    fprintf(stdout, "runtime_blockers: source-only target; YVEX-produced GGUF emission planned; GLM runtime family mapping planned; GLM runtime unsupported\n");
    fprintf(stdout, "next_required_rows: OWI.HUGE.0,MODEL.CLASS.3,TENSOR.COLLECTION.2,ATTENTION.CLASS.0,KV.CACHE.0,MOE.CLASS.0,GLM-YVEX-produced-GGUF,GLM-runtime-family-mapping\n");
    fprintf(stdout, "cleanup_attempted: false\n");
    fprintf(stdout, "cleanup_status: not-needed\n");
                                               ...
                                               unsigned long long artifact_bytes)
{
    ...
    fprintf(stdout, "residency: %s\n", residency);
    fprintf(stdout, "plan_status: failed\n");
    fprintf(stdout, "materialization_plan_ready: false\n");
    fprintf(stdout, "materialization_attempted: false\n");
    fprintf(stdout, "full_materialization_proof: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    ...
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "plan_id: unavailable\n");
    fprintf(stdout, "plan_kind: full-model-materialization\n");
    fprintf(stdout, "plan_source: tensor-inventory\n");
    fprintf(stdout, "plan_backend: %s\n", backend);
    fprintf(stdout, "plan_residency: %s\n", residency);
    fprintf(stdout, "plan_tensor_count: 0\n");
    fprintf(stdout, "plan_tensor_bytes: 0\n");
    ...
    fprintf(stdout, "backend_memory_available_bytes: unknown\n");
    fprintf(stdout, "backend_required_bytes: 0\n");
    fprintf(stdout, "backend_fit_status: unknown\n");
    fprintf(stdout, "backend_fit_reason: inventory failed before backend fit preflight\n");
    fprintf(stdout, "backend_allocation_attempted: false\n");
    ...
    fprintf(stdout, "next_required_row: FULLMODEL.2\n");
    fprintf(stdout, "proof_ready_for_fullmodel_2: false\n");
    fprintf(stdout, "fullmodel_2_blockers: tensor inventory unavailable; materialization plan unavailable\n");
}
                                          ...
                                          const char *resolved_path)
{
    ...
    if (is_materialize) {
        ...
        fullmodel_print_materialize_report(&report);
        fprintf(stdout, "reason: artifact path does not exist\n");
        return exit_for_status(YVEX_ERR_IO);
    }

    if (is_descriptor) {
        fprintf(stdout, "fullmodel: descriptor\n");
        fprintf(stdout, "status: fullmodel-descriptor-fail\n");
        fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
        fprintf(stdout, "model_resolved_path: %s\n", resolved_path ? resolved_path : "");
        fprintf(stdout, "target_id: %s\n", options && options->target ? options->target : "unknown");
        fprintf(stdout, "target_class: unresolved-artifact\n");
        fprintf(stdout, "backend: %s\n", options && options->backend ? options->backend : "cpu");
        fprintf(stdout, "artifact_identity_status: unavailable\n");
        fprintf(stdout, "tensor_inventory_status: failed\n");
        fprintf(stdout, "materialization_plan_status: unavailable\n");
        fprintf(stdout, "materialization_proof_status: unavailable\n");
        fprintf(stdout, "runtime_descriptor: report-only\n");
        fprintf(stdout, "runtime_descriptor_status: fail\n");
        fprintf(stdout, "runtime_descriptor_kind: fullmodel-planning\n");
        fprintf(stdout, "family: unknown\n");
        fprintf(stdout, "architecture: unknown\n");
        fprintf(stdout, "model_class: unresolved\n");
        fprintf(stdout, "full_runtime_model: false\n");
        fprintf(stdout, "full_model_execution: unsupported\n");
        fprintf(stdout, "full_model_materialization: unavailable\n");
        fprintf(stdout, "generation_ready: false\n");
        fprintf(stdout, "generation: unsupported-full-model\n");
        fprintf(stdout, "benchmark_status: not-measured\n");
        fprintf(stdout, "tensor_role_map_status: unavailable\n");
        fprintf(stdout, "tensor_collection_map_status: unavailable\n");
        fprintf(stdout, "required_role_coverage: none\n");
        fprintf(stdout, "missing_required_roles: artifact\n");
        fprintf(stdout, "unsupported_required_roles: full-runtime-model\n");
        fprintf(stdout, "runtime_blockers: artifact path missing\n");
        fprintf(stdout, "descriptor_blockers: artifact path missing\n");
        fprintf(stdout, "prefill_ready: false\n");
        fprintf(stdout, "decode_ready: false\n");
        fprintf(stdout, "logits_ready: false\n");
        fprintf(stdout, "sampling_ready: false\n");
    ...
    if (is_family_runtime) {
        fprintf(stdout, "family_runtime: report\n");
        fprintf(stdout, "status: fullmodel-family-runtime-fail\n");
        fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
        fprintf(stdout, "model_resolved_path: %s\n", resolved_path ? resolved_path : "");
        fprintf(stdout, "target_id: %s\n", options && options->target ? options->target : "unknown");
        fprintf(stdout, "target_class: unresolved-artifact\n");
        fprintf(stdout, "backend: %s\n", options && options->backend ? options->backend : "cpu");
        fprintf(stdout, "family: unknown\n");
        fprintf(stdout, "family_detected: unknown\n");
        fprintf(stdout, "family_requested: %s\n", fullmodel_requested_family(options));
        fprintf(stdout, "family_adapter: unavailable\n");
        fprintf(stdout, "family_adapter_status: failed\n");
        fprintf(stdout, "family_runtime_stage: report-only\n");
        fprintf(stdout, "runtime_claim: unsupported\n");
        fprintf(stdout, "generation: unsupported-full-model\n");
        fprintf(stdout, "benchmark_status: not-measured\n");
        fprintf(stdout, "descriptor_status: fail\n");
        fprintf(stdout, "descriptor_source: unavailable\n");
        fprintf(stdout, "full_runtime_model: false\n");
        fprintf(stdout, "full_model_execution: unsupported\n");
        fprintf(stdout, "generation_ready: false\n");
        fprintf(stdout, "runtime_execution_ready: false\n");
    ...
    fprintf(stdout, "fullmodel: %s\n", is_plan ? "materialization-plan" : "report");
    fprintf(stdout, "status: %s\n", is_plan ? "fullmodel-materialization-plan-fail" : "fullmodel-report-fail");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
    fprintf(stdout, "model_resolved_path: %s\n", resolved_path ? resolved_path : "");
    fprintf(stdout, "target_id: %s\n", options && options->target ? options->target : "unknown");
    fprintf(stdout, "target_class: unresolved-artifact\n");
    fprintf(stdout, "source_artifact_class: unknown\n");
    fprintf(stdout, "target_artifact_class: GGUF artifact\n");
    fprintf(stdout, "artifact_exists: false\n");
    fprintf(stdout, "artifact_bytes: 0\n");
    fprintf(stdout, "artifact_identity_status: unavailable\n");
    fprintf(stdout, "tensor_count: 0\n");
    fprintf(stdout, "tensor_inventory_status: failed\n");
    fprintf(stdout, "metadata_status: failed\n");
    fprintf(stdout, "architecture: unknown\n");
    fprintf(stdout, "family: unknown\n");
    fprintf(stdout, "model_class: unresolved\n");
    fprintf(stdout, "fullmodel_inventory: unavailable\n");
    fprintf(stdout, "qtype_summary: none\n");
    fprintf(stdout, "dtype_summary: none\n");
    fprintf(stdout, "total_tensor_bytes: 0\n");
    fprintf(stdout, "estimated_cpu_resident_bytes: unknown\n");
    ...
    fprintf(stdout, "estimated_total_runtime_bytes: unknown\n");
    fprintf(stdout, "backend: %s\n", options && options->backend ? options->backend : "cpu");
    fprintf(stdout, "backend_placement_status: failed-missing-artifact\n");
    fprintf(stdout, "cpu_placement: unavailable\n");
    fprintf(stdout, "cuda_placement: unavailable\n");
    fprintf(stdout, "cuda_available: %s\n", yvex_backend_cuda_available() ? "true" : "false");
    fprintf(stdout, "cuda_memory_status: unavailable\n");
    fprintf(stdout, "residency_plan: unavailable\n");
    fprintf(stdout, "tensor_collections_status: unavailable\n");
    fprintf(stdout, "collection_detected: no\n");
    fprintf(stdout, "collection_supported: false\n");
    fprintf(stdout, "runtime_consumer: unsupported\n");
    fprintf(stdout, "embedding_tensors: 0\n");
                                                ...
                                                int rc)
{
    ...
    if (is_materialize) {
        ...
        fullmodel_print_materialize_report(&report);
        fprintf(stdout, "artifact_bytes: %llu\n", artifact_bytes);
    ...
    if (is_descriptor) {
        ...
        fprintf(stdout, "target_class: GGUF-artifact\n");
        fprintf(stdout, "backend: %s\n", options && options->backend ? options->backend : "cpu");
        fprintf(stdout, "artifact_identity_status: not-checked\n");
        fprintf(stdout, "tensor_inventory_status: failed\n");
        fprintf(stdout, "materialization_plan_status: failed\n");
        fprintf(stdout, "materialization_proof_status: unavailable\n");
        fprintf(stdout, "runtime_descriptor: report-only\n");
        fprintf(stdout, "runtime_descriptor_status: fail\n");
        fprintf(stdout, "runtime_descriptor_kind: fullmodel-planning\n");
        fprintf(stdout, "family: unknown\n");
        fprintf(stdout, "architecture: unknown\n");
        fprintf(stdout, "model_class: parse-failed\n");
        fprintf(stdout, "full_runtime_model: false\n");
        fprintf(stdout, "full_model_execution: unsupported\n");
        fprintf(stdout, "full_model_materialization: unavailable\n");
        fprintf(stdout, "generation_ready: false\n");
        ...
        fprintf(stdout, "tensor_collection_map_status: unavailable\n");
        fprintf(stdout, "required_role_coverage: none\n");
        fprintf(stdout, "missing_required_roles: parseable-GGUF-tensor-directory\n");
        fprintf(stdout, "unsupported_required_roles: full-runtime-model\n");
        fprintf(stdout, "runtime_blockers: GGUF metadata or tensor directory parse failed\n");
        fprintf(stdout, "descriptor_blockers: GGUF metadata or tensor directory parse failed\n");
        fprintf(stdout, "prefill_ready: false\n");
        fprintf(stdout, "decode_ready: false\n");
        fprintf(stdout, "logits_ready: false\n");
        fprintf(stdout, "sampling_ready: false\n");
    ...
    fprintf(stdout, "source_artifact_class: unknown\n");
    fprintf(stdout, "target_artifact_class: GGUF artifact\n");
    fprintf(stdout, "artifact_exists: true\n");
    fprintf(stdout, "artifact_bytes: %llu\n", artifact_bytes);
    fprintf(stdout, "artifact_identity_status: not-checked\n");
    fprintf(stdout, "tensor_count: 0\n");
    fprintf(stdout, "tensor_inventory_status: failed\n");
    ...
    fprintf(stdout, "estimated_total_runtime_bytes: unknown\n");
    fprintf(stdout, "backend: %s\n", options && options->backend ? options->backend : "cpu");
    fprintf(stdout, "backend_placement_status: failed-parse\n");
    fprintf(stdout, "cpu_placement: unavailable\n");
    fprintf(stdout, "cuda_placement: unavailable\n");
    fprintf(stdout, "cuda_available: %s\n", yvex_backend_cuda_available() ? "true" : "false");
    fprintf(stdout, "cuda_memory_status: unavailable\n");
    fprintf(stdout, "residency_plan: unavailable\n");
    fprintf(stdout, "tensor_collections_status: failed\n");
    fprintf(stdout, "collection_detected: no\n");
    fprintf(stdout, "collection_supported: false\n");
    fprintf(stdout, "runtime_consumer: unsupported\n");
    fprintf(stdout, "embedding_tensors: 0\n");
...
static void attention_print_phase(unsigned int index,
                                  ...
                                  const char *status)
{
    fprintf(stdout, "attention_phase.%u.name: %s\n", index, name ? name : "");
    fprintf(stdout, "attention_phase.%u.status: %s\n", index, status ? status : "planned");
}
                                            ...
                                            const char *prefix)
{
    ...
    dims[0] = '\0';
    if (tensor) dims_to_text(tensor->dims, tensor->rank, dims, sizeof(dims));
    fprintf(stdout, "%s_required: true\n", prefix);
    fprintf(stdout, "%s_status: %s\n", prefix, status);
    fprintf(stdout, "%s_tensor: %s\n", prefix, tensor && tensor->name ? tensor->name : "none");
    fprintf(stdout, "%s_shape: %s\n", prefix, tensor ? dims : "unknown");
    fprintf(stdout, "%s_dtype: %s\n", prefix, tensor ? yvex_dtype_name(tensor->dtype) : "unknown");
    fprintf(stdout, "%s_qtype: %s\n", prefix, tensor ? yvex_dtype_name(tensor->dtype) : "unknown");
    fprintf(stdout, "%s_bytes: %llu\n", prefix, tensor ? tensor->storage_bytes : 0ull);
    fprintf(stdout, "%s_runtime_consumer: %s\n", prefix,
                                               ...
                                               const char *phase)
{
    ...
    fprintf(stdout, "family_runtime_status: unsupported\n");
    fprintf(stdout, "attention_class_status: unsupported\n");
    fprintf(stdout, "attention_stage: report-only\n");
    fprintf(stdout, "runtime_claim: unsupported\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "attention_family: unsupported\n");
    fprintf(stdout, "attention_type: unknown\n");
    fprintf(stdout, "attention_support_status: report-only\n");
    fprintf(stdout, "full_transformer_attention: unsupported\n");
    fprintf(stdout, "attention_runtime_ready: false\n");
    fprintf(stdout, "attention_backend_ready: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "q_projection_required: true\n");
    fprintf(stdout, "k_projection_required: true\n");
    fprintf(stdout, "v_projection_required: true\n");
    fprintf(stdout, "o_projection_required: true\n");
    fprintf(stdout, "q_projection_status: unknown\n");
    fprintf(stdout, "k_projection_status: unknown\n");
    fprintf(stdout, "v_projection_status: unknown\n");
    fprintf(stdout, "o_projection_status: unknown\n");
    fprintf(stdout, "attention_heads: unknown\n");
    fprintf(stdout, "kv_heads: unknown\n");
    fprintf(stdout, "head_dim: unknown\n");
    fprintf(stdout, "hidden_size: unknown\n");
    fprintf(stdout, "head_layout_status: unknown\n");
    fprintf(stdout, "head_layout_source: unknown\n");
    fprintf(stdout, "head_layout_blockers: attention class unavailable\n");
    fprintf(stdout, "position_policy: planned\n");
    fprintf(stdout, "rope_required: true\n");
    fprintf(stdout, "rope_status: planned\n");
    fprintf(stdout, "rope_base: unknown\n");
    fprintf(stdout, "rope_scaling: unknown\n");
    fprintf(stdout, "rope_dimension: unknown\n");
    fprintf(stdout, "rope_runtime_ready: false\n");
    fprintf(stdout, "mask_policy: planned\n");
    fprintf(stdout, "causal_mask_required: true\n");
    fprintf(stdout, "mask_status: planned\n");
    fprintf(stdout, "mask_runtime_ready: false\n");
    fprintf(stdout, "kv_required: true\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    fprintf(stdout, "kv_layers: unknown\n");
    ...
    fprintf(stdout, "kv_write_ready: false\n");
    fprintf(stdout, "kv_read_ready: false\n");
    fprintf(stdout, "attention_kv_runtime_ready: false\n");
    fprintf(stdout, "kv_boundary: diagnostic-kv-exists-real-attention-kv-unsupported\n");
    fprintf(stdout, "context_policy: planned\n");
    fprintf(stdout, "max_context: unknown\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_status: planned\n");
    fprintf(stdout, "context_blockers: attention class unavailable\n");
    fprintf(stdout, "graph_rope_primitive: implemented\n");
    fprintf(stdout, "graph_attention_primitive: implemented-fixture\n");
    fprintf(stdout, "graph_qkv_projection: unsupported\n");
    fprintf(stdout, "graph_o_projection: unsupported\n");
    fprintf(stdout, "graph_full_transformer_attention: unsupported\n");
    fprintf(stdout, "graph_backend_status: report-only\n");
    fprintf(stdout, "unsupported_graph_ops: full-transformer-attention,real-attention-backed-kv,real-transformer-prefill\n");
    fprintf(stdout, "backend_attention_status: implemented-fixture-full-transformer-unsupported\n");
    fprintf(stdout, "backend_rope_status: implemented-primitive\n");
    fprintf(stdout, "backend_softmax_status: implemented-inside-attention-fixture\n");
    fprintf(stdout, "backend_matmul_status: implemented-primitive\n");
    fprintf(stdout, "backend_kv_status: unsupported-real-attention-kv\n");
    fprintf(stdout, "attention_blockers: %s\n", reason ? reason : "attention class unavailable");
    fprintf(stdout, "next_required_rows: ATTENTION.CLASS.0,KV.CACHE.0,CONTEXT.CLASS.0\n");
    fprintf(stdout, "cleanup_attempted: false\n");
    fprintf(stdout, "cleanup_status: not-needed\n");
                                  ...
                                  int selected_target)
{
    ...
    if (options && options->output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "report: attention\n");
        fprintf(stdout, "model: %s\n", options->model ? options->model : "");
        fprintf(stdout, "family: deepseek backend=%s\n", backend);
        fprintf(stdout, "status: %s\n", attention_class_status);
        fprintf(stdout, "top_blocker: %s\n", has_all_qkvo ? "runtime attention integration missing" : "missing Q/K/V/O projection tensors");
        fprintf(stdout, "next: V010.ATTN.9\n");
        fprintf(stdout, "boundary: report-only, no runtime execution\n");
        return 0;
    }

    fprintf(stdout, "attention: report\n");
    fprintf(stdout, "status: attention-report\n");
    fprintf(stdout, "model: %s\n", options && options->model ? options->model : "");
    fprintf(stdout, "model_resolved_path: %s\n", ref && ref->path ? ref->path : "");
    fprintf(stdout, "target_id: %s\n", target_id ? target_id : "path");
    fprintf(stdout, "target_class: %s\n", target_class ? target_class : "candidate-GGUF-path");
    ...
    fprintf(stdout, "family_detected: %s\n", detected);
    fprintf(stdout, "family_requested: %s\n", requested);
    fprintf(stdout, "family_runtime_status: %s\n", attention_class_status);
    fprintf(stdout, "attention_class_status: %s\n", attention_class_status);
    fprintf(stdout, "attention_stage: report-only\n");
    fprintf(stdout, "runtime_claim: unsupported\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    ...
    fprintf(stdout, "tensor_count: %llu\n", tensor_count);
    fprintf(stdout, "total_tensor_bytes: %llu\n", total_tensor_bytes);
    fprintf(stdout, "attention_family: model-family-specific\n");
    fprintf(stdout, "attention_type: unknown\n");
    fprintf(stdout, "attention_support_status: report-only\n");
    fprintf(stdout, "full_transformer_attention: unsupported\n");
    fprintf(stdout, "attention_runtime_ready: false\n");
    ...
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "attention_metadata_status: incomplete\n");
    fprintf(stdout, "attention_blocker: attention metadata incomplete\n");
    ...
    fprintf(stdout, "head_layout_status: unknown\n");
    fprintf(stdout, "head_layout_source: unknown\n");
    fprintf(stdout, "head_layout_blockers: attention metadata incomplete; head count, KV head count, and head dimension unavailable\n");
    fprintf(stdout, "position_policy: rope-or-family-specific-planned\n");
    fprintf(stdout, "rope_required: true\n");
    fprintf(stdout, "rope_status: planned\n");
    fprintf(stdout, "rope_base: unknown\n");
    ...
    fprintf(stdout, "graph_rope_primitive: implemented\n");
    fprintf(stdout, "rope_runtime_ready: false\n");
    fprintf(stdout, "mask_policy: causal-or-family-specific-planned\n");
    fprintf(stdout, "causal_mask_required: true\n");
    fprintf(stdout, "mask_status: planned\n");
    fprintf(stdout, "mask_runtime_ready: false\n");
    fprintf(stdout, "kv_required: true\n");
    ...
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_status: planned\n");
    fprintf(stdout, "context_blockers: context class report pending; attention head layout incomplete\n");
    fprintf(stdout, "graph_attention_primitive: implemented-fixture\n");
    fprintf(stdout, "graph_matmul_primitive: implemented\n");
    fprintf(stdout, "graph_qkv_projection: %s\n", graph_projection_status);
    fprintf(stdout, "graph_o_projection: %s\n", has_o ? "planned" : "missing-tensor");
    fprintf(stdout, "graph_model_qkv_projection: unsupported\n");
    fprintf(stdout, "graph_attention_kv_write: unsupported\n");
    fprintf(stdout, "graph_layer_integrated_attention: unsupported\n");
    fprintf(stdout, "graph_full_transformer_attention: unsupported\n");
    fprintf(stdout, "graph_backend_status: report-only\n");
    fprintf(stdout, "unsupported_graph_ops: full-transformer-attention,real-qkv-projection,attention-backed-kv-write,layer-integrated-attention,real-transformer-prefill\n");
    fprintf(stdout, "backend_attention_status: implemented-fixture-full-transformer-unsupported\n");
    fprintf(stdout, "backend_rope_status: implemented-primitive\n");
    fprintf(stdout, "backend_softmax_status: implemented-inside-attention-fixture\n");
    fprintf(stdout, "backend_matmul_status: implemented-primitive\n");
    ...
    fprintf(stdout, "sampling_ready: false\n");
    fprintf(stdout, "runtime_execution_ready: false\n");
    fprintf(stdout, "next_required_rows: KV.CACHE.0,CONTEXT.CLASS.0,real-transformer-prefill,real-attention-backed-KV,real-decode,GEN.DEEPSEEK.0\n");
    fprintf(stdout, "cleanup_attempted: false\n");
    fprintf(stdout, "cleanup_status: not-needed\n");
...
{
    if (report && report->output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        fprintf(stdout, "report: tensor-collection\n");
        fprintf(stdout, "model: %s\n", report->model ? report->model : "");
        fprintf(stdout, "family: %s collection=%s\n",
               report->family ? report->family : "unknown",
...
static void context_print_phase(unsigned int index,
                                ...
                                const char *status)
{
    fprintf(stdout, "context_phase.%u.name: %s\n", index, name ? name : "");
    fprintf(stdout, "context_phase.%u.status: %s\n", index, status ? status : "unknown");
}
                                        ...
                                        const char *kv_status)
{
    ...
    fprintf(stdout, "kv_class_status: %s\n", kv_status ? kv_status : "unknown");
    fprintf(stdout, "context_class_status: report-only\n");
    fprintf(stdout, "context_stage: report-only\n");
    fprintf(stdout, "runtime_claim: unsupported\n");
    fprintf(stdout, "generation: unsupported-full-model\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
}
...
static void context_print_unsupported_source_only(const yvex_cli_context_options *options)
{
    ...
    context_print_common_header(options,
                                ...
                                "unsupported",
                                "unsupported");
    fprintf(stdout, "model_max_context: unknown\n");
    fprintf(stdout, "model_max_context_source: source-manifest-planned\n");
    fprintf(stdout, "model_max_context_status: unsupported-source-only\n");
    fprintf(stdout, "requested_context: %s", options && options->context_length_seen ? "" : "not-requested");
    if (options && options->context_length_seen) fprintf(stdout, "%llu", options->context_length);
    fprintf(stdout, "\n");
    fprintf(stdout, "requested_context_source: %s\n", options && options->context_length_seen ? "operator-request" : "none");
    fprintf(stdout, "requested_context_status: %s\n", options && options->context_length_seen ? "reported-only" : "not-requested");
    fprintf(stdout, "active_context: unsupported\n");
    fprintf(stdout, "active_context_source: source-only-target\n");
    fprintf(stdout, "active_context_status: unsupported\n");
    fprintf(stdout, "token_input_status: not-performed-source-only-target\n");
    fprintf(stdout, "token_count: 0\n");
    fprintf(stdout, "prompt_token_count: 0\n");
    fprintf(stdout, "prefill_token_count: 0\n");
    fprintf(stdout, "generated_token_count: 0\n");
    fprintf(stdout, "token_range_start: none\n");
    fprintf(stdout, "token_range_end: none\n");
    fprintf(stdout, "chunking_required: false\n");
    fprintf(stdout, "chunk_size: not-requested\n");
    fprintf(stdout, "chunk_count: 0\n");
    fprintf(stdout, "last_chunk_size: 0\n");
    fprintf(stdout, "chunking_policy: unsupported-source-only\n");
    fprintf(stdout, "chunking_status: unsupported\n");
    fprintf(stdout, "prefill_boundary_status: unsupported\n");
    fprintf(stdout, "prefill_position_start: unknown\n");
    fprintf(stdout, "prefill_position_end: unknown\n");
    fprintf(stdout, "prefill_context_ready: false\n");
    fprintf(stdout, "full_transformer_prefill_ready: false\n");
    fprintf(stdout, "decode_position_policy: unsupported-source-only\n");
    fprintf(stdout, "decode_start_position: unknown\n");
    fprintf(stdout, "decode_position_status: unsupported\n");
    fprintf(stdout, "decode_context_ready: false\n");
    fprintf(stdout, "decode_ready: false\n");
    fprintf(stdout, "overflow_policy: unsupported-source-only\n");
    fprintf(stdout, "context_overflow: unsupported\n");
    fprintf(stdout, "overflow_check_status: unsupported\n");
    fprintf(stdout, "overflow_stop_reason: unsupported\n");
    fprintf(stdout, "overflow_mutates_state: false\n");
    fprintf(stdout, "attention_dependency_status: unsupported-source-only\n");
    fprintf(stdout, "attention_context_ready: false\n");
    fprintf(stdout, "attention_position_policy: planned\n");
    fprintf(stdout, "rope_context_status: planned\n");
    fprintf(stdout, "mask_context_status: planned\n");
    fprintf(stdout, "kv_dependency_status: unsupported-source-only\n");
    fprintf(stdout, "kv_capacity_required: true\n");
    fprintf(stdout, "kv_capacity_status: planned\n");
    fprintf(stdout, "kv_context_positions: unknown\n");
    fprintf(stdout, "kv_context_ready: false\n");
    fprintf(stdout, "real_attention_kv_ready: false\n");
    fprintf(stdout, "generation_context_status: unsupported-full-model\n");
    fprintf(stdout, "bounded_generation_context_policy: diagnostic-only-not-run\n");
    fprintf(stdout, "full_generation_context_ready: false\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "context_blockers: source-only target has no YVEX-produced GGUF tensor inventory; GLM context mapping planned; real transformer prefill unsupported\n");
    fprintf(stdout, "next_required_rows: MOE.CLASS.0,source-manifest-context-profile,YVEX-produced-GGUF,real-transformer-prefill,real-decode,GEN.DEEPSEEK.0\n");
    fprintf(stdout, "cleanup_attempted: true\n");
    fprintf(stdout, "cleanup_status: pass\n");
                                            ...
                                            const char *reason)
{
    context_print_common_header(options,
                                ...
                                "unsupported");
    fprintf(stdout, "model_max_context: unknown\n");
    fprintf(stdout, "model_max_context_source: unsupported-family\n");
    fprintf(stdout, "model_max_context_status: unsupported\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "requested_context_source: none\n");
    fprintf(stdout, "requested_context_status: not-requested\n");
    fprintf(stdout, "active_context: unsupported\n");
    fprintf(stdout, "active_context_source: unsupported-family\n");
    fprintf(stdout, "active_context_status: unsupported\n");
    fprintf(stdout, "token_input_status: not-performed\n");
    fprintf(stdout, "token_count: 0\n");
    fprintf(stdout, "prompt_token_count: 0\n");
    fprintf(stdout, "prefill_token_count: 0\n");
    fprintf(stdout, "generated_token_count: 0\n");
    ...
    fprintf(stdout, "chunk_count: 0\n");
    fprintf(stdout, "last_chunk_size: 0\n");
    fprintf(stdout, "chunking_policy: unsupported-family\n");
    fprintf(stdout, "chunking_status: unsupported\n");
    fprintf(stdout, "prefill_boundary_status: unsupported\n");
    fprintf(stdout, "prefill_position_start: unknown\n");
    fprintf(stdout, "prefill_position_end: unknown\n");
    fprintf(stdout, "prefill_context_ready: false\n");
    fprintf(stdout, "full_transformer_prefill_ready: false\n");
    fprintf(stdout, "decode_position_policy: unsupported-family\n");
    fprintf(stdout, "decode_start_position: unknown\n");
    fprintf(stdout, "decode_position_status: unsupported\n");
    fprintf(stdout, "decode_context_ready: false\n");
    fprintf(stdout, "decode_ready: false\n");
    fprintf(stdout, "overflow_policy: unsupported-family\n");
    fprintf(stdout, "context_overflow: unsupported\n");
    fprintf(stdout, "overflow_check_status: unsupported\n");
    fprintf(stdout, "overflow_stop_reason: unsupported\n");
    fprintf(stdout, "overflow_mutates_state: false\n");
    fprintf(stdout, "attention_dependency_status: unsupported-family\n");
    fprintf(stdout, "attention_context_ready: false\n");
    fprintf(stdout, "attention_position_policy: unsupported\n");
    fprintf(stdout, "rope_context_status: unsupported\n");
    fprintf(stdout, "mask_context_status: unsupported\n");
    fprintf(stdout, "kv_dependency_status: unsupported-family\n");
    fprintf(stdout, "kv_capacity_required: unknown\n");
    fprintf(stdout, "kv_capacity_status: unsupported\n");
    fprintf(stdout, "kv_context_positions: unknown\n");
    fprintf(stdout, "kv_context_ready: false\n");
    fprintf(stdout, "real_attention_kv_ready: false\n");
    fprintf(stdout, "generation_context_status: unsupported-full-model\n");
    ...
    fprintf(stdout, "full_generation_context_ready: false\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "context_blockers: %s\n", reason ? reason : "unsupported context family");
    fprintf(stdout, "next_required_rows: CONTEXT.CLASS.0,MOE.CLASS.0,real-transformer-prefill,real-decode,GEN.DEEPSEEK.0\n");
    fprintf(stdout, "cleanup_attempted: true\n");
    fprintf(stdout, "cleanup_status: pass\n");
                                ...
                                int selected_target)
{
    ...
    if (options && options->tokens_text) {
        ...
        if (rc != YVEX_OK) {
            context_print_common_header(options,
                                        ...
                                        coverage_status,
                                        coverage_status);
            fprintf(stdout, "token_input_status: failed\n");
            fprintf(stdout, "reason: %s\n", yvex_error_message(&err));
            fprintf(stdout, "cleanup_attempted: true\n");
            fprintf(stdout, "cleanup_status: pass\n");
    ...
    if (options && options->output_mode != YVEX_MODELS_OUTPUT_AUDIT) {
        ...
        fprintf(stdout, "next: V010.CONTEXT.*\n");
        fprintf(stdout, "boundary: report-only, no runtime execution\n");
        return 0;
    }
    ...
    context_print_common_header(options,
                                ...
                                coverage_status,
                                coverage_status);
    fprintf(stdout, "report_options.include_attention: %s\n", options && options->include_attention ? "true" : "false");
    fprintf(stdout, "report_options.include_kv: %s\n", options && options->include_kv ? "true" : "false");
    fprintf(stdout, "report_options.include_prefill: %s\n", options && options->include_prefill ? "true" : "false");
    fprintf(stdout, "report_options.include_decode: %s\n", options && options->include_decode ? "true" : "false");
    fprintf(stdout, "report_options.include_blockers: %s\n", options && options->include_blockers ? "true" : "false");

    fprintf(stdout, "model_max_context: ");
    if (model_max_context > 0ull) fprintf(stdout, "%llu", model_max_context);
    else fprintf(stdout, "unknown");
    fprintf(stdout, "\n");
    fprintf(stdout, "model_max_context_source: %s\n", model_max_context > 0ull ? source : "metadata-missing-or-selected-slice");
    fprintf(stdout, "model_max_context_status: %s\n", model_max_context > 0ull ? "available" : "planned");
    fprintf(stdout, "requested_context: ");
    if (options && options->context_length_seen) fprintf(stdout, "%llu", options->context_length);
    else fprintf(stdout, "not-requested");
    fprintf(stdout, "\n");
    fprintf(stdout, "requested_context_source: %s\n", options && options->context_length_seen ? "operator-request" : "none");
    fprintf(stdout, "requested_context_status: %s\n", options && options->context_length_seen ? "reported-only" : "not-requested");
    fprintf(stdout, "active_context: ");
    if (active_context > 0ull) fprintf(stdout, "%llu", active_context);
    else fprintf(stdout, "diagnostic");
    fprintf(stdout, "\n");
    fprintf(stdout, "active_context_source: %s\n",
    ...
    fprintf(stdout, "active_context_status: %s\n", active_context > 0ull ? "reported-only" : "diagnostic");

    fprintf(stdout, "token_input_status: %s\n", options && options->tokens_text ? "available" : "not-provided");
    fprintf(stdout, "token_count: %llu\n", token_count);
    fprintf(stdout, "prompt_token_count: %llu\n", token_count);
    fprintf(stdout, "prefill_token_count: %llu\n", token_count);
    fprintf(stdout, "generated_token_count: 0\n");
    fprintf(stdout, "token_range_start: %s\n", token_count > 0ull ? "0" : "none");
    fprintf(stdout, "token_range_end: ");
    if (token_count > 0ull) fprintf(stdout, "%llu", token_count - 1ull);
    else fprintf(stdout, "none");
    fprintf(stdout, "\n");

    fprintf(stdout, "chunking_required: %s\n", options && options->chunk_size_seen ? "true" : "false");
    fprintf(stdout, "chunk_size: ");
    if (options && options->chunk_size_seen) fprintf(stdout, "%llu", options->chunk_size);
    else fprintf(stdout, "not-requested");
    fprintf(stdout, "\n");
    fprintf(stdout, "chunk_count: %llu\n", chunk_count);
    fprintf(stdout, "last_chunk_size: %llu\n", last_chunk);
    fprintf(stdout, "chunking_policy: %s\n", options && options->chunk_size_seen ? "explicit-token-chunks-report-only" : "not-requested");
    fprintf(stdout, "chunking_status: %s\n", options && options->chunk_size_seen ? "report-only" : "planned");

    fprintf(stdout, "prefill_boundary_status: report-only\n");
    fprintf(stdout, "prefill_position_start: %s\n", token_count > 0ull ? "0" : "none");
    fprintf(stdout, "prefill_position_end: %llu\n", token_count);
    fprintf(stdout, "prefill_context_ready: false\n");
    fprintf(stdout, "full_transformer_prefill_ready: false\n");
    fprintf(stdout, "decode_position_policy: prefill-end-token-count\n");
    fprintf(stdout, "decode_start_position: %llu\n", decode_start);
    fprintf(stdout, "decode_position_status: report-only\n");
    fprintf(stdout, "decode_context_ready: false\n");
    fprintf(stdout, "decode_ready: false\n");

    fprintf(stdout, "overflow_policy: bounded-diagnostic-refuse-before-mutation\n");
    fprintf(stdout, "context_overflow: %s\n", overflow_known ? (overflow ? "overflow" : "none") : "unknown");
    fprintf(stdout, "overflow_check_status: %s\n", overflow_known ? "pass" : "unknown");
    fprintf(stdout, "overflow_stop_reason: %s\n", overflow ? "context-limit" : "none");
    fprintf(stdout, "overflow_mutates_state: false\n");

    fprintf(stdout, "attention_dependency_status: %s\n", has_qkv ? "blocked-runtime-integration" : "blocked-missing-qkv");
    fprintf(stdout, "attention_context_ready: false\n");
    fprintf(stdout, "attention_position_policy: rope-or-family-specific-planned\n");
    fprintf(stdout, "rope_context_status: planned\n");
    fprintf(stdout, "mask_context_status: planned\n");
    fprintf(stdout, "kv_dependency_status: %s\n", has_qkv ? "blocked-runtime-integration" : "blocked-missing-qkv");
    fprintf(stdout, "kv_capacity_required: true\n");
    fprintf(stdout, "kv_capacity_status: planned\n");
    fprintf(stdout, "kv_context_positions: %s", active_context > 0ull ? "" : "unknown");
    if (active_context > 0ull) fprintf(stdout, "%llu", active_context);
    fprintf(stdout, "\n");
    fprintf(stdout, "kv_context_ready: false\n");
    fprintf(stdout, "real_attention_kv_ready: false\n");
    fprintf(stdout, "generation_context_status: bounded-diagnostic-only\n");
    fprintf(stdout, "bounded_generation_context_policy: context-limit-pre-append\n");
    fprintf(stdout, "full_generation_context_ready: false\n");
    fprintf(stdout, "generation_ready: false\n");
    fprintf(stdout, "context_blockers: %s\n",
           selected_target
    ...
    fprintf(stdout, "next_required_rows: MOE.CLASS.0,RUNTIME.KV.1,real-transformer-prefill,real-decode,real-output-head-logits,GEN.DEEPSEEK.0\n");
    fprintf(stdout, "cleanup_attempted: true\n");
    fprintf(stdout, "cleanup_status: pass\n");
                                    ...
                                    unsigned int top_count)
{
    ...
    for (i = 0; i < top_count; ++i) {
        ...
        fprintf(stdout, "largest_tensor_%u: name=%s dtype=%s role=%s dims=%s bytes=%llu\n",
               i,
...
static void fullmodel_print_phase(unsigned int index,
                                  ...
                                  const char *blocker)
{
    fprintf(stdout, "phase.%u.name: %s\n", index, name ? name : "");
    fprintf(stdout, "phase.%u.status: %s\n", index, status ? status : "planned");
    fprintf(stdout, "phase.%u.tensor_count: %llu\n", index, tensor_count);
    fprintf(stdout, "phase.%u.tensor_bytes: %llu\n", index, tensor_bytes);
    fprintf(stdout, "phase.%u.residency: %s\n", index, residency ? residency : "not-applicable");
    fprintf(stdout, "phase.%u.required: %s\n", index, required ? "true" : "false");
    fprintf(stdout, "phase.%u.blocked: %s\n", index, blocked ? "true" : "false");
    fprintf(stdout, "phase.%u.blocker: %s\n", index, blocker && blocker[0] ? blocker : "none");
}
                                            ...
                                            const char *blocker)
{
    ...
    fprintf(stdout, "collection.%s.tensor_count: %llu\n", name, tensor_count);
    fprintf(stdout, "collection.%s.tensor_bytes: %llu\n", name, tensor_bytes);
    fprintf(stdout, "collection.%s.required_for_generation: %s\n",
           name, fullmodel_required_bool(required_for_generation));
    fprintf(stdout, "collection.%s.present: %s\n", name, fullmodel_required_bool(present));
    fprintf(stdout, "collection.%s.placement: %s\n", name, placement ? placement : "unknown");
    fprintf(stdout, "collection.%s.materialization_phase: %s\n", name, phase ? phase : "collection-grouping");
    fprintf(stdout, "collection.%s.runtime_consumer: %s\n", name, runtime_consumer ? runtime_consumer : "planned");
    fprintf(stdout, "collection.%s.blocker: %s\n", name, blocker && blocker[0] ? blocker : "none");
}
...
static unsigned int fullmodel_print_blocker(unsigned int index,
                                            ...
                                            int blocks_generation)
{
    fprintf(stdout, "blocker.%u.category: %s\n", index, category ? category : "runtime-consumer");
    fprintf(stdout, "blocker.%u.severity: %s\n", index, severity ? severity : "warning");
    fprintf(stdout, "blocker.%u.message: %s\n", index, message ? message : "");
    fprintf(stdout, "blocker.%u.blocks_full_materialization: %s\n",
           index, blocks_full_materialization ? "true" : "false");
    fprintf(stdout, "blocker.%u.blocks_generation: %s\n",
                                                 ...
                                                 int selected_target)
{
    ...
    fprintf(stdout, "architecture: %s\n", yvex_arch_name(arch));
    fprintf(stdout, "family: %s\n", fullmodel_family_from_arch(arch));
    fprintf(stdout, "qtype_summary: %s\n", dtype_summary ? dtype_summary : "none");
    fprintf(stdout, "dtype_summary: %s\n", dtype_summary ? dtype_summary : "none");
    fprintf(stdout, "required_role_coverage: %s\n", role_coverage ? role_coverage : "partial");
    fprintf(stdout, "missing_required_roles: %s\n", missing_roles ? missing_roles : "unknown");
    fprintf(stdout, "backend: %s\n", backend);
    fprintf(stdout, "residency: %s\n", residency);
    fprintf(stdout, "plan_status: %s\n", plan_status);
    fprintf(stdout, "materialization_plan_ready: %s\n", materialization_plan_ready ? "true" : "false");
    fprintf(stdout, "materialization_attempted: false\n");
    fprintf(stdout, "full_materialization_proof: false\n");
    fprintf(stdout, "full_model_execution: unsupported\n");
    fprintf(stdout, "generation_ready: false\n");
    ...
    fprintf(stdout, "plan_backend: %s\n", backend);
    fprintf(stdout, "plan_residency: %s\n", residency);
    fprintf(stdout, "plan_tensor_count: %llu\n", tensor_count);
    fprintf(stdout, "plan_tensor_bytes: %llu\n", total_tensor_bytes);
    fprintf(stdout, "plan_collection_count: 10\n");
    fprintf(stdout, "plan_phase_count: 11\n");
    fprintf(stdout, "plan_blocker_count: %u\n", blocker_count);
    fprintf(stdout, "plan_cleanup_required: true\n");
    fprintf(stdout, "plan_cleanup_phases: release-backend-buffers,release-host-staging,release-scratch,clear-partial-residency,preserve-failure-report\n");

    fprintf(stdout, "backend_available: %s\n", fit.available ? "true" : "false");
    fprintf(stdout, "backend_memory_known: %s\n", fit.memory_known ? "true" : "false");
    if (fit.memory_known) {
        fprintf(stdout, "backend_memory_total_bytes: %llu\n", fit.total_bytes);
        fprintf(stdout, "backend_memory_available_bytes: %llu\n", fit.available_bytes);
    } else {
        fprintf(stdout, "backend_memory_total_bytes: unknown\n");
        fprintf(stdout, "backend_memory_available_bytes: unknown\n");
    }
    fprintf(stdout, "backend_required_bytes: %llu\n", fit.required_bytes);
    fprintf(stdout, "backend_fit_status: %s\n", fit.fit_status);
    fprintf(stdout, "backend_fit_reason: %s\n", fit.fit_reason);
    fprintf(stdout, "backend_allocation_attempted: false\n");
    ...
    fprintf(stdout, "cleanup_plan_required: true\n");
    fprintf(stdout, "cleanup_plan_phases: release-backend-buffers,release-host-staging,release-scratch,clear-partial-residency,preserve-failure-report\n");
    fprintf(stdout, "cleanup_idempotent_required: true\n");
    fprintf(stdout, "cleanup_failure_policy: preserve-failure-report\n");
    fprintf(stdout, "next_required_row: FULLMODEL.2\n");
    fprintf(stdout, "proof_ready_for_fullmodel_2: false\n");
    fprintf(stdout, "fullmodel_2_blockers: %s\n",
           selected_target
...
int yvex_fullmodel_command(int arg_count, char **args)
{
    ...
    fprintf(stdout, "fullmodel: report\n");
    fprintf(stdout, "status: fullmodel-report\n");
    fprintf(stdout, "model: %s\n", options.model);
    fprintf(stdout, "model_resolved_path: %s\n", ref.path ? ref.path : "");
    fprintf(stdout, "target_id: %s\n", target_id);
    fprintf(stdout, "target_class: %s\n", target_class);
    ...
    fprintf(stdout, "tensor_count: %llu\n", tensor_count);
    fprintf(stdout, "tensor_inventory_status: pass\n");
    fprintf(stdout, "metadata_status: pass\n");
    fprintf(stdout, "architecture: %s\n", yvex_arch_name(arch));
    fprintf(stdout, "family: %s\n", fullmodel_family_from_arch(arch));
    fprintf(stdout, "model_class: %s\n", model_class);
    fprintf(stdout, "fullmodel_inventory: %s\n", inventory_status);
    fprintf(stdout, "full_runtime_model: false\n");
    fprintf(stdout, "qtype_summary: %s\n", dtype_summary);
    fprintf(stdout, "dtype_summary: %s\n", dtype_summary);
    fprintf(stdout, "total_tensor_bytes: %llu\n", total_tensor_bytes);
    fprintf(stdout, "estimated_cpu_resident_bytes: %llu\n", total_tensor_bytes);
    fprintf(stdout, "estimated_cuda_resident_bytes: %llu\n", total_tensor_bytes);
    fprintf(stdout, "estimated_kv_bytes: planned\n");
    fprintf(stdout, "estimated_scratch_bytes: planned\n");
    fprintf(stdout, "estimated_total_runtime_bytes: unknown\n");
    fprintf(stdout, "backend: %s\n", options.backend);
    fprintf(stdout, "backend_placement_status: %s\n", backend_placement_status);
    fprintf(stdout, "cpu_placement: %s\n", cpu_placement);
    fprintf(stdout, "cuda_placement: %s\n", cuda_placement);
    fprintf(stdout, "cuda_available: %s\n", yvex_backend_cuda_available() ? "true" : "false");
    fprintf(stdout, "cuda_memory_status: %s\n", yvex_backend_cuda_available() ? "probe-available-no-allocation" : "unavailable");
    fprintf(stdout, "residency_plan: report-only-no-allocation\n");
    fprintf(stdout, "tensor_collections_status: %s\n", role_coverage);
    fprintf(stdout, "collection_detected: %s\n", tensor_count > 0ull ? "yes" : "no");
    fprintf(stdout, "collection_supported: partial\n");
    fprintf(stdout, "runtime_consumer: unsupported\n");
    fprintf(stdout, "embedding_tensors: %llu\n", collections.embedding);
    fprintf(stdout, "normalization_tensors: %llu\n", collections.normalization);
    fprintf(stdout, "attention_tensors: %llu\n", collections.attention);
    fprintf(stdout, "kv_cache_requirements: planned\n");
    fprintf(stdout, "mlp_tensors: %llu\n", collections.mlp);
    fprintf(stdout, "moe_tensors: %llu\n", collections.moe);
    fprintf(stdout, "output_tensors: %llu\n", collections.output);
    fprintf(stdout, "tokenizer_tensors: %llu\n", collections.tokenizer);
    fprintf(stdout, "unknown_tensors: %llu\n", collections.unknown);
    fprintf(stdout, "required_role_coverage: %s\n", role_coverage);
    fprintf(stdout, "missing_required_roles: %s\n", missing_roles);
    fprintf(stdout, "unsupported_required_roles: %s\n", unsupported_roles);
    fprintf(stdout, "runtime_blockers: full tensor set missing; attention projection tensors may be missing; MLP/MoE tensors may be missing; output head may be missing; real transformer prefill unsupported; real DeepSeek decode unsupported; real output-head logits unsupported; real vocabulary sampling unsupported; full model materialization not implemented\n");
    print_fullmodel_common_boundaries();
    fprintf(stdout, "largest_tensor_report_limit: %llu\n", options.limit_tensors);
