...
static void sample_print_summary(const yvex_sampling_summary *summary,
                                 ...
                                 const char *status)
{
    fprintf(stdout, "sample: token\n");
    fprintf(stdout, "status: sample-token\n");
    ...
    fprintf(stdout, "bounded_sampling_ready: %s\n",
           summary && summary->bounded_sampling_ready ? "true" : "false");
    fprintf(stdout, "real_vocab_sampling: false\n");
    fprintf(stdout, "real_model_sampling: false\n");
...
int yvex_sample_command(int arg_count, char **args)
{
    ...
    for (i = 2; i < arg_count; ++i) {
        if (strcmp(args[i], "--model") == 0) {
            ...
            if (i + 1 >= arg_count || !sample_parse_strategy(args[i + 1], &strategy)) {
