/*
 * yvex_kv_report.c - typed KV report construction.
 *
 * Owner:
 *   src/generation
 *
                                   ...
                                   yvex_error *err)
{
    ...
    if (request->append_demo) {
        ...
        for (i = 0ull; i < append_target; ++i) {
            fill_kv_demo_values(append_values, value_count, i);
    ...
    if (request->read_requested) {
        ...
        report->read_checksum = checksum_kv_values(read_values, value_count);
    }
