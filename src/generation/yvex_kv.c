...
static void kv_print_phases(const char *status, const char *failed_phase)
{
    ...
    for (i = 0u; i < sizeof(names) / sizeof(names[0]); ++i) {
        ...
        fprintf(stdout, "kv_phase.%lu.name: %s\n", i, names[i]);
        fprintf(stdout, "kv_phase.%lu.status: %s\n", i, phase_status);
    }
}

static void kv_print_next_rows(void)
{
    fprintf(stdout, "next_required_rows: ATTENTION.CLASS.0 complete,CONTEXT.CLASS.0,RUNTIME.KV.1,RUNTIME.KV.2,RUNTIME.KV.3,real-transformer-prefill,real-decode,real-output-head-logits,GEN.DEEPSEEK.0\n");
}
                                   ...
                                   const char *kv_class_status)
{
    ...
    fprintf(stdout, "kv_stage: report-only\n");
    fprintf(stdout, "kv_support_status: report-only\n");
...
{
    fprintf(stdout, "diagnostic_kv_available: true\n");
    fprintf(stdout, "diagnostic_kv_boundary: segment-summary/minimal diagnostic KV\n");
    fprintf(stdout, "real_attention_kv: unsupported\n");
    fprintf(stdout, "real_attention_kv_write_ready: false\n");
    fprintf(stdout, "real_attention_kv_read_ready: false\n");
    fprintf(stdout, "decode_kv_consumer_ready: false\n");
}
                                        ...
                                        unsigned long long max_context)
{
    ...
    fprintf(stdout, "kv_source: attention-qkv-requirements\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_layout_status: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    fprintf(stdout, "kv_dtype_status: planned\n");
    fprintf(stdout, "kv_layers: unknown\n");
    fprintf(stdout, "kv_layers_status: planned\n");
    fprintf(stdout, "kv_heads: unknown\n");
    fprintf(stdout, "kv_heads_status: planned\n");
    fprintf(stdout, "kv_head_dim: unknown\n");
    fprintf(stdout, "kv_head_dim_status: planned\n");
    fprintf(stdout, "kv_positions: context-dependent\n");
    fprintf(stdout, "kv_capacity: planned\n");
    fprintf(stdout, "kv_capacity_status: planned\n");
    fprintf(stdout, "kv_indexing: layer-head-position-token-order\n");
    fprintf(stdout, "kv_layer_indexing: planned\n");
    fprintf(stdout, "kv_head_indexing: planned\n");
    fprintf(stdout, "kv_position_indexing: planned\n");
    fprintf(stdout, "kv_token_order_policy: prompt-order-then-append-order\n");
    fprintf(stdout, "kv_residency_class: planned\n");
    fprintf(stdout, "kv_residency_status: planned\n");
    fprintf(stdout, "kv_cpu_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_cuda_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_host_staged_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_ssd_staged_status: planned\n");
    fprintf(stdout, "kv_ssd_streamed_status: planned\n");
    fprintf(stdout, "kv_paged_status: planned\n");
    fprintf(stdout, "kv_chunked_status: planned\n");
    fprintf(stdout, "kv_quantized_status: planned\n");
    fprintf(stdout, "kv_managed_memory_status: planned\n");
    fprintf(stdout, "context_required: true\n");
    fprintf(stdout, "context_length_source: %s\n", max_context > 0ull ? "model-metadata" : "planned");
    fprintf(stdout, "max_context: %s", max_context > 0ull ? "" : "unknown");
    if (max_context > 0ull) {
        fprintf(stdout, "%llu", max_context);
    }
    fprintf(stdout, "\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_capacity_status: planned\n");
    fprintf(stdout, "context_overflow_policy: planned-refusal-before-mutation\n");
    ...
    fprintf(stdout, "attention_q_status: %s\n", kv_role_status(scan && scan->has_q));
    fprintf(stdout, "attention_k_status: %s\n", kv_role_status(scan && scan->has_k));
    fprintf(stdout, "attention_v_status: %s\n", kv_role_status(scan && scan->has_v));
    fprintf(stdout, "attention_o_status: %s\n", kv_role_status(scan && scan->has_o));
    fprintf(stdout, "attention_runtime_ready: false\n");
    fprintf(stdout, "full_transformer_attention_ready: false\n");
    fprintf(stdout, "prefill_kv_write_required: true\n");
    fprintf(stdout, "prefill_kv_write_ready: false\n");
    fprintf(stdout, "decode_kv_read_required: true\n");
    fprintf(stdout, "decode_kv_read_ready: false\n");
    fprintf(stdout, "qkv_role_coverage: %s\n", has_qkv ? "present" : "missing");
}

static void kv_print_role_summary(const yvex_kv_role_scan *scan)
{
    fprintf(stdout, "tensor_inventory_status: loaded-gguf-directory\n");
    fprintf(stdout, "tensor_count: %llu\n", scan ? scan->tensor_count : 0ull);
    fprintf(stdout, "tensor_bytes: %llu\n", scan ? scan->total_tensor_bytes : 0ull);
    fprintf(stdout, "role.token_embedding.status: %s\n", kv_role_status(scan && scan->has_token_embedding));
    fprintf(stdout, "role.attention_norm.status: %s\n", kv_role_status(scan && scan->has_attention_norm));
    fprintf(stdout, "role.q_projection.status: %s\n", kv_role_status(scan && scan->has_q));
    fprintf(stdout, "role.k_projection.status: %s\n", kv_role_status(scan && scan->has_k));
    fprintf(stdout, "role.v_projection.status: %s\n", kv_role_status(scan && scan->has_v));
    fprintf(stdout, "role.o_projection.status: %s\n", kv_role_status(scan && scan->has_o));
    fprintf(stdout, "role.output_head.status: %s\n", kv_role_status(scan && scan->has_output_head));
}
...
{
    if (!scan || !scan->has_q || !scan->has_k || !scan->has_v) {
        fprintf(stdout, "kv_blockers: ");
        if (!scan || !scan->has_q) {
            fprintf(stdout, "q projection tensor missing; ");
        }
        if (!scan || !scan->has_k) {
            fprintf(stdout, "k projection tensor missing; ");
        }
        if (!scan || !scan->has_v) {
            fprintf(stdout, "v projection tensor missing; ");
        }
        fprintf(stdout, "real attention-backed KV writes unsupported; decode KV consumer unsupported; context class report pending; KV capacity estimator pending\n");
        return;
    }
    fprintf(stdout, "kv_blockers: real attention-backed KV writes unsupported; decode KV consumer unsupported; KV capacity estimator pending; context class report pending; full transformer prefill unsupported\n");
}
...
static int kv_print_source_only_report(const yvex_kv_report_options *options)
{
    ...
    if (options && !options->audit_output) {
        fprintf(stdout, "report: kv\n");
        fprintf(stdout, "model: %s\n", options->model ? options->model : "");
        fprintf(stdout, "family: %s\n", family ? family : "unknown");
        fprintf(stdout, "status: unsupported\n");
        fprintf(stdout, "top_blocker: source-only target has no YVEX-produced GGUF tensor inventory\n");
        fprintf(stdout, "next: V010.KV.*\n");
    ...
    fprintf(stdout, "kv_required: true\n");
    fprintf(stdout, "kv_source: planned-family-mapping\n");
    fprintf(stdout, "kv_layout: planned\n");
    fprintf(stdout, "kv_layout_status: planned\n");
    fprintf(stdout, "kv_dtype: planned\n");
    fprintf(stdout, "kv_dtype_status: planned\n");
    ...
    fprintf(stdout, "kv_head_dim: unknown\n");
    fprintf(stdout, "kv_head_dim_status: planned\n");
    fprintf(stdout, "kv_positions: planned\n");
    fprintf(stdout, "kv_capacity: planned\n");
    fprintf(stdout, "kv_capacity_status: planned\n");
    fprintf(stdout, "kv_indexing: planned\n");
    fprintf(stdout, "kv_layer_indexing: planned\n");
    fprintf(stdout, "kv_head_indexing: planned\n");
    fprintf(stdout, "kv_position_indexing: planned\n");
    fprintf(stdout, "kv_token_order_policy: planned\n");
    fprintf(stdout, "kv_residency_class: planned\n");
    fprintf(stdout, "kv_residency_status: planned\n");
    fprintf(stdout, "kv_cpu_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_cuda_bytes_estimate: unknown\n");
    ...
    fprintf(stdout, "kv_quantized_status: planned\n");
    fprintf(stdout, "context_required: true\n");
    fprintf(stdout, "context_length_source: planned-source-manifest\n");
    fprintf(stdout, "max_context: unknown\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_capacity_status: planned\n");
    fprintf(stdout, "context_overflow_policy: planned\n");
    ...
    fprintf(stdout, "decode_kv_read_required: true\n");
    fprintf(stdout, "decode_kv_read_ready: false\n");
    fprintf(stdout, "kv_blockers: source-only target has no YVEX-produced GGUF tensor inventory; GLM KV mapping planned; real attention-backed KV writes unsupported\n");
    kv_print_next_rows();
                                              ...
                                              const char *reason)
{
    ...
    kv_print_report_header(options, NULL, "kv-report-unsupported",
                           ...
                           "unsupported");
    kv_print_boundary_fields();
    fprintf(stdout, "tensor_inventory_status: not-performed\n");
    fprintf(stdout, "kv_required: unknown\n");
    fprintf(stdout, "kv_source: unsupported-family\n");
    fprintf(stdout, "kv_layout: unsupported\n");
    fprintf(stdout, "kv_layout_status: unsupported\n");
    fprintf(stdout, "kv_dtype: unsupported\n");
    fprintf(stdout, "kv_dtype_status: unsupported\n");
    fprintf(stdout, "kv_layers: unknown\n");
    fprintf(stdout, "kv_layers_status: unsupported\n");
    fprintf(stdout, "kv_heads: unknown\n");
    fprintf(stdout, "kv_heads_status: unsupported\n");
    fprintf(stdout, "kv_head_dim: unknown\n");
    fprintf(stdout, "kv_head_dim_status: unsupported\n");
    fprintf(stdout, "kv_positions: unknown\n");
    fprintf(stdout, "kv_capacity: unsupported\n");
    fprintf(stdout, "kv_capacity_status: unsupported\n");
    fprintf(stdout, "kv_indexing: unsupported\n");
    fprintf(stdout, "kv_layer_indexing: unsupported\n");
    fprintf(stdout, "kv_head_indexing: unsupported\n");
    fprintf(stdout, "kv_position_indexing: unsupported\n");
    fprintf(stdout, "kv_token_order_policy: unsupported\n");
    fprintf(stdout, "kv_residency_class: unsupported\n");
    fprintf(stdout, "kv_residency_status: unsupported\n");
    fprintf(stdout, "kv_cpu_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_cuda_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_host_staged_bytes_estimate: unknown\n");
    fprintf(stdout, "kv_ssd_staged_status: unsupported\n");
    fprintf(stdout, "kv_paged_status: unsupported\n");
    fprintf(stdout, "kv_chunked_status: unsupported\n");
    fprintf(stdout, "kv_quantized_status: unsupported\n");
    fprintf(stdout, "context_required: unknown\n");
    fprintf(stdout, "context_length_source: unsupported-family\n");
    fprintf(stdout, "max_context: unknown\n");
    fprintf(stdout, "requested_context: not-requested\n");
    fprintf(stdout, "context_capacity_status: unsupported\n");
    fprintf(stdout, "context_overflow_policy: unsupported\n");
    fprintf(stdout, "attention_dependency_status: unsupported-family\n");
    fprintf(stdout, "attention_q_required: unknown\n");
    fprintf(stdout, "attention_k_required: unknown\n");
    fprintf(stdout, "attention_v_required: unknown\n");
    fprintf(stdout, "attention_runtime_ready: false\n");
    fprintf(stdout, "full_transformer_attention_ready: false\n");
    fprintf(stdout, "prefill_kv_write_required: unknown\n");
    fprintf(stdout, "prefill_kv_write_ready: false\n");
    fprintf(stdout, "decode_kv_read_required: unknown\n");
    fprintf(stdout, "decode_kv_read_ready: false\n");
    fprintf(stdout, "kv_blockers: %s\n", reason ? reason : "unsupported family");
    kv_print_next_rows();
    fprintf(stdout, "cleanup_attempted: true\n");
    fprintf(stdout, "cleanup_status: pass\n");
...
static int command_kv_report(int arg_count, char **args)
{
    ...
    for (i = 3; i < arg_count; ++i) {
        if (strcmp(args[i], "--model") == 0) {
            ...
            if (!kv_parse_output_mode(args[++i], &options.audit_output)) {
    ...
    if (!options.audit_output) {
        ...
        fprintf(stdout, "top_blocker: real attention-backed KV unsupported\n");
        fprintf(stdout, "next: V010.KV.*\n");
        fprintf(stdout, "boundary: report-only, no runtime execution\n");
        close_model_context(&ctx);
        yvex_model_ref_clear(&ref);
    ...
    fprintf(stdout, "report_options.include_context: %s\n", kv_bool(options.include_context));
    ...
    fprintf(stdout, "full_kv_allocation_proof: false\n");
    fprintf(stdout, "cuda_full_kv_allocation_proof: false\n");
    fprintf(stdout, "paged_kv_implementation: false\n");
    fprintf(stdout, "chunked_kv_runtime_implementation: false\n");
    fprintf(stdout, "ssd_backed_kv: false\n");
    fprintf(stdout, "quantized_kv_runtime: false\n");
...
static int command_kv(int arg_count, char **args)
{
    yvex_kv_shape shape;
    yvex_kv_cache *kv = NULL;
    yvex_kv_summary summary;
    ...
    if (strcmp(args[2], "report") == 0) {
        return command_kv_report(arg_count, args);
    }

    for (i = 2; i < arg_count; ++i) {
        if (strcmp(args[i], "--layers") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &shape.layer_count)) {
                fprintf(stderr, "yvex: --layers requires a positive integer\n");
                return 2;
            }
            i += 1;
        } else if (strcmp(args[i], "--heads") == 0 || strcmp(args[i], "--lanes") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &shape.kv_head_count)) {
                fprintf(stderr, "yvex: --heads requires a positive integer\n");
                return 2;
            }
            i += 1;
        } else if (strcmp(args[i], "--head-dim") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &shape.head_dim)) {
                fprintf(stderr, "yvex: --head-dim requires a positive integer\n");
                return 2;
            }
            i += 1;
        } else if (strcmp(args[i], "--capacity") == 0) {
            if (i + 1 >= arg_count || !parse_positive_ull(args[i + 1], &shape.capacity)) {
                fprintf(stderr, "yvex: --capacity requires a positive integer\n");
                return 2;
            }
            i += 1;
        } else if (strcmp(args[i], "--append-demo") == 0) {
            append_demo = 1;
        } else if (strcmp(args[i], "--read-position") == 0) {
            if (i + 1 >= arg_count || !parse_ull_allow_zero(args[i + 1], &read_position)) {
    ...
    fprintf(stdout, "kv: ownership\n");
    fprintf(stdout, "kv_created: true\n");
    fprintf(stdout, "session_owned: %s\n", summary.session_owned ? "true" : "false");
    fprintf(stdout, "layers: %llu\n", summary.layer_count);
    fprintf(stdout, "heads: %llu\n", summary.kv_head_count);
    ...
    fprintf(stdout, "values_per_position: %llu\n", summary.values_per_position);
    fprintf(stdout, "bytes_per_position: %llu\n", summary.bytes_per_position);
    fprintf(stdout, "planned_bytes: %llu\n", summary.bytes);
    fprintf(stdout, "allocated_bytes: %llu\n", summary.allocated_bytes);
    fprintf(stdout, "append_count: %llu\n", summary.append_count);
    fprintf(stdout, "read_count: %llu\n", summary.read_count);
    fprintf(stdout, "written_positions: %llu\n", summary.written_positions);
    fprintf(stdout, "last_appended_position: %llu\n", appended_position);
    if (read_requested) {
        unsigned long long sample_count = value_count < 8ull ? value_count : 8ull;
        unsigned long long j;
        fprintf(stdout, "read_position: %llu\n", read_position);
        fprintf(stdout, "read_value_count: %llu\n", value_count);
        fprintf(stdout, "read_checksum: %llu\n", read_checksum);
        fprintf(stdout, "read_sample_values:");
        for (j = 0; j < sample_count; ++j) {
            fprintf(stdout, "%s%.9g", j == 0 ? " " : ",", (double)read_values[j]);
        }
        fprintf(stdout, "\n");
    } else {
        fprintf(stdout, "read_position: not-requested\n");
        fprintf(stdout, "read_value_count: 0\n");
        fprintf(stdout, "read_checksum: 0\n");
    }
    fprintf(stdout, "overflow_status: %s\n",
    ...
    fprintf(stdout, "status: kv-owned\n");
...
{
    return command_kv(arg_count, args);
}
