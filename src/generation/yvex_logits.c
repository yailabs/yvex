                                 ...
                                 const char *status)
{
    ...
    fprintf(stdout, "logits: buffer\n");
    fprintf(stdout, "status: logits-buffer\n");
    ...
    fprintf(stdout, "bounded_logits_ready: %s\n",
           summary && summary->bounded_logits_ready ? "true" : "false");
    fprintf(stdout, "real_model_logits: false\n");
    fprintf(stdout, "real_model_output_head: false\n");
...
int yvex_logits_command(int arg_count, char **args)
{
    ...
    for (i = 2; i < arg_count; ++i) {
        if (strcmp(args[i], "--model") == 0) {
            ...
            fprintf(stderr, "yvex: unknown logits option: %s\n", args[i]);
            fprintf(stderr, "Try '" "yvex help logits' for usage.\n");
            return 2;
        }
    }

    if (!model_arg || !backend_name || !tokens_text || !segment_name) {
        fprintf(stderr, "usage: " "yvex logits --model FILE_OR_ALIAS --backend cpu|cuda --segment embedding-rmsnorm --tokens IDS [--layers N [--layer-hidden-dim N --layer-head-dim N --layer-ffn-dim N]] [--chunk-size N] [--position-start N] [--context-length N] [--attach-kv --kv-layers N --kv-heads N --kv-head-dim N --kv-capacity N] [--logits-count N]\n");
        return 2;
    }
    ...
    if (strcmp(segment_name, "embedding-rmsnorm") != 0) {
        fprintf(stderr, "yvex: unsupported logits segment: %s\n", segment_name);
        return 2;
    }
...
{
    fprintf(fp, "usage: " "yvex logits --model FILE_OR_ALIAS --backend cpu|cuda --segment embedding-rmsnorm --tokens IDS [--layers N [--layer-hidden-dim N --layer-head-dim N --layer-ffn-dim N]] [--chunk-size N] [--position-start N] [--context-length N] [--attach-kv --kv-layers N --kv-heads N --kv-head-dim N --kv-capacity N] [--logits-count N]\n\nLogits creates a bounded diagnostic logits buffer from the implemented decode state. It does not run the real model output head, sample, generate, or claim DeepSeek logits.\n");
}
