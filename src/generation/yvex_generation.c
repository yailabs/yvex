...
static void generate_print_optional_ull(const char *label,
                                        ...
                                        unsigned long long value)
{
    fprintf(stdout, "%s: ", label ? label : "value");
    if (seen) {
                                      ...
                                      unsigned long long count)
{
    ...
    fprintf(stdout, "%s: ", label ? label : "tokens");
    for (i = 0ull; tokens && i < count; ++i) {
        if (i > 0ull) {
            fprintf(stdout, ",");
        }
        fprintf(stdout, "%u", tokens[i]);
    }
    fprintf(stdout, "\n");
}
...
static void generate_print_runtime_sequence(const yvex_generate_summary *summary)
{
    ...
    fprintf(stdout, "runtime_token_sequence: ");
    if (summary) {
        for (i = 0ull; i < summary->prompt_token_count && i < YVEX_TOKEN_INPUT_MAX_TOKENS; ++i) {
            if (emitted > 0ull) {
                fprintf(stdout, ",");
            }
            fprintf(stdout, "%u", summary->prompt_tokens[i]);
            emitted += 1ull;
        }
        for (i = 0ull; i < summary->generated_token_count && i < YVEX_TOKEN_INPUT_MAX_TOKENS; ++i) {
            ...
            fprintf(stdout, "%u", summary->generated_tokens[i]);
            emitted += 1ull;
        }
    }
    fprintf(stdout, "\n");
                                            ...
                                            unsigned long long *category_count)
{
    ...
    fprintf(stdout, "trace.tokens.runtime_sequence: ");
    for (i = 0ull; i < summary->prompt_token_count && i < YVEX_TOKEN_INPUT_MAX_TOKENS; ++i) {
        if (emitted > 0ull) {
            fprintf(stdout, ",");
        }
                                   ...
                                   const char *segment_name)
{
    ...
    fprintf(stdout, "cancel_timing: %s\n",
           summary && summary->state.cancel_timing ?
               summary->state.cancel_timing : "none");
    fprintf(stdout, "cancel_safe_point: %s\n",
           summary && summary->state.cancel_safe_point ?
               summary->state.cancel_safe_point : "none");
    fprintf(stdout, "partial_output_available: %s\n",
...
static void generate_print_normal_summary(const yvex_generate_summary *summary,
                                          ...
                                          const char *backend_name)
{
    fprintf(stdout, "status: diagnostic-generation\n");
    fprintf(stdout, "model: %s\n", model_arg ? model_arg : "");
    fprintf(stdout, "backend: %s\n", backend_name ? backend_name : "cpu");
    fprintf(stdout, "tokens: %llu -> %llu diagnostic\n",
           summary ? summary->prompt_token_count : 0ull,
           summary ? summary->generated_token_count : 0ull);
    fprintf(stdout, "stop: %s\n",
           summary && summary->stop_reason ? summary->stop_reason : "internal-error");
    fprintf(stdout, "boundary: full-model generation unsupported\n");
    fprintf(stdout, "benchmark_status: not-measured\n");
    fprintf(stdout, "hint: use --" "audit or --trace-level full for diagnostic internals\n");
}
 ...
 *
 * Inputs:
 *   arg_count/args are borrowed CLI arguments; model references and runtime objects
...
int yvex_generate_command(int arg_count, char **args)
{
    ...
    for (i = 2; i < arg_count; ++i) {
        if (strcmp(args[i], "--model") == 0) {
            ...
            tokens_text = args[++i];
        } else if (strcmp(args[i], "--max-new-tokens") == 0) {
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &max_new_tokens)) {
                fprintf(stderr, "error: --max-new-tokens must be an integer greater than 0\n");
                return 2;
            }
            max_new_tokens_seen = 1;
            i += 1;
        } else if (strcmp(args[i], "--strategy") == 0) {
            if (i + 1 >= arg_count || !generate_parse_strategy(args[i + 1])) {
                fprintf(stderr, "error: --strategy currently supports greedy only\n");
                return 2;
            }
            i += 1;
        } else if (strcmp(args[i], "--trace-level") == 0) {
            if (i + 1 >= arg_count || !generate_parse_trace_level(args[i + 1], &trace_level)) {
            ...
            if (!generate_parse_output_mode(args[++i], &output_mode)) {
            ...
            return 2;
        } else if (strcmp(args[i], "--cancel-after-steps") == 0) {
            if (i + 1 >= arg_count ||
                !generate_parse_ull_allow_zero_cli(args[i + 1], &cancel_after_steps)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &logits_count) ||
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &kv_shape.layer_count)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &kv_shape.kv_head_count)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &kv_shape.head_dim)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &kv_shape.capacity)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &layer_count)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &layer_hidden_dim)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &layer_head_dim)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &layer_ffn_dim)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &chunk_size)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_ull_allow_zero_cli(args[i + 1], &position_start)) {
            ...
            if (i + 1 >= arg_count ||
                !generate_parse_positive_ull_cli(args[i + 1], &context_length)) {
