/*
 * Owner: generation.prefill (generation).
 ...
 *
 * prefill.c - Prefill state runtime boundary.
 *
                                     ...
                                     yvex_error *err)
{
    ...
    float *prefill_scratch_values = NULL;
    float *kv_values = NULL;
    float *kv_read_values = NULL;
    unsigned long long aggregate = 1469598103934665603ull;
    unsigned long long chunk_checksum = 1469598103934665603ull;
    unsigned long long effective_chunk_size = 1ull;
    unsigned long long chunk_start = 0ull;
    unsigned long long chunk_end = 0ull;
    unsigned long long chunk_index = 0ull;
    unsigned long long context_length = 0ull;
    unsigned long long default_context_length = 0ull;
    unsigned long long scratch_slots = 0ull;
    ...
    chunked_prefill_requested = options->chunk_size > 0ull;
    ...
    out->chunked_prefill_requested = chunked_prefill_requested;
    out->chunk_execution_mode = chunked_prefill_requested
                                    ? "bounded-token-chunks"
                                    : "token-loop";
    out->chunk_size = options->chunk_size;
    out->context_boundary_status = "unchecked";
    out->prefill_scratch_kind = "host-diagnostic-reuse";
    out->prefill_scratch_reuse = chunked_prefill_requested;
    out->prefill_scratch_cleanup_status = "not-needed";
    out->kv_ready = 0;
    ...
    out->prefill_phase = "context-boundary";
    if (options->chunk_size > input->token_count) {
        yvex_error_set(err, YVEX_ERR_INVALID_ARG, "yvex_engine_create_prefill_state",
                       "chunk size cannot exceed token count");
        return YVEX_ERR_INVALID_ARG;
    }
    if (input->token_count > ULLONG_MAX - options->position_start) {
        out->context_boundary_status = "fail";
        out->position_end = 0ull;
        yvex_error_set(err, YVEX_ERR_BOUNDS, "yvex_engine_create_prefill_state",
                       "prefill position range overflow");
        return YVEX_ERR_BOUNDS;
    }
    out->position_end = options->position_start + input->token_count - 1ull;
    if (options->context_length > 0ull) {
        context_length = options->context_length;
    } else if (engine->model && yvex_model_context_length(engine->model) > 0ull) {
        context_length = yvex_model_context_length(engine->model);
    } else {
        if (!prefill_checked_add_ull(options->position_start,
                                     input->token_count,
                                     &default_context_length)) {
            out->context_boundary_status = "fail";
            out->position_end = 0ull;
            yvex_error_set(err, YVEX_ERR_BOUNDS, "yvex_engine_create_prefill_state",
                           "default prefill context length overflow");
            return YVEX_ERR_BOUNDS;
        }
        context_length = default_context_length;
    }
    out->context_length = context_length;
    if (input->token_count > context_length ||
        options->position_start > context_length - input->token_count) {
        out->context_boundary_status = "fail";
        yvex_error_set(err, YVEX_ERR_BOUNDS, "yvex_engine_create_prefill_state",
    ...
    if (chunked_prefill_requested) {
        out->chunk_count = prefill_chunk_count(input->token_count, options->chunk_size);
        effective_chunk_size = options->chunk_size;
        if (prefill_test_env_enabled("YVEX_TEST_FAIL_PREFILL_CHUNK_SCRATCH_ALLOC")) {
            out->prefill_phase = "scratch-allocation";
            yvex_error_set(err, YVEX_ERR_NOMEM, "yvex_engine_create_prefill_state",
                           "test prefill chunk scratch allocation failure");
            return YVEX_ERR_NOMEM;
        }
        out->prefill_phase = "scratch-allocation";
        scratch_slots = effective_chunk_size > 0ull ? effective_chunk_size : 1ull;
        if (scratch_slots > (unsigned long long)(SIZE_MAX / sizeof(float))) {
            yvex_error_set(err, YVEX_ERR_BOUNDS, "yvex_engine_create_prefill_state",
                           "prefill scratch byte count overflow");
            return YVEX_ERR_BOUNDS;
        }
        prefill_scratch_values = (float *)calloc((size_t)scratch_slots, sizeof(float));
        if (!prefill_scratch_values) {
            yvex_error_set(err, YVEX_ERR_NOMEM, "yvex_engine_create_prefill_state",
                           "failed to allocate prefill chunk scratch");
            return YVEX_ERR_NOMEM;
        }
        out->prefill_scratch_allocations = 1ull;
        if (prefill_test_env_enabled("YVEX_TEST_FAIL_PREFILL_CHUNK_AFTER_SCRATCH_ALLOC")) {
            prefill_cleanup_scratch(&prefill_scratch_values, out);
    ...
    if (options->attach_kv) {
        ...
        if (rc != YVEX_OK) {
            prefill_cleanup_runtime(&kv, &kv_values, &kv_read_values,
                                    &prefill_scratch_values, out);
            return rc;
        }
    ...
    for (chunk_start = 0ull, chunk_index = 0ull;
         chunk_start < input->token_count;
         chunk_start += effective_chunk_size, ++chunk_index) {
        chunk_end = chunk_start + effective_chunk_size - 1ull;
        if (chunk_end >= input->token_count) {
            chunk_end = input->token_count - 1ull;
        }
        if (chunked_prefill_requested) {
            out->current_chunk_start = options->position_start + chunk_start;
            out->current_chunk_end = options->position_start + chunk_end;
            chunk_checksum = 1469598103934665603ull;
            chunk_checksum = prefill_mix_checksum_u64(chunk_checksum, chunk_index);
            chunk_checksum = prefill_mix_checksum_u64(chunk_checksum, out->current_chunk_start);
            chunk_checksum = prefill_mix_checksum_u64(chunk_checksum, out->current_chunk_end);
        }
        ...
        for (i = chunk_start; i <= chunk_end; ++i) {
            ...
            out->prefill_phase = chunked_prefill_requested ? "chunk-execution" : "token-execution";
            rc = yvex_engine_execute_segment_graph(engine, &segment_options, &segment_result, err);
            if (rc != YVEX_OK) {
                out->failed_token_index = i;
                out->failed_chunk_index = chunked_prefill_requested ? chunk_index : 0ull;
                if (kv) {
                    prefill_cleanup_runtime(&kv, &kv_values, &kv_read_values,
                                            &prefill_scratch_values, out);
                } else {
            ...
            if (chunked_prefill_requested && prefill_scratch_values) {
                prefill_scratch_values[(i - chunk_start) % scratch_slots] =
                    (float)((double)input->tokens[i] +
                            ((double)segment_result.output_checksum * 0.000000000000001));
                out->prefill_scratch_reuse_count += 1ull;
            }
            ...
            if (chunked_prefill_requested) {
                chunk_checksum = prefill_mix_checksum_u64(chunk_checksum,
            ...
            if (layer_prefill_requested) {
                ...
                if (chunked_prefill_requested) {
                    chunk_checksum = prefill_mix_checksum_u64(chunk_checksum,
                                                              layer_result.final_output_checksum);
                }
            ...
            if (kv) {
                ...
                if (chunked_prefill_requested) {
                    chunk_checksum = prefill_mix_checksum_u64(chunk_checksum, kv_position);
                }
                rc = yvex_kv_cache_get_summary(kv, &kv_summary, err);
                if (rc != YVEX_OK) {
                    out->failed_token_index = i;
        ...
        if (chunked_prefill_requested) {
            out->final_chunk_checksum = chunk_checksum;
            aggregate = prefill_mix_checksum_u64(aggregate, chunk_checksum);
            out->chunks_processed += 1ull;
            if (prefill_test_env_enabled("YVEX_TEST_FAIL_PREFILL_CHUNK_AFTER_CHUNK_0") &&
                chunk_index == 0ull) {
                out->failed_chunk_index = 1ull;
                out->prefill_phase = "chunk-execution";
                if (kv) {
                    prefill_cleanup_runtime(&kv, &kv_values, &kv_read_values,
                                            &prefill_scratch_values, out);
                } else {
