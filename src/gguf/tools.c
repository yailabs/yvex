...
static int command_source_manifest(int arg_count, char **args)
{
    ...
    if (strcmp(args[2], "report") == 0) {
        return yvex_source_manifest_report_command(arg_count, args);
    }
    if (strcmp(args[2], "inspect") == 0) {
    ...
    fprintf(stdout, "source manifest: written\n");
    fprintf(stdout, "repo: %s\n", options.repo);
    fprintf(stdout, "revision: %s\n", options.revision);
    fprintf(stdout, "local_path: %s\n", options.local_path);
    fprintf(stdout, "status: %s\n", yvex_source_status_name(options.status));
    fprintf(stdout, "files: %llu\n", summary.file_count);
    fprintf(stdout, "safetensors: %llu\n", summary.safetensors_count);
    fprintf(stdout, "total_size_bytes: %llu\n", summary.total_size_bytes);
    fprintf(stdout, "out: %s\n", out_path);
    fprintf(stdout, "status: source-manifest-written\n");
    return 0;
}
...
{
    return command_source_manifest(arg_count, args);
}
