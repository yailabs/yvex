/*
 * gguf/core.c - file-backed GGUF v3 structural parser.
 *
 * Owner:
...
static int parse_header(yvex_file_cursor *cur, yvex_gguf_header *out)
{
    ...
    if (out->version != YVEX_GGUF_SUPPORTED_VERSION) {
        return cursor_fail(cur, YVEX_GGUF_PARSE_UNSUPPORTED_VERSION,
                           YVEX_GGUF_PARSE_SECTION_CONTAINER,
                           "core.container", "unsupported GGUF container version");
    }
    rc = cursor_read_u64le(cur, &out->tensor_count, "core.container", "truncated GGUF tensor count");
    if (rc != YVEX_OK) return rc;
    rc = cursor_read_u64le(cur, &out->metadata_count, "core.container", "truncated GGUF metadata count");
    if (rc != YVEX_OK) return rc;
