...
static int command_backend(int argc, char **argv)
{
    ...
    if (argc != 3 || strcmp(argv[2], "--help") == 0 || strcmp(argv[2], "-h") == 0) {
        ...
        yvex_cli_out_writef(stderr, "usage: yvex backend cpu|cuda\n");
        return 2;
    }
    ...
    if (strcmp(argv[2], "cpu") == 0) {
        ...
        yvex_cli_out_writef(stderr, "Try 'yvex help backend' for usage.\n");
        return 2;
    }
    ...
    if (rc == YVEX_ERR_UNSUPPORTED) {
        ...
        yvex_cli_out_writef(stdout, "status: backend-unsupported\n");
        return 5;
    }
    if (rc != YVEX_OK) {
    ...
    yvex_cli_out_writef(stdout, "capabilities:\n");
    ...
    yvex_cli_out_writef(stdout, "status: backend-ready\n");
...
static int command_cuda_info(int argc, char **argv)
{
    ...
    if (argc != 2 && !(argc == 3 && (strcmp(argv[2], "--help") == 0 ||
                                    strcmp(argv[2], "-h") == 0))) {
        yvex_cli_out_writef(stderr, "usage: yvex cuda-info\n");
        return 2;
    }
    if (argc == 3) {
    ...
    rc = yvex_backend_open(&backend, &options, &err);
    if (rc == YVEX_ERR_UNSUPPORTED) {
        yvex_cli_out_writef(stdout, "cuda: unavailable\n");
        yvex_cli_out_writef(stdout, "reason: %s\n", yvex_error_message(&err));
        yvex_cli_out_writef(stdout, "status: cuda-unavailable\n");
        return 5;
    }
    if (rc != YVEX_OK) {
        return print_yvex_error(&err, exit_for_status(rc));
    ...
    yvex_cli_out_writef(stdout, "cuda: available\n");
    yvex_cli_out_writef(stdout, "device_count: >=1\n");
    ...
    yvex_cli_out_writef(stdout, "  global_memory_bytes: %llu\n", info.global_memory_bytes);
    yvex_cli_out_writef(stdout, "  free_memory_bytes: %llu\n", info.free_memory_bytes);
    yvex_cli_out_writef(stdout, "  total_memory_bytes: %llu\n", info.total_memory_bytes);
    yvex_cli_out_writef(stdout, "  unified_addressing: %s\n", yes_no(info.unified_addressing));
    yvex_cli_out_writef(stdout, "  managed_memory: %s\n", yes_no(info.managed_memory));
    yvex_cli_out_writef(stdout, "\n");
    yvex_cli_out_writef(stdout, "status: cuda-info\n");
    yvex_backend_close(backend);
    return 0;
}
