/*
 * yvex_source_cli.c - source command quarantine.
 *
 * Owner:
 *   src/cli/commands
 *
 ...
 *   summaries, footprint summaries, native safetensors header inventory, and
 *   CLI output routing during TOPOLOGY.CLI.PRINT.ALL.0.
 *
 * Does not own:
...
static void yvex_sm_json_string(FILE *fp, const char *s)
{
    ...
    while (*p) {
        ...
        case '\\':
            ...
            break;
        case '\b':
            yvex_cli_out_fputs("\\b", fp);
            break;
        case '\f':
            yvex_cli_out_fputs("\\f", fp);
            break;
        case '\n':
            yvex_cli_out_fputs("\\n", fp);
            break;
        ...
        default:
            if (ch < 32) {
                yvex_cli_out_writef(fp, "\\u%04x", (unsigned int)ch);
            } else {
                fputc((int)ch, fp);
            }
            break;
                                         ...
                                         yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "  },\n");
    yvex_cli_out_writef(fp, "  \"download\": {\n");
                                    ...
                                    yvex_qwen_source_pressure_report *report)
{
    ...
    if (options->source) {
        if (!qwen_source_path_format(report->source_path, sizeof(report->source_path),
            ...
            yvex_cli_out_writef(stderr, "source-manifest report: source path is too long\n");
            return 2;
        }
...
static void qwen_source_print_normal(const yvex_qwen_source_report_options *options,
                                     const yvex_qwen_source_pressure_report *report)
{
    yvex_cli_out_writef(stdout, "report: %s\n", options->profile->report_name);
    ...
    yvex_cli_out_writef(stdout, "files: %llu  safetensors: %llu  bytes: %llu  footprint: %s\n",
           report->source_file_count,
           report->safetensors_count,
           report->total_size_bytes,
           qwen_source_footprint_class(report));
    yvex_cli_out_writef(stdout, "provenance: %s status=%s revision=unknown\n",
           qwen_source_provenance_origin_normal(report),
           qwen_source_provenance_status(report));
    yvex_cli_out_writef(stdout, "native: %s  files=%llu  tensors=%llu  header_bytes=%llu\n",
           qwen_source_native_inventory_status(report),
           report->native_safetensors_count,
           report->native_tensor_count,
           report->native_safetensors_header_bytes);
    yvex_cli_out_writef(stdout, "metadata: %s  tensors=%llu  dtypes=%llu  max_rank=%llu\n",
           qwen_source_tensor_metadata_status(report),
           report->source_tensor_count,
           report->source_tensor_dtype_count,
           report->source_tensor_max_rank);
    yvex_cli_out_writef(stdout, "manifest: %s  consistency=%s\n",
    ...
    yvex_cli_out_writef(stdout, "boundary: source report only; no artifact/runtime/generation/benchmark\n");
...
static void qwen_source_print_table(const yvex_qwen_source_report_options *options,
                                    const yvex_qwen_source_pressure_report *report)
{
    yvex_cli_out_writef(stdout, "SOURCE PRESSURE  release=%s\n\n", options->release);
    yvex_cli_out_writef(stdout, "%-6s  %-24s  %-7s  %7s  %-8s  %-11s  %s\n",
           "FAMILY", "TARGET", "SOURCE", "TENSORS", "MANIFEST",
           "CONSISTENCY", "NEXT");
    yvex_cli_out_writef(stdout, "%-6s  %-24s  %-7s  %7llu  %-8s  %-11s  %s\n",
           options->profile->family_key,
           options->target,
           report->source_state,
           report->source_tensor_count,
                                    ...
                                    const yvex_qwen_source_pressure_report *report)
{
    ...
    yvex_cli_out_writef(stdout, "source-report: %s\n", options->profile->family_key);
    yvex_cli_out_writef(stdout, "status: %s\n", report->status);
    yvex_cli_out_writef(stdout, "release: %s\n", options->release);
    yvex_cli_out_writef(stdout, "family: %s\n", options->profile->display_family);
    yvex_cli_out_writef(stdout, "family_key: %s\n", options->profile->family_key);
    ...
    yvex_cli_out_writef(stdout, "source_target_status: %s\n", options->profile->source_target_status);
    yvex_cli_out_writef(stdout, "source_family_profile_status: %s\n",
    ...
    yvex_cli_out_writef(stdout, "hardware_lane: %s\n", options->profile->hardware_lane);
    yvex_cli_out_writef(stdout, "backend_lane: %s\n", options->profile->backend_lane);
    ...
    yvex_cli_out_writef(stdout, "download_report_status: %s\n",
           report->download_report_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "download_repo_id: %s\n",
           report->identity_repo_id[0] ? report->identity_repo_id : "unknown");
    yvex_cli_out_writef(stdout, "download_revision: %s\n",
           report->identity_revision[0] ? report->identity_revision : "unknown");
    yvex_cli_out_writef(stdout, "source_file_count: %llu\n", report->source_file_count);
    yvex_cli_out_writef(stdout, "source_regular_file_count: %llu\n", report->source_regular_file_count);
    yvex_cli_out_writef(stdout, "source_safetensors_count: %llu\n", report->safetensors_count);
    yvex_cli_out_writef(stdout, "source_bin_count: %llu\n", report->bin_count);
    yvex_cli_out_writef(stdout, "source_dat_count: %llu\n", report->dat_count);
    yvex_cli_out_writef(stdout, "source_json_count: %llu\n", report->json_count);
    yvex_cli_out_writef(stdout, "source_tokenizer_file_count: %llu\n", report->tokenizer_file_count);
    yvex_cli_out_writef(stdout, "source_config_file_count: %llu\n", report->config_file_count);
    yvex_cli_out_writef(stdout, "source_total_size_bytes: %llu\n", report->total_size_bytes);
    yvex_cli_out_writef(stdout, "source_safetensors_size_bytes: %llu\n", report->safetensors_size_bytes);
    yvex_cli_out_writef(stdout, "source_sidecar_size_bytes: %llu\n", report->sidecar_size_bytes);
    yvex_cli_out_writef(stdout, "source_other_size_bytes: %llu\n", report->other_size_bytes);
    yvex_cli_out_writef(stdout, "source_footprint_class: %s\n", qwen_source_footprint_class(report));
    yvex_cli_out_writef(stdout, "source_footprint_status: %s\n", qwen_source_footprint_status(report));
    yvex_cli_out_writef(stdout, "source_count_scope: top-level-regular-files\n");
    yvex_cli_out_writef(stdout, "source_payload_loaded: false\n");
    yvex_cli_out_writef(stdout, "largest_source_file_bytes: %llu\n", report->largest_source_file_bytes);
    yvex_cli_out_writef(stdout, "largest_source_file_name: %s\n",
           report->largest_source_file_name[0]
               ? report->largest_source_file_name
               : "none");
    yvex_cli_out_writef(stdout, "config_status: %s\n", qwen_source_present_missing(report->config_exists));
    ...
    yvex_cli_out_writef(stdout, "safetensors_status: %s\n", qwen_source_safetensors_status(report));
    yvex_cli_out_writef(stdout, "safetensors_count: %llu\n", report->safetensors_count);
    yvex_cli_out_writef(stdout, "source_manifest_expected: true\n");
    yvex_cli_out_writef(stdout, "source_manifest_status: %s\n", qwen_source_manifest_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_path: %s\n",
           report->manifest_path[0] ? report->manifest_path : "unknown");
    yvex_cli_out_writef(stdout, "source_manifest_schema_status: %s\n",
           qwen_source_manifest_schema_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_schema_version: %s\n",
           report->manifest_schema_version[0]
               ? report->manifest_schema_version
               : "unknown");
    yvex_cli_out_writef(stdout, "source_manifest_family: %s\n", options->profile->family_key);
    yvex_cli_out_writef(stdout, "source_manifest_family_status: %s\n",
           qwen_source_manifest_family_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_target_id: %s\n", options->target);
    yvex_cli_out_writef(stdout, "source_manifest_target_status: %s\n",
           qwen_source_manifest_target_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_source_path_status: %s\n", report->source_state);
    yvex_cli_out_writef(stdout, "source_manifest_artifact_class_status: %s\n",
           qwen_source_manifest_artifact_class_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_footprint_status: %s\n",
           qwen_source_manifest_footprint_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_authority: %s\n",
           qwen_source_manifest_authority(report));
    yvex_cli_out_writef(stdout, "source_manifest_provenance_status: %s\n",
           qwen_source_manifest_provenance_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_native_inventory_status: %s\n",
           qwen_source_manifest_native_inventory_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_tensor_metadata_status: %s\n",
           qwen_source_manifest_tensor_metadata_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_consistency_status: %s\n",
           qwen_source_manifest_consistency_status(report));
    yvex_cli_out_writef(stdout, "source_manifest_hardening_status: report-only\n");
    yvex_cli_out_writef(stdout, "source_manifest_creation_performed: false\n");
    yvex_cli_out_writef(stdout, "source_manifest_payload_loaded: false\n");
    yvex_cli_out_writef(stdout, "source_manifest_remote_checked: false\n");
    yvex_cli_out_writef(stdout, "source_manifest_hash_computed: false\n");
    yvex_cli_out_writef(stdout, "source_revision: unknown\n");
    yvex_cli_out_writef(stdout, "source_revision_status: unknown\n");
    yvex_cli_out_writef(stdout, "source_commit: unknown\n");
    yvex_cli_out_writef(stdout, "source_commit_status: unknown\n");
    yvex_cli_out_writef(stdout, "source_tag: unknown\n");
    yvex_cli_out_writef(stdout, "source_tag_status: unknown\n");
    yvex_cli_out_writef(stdout, "source_license_status: %s\n",
           qwen_source_presence_verification_status(report->license_exists));
    yvex_cli_out_writef(stdout, "source_readme_status: %s\n",
    ...
    yvex_cli_out_writef(stdout, "source_digest_status: not-computed\n");
    yvex_cli_out_writef(stdout, "source_hash_status: not-computed\n");
    yvex_cli_out_writef(stdout, "source_verification_status: not-verified\n");
    yvex_cli_out_writef(stdout, "source_remote_checked: false\n");
    yvex_cli_out_writef(stdout, "native_inventory_status: %s\n",
           qwen_source_native_inventory_status(report));
    yvex_cli_out_writef(stdout, "native_inventory_scope: top-level-safetensors-headers\n");
    yvex_cli_out_writef(stdout, "native_inventory_source: %s\n",
           qwen_source_native_inventory_source(report));
    yvex_cli_out_writef(stdout, "native_safetensors_count: %llu\n", report->native_safetensors_count);
    yvex_cli_out_writef(stdout, "native_safetensors_opened: %llu\n", report->native_safetensors_opened);
    yvex_cli_out_writef(stdout, "native_safetensors_header_read_count: %llu\n",
           report->native_safetensors_header_read_count);
    yvex_cli_out_writef(stdout, "native_safetensors_header_error_count: %llu\n",
           report->native_safetensors_header_error_count);
    yvex_cli_out_writef(stdout, "native_safetensors_header_bytes: %llu\n",
           report->native_safetensors_header_bytes);
    yvex_cli_out_writef(stdout, "native_safetensors_payload_loaded: false\n");
    yvex_cli_out_writef(stdout, "native_safetensors_payload_bytes_read: 0\n");
    yvex_cli_out_writef(stdout, "native_tensor_count: %llu\n", report->native_tensor_count);
    yvex_cli_out_writef(stdout, "native_tensor_metadata_status: %s\n",
           qwen_source_native_tensor_metadata_status(report));
    yvex_cli_out_writef(stdout, "native_tensor_payload_status: %s\n",
           qwen_source_native_tensor_payload_status(report));
    yvex_cli_out_writef(stdout, "native_declared_data_bytes: %llu\n",
           report->native_declared_data_bytes);
    yvex_cli_out_writef(stdout, "native_declared_tensor_bytes: %llu\n",
           report->native_declared_tensor_bytes);
    yvex_cli_out_writef(stdout, "native_max_rank: %llu\n", report->native_max_rank);
    yvex_cli_out_writef(stdout, "native_max_tensor_elements: %llu\n",
           report->native_max_tensor_elements);
    yvex_cli_out_writef(stdout, "native_largest_tensor_name: %s\n",
           report->native_largest_tensor_name[0]
               ? report->native_largest_tensor_name
               : "none");
    yvex_cli_out_writef(stdout, "native_largest_tensor_bytes: %llu\n",
           report->native_largest_tensor_bytes);
    yvex_cli_out_writef(stdout, "native_dtype_f16_count: %llu\n", report->native_dtype_f16_count);
    yvex_cli_out_writef(stdout, "native_dtype_bf16_count: %llu\n", report->native_dtype_bf16_count);
    yvex_cli_out_writef(stdout, "native_dtype_f32_count: %llu\n", report->native_dtype_f32_count);
    yvex_cli_out_writef(stdout, "native_dtype_i8_count: %llu\n", report->native_dtype_i8_count);
    yvex_cli_out_writef(stdout, "native_dtype_i16_count: %llu\n", report->native_dtype_i16_count);
    yvex_cli_out_writef(stdout, "native_dtype_i32_count: %llu\n", report->native_dtype_i32_count);
    yvex_cli_out_writef(stdout, "native_dtype_i64_count: %llu\n", report->native_dtype_i64_count);
    yvex_cli_out_writef(stdout, "native_dtype_u8_count: %llu\n", report->native_dtype_u8_count);
    yvex_cli_out_writef(stdout, "native_dtype_other_count: %llu\n", report->native_dtype_other_count);
    yvex_cli_out_writef(stdout, "native_invalid_file_count: %llu\n", report->native_invalid_file_count);
    yvex_cli_out_writef(stdout, "native_inventory_error_count: %llu\n",
           report->native_inventory_error_count);
    yvex_cli_out_writef(stdout, "native_inventory_report_status: %s\n",
    ...
    yvex_cli_out_writef(stdout, "source_tensor_metadata_status: %s\n",
           qwen_source_tensor_metadata_status(report));
    yvex_cli_out_writef(stdout, "source_tensor_metadata_scope: safetensors-header\n");
    yvex_cli_out_writef(stdout, "source_tensor_metadata_source: %s\n",
    ...
    yvex_cli_out_writef(stdout, "source_tensor_name_count: %llu\n", report->source_tensor_name_count);
    yvex_cli_out_writef(stdout, "source_tensor_file_count: %llu\n", report->source_tensor_file_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_count: %llu\n", report->source_tensor_dtype_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_count: %llu\n", report->source_tensor_rank_count);
    yvex_cli_out_writef(stdout, "source_tensor_shape_count: %llu\n", report->source_tensor_shape_count);
    yvex_cli_out_writef(stdout, "source_tensor_declared_data_bytes: %llu\n",
           report->source_tensor_declared_data_bytes);
    yvex_cli_out_writef(stdout, "source_tensor_declared_tensor_bytes: %llu\n",
           report->source_tensor_declared_tensor_bytes);
    yvex_cli_out_writef(stdout, "source_tensor_total_elements: %llu\n",
           report->source_tensor_total_elements);
    yvex_cli_out_writef(stdout, "source_tensor_max_rank: %llu\n", report->source_tensor_max_rank);
    yvex_cli_out_writef(stdout, "source_tensor_max_elements: %llu\n", report->source_tensor_max_elements);
    yvex_cli_out_writef(stdout, "source_tensor_largest_name: %s\n",
           report->source_tensor_largest_name[0]
               ? report->source_tensor_largest_name
               : "none");
    yvex_cli_out_writef(stdout, "source_tensor_largest_file: %s\n",
           report->source_tensor_largest_file[0]
               ? report->source_tensor_largest_file
               : "none");
    yvex_cli_out_writef(stdout, "source_tensor_largest_dtype: %s\n",
           report->source_tensor_largest_dtype[0]
               ? report->source_tensor_largest_dtype
               : "none");
    yvex_cli_out_writef(stdout, "source_tensor_largest_rank: %llu\n",
           report->source_tensor_largest_rank);
    yvex_cli_out_writef(stdout, "source_tensor_largest_shape: %s\n",
           report->source_tensor_largest_shape[0]
               ? report->source_tensor_largest_shape
               : "[]");
    yvex_cli_out_writef(stdout, "source_tensor_largest_elements: %llu\n",
           report->source_tensor_largest_elements);
    yvex_cli_out_writef(stdout, "source_tensor_largest_declared_bytes: %llu\n",
           report->source_tensor_largest_declared_bytes);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_f16_count: %llu\n",
           report->source_tensor_dtype_f16_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_bf16_count: %llu\n",
           report->source_tensor_dtype_bf16_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_f32_count: %llu\n",
           report->source_tensor_dtype_f32_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_i8_count: %llu\n",
           report->source_tensor_dtype_i8_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_i16_count: %llu\n",
           report->source_tensor_dtype_i16_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_i32_count: %llu\n",
           report->source_tensor_dtype_i32_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_i64_count: %llu\n",
           report->source_tensor_dtype_i64_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_u8_count: %llu\n",
           report->source_tensor_dtype_u8_count);
    yvex_cli_out_writef(stdout, "source_tensor_dtype_other_count: %llu\n",
           report->source_tensor_dtype_other_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_0_count: %llu\n",
           report->source_tensor_rank_0_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_1_count: %llu\n",
           report->source_tensor_rank_1_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_2_count: %llu\n",
           report->source_tensor_rank_2_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_3_count: %llu\n",
           report->source_tensor_rank_3_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_4_count: %llu\n",
           report->source_tensor_rank_4_count);
    yvex_cli_out_writef(stdout, "source_tensor_rank_other_count: %llu\n",
           report->source_tensor_rank_other_count);
    yvex_cli_out_writef(stdout, "source_tensor_name_pattern_status: lexical-only\n");
    yvex_cli_out_writef(stdout, "source_tensor_name_embed_count: %llu\n",
           report->source_tensor_name_embed_count);
    yvex_cli_out_writef(stdout, "source_tensor_name_attn_count: %llu\n",
           report->source_tensor_name_attn_count);
    yvex_cli_out_writef(stdout, "source_tensor_name_mlp_count: %llu\n",
           report->source_tensor_name_mlp_count);
    yvex_cli_out_writef(stdout, "source_tensor_name_norm_count: %llu\n",
           report->source_tensor_name_norm_count);
    yvex_cli_out_writef(stdout, "source_tensor_name_lm_head_count: %llu\n",
           report->source_tensor_name_lm_head_count);
    yvex_cli_out_writef(stdout, "source_tensor_name_other_count: %llu\n",
           report->source_tensor_name_other_count);
    yvex_cli_out_writef(stdout, "source_tensor_metadata_error_count: %llu\n",
           report->source_tensor_metadata_error_count);
    yvex_cli_out_writef(stdout, "source_tensor_sample_count: %llu\n",
           report->source_tensor_sample_count);
    for (i = 0; i < report->source_tensor_sample_count; ++i) {
        const yvex_source_tensor_sample *sample = &report->source_tensor_samples[i];
        yvex_cli_out_writef(stdout, "source_tensor_%lu_name: %s\n", i, sample->name);
        yvex_cli_out_writef(stdout, "source_tensor_%lu_file: %s\n", i, sample->file);
        yvex_cli_out_writef(stdout, "source_tensor_%lu_dtype: %s\n", i, sample->dtype);
        yvex_cli_out_writef(stdout, "source_tensor_%lu_rank: %llu\n", i, sample->rank);
        yvex_cli_out_writef(stdout, "source_tensor_%lu_shape: %s\n", i, sample->shape);
        yvex_cli_out_writef(stdout, "source_tensor_%lu_elements: %llu\n", i, sample->elements);
        yvex_cli_out_writef(stdout, "source_tensor_%lu_declared_bytes: %llu\n",
               i,
               sample->declared_bytes);
    }
    ...
    yvex_cli_out_writef(stdout, "blocker_count: %lu\n", report->blocker_count);
...
int yvex_source_manifest_report_command(int argc, char **argv)
{
    ...
    for (i = 3; i < argc; ++i) {
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            ...
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --family requires qwen|gemma\n");
                return 2;
            }
            options.family = argv[++i];
        } else if (strcmp(argv[i], "--release") == 0) {
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --release requires VERSION\n");
                return 2;
            }
            options.release = argv[++i];
        } else if (strcmp(argv[i], "--models-root") == 0) {
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --models-root requires DIR\n");
                return 2;
            }
            options.models_root = argv[++i];
        } else if (strcmp(argv[i], "--source") == 0) {
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --source requires DIR\n");
                return 2;
            }
            options.source = argv[++i];
        } else if (strcmp(argv[i], "--target") == 0) {
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --target requires TARGET\n");
                return 2;
            }
            ...
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --tensor-limit requires N\n");
                return 2;
            }
            if (!qwen_source_parse_positive_ull(argv[++i], &options.tensor_limit)) {
                yvex_cli_out_writef(stderr, "source-manifest report: --tensor-limit requires a positive integer\n");
                return 2;
            }
            ...
            if (i + 1 >= argc) {
                yvex_cli_out_writef(stderr, "source-manifest report: --output requires normal|table|audit\n");
                return 2;
            }
            if (!qwen_source_output_mode_parse(argv[++i], &options.output_mode)) {
                yvex_cli_out_writef(stderr, "source-manifest report: unsupported output mode: %s\n", argv[i]);
                return 2;
            }
        } else if (strcmp(argv[i], "--json") == 0) {
            yvex_cli_out_writef(stderr, "source-manifest report: JSON output is unsupported; use --output normal|table|audit\n");
            return 2;
        } else {
            yvex_cli_out_writef(stderr, "source-manifest report: unknown option: %s\n", argv[i]);
            return 2;
        }
    }

    if (!options.family || options.family[0] == '\0') {
        yvex_cli_out_writef(stderr, "source-manifest report: --family is required\n");
        qwen_source_report_usage(stderr);
        return 2;
    }
    options.profile = qwen_source_find_profile(options.family);
    if (!options.profile) {
        yvex_cli_out_writef(stderr, "source-manifest report: unsupported family: %s\n", options.family);
        return 2;
    }
    ...
    if (!options.release || options.release[0] == '\0') {
        yvex_cli_out_writef(stderr, "source-manifest report: --release is required\n");
        qwen_source_report_usage(stderr);
        return 2;
    }
    if (strcmp(options.release, "v0.1.0") != 0) {
        yvex_cli_out_writef(stderr, "source-manifest report: unsupported release: %s\n", options.release);
        return 2;
    }
    if (!qwen_source_target_is_supported(options.profile, options.target)) {
        yvex_cli_out_writef(stderr, "source-manifest report: unsupported target: %s\n", options.target);
        return 2;
    }
