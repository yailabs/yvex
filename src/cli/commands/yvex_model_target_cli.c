...
{
    yvex_cli_out_writef(fp, "usage: yvex model-target decision --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target decision --help\n");
    yvex_cli_out_writef(fp, "\noptions:\n");
    yvex_cli_out_writef(fp, "  --candidate TARGET             report decision facts for one target\n");
    yvex_cli_out_writef(fp, "  --include-candidates           include target candidate classifications\n");
    yvex_cli_out_writef(fp, "  --include-pressure-targets     include pressure-lane status fields\n");
    yvex_cli_out_writef(fp, "  --include-blockers             include blocker row fields\n");
    yvex_cli_out_writef(fp, "  --include-critical-path        include release-critical track fields\n");
    yvex_cli_out_writef(fp, "  --include-next                 include deterministic next row fields\n");
    yvex_cli_out_writef(fp, "  --strict                       keep invalid usage fatal; honest blocked decisions still pass\n");
...
static void print_model_target_decision_help(FILE *fp)
{
    ...
    yvex_cli_out_writef(fp, "v0.1.0 requires an honest full-runtime-candidate target before runtime graph, prefill, KV, decode, logits, sampling, and generation rows can advance.\n");
    yvex_cli_out_writef(fp, "Selected runtime slices, source-only pressure targets, external references, and fixture-only targets are ineligible for full-runtime closure.\n");
}

static void print_model_target_candidate_usage(FILE *fp)
{
    yvex_cli_out_writef(fp, "usage: yvex model-target candidate --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target candidate --help\n");
    yvex_cli_out_writef(fp, "\noptions:\n");
    yvex_cli_out_writef(fp, "  --target TARGET                report one candidate target\n");
    yvex_cli_out_writef(fp, "  --include-candidates           include candidate classification blocks\n");
    yvex_cli_out_writef(fp, "  --include-pressure-targets     include pressure target count fields\n");
    yvex_cli_out_writef(fp, "  --include-blockers             include stable blocker fields\n");
    yvex_cli_out_writef(fp, "  --include-next                 include next required row fields\n");
    yvex_cli_out_writef(fp, "  --audit | --output normal|table|audit\n");
}
...
{
    yvex_cli_out_writef(fp, "usage: yvex model-target dense-candidate --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target dense-candidate --help\n");
    yvex_cli_out_writef(fp, "\noptions:\n");
    yvex_cli_out_writef(fp, "  --target TARGET                report one dense or dense-adjacent target\n");
    yvex_cli_out_writef(fp, "  --include-candidates           include dense candidate classification blocks\n");
    yvex_cli_out_writef(fp, "  --include-requirements         include required dense runtime role groups\n");
    yvex_cli_out_writef(fp, "  --include-blockers             include stable blocker fields\n");
    yvex_cli_out_writef(fp, "  --include-next                 include next required row fields\n");
    yvex_cli_out_writef(fp, "  --audit | --output normal|table|audit\n");
}
...
{
    yvex_cli_out_writef(fp, "usage: yvex model-target qwen-metal --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target qwen-metal --help\n");
    yvex_cli_out_writef(fp, "\noptions:\n");
    yvex_cli_out_writef(fp, "  --target TARGET                report one planned Qwen/Metal candidate slot\n");
    yvex_cli_out_writef(fp, "  --include-candidates           include planned candidate slot blocks\n");
    yvex_cli_out_writef(fp, "  --include-hardware             include Apple Silicon / Metal hardware pressure fields\n");
    yvex_cli_out_writef(fp, "  --include-backend              include Metal backend pressure fields\n");
    yvex_cli_out_writef(fp, "  --include-source               include Qwen source/config pressure fields\n");
    yvex_cli_out_writef(fp, "  --include-blockers             include stable blocker fields\n");
    yvex_cli_out_writef(fp, "  --include-next                 include next required row fields\n");
    yvex_cli_out_writef(fp, "  --audit | --output normal|table|audit\n");
}
    ...
    yvex_model_class_profile *profile)
{
    ...
    if (source_override) {
        ...
        if (n < 0 || (size_t)n >= sizeof(profile->source_path)) {
            yvex_cli_out_writef(stderr, "model-target class-profile: source path is too long\n");
            return 2;
        }
...
static void print_model_class_profile_normal(
    const yvex_model_class_profile *profile)
{
    yvex_cli_out_writef(stdout, "model-class: %s\n", profile->spec->family_key);
    ...
    yvex_cli_out_writef(stdout, "evidence: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "patterns: tensors=%llu attn=%llu mlp=%llu norm=%llu head=%llu moe=%llu\n",
    ...
    yvex_cli_out_writef(stdout, "boundary: model-class profile only; no tensor role mapping/runtime/generation\n");
}

static void print_model_class_profile_table(
    const yvex_model_class_profile *profile)
{
    yvex_cli_out_writef(stdout, "MODEL CLASS PROFILE\n\n");
    yvex_cli_out_writef(stdout, "%-6s  %-24s  %-16s  %7s  %4s  %3s  %4s  %4s  %3s  %s\n",
           "FAMILY", "TARGET", "STATUS", "TENSORS",
           "ATTN", "MLP", "NORM", "HEAD", "MOE", "NEXT");
    yvex_cli_out_writef(stdout, "%-6s  %-24s  %-16s  %7llu  %4llu  %3llu  %4llu  %4llu  %3llu  %s\n",
           profile->spec->family_key,
           profile->record->target_id,
           profile->status,
           profile->tensor_count,
...
static void print_model_class_profile_audit(
    const yvex_model_class_profile *profile)
{
    yvex_cli_out_writef(stdout, "model_class_profile_status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "model_class_family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "model_class_target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "model_class_name: %s\n", profile->spec->class_name);
    yvex_cli_out_writef(stdout, "model_class_runtime_shape: %s\n", profile->spec->runtime_shape);
    yvex_cli_out_writef(stdout, "model_class_evidence_basis: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "model_class_config_status: %s\n",
           profile->config_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "model_class_tokenizer_status: %s\n",
           profile->tokenizer_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "model_class_source_metadata_status: %s\n",
           profile->source_metadata_status);
    yvex_cli_out_writef(stdout, "model_class_tensor_count: %llu\n", profile->tensor_count);
    yvex_cli_out_writef(stdout, "model_class_embedding_pattern_count: %llu\n",
           profile->embedding_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_attention_q_pattern_count: %llu\n",
           profile->attention_q_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_attention_k_pattern_count: %llu\n",
           profile->attention_k_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_attention_v_pattern_count: %llu\n",
           profile->attention_v_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_attention_o_pattern_count: %llu\n",
           profile->attention_o_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_mlp_gate_pattern_count: %llu\n",
           profile->mlp_gate_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_mlp_up_pattern_count: %llu\n",
           profile->mlp_up_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_mlp_down_pattern_count: %llu\n",
           profile->mlp_down_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_norm_pattern_count: %llu\n",
           profile->norm_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_output_head_pattern_count: %llu\n",
           profile->output_head_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_moe_router_pattern_count: %llu\n",
           profile->moe_router_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_moe_expert_pattern_count: %llu\n",
           profile->moe_expert_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_other_pattern_count: %llu\n",
           profile->other_pattern_count);
    yvex_cli_out_writef(stdout, "model_class_pattern_status: lexical-only\n");
    yvex_cli_out_writef(stdout, "model_class_role_mapping_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "model_class_runtime_status: unsupported\n");
    yvex_cli_out_writef(stdout, "backend_selection: deferred\n");
    yvex_cli_out_writef(stdout, "backend_pressure: %s\n", profile->spec->backend_pressure);
    ...
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    yvex_cli_out_writef(stdout, "source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "source_path_source: %s\n", profile->source_path_source);
...
static void print_model_class_audit_hint(const yvex_model_target_record *record)
{
    ...
    yvex_cli_out_writef(stdout, "model_class_profile_status: command-visible\n");
    yvex_cli_out_writef(stdout, "model_class_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "model_class_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "model_class_name: %s\n", spec->class_name);
    yvex_cli_out_writef(stdout, "model_class_runtime_shape: %s\n", spec->runtime_shape);
...
static void print_tensor_collection_normal(
    const yvex_tensor_collection_profile *profile)
{
    yvex_cli_out_writef(stdout, "tensor-collection: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "target: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "stage: header-collection-inventory\n");
    yvex_cli_out_writef(stdout, "evidence: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "collections: embedding=%llu attention_qkvo=%llu mlp_gud=%llu norm=%llu head=%llu moe=%llu\n",
           profile->embedding_tensor_count,
           profile->attention_complete_qkvo_layer_count,
           profile->mlp_complete_gud_layer_count,
           profile->norm_tensor_count,
    ...
    if (profile->source_exists && profile->tensor_count > 0) {
        yvex_cli_out_writef(stdout, "layers_observed: %llu\n", profile->layer_count_observed);
    }
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next: %s\n", YVEX_TENSOR_COLLECTION_NEXT_ROW);
    yvex_cli_out_writef(stdout, "boundary: tensor collection inventory only; no role mapping/runtime/generation\n");
}

static void print_tensor_collection_table(
    const yvex_tensor_collection_profile *profile)
{
    yvex_cli_out_writef(stdout, "TENSOR COLLECTION INVENTORY\n\n");
    yvex_cli_out_writef(stdout, "%-6s  %-10s  %-19s  %5s  %9s  %7s  %4s  %4s  %3s  %6s  %s\n",
           "FAMILY", "TARGET", "STATUS", "EMBED", "ATTN_QKVO",
           "MLP_GUD", "NORM", "HEAD", "MOE", "LAYERS", "NEXT");
    yvex_cli_out_writef(stdout, "%-6s  %-10s  %-19s  %5llu  %9llu  %7llu  %4llu  %4llu  %3llu  %6llu  %s\n",
           profile->spec->family_key,
           profile->record->target_id,
           profile->status,
           profile->embedding_tensor_count,
...
static void print_tensor_collection_audit(
    const yvex_tensor_collection_profile *profile)
{
    yvex_cli_out_writef(stdout, "tensor_collection_status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "tensor_collection_family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "tensor_collection_target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "tensor_collection_stage: header-collection-inventory\n");
    yvex_cli_out_writef(stdout, "tensor_collection_evidence_basis: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "tensor_collection_source_status: %s\n",
           profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_collection_source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "tensor_collection_manifest_status: not-checked\n");
    yvex_cli_out_writef(stdout, "tensor_collection_config_status: %s\n",
           profile->config_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_collection_tokenizer_status: %s\n",
           profile->tokenizer_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_collection_tensor_count: %llu\n", profile->tensor_count);
    yvex_cli_out_writef(stdout, "tensor_collection_layer_count_observed: %llu\n",
           profile->layer_count_observed);
    yvex_cli_out_writef(stdout, "tensor_collection_embedding_status: %s\n",
           tensor_collection_present_status(profile->embedding_tensor_count));
    yvex_cli_out_writef(stdout, "tensor_collection_embedding_tensor_count: %llu\n",
           profile->embedding_tensor_count);
    yvex_cli_out_writef(stdout, "tensor_collection_attention_status: %s\n",
           tensor_collection_attention_status(profile));
    yvex_cli_out_writef(stdout, "tensor_collection_attention_q_count: %llu\n", profile->attention_q_count);
    yvex_cli_out_writef(stdout, "tensor_collection_attention_k_count: %llu\n", profile->attention_k_count);
    yvex_cli_out_writef(stdout, "tensor_collection_attention_v_count: %llu\n", profile->attention_v_count);
    yvex_cli_out_writef(stdout, "tensor_collection_attention_o_count: %llu\n", profile->attention_o_count);
    yvex_cli_out_writef(stdout, "tensor_collection_attention_complete_qkvo_layer_count: %llu\n",
           profile->attention_complete_qkvo_layer_count);
    yvex_cli_out_writef(stdout, "tensor_collection_mlp_status: %s\n",
           tensor_collection_mlp_status(profile));
    yvex_cli_out_writef(stdout, "tensor_collection_mlp_gate_count: %llu\n", profile->mlp_gate_count);
    yvex_cli_out_writef(stdout, "tensor_collection_mlp_up_count: %llu\n", profile->mlp_up_count);
    yvex_cli_out_writef(stdout, "tensor_collection_mlp_down_count: %llu\n", profile->mlp_down_count);
    yvex_cli_out_writef(stdout, "tensor_collection_mlp_complete_gud_layer_count: %llu\n",
           profile->mlp_complete_gud_layer_count);
    yvex_cli_out_writef(stdout, "tensor_collection_norm_status: %s\n",
           tensor_collection_present_status(profile->norm_tensor_count));
    yvex_cli_out_writef(stdout, "tensor_collection_norm_tensor_count: %llu\n",
           profile->norm_tensor_count);
    yvex_cli_out_writef(stdout, "tensor_collection_output_head_status: %s\n",
           tensor_collection_present_status(profile->output_head_tensor_count));
    yvex_cli_out_writef(stdout, "tensor_collection_output_head_tensor_count: %llu\n",
           profile->output_head_tensor_count);
    yvex_cli_out_writef(stdout, "tensor_collection_moe_status: %s\n",
           (profile->moe_router_count + profile->moe_expert_count) > 0
               ? "observed"
               : "not-observed");
    yvex_cli_out_writef(stdout, "tensor_collection_moe_router_count: %llu\n", profile->moe_router_count);
    yvex_cli_out_writef(stdout, "tensor_collection_moe_expert_count: %llu\n", profile->moe_expert_count);
    yvex_cli_out_writef(stdout, "tensor_collection_tokenizer_collection_status: %s\n",
           profile->tokenizer_present ? "sidecar-observed" : "missing");
    yvex_cli_out_writef(stdout, "tensor_collection_kv_runtime_state_status: runtime-state-required-not-implemented\n");
    yvex_cli_out_writef(stdout, "tensor_collection_validation_status: lexical-and-header-only\n");
    yvex_cli_out_writef(stdout, "tensor_collection_role_mapping_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tensor_collection_runtime_descriptor_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tensor_collection_graph_consumer_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    ...
    const yvex_model_target_record *record)
{
    ...
    spec = find_model_class_profile_spec(record->target_id);
    if (!spec) return;
    yvex_cli_out_writef(stdout, "tensor_collection_status: command-visible\n");
    yvex_cli_out_writef(stdout, "tensor_collection_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "tensor_collection_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "tensor_collection_stage: header-collection-inventory\n");
    yvex_cli_out_writef(stdout, "tensor_collection_evidence_basis: header-metadata-only\n");
...
static void print_tensor_naming_normal(
    const yvex_tensor_naming_profile *profile)
{
    yvex_cli_out_writef(stdout, "tensor-map: %s [%s]\n",
           profile->record->target_id,
           compact_status_bracket(profile->status));
    yvex_cli_out_writef(stdout, "family: %s  stage: header-naming-map  evidence: header-only\n",
           profile->spec->family_key);
    yvex_cli_out_writef(stdout, "roles: total=%llu embedding=%llu attention=%llu mlp=%llu norm=%llu head=%llu moe=%llu unknown=%llu\n",
           profile->mapped_total_count,
           profile->embedding_count,
           profile->attention_count,
           profile->mlp_count,
    ...
    yvex_cli_out_writef(stdout, "boundary: report-only; use --audit for tensor entries\n");
}

static void print_tensor_naming_table(
    const yvex_tensor_naming_profile *profile)
{
    yvex_cli_out_writef(stdout, "TENSOR NAMING MAP\n\n");
    yvex_cli_out_writef(stdout, "%-6s  %-20s  %-24s  %7s  %6s  %6s  %6s  %6s  %6s  %6s  %8s  %7s  %s\n",
           "FAMILY", "TARGET", "STATUS", "TOTAL", "EMBED", "ATTN",
           "MLP", "NORM", "HEAD", "MOE", "UNKNOWN", "LAYERS", "NEXT");
    yvex_cli_out_writef(stdout, "%-6s  %-20s  %-24s  %7llu  %6llu  %6llu  %6llu  %6llu  %6llu  %6llu  %8llu  %7llu  %s\n",
           profile->spec->family_key,
           profile->record->target_id,
           profile->status,
           profile->mapped_total_count,
    ...
    const yvex_tensor_naming_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "tensor_map_family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "tensor_map_target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "tensor_map_stage: header-naming-map\n");
    yvex_cli_out_writef(stdout, "tensor_map_evidence_basis: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "tensor_map_source_status: %s\n",
           profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_map_source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "tensor_map_manifest_status: not-checked\n");
    yvex_cli_out_writef(stdout, "tensor_map_config_status: %s\n",
           profile->config_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_map_tokenizer_status: %s\n",
           profile->tokenizer_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_map_tensor_count: %llu\n", profile->tensor_count);
    yvex_cli_out_writef(stdout, "tensor_map_mapped_total_count: %llu\n", profile->mapped_total_count);
    yvex_cli_out_writef(stdout, "tensor_map_unmapped_unknown_count: %llu\n",
           profile->unmapped_unknown_count);
    yvex_cli_out_writef(stdout, "tensor_map_ambiguous_count: %llu\n", profile->ambiguous_count);
    yvex_cli_out_writef(stdout, "tensor_map_layer_count_observed: %llu\n",
           profile->layer_count_observed);
    yvex_cli_out_writef(stdout, "tensor_map_embedding_count: %llu\n", profile->embedding_count);
    yvex_cli_out_writef(stdout, "tensor_map_attention_count: %llu\n", profile->attention_count);
    yvex_cli_out_writef(stdout, "tensor_map_attention_q_count: %llu\n", profile->attention_q_count);
    yvex_cli_out_writef(stdout, "tensor_map_attention_k_count: %llu\n", profile->attention_k_count);
    yvex_cli_out_writef(stdout, "tensor_map_attention_v_count: %llu\n", profile->attention_v_count);
    yvex_cli_out_writef(stdout, "tensor_map_attention_o_count: %llu\n", profile->attention_o_count);
    yvex_cli_out_writef(stdout, "tensor_map_mlp_count: %llu\n", profile->mlp_count);
    yvex_cli_out_writef(stdout, "tensor_map_mlp_gate_count: %llu\n", profile->mlp_gate_count);
    yvex_cli_out_writef(stdout, "tensor_map_mlp_up_count: %llu\n", profile->mlp_up_count);
    yvex_cli_out_writef(stdout, "tensor_map_mlp_down_count: %llu\n", profile->mlp_down_count);
    yvex_cli_out_writef(stdout, "tensor_map_norm_count: %llu\n", profile->norm_count);
    yvex_cli_out_writef(stdout, "tensor_map_output_head_count: %llu\n", profile->output_head_count);
    yvex_cli_out_writef(stdout, "tensor_map_qwen_linear_attn_count: %llu\n",
           profile->qwen_linear_attn_count);
    yvex_cli_out_writef(stdout, "tensor_map_moe_router_count: %llu\n", profile->moe_router_count);
    yvex_cli_out_writef(stdout, "tensor_map_moe_expert_count: %llu\n", profile->moe_expert_count);
    yvex_cli_out_writef(stdout, "tensor_map_moe_shared_count: %llu\n", profile->moe_shared_count);
    yvex_cli_out_writef(stdout, "tensor_map_required_role_coverage_status: %s\n",
           tensor_naming_required_groups_present(profile)
               ? "required-groups-present"
               : "required-groups-missing");
    yvex_cli_out_writef(stdout, "tensor_map_tokenizer_sidecar_status: %s\n",
           profile->tokenizer_present ? "sidecar-observed" : "missing");
    yvex_cli_out_writef(stdout, "tensor_map_config_sidecar_status: %s\n",
           profile->config_present ? "sidecar-observed" : "missing");
    yvex_cli_out_writef(stdout, "tensor_map_validation_status: lexical-and-header-only\n");
    yvex_cli_out_writef(stdout, "tensor_map_canonical_role_status: mapped-candidates\n");
    yvex_cli_out_writef(stdout, "tensor_map_runtime_role_coverage_status: report-only\n");
    yvex_cli_out_writef(stdout, "tensor_map_artifact_contract_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tensor_map_runtime_descriptor_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tensor_map_graph_consumer_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    ...
    for (i = 0; i < profile->entry_count; ++i) {
        ...
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.native_name: %s\n", i, entry->native_name);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.canonical_role: %s\n", i, entry->canonical_role);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.family: %s\n", i, entry->family);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.target_id: %s\n", i, entry->target_id);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.collection: %s\n", i, entry->collection);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.layer_index: %s\n", i, entry->layer_index);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.expert_index: %s\n", i, entry->expert_index);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.dtype: %s\n", i, entry->dtype);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.rank: %s\n", i, entry->rank);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.shape: %s\n", i,
               entry->shape[0] ? entry->shape : "unknown");
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.source_file: %s\n", i, entry->source_file);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.mapping_status: %s\n",
               i, entry->mapping_status);
        yvex_cli_out_writef(stdout, "tensor_map.entry.%llu.mapping: %s -> %s\n",
               i, entry->native_name, entry->canonical_role);
    }
    yvex_cli_out_writef(stdout, "boundary: %stensor naming map only; no runtime descriptor/graph/runtime/generation\n",
...
static void print_tensor_map_audit_hint(const yvex_model_target_record *record)
{
    ...
    spec = find_model_class_profile_spec(record->target_id);
    if (!spec) return;
    yvex_cli_out_writef(stdout, "tensor_map_status: not-run\n");
    yvex_cli_out_writef(stdout, "tensor_map_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "tensor_map_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "tensor_map_stage: header-naming-map\n");
    yvex_cli_out_writef(stdout, "tensor_map_evidence_basis: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "tensor_map_next: %s\n", YVEX_TENSOR_NAMING_NEXT_ROW);
}
...
static void print_output_head_map_audit_hint(const yvex_model_target_record *record)
{
    ...
    spec = find_model_class_profile_spec(record->target_id);
    if (!spec) return;
    yvex_cli_out_writef(stdout, "output_head_map_status: not-run\n");
    yvex_cli_out_writef(stdout, "output_head_map_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "output_head_map_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "output_head_map_stage: header-output-head-map\n");
    yvex_cli_out_writef(stdout, "output_head_map_evidence_basis: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "output_head_map_next: %s\n", YVEX_OUTPUT_HEAD_MAP_NEXT_ROW);
}
...
static void print_output_head_map_normal(
    const yvex_output_head_map_profile *profile)
{
    yvex_cli_out_writef(stdout, "output-head-map: %s [%s]\n",
           profile->record->target_id,
           compact_status_bracket(profile->status));
    yvex_cli_out_writef(stdout, "family: %s  evidence: header-only\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "head: %s  final_norm: %s  embedding: %s  tie: %s\n",
    ...
    yvex_cli_out_writef(stdout, "shape: %s\n", profile->shape_relation_status);
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next: %s\n", YVEX_OUTPUT_HEAD_MAP_NEXT_ROW);
    yvex_cli_out_writef(stdout, "boundary: mapping only; no logits/runtime/generation\n");
}

static void print_output_head_map_table(
    const yvex_output_head_map_profile *profile)
{
    yvex_cli_out_writef(stdout, "OUTPUT HEAD TENSOR MAP\n\n");
    yvex_cli_out_writef(stdout, "%-6s  %-20s  %-31s  %-4s  %-10s  %-5s  %-34s  %-24s  %s\n",
...
static void print_output_head_entry_audit(
    ...
    const yvex_output_head_map_entry *entry)
{
    yvex_cli_out_writef(stdout, "output_head.entry.%s.native_name: %s\n", name, entry->native_name);
    yvex_cli_out_writef(stdout, "output_head.entry.%s.canonical_role: %s\n",
           name, entry->canonical_role);
    yvex_cli_out_writef(stdout, "output_head.entry.%s.mapping_status: %s\n",
           name, entry->mapping_status);
    yvex_cli_out_writef(stdout, "output_head.entry.%s.dtype: %s\n", name, entry->dtype);
    yvex_cli_out_writef(stdout, "output_head.entry.%s.rank: %s\n", name, entry->rank);
    yvex_cli_out_writef(stdout, "output_head.entry.%s.shape: %s\n", name, entry->shape);
    yvex_cli_out_writef(stdout, "output_head.entry.%s.source_file: %s\n", name, entry->source_file);
}
    ...
    const yvex_output_head_map_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "output_head_map_stage: header-output-head-map\n");
    yvex_cli_out_writef(stdout, "output_head_map_evidence_basis: header-metadata-only\n");
    yvex_cli_out_writef(stdout, "output_head_map_source_status: %s\n",
           profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "output_head_map_source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "output_head_map_manifest_status: not-checked\n");
    yvex_cli_out_writef(stdout, "output_head_map_config_status: %s\n",
           profile->config_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "output_head_map_tokenizer_status: %s\n",
           profile->tokenizer_present ? "present" : "missing");
    yvex_cli_out_writef(stdout, "output_head_native_name: %s\n", profile->output_head.native_name);
    yvex_cli_out_writef(stdout, "output_head_canonical_role: %s\n",
           profile->output_head.canonical_role);
    yvex_cli_out_writef(stdout, "output_head_mapping_status: %s\n",
           profile->output_head.mapping_status);
    yvex_cli_out_writef(stdout, "output_head_candidate_count: %llu\n",
           profile->output_head_candidate_count);
    yvex_cli_out_writef(stdout, "output_head_ambiguous_count: %llu\n",
           profile->output_head_ambiguous_count);
    yvex_cli_out_writef(stdout, "output_head_missing_status: %s\n",
           profile->output_head_missing_status);
    yvex_cli_out_writef(stdout, "output_head_dtype: %s\n", profile->output_head.dtype);
    yvex_cli_out_writef(stdout, "output_head_rank: %s\n", profile->output_head.rank);
    yvex_cli_out_writef(stdout, "output_head_shape: %s\n", profile->output_head.shape);
    yvex_cli_out_writef(stdout, "output_head_vocab_dim_candidate: %s\n",
           profile->output_head.vocab_dim_candidate);
    yvex_cli_out_writef(stdout, "output_head_hidden_dim_candidate: %s\n",
           profile->output_head.hidden_dim_candidate);
    yvex_cli_out_writef(stdout, "embedding_native_name: %s\n", profile->embedding.native_name);
    yvex_cli_out_writef(stdout, "embedding_canonical_role: %s\n", profile->embedding.canonical_role);
    yvex_cli_out_writef(stdout, "embedding_dtype: %s\n", profile->embedding.dtype);
    yvex_cli_out_writef(stdout, "embedding_rank: %s\n", profile->embedding.rank);
    yvex_cli_out_writef(stdout, "embedding_shape: %s\n", profile->embedding.shape);
    yvex_cli_out_writef(stdout, "final_norm_native_name: %s\n", profile->final_norm.native_name);
    yvex_cli_out_writef(stdout, "final_norm_canonical_role: %s\n",
           profile->final_norm.canonical_role);
    yvex_cli_out_writef(stdout, "final_norm_dtype: %s\n", profile->final_norm.dtype);
    yvex_cli_out_writef(stdout, "final_norm_rank: %s\n", profile->final_norm.rank);
    yvex_cli_out_writef(stdout, "final_norm_shape: %s\n", profile->final_norm.shape);
    yvex_cli_out_writef(stdout, "tie_policy_status: %s\n", profile->tie_policy_status);
    yvex_cli_out_writef(stdout, "config_tie_word_embeddings_status: %s\n",
           profile->config_tie_word_embeddings_status);
    yvex_cli_out_writef(stdout, "shape_relation_status: %s\n", profile->shape_relation_status);
    yvex_cli_out_writef(stdout, "output_head_runtime_consumer_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "output_head_logits_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "output_head_artifact_contract_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "output_head_runtime_descriptor_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "output_head_graph_consumer_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    ...
    yvex_cli_out_writef(stdout, "boundary: output-head tensor mapping only; no logits/runtime/generation\n");
}
...
static void print_tokenizer_map_normal(
    const yvex_tokenizer_map_profile *profile)
{
    yvex_cli_out_writef(stdout, "tokenizer-map: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "tokenizer: %s\n",
           tokenizer_map_normal_sidecar(&profile->tokenizer_json, "present"));
    yvex_cli_out_writef(stdout, "vocab: %s\n", tokenizer_map_vocab_status(profile));
    yvex_cli_out_writef(stdout, "merges: %s\n", tokenizer_map_merges_status(profile));
    yvex_cli_out_writef(stdout, "chat_template: %s\n",
           strcmp(profile->chat_template_status, "present") == 0
               ? "present"
               : profile->chat_template_status);
    yvex_cli_out_writef(stdout, "specials: %s\n", profile->special_tokens_status);
    yvex_cli_out_writef(stdout, "runtime: unsupported\n");
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next: %s\n", tokenizer_map_next_row(profile));
    yvex_cli_out_writef(stdout, "boundary: tokenizer metadata mapping only; no tokenization/detokenization/runtime/generation\n");
}

static void print_tokenizer_map_table(
    const yvex_tokenizer_map_profile *profile)
{
    yvex_cli_out_writef(stdout, "TOKENIZER METADATA MAP\n\n");
    yvex_cli_out_writef(stdout, "%-20s  %-6s  %-19s  %-9s  %-28s  %-22s  %-13s  %-8s  %s\n",
...
static void print_tokenizer_map_audit(
    const yvex_tokenizer_map_profile *profile)
{
    yvex_cli_out_writef(stdout, "schema_version: yvex.source.tokenizer_map.v1\n");
    yvex_cli_out_writef(stdout, "tokenizer_map_status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "tokenizer_map_family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "tokenizer_map_target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "tokenizer_map_stage: metadata-tokenizer-map\n");
    yvex_cli_out_writef(stdout, "tokenizer_map_evidence_basis: sidecar-json-only\n");
    yvex_cli_out_writef(stdout, "evidence_basis: sidecar-json-only\n");
    yvex_cli_out_writef(stdout, "tokenizer_map_source_status: %s\n",
           profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tokenizer_map_source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "tokenizer_json_status: %s\n", profile->tokenizer_json.status);
    yvex_cli_out_writef(stdout, "tokenizer_json_path: %s\n", profile->tokenizer_json.path);
    yvex_cli_out_writef(stdout, "tokenizer_config_status: %s\n", profile->tokenizer_config.status);
    yvex_cli_out_writef(stdout, "tokenizer_config_path: %s\n", profile->tokenizer_config.path);
    yvex_cli_out_writef(stdout, "special_tokens_map_status: %s\n",
           profile->special_tokens_map.status);
    yvex_cli_out_writef(stdout, "special_tokens_map_path: %s\n", profile->special_tokens_map.path);
    yvex_cli_out_writef(stdout, "generation_config_status: %s\n",
           profile->generation_config.status);
    yvex_cli_out_writef(stdout, "generation_config_path: %s\n", profile->generation_config.path);
    yvex_cli_out_writef(stdout, "config_json_status: %s\n", profile->config_json.status);
    yvex_cli_out_writef(stdout, "config_json_path: %s\n", profile->config_json.path);
    yvex_cli_out_writef(stdout, "vocab_json_status: %s\n", profile->vocab_json.status);
    yvex_cli_out_writef(stdout, "vocab_status: %s\n", tokenizer_map_vocab_status(profile));
    yvex_cli_out_writef(stdout, "merges_txt_status: %s\n", profile->merges_txt.status);
    yvex_cli_out_writef(stdout, "merges_status: %s\n", tokenizer_map_merges_status(profile));
    yvex_cli_out_writef(stdout, "tokenizer_model_status: %s\n", profile->tokenizer_model.status);
    yvex_cli_out_writef(stdout, "chat_template_file_status: %s\n",
           profile->chat_template_file.status);
    yvex_cli_out_writef(stdout, "tokenizer_class: %s\n", profile->tokenizer_class);
    yvex_cli_out_writef(stdout, "model_type: %s\n", profile->model_type);
    yvex_cli_out_writef(stdout, "tokenizer_model_type: %s\n", profile->model_type);
    yvex_cli_out_writef(stdout, "tokenizer_backend_type: %s\n", profile->tokenizer_backend_type);
    yvex_cli_out_writef(stdout, "vocab_size_status: %s\n", profile->vocab_size_status);
    yvex_cli_out_writef(stdout, "vocab_size: %s\n", profile->vocab_size);
    yvex_cli_out_writef(stdout, "config_vocab_size: %s\n", profile->config_vocab_size);
    yvex_cli_out_writef(stdout, "tokenizer_vocab_size: %s\n", profile->tokenizer_vocab_size);
    yvex_cli_out_writef(stdout, "output_head_vocab_dim_candidate: %s\n",
           profile->output_head_vocab_dim_candidate);
    yvex_cli_out_writef(stdout, "output_head_vocab_relation_status: %s\n",
           profile->output_head_vocab_relation_status);
    yvex_cli_out_writef(stdout, "bos_token_id_status: %s\n", profile->bos_token_id_status);
    yvex_cli_out_writef(stdout, "bos_token_id: %s\n", profile->bos_token_id);
    yvex_cli_out_writef(stdout, "bos_token_status: %s\n", profile->bos_token_status);
    yvex_cli_out_writef(stdout, "bos_token: %s\n", profile->bos_token);
    yvex_cli_out_writef(stdout, "eos_token_id_status: %s\n", profile->eos_token_id_status);
    yvex_cli_out_writef(stdout, "eos_token_id: %s\n", profile->eos_token_id);
    yvex_cli_out_writef(stdout, "eos_token_status: %s\n", profile->eos_token_status);
    yvex_cli_out_writef(stdout, "eos_token: %s\n", profile->eos_token);
    yvex_cli_out_writef(stdout, "pad_token_id_status: %s\n", profile->pad_token_id_status);
    yvex_cli_out_writef(stdout, "pad_token_id: %s\n", profile->pad_token_id);
    yvex_cli_out_writef(stdout, "pad_token_status: %s\n", profile->pad_token_status);
    yvex_cli_out_writef(stdout, "pad_token: %s\n", profile->pad_token);
    yvex_cli_out_writef(stdout, "unk_token_id_status: %s\n", profile->unk_token_id_status);
    yvex_cli_out_writef(stdout, "unk_token_id: %s\n", profile->unk_token_id);
    yvex_cli_out_writef(stdout, "unk_token_status: %s\n", profile->unk_token_status);
    yvex_cli_out_writef(stdout, "unk_token: %s\n", profile->unk_token);
    yvex_cli_out_writef(stdout, "sep_token_id_status: %s\n", profile->sep_token_id_status);
    yvex_cli_out_writef(stdout, "sep_token_id: %s\n", profile->sep_token_id);
    yvex_cli_out_writef(stdout, "sep_token_status: %s\n", profile->sep_token_status);
    yvex_cli_out_writef(stdout, "sep_token: %s\n", profile->sep_token);
    yvex_cli_out_writef(stdout, "added_tokens_status: %s\n", profile->added_tokens_status);
    yvex_cli_out_writef(stdout, "added_tokens_count: %s\n", profile->added_tokens_count);
    yvex_cli_out_writef(stdout, "special_tokens_status: %s\n", profile->special_tokens_status);
    yvex_cli_out_writef(stdout, "additional_special_tokens_status: %s\n",
           profile->additional_special_tokens_status);
    yvex_cli_out_writef(stdout, "additional_special_tokens_count: %s\n",
           profile->additional_special_tokens_count);
    yvex_cli_out_writef(stdout, "stop_token_candidate_status: %s\n",
           profile->stop_token_candidate_status);
    yvex_cli_out_writef(stdout, "stop_token_candidate_count: %s\n",
           profile->stop_token_candidate_count);
    yvex_cli_out_writef(stdout, "stop_token_candidate.0.id: %s\n",
           profile->stop_token_candidate_0_id);
    yvex_cli_out_writef(stdout, "stop_token_candidate.0.text: %s\n",
           profile->stop_token_candidate_0_text);
    yvex_cli_out_writef(stdout, "chat_template_status: %s\n", profile->chat_template_status);
    yvex_cli_out_writef(stdout, "chat_template_present: %s\n", profile->chat_template_present);
    yvex_cli_out_writef(stdout, "chat_template_source: %s\n", profile->chat_template_source);
    yvex_cli_out_writef(stdout, "chat_template_hash_status: not-computed\n");
    yvex_cli_out_writef(stdout, "prompt_template_status: %s\n", profile->prompt_template_status);
    yvex_cli_out_writef(stdout, "chat_template_runtime_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tokenizer_runtime_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tokenization_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "detokenization_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "gguf_tokenizer_contract_status: planned\n");
    yvex_cli_out_writef(stdout, "eos_stop_policy_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "stop_token_policy_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "prompt_template_runtime_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next_required_rows: %s\n", tokenizer_map_next_row(profile));
    yvex_cli_out_writef(stdout, "boundary: tokenizer metadata mapping only; no tokenization/runtime/generation\n");
}
    ...
    const yvex_tokenizer_map_profile *profile)
{
    if (!profile) return;
    yvex_cli_out_writef(stdout, "{\"schema_version\":\"yvex.source.tokenizer_map.v1\",");
    yvex_cli_out_writef(stdout, "\"status\":");
...
static void print_tokenizer_map_audit_hint(const yvex_model_target_record *record)
{
    ...
    spec = find_model_class_profile_spec(record->target_id);
    if (!spec) return;
    yvex_cli_out_writef(stdout, "tokenizer_map_status: not-run\n");
    yvex_cli_out_writef(stdout, "tokenizer_map_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "tokenizer_map_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "tokenizer_map_stage: metadata-tokenizer-map\n");
    yvex_cli_out_writef(stdout, "tokenizer_map_evidence_basis: sidecar-metadata-only\n");
    yvex_cli_out_writef(stdout, "tokenizer_runtime_status: not-implemented\n");
    yvex_cli_out_writef(stdout, "tokenizer_map_next: %s\n", YVEX_TOKENIZER_MISSING_NEXT_ROW);
}
                                    ...
                                    int metadata)
{
    ...
    for (i = 0; i < profile->missing_entry_count; ++i) {
        ...
        yvex_cli_out_writef(stdout, "%s%s", first ? "" : ",", entry->name);
        first = 0;
    }
    if (first) yvex_cli_out_writef(stdout, "none");
    yvex_cli_out_writef(stdout, "\n");
}
...
static void print_missing_role_report_normal(
    const yvex_missing_role_report_profile *profile)
{
    yvex_cli_out_writef(stdout, "missing-roles: %s [%s]\n",
           profile->record->target_id,
           compact_status_bracket(profile->status));
    yvex_cli_out_writef(stdout, "family: %s  evidence: header+sidecar-only\n",
           profile->spec->family_key);
    yvex_cli_out_writef(stdout, "source_roles: %llu/%u present, %llu missing, %llu ambiguous\n",
           profile->source_role_observed_count,
           missing_role_source_required_count(profile),
           profile->source_role_missing_count,
           profile->source_role_ambiguous_count);
    yvex_cli_out_writef(stdout, "metadata_roles: %llu/4 present, %llu missing, %llu ambiguous\n",
           profile->metadata_observed_count,
           profile->metadata_missing_count,
           profile->metadata_ambiguous_count);
    ...
    yvex_cli_out_writef(stdout, "boundary: report-only; use --audit for role details\n");
}

static void print_missing_role_report_table(
    const yvex_missing_role_report_profile *profile)
{
    yvex_cli_out_writef(stdout, "MISSING ROLE BLOCKER REPORT\n\n");
    yvex_cli_out_writef(stdout, "%-6s  %-15s  %-29s  %7s  %8s  %9s  %8s  %9s  %-30s  %s\n",
           "FAMILY", "TARGET", "STATUS", "OBS_SRC", "MISS_SRC",
           "AMBIG_SRC", "OBS_META", "MISS_META", "TOP_BLOCKER", "NEXT");
    yvex_cli_out_writef(stdout, "%-6s  %-15s  %-29s  %7llu  %8llu  %9llu  %8llu  %9llu  %-30s  %s\n",
           profile->spec->family_key,
           profile->record->target_id,
           profile->status,
           profile->source_role_observed_count,
    ...
    const yvex_missing_role_report_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "missing_role_report_status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "missing_role_report_family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "missing_role_report_target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "missing_role_report_stage: missing-role-blocker-report\n");
    yvex_cli_out_writef(stdout, "missing_role_report_evidence_basis: header-and-sidecar-metadata-only\n");
    yvex_cli_out_writef(stdout, "missing_role_report_source_status: %s\n",
           profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "missing_role_report_source_path: %s\n", profile->source_path);
    yvex_cli_out_writef(stdout, "missing_role_source_role_required_count: %u\n",
           missing_role_source_required_count(profile));
    yvex_cli_out_writef(stdout, "missing_role_source_role_observed_count: %llu\n",
           profile->source_role_observed_count);
    yvex_cli_out_writef(stdout, "missing_role_source_role_missing_count: %llu\n",
           profile->source_role_missing_count);
    yvex_cli_out_writef(stdout, "missing_role_source_role_ambiguous_count: %llu\n",
           profile->source_role_ambiguous_count);
    yvex_cli_out_writef(stdout, "missing_role_metadata_required_count: 4\n");
    yvex_cli_out_writef(stdout, "missing_role_metadata_observed_count: %llu\n",
           profile->metadata_observed_count);
    yvex_cli_out_writef(stdout, "missing_role_metadata_missing_count: %llu\n",
           profile->metadata_missing_count);
    yvex_cli_out_writef(stdout, "missing_role_metadata_ambiguous_count: %llu\n",
           profile->metadata_ambiguous_count);
    yvex_cli_out_writef(stdout, "missing_role_embedding_status: %s\n", profile->embedding_status);
    yvex_cli_out_writef(stdout, "missing_role_attention_norm_status: %s\n",
           profile->attention_norm_status);
    yvex_cli_out_writef(stdout, "missing_role_attention_q_status: %s\n",
           profile->attention_q_status);
    yvex_cli_out_writef(stdout, "missing_role_attention_k_status: %s\n",
           profile->attention_k_status);
    yvex_cli_out_writef(stdout, "missing_role_attention_v_status: %s\n",
           profile->attention_v_status);
    yvex_cli_out_writef(stdout, "missing_role_attention_o_status: %s\n",
           profile->attention_o_status);
    yvex_cli_out_writef(stdout, "missing_role_mlp_norm_status: %s\n", profile->mlp_norm_status);
    yvex_cli_out_writef(stdout, "missing_role_mlp_gate_status: %s\n", profile->mlp_gate_status);
    yvex_cli_out_writef(stdout, "missing_role_mlp_up_status: %s\n", profile->mlp_up_status);
    yvex_cli_out_writef(stdout, "missing_role_mlp_down_status: %s\n", profile->mlp_down_status);
    yvex_cli_out_writef(stdout, "missing_role_final_norm_status: %s\n", profile->final_norm_status);
    yvex_cli_out_writef(stdout, "missing_role_output_head_status: %s\n",
           profile->output_head_status);
    yvex_cli_out_writef(stdout, "missing_role_tied_head_policy_status: %s\n",
           profile->tied_head_policy_status);
    yvex_cli_out_writef(stdout, "missing_role_qwen_linear_attn_status: %s\n",
           profile->qwen_linear_attn_status);
    yvex_cli_out_writef(stdout, "missing_role_qwen_linear_attn_count: %llu\n",
           profile->qwen_linear_attn_count);
    yvex_cli_out_writef(stdout, "missing_role_moe_router_status: %s\n",
           profile->moe_router_status);
    yvex_cli_out_writef(stdout, "missing_role_moe_router_count: %llu\n",
           profile->moe_router_count);
    yvex_cli_out_writef(stdout, "missing_role_moe_expert_status: %s\n",
           profile->moe_expert_status);
    yvex_cli_out_writef(stdout, "missing_role_moe_expert_count: %llu\n",
           profile->moe_expert_count);
    yvex_cli_out_writef(stdout, "missing_role_moe_shared_status: %s\n",
           profile->moe_shared_status);
    yvex_cli_out_writef(stdout, "missing_role_moe_shared_count: %llu\n",
           profile->moe_shared_count);
    yvex_cli_out_writef(stdout, "missing_role_unknown_tensor_status: %s\n",
           profile->unknown_tensor_status);
    yvex_cli_out_writef(stdout, "missing_role_unknown_tensor_count: %llu\n",
           profile->unmapped_unknown_count);
    yvex_cli_out_writef(stdout, "missing_role_tokenizer_metadata_status: %s\n",
           profile->tokenizer_metadata_status);
    yvex_cli_out_writef(stdout, "missing_role_config_metadata_status: %s\n",
           profile->config_metadata_status);
    yvex_cli_out_writef(stdout, "missing_role_generation_metadata_status: %s\n",
           profile->generation_metadata_status);
    yvex_cli_out_writef(stdout, "missing_role_special_tokens_status: %s\n",
           profile->special_tokens_status);
    yvex_cli_out_writef(stdout, "missing_role_artifact_contract_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_runtime_descriptor_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_graph_consumer_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_backend_residency_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_attention_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_kv_runtime_state_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_prefill_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_decode_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_logits_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_tokenizer_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_sampling_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_generation_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_eval_benchmark_status: missing\n");
    yvex_cli_out_writef(stdout, "missing_role_top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "missing_role_next_required_row: %s\n",
           YVEX_MISSING_ROLE_REPORT_NEXT_ROW);
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    ...
    for (i = 0; i < profile->missing_entry_count; ++i) {
        const yvex_missing_role_entry *entry = &profile->missing_entries[i];
        yvex_cli_out_writef(stdout, "missing_role.entry.%llu.role: %s\n", i, entry->name);
        yvex_cli_out_writef(stdout, "missing_role.entry.%llu.blocker_class: %s\n",
               i, entry->blocker_class);
        yvex_cli_out_writef(stdout, "missing_role.entry.%llu.status: %s\n", i, entry->status);
    }
    yvex_cli_out_writef(stdout, "boundary: missing-role report only; no artifact/runtime descriptor/graph/runtime/generation\n");
}
    ...
    const yvex_model_target_record *record)
{
    ...
    spec = find_model_class_profile_spec(record->target_id);
    if (!spec) return;
    yvex_cli_out_writef(stdout, "missing_role_report_status: not-run\n");
    yvex_cli_out_writef(stdout, "missing_role_report_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "missing_role_report_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "missing_role_report_stage: missing-role-blocker-report\n");
    yvex_cli_out_writef(stdout, "missing_role_top_blocker: not-run\n");
    yvex_cli_out_writef(stdout, "missing_role_next_required_row: %s\n",
           YVEX_MISSING_ROLE_REPORT_NEXT_ROW);
}
...
static void print_missing_roles_row(const char *group,
                                    ...
                                    const char *detail)
{
    yvex_cli_out_writef(stdout, "%-16s  %-10s  %5llu  %s\n",
           group,
    ...
    const yvex_missing_roles_porcelain_context *ctx)
{
    ...
    yvex_cli_out_writef(stdout, "missing-roles: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "status: blocked\n");
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", ctx->top_blocker);
    yvex_cli_out_writef(stdout, "next: %s\n\n", missing_roles_porcelain_next_row(ctx));
    yvex_cli_out_writef(stdout, "%-16s  %-10s  %5s  %s\n",
    ...
    const yvex_missing_roles_porcelain_context *ctx)
{
    ...
    moe_total = profile->moe_router_count + profile->moe_expert_count +
                profile->moe_shared_count;
    yvex_cli_out_writef(stdout, "%-18s  %-6s  %-7s  %-24s  %5s  %5s  %5s  %5s  %5s  %3s  %7s  %-9s  %-8s  %s\n",
           "TARGET", "FAMILY", "STATUS", "TOP_BLOCKER", "EMBED", "ATTN",
           "MLP", "NORM", "HEAD", "MOE", "UNKNOWN", "TOKENIZER",
           "ARTIFACT", "NEXT");
    yvex_cli_out_writef(stdout, "%-18s  %-6s  %-7s  %-24s  %5llu  %5llu  %5llu  %5llu  %5llu  %3llu  %7llu  %-9s  %-8s  %s\n",
           profile->record->target_id,
           profile->spec->family_key,
           "blocked",
           ctx->top_blocker,
    ...
    const yvex_missing_roles_porcelain_context *ctx)
{
    ...
    yvex_cli_out_writef(stdout, "target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "source_status: %s\n", profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "model_class_status: %s\n", profile->source_exists ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_map_status: %s\n", ctx->tensor_map_status);
    yvex_cli_out_writef(stdout, "tensor_map_path: %s\n", ctx->tensor_map_path);
    yvex_cli_out_writef(stdout, "tensor_map_mapped_total_count: %llu\n", profile->mapped_total_count);
    yvex_cli_out_writef(stdout, "tensor_map_unmapped_unknown_count: %llu\n",
           profile->unmapped_unknown_count);
    yvex_cli_out_writef(stdout, "tensor_map_role_counts: embed=%llu attention=%llu qwen_linear_attn=%llu mlp=%llu norm=%llu head=%llu moe_router=%llu moe_expert=%llu moe_shared=%llu unknown=%llu\n",
           profile->embedding_count,
           profile->attention_count,
           profile->qwen_linear_attn_count,
           profile->mlp_count,
           ...
           profile->moe_shared_count,
           profile->unmapped_unknown_count);
    yvex_cli_out_writef(stdout, "role_group.embedding.status: %s\n", profile->embedding_status);
    yvex_cli_out_writef(stdout, "role_group.attention.status: %s\n",
           profile->attention_count ? "present" : "missing");
    yvex_cli_out_writef(stdout, "role_group.qwen_linear_attn.status: %s\n",
           profile->qwen_linear_attn_status);
    yvex_cli_out_writef(stdout, "role_group.mlp.status: %s\n",
           profile->mlp_count ? "present" : "missing");
    yvex_cli_out_writef(stdout, "role_group.moe_router.status: %s\n",
           profile->moe_router_status);
    yvex_cli_out_writef(stdout, "role_group.moe_experts.status: %s\n",
           profile->moe_expert_status);
    yvex_cli_out_writef(stdout, "role_group.shared_expert.status: %s\n",
           profile->moe_shared_status);
    yvex_cli_out_writef(stdout, "role_group.output_head.status: %s\n",
           profile->output_head_status);
    yvex_cli_out_writef(stdout, "role_group.tied_head_policy.status: %s\n",
           profile->tied_head_policy_status);
    yvex_cli_out_writef(stdout, "role_group.unknown_tensors.status: %s\n",
           profile->unknown_tensor_status);
    ...
    yvex_cli_out_writef(stdout, "expected_artifact_path: %s\n", ctx->expected_artifact_path);
    yvex_cli_out_writef(stdout, "artifact_plan_status: planned-full-gguf\n");
    yvex_cli_out_writef(stdout, "artifact_emission_status: not-performed\n");
    yvex_cli_out_writef(stdout, "artifact_identity_status: %s\n", ctx->artifact_identity_status);
    yvex_cli_out_writef(stdout, "prepare_blocker_count: %u\n", ctx->blocker_count);
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", ctx->top_blocker);
    yvex_cli_out_writef(stdout, "missing_role_count: %llu\n", profile->missing_entry_count);
    for (i = 0; i < profile->missing_entry_count; ++i) {
        const yvex_missing_role_entry *entry = &profile->missing_entries[i];
        yvex_cli_out_writef(stdout, "missing_role.%llu.name: %s\n", i, entry->name);
        yvex_cli_out_writef(stdout, "missing_role.%llu.status: %s\n", i, entry->status);
        yvex_cli_out_writef(stdout, "missing_role.%llu.detail: %s\n", i, entry->blocker_class);
    }
    yvex_cli_out_writef(stdout, "next: %s\n", missing_roles_porcelain_next_row(ctx));
    yvex_cli_out_writef(stdout, "runtime_execution: not-performed\n");
    yvex_cli_out_writef(stdout, "generation: unsupported\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "boundary: missing-role report only; no GGUF/runtime/generation\n");
}
...
{
    if (!profile || !ctx) return;
    yvex_cli_out_writef(stdout, "{\"status\":\"blocked\",\"target_id\":");
    model_target_json_write_escaped(stdout, profile->record->target_id);
    yvex_cli_out_writef(stdout, ",\"family\":");
    model_target_json_write_escaped(stdout, profile->spec->family_key);
    ...
    const yvex_tensor_mapping_gate_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "tensor-mapping-gate: %s [%s]\n",
           profile->record->target_id,
           compact_status_bracket(profile->status));
    yvex_cli_out_writef(stdout, "gate: v0.1.0  family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "roles: source %llu/12, metadata %llu/4, missing %llu, ambiguous %llu\n",
    ...
    if (strcmp(profile->missing_roles, "none") != 0) {
        yvex_cli_out_writef(stdout, "missing: %s\n", profile->missing_roles);
    }
    if (strcmp(profile->ambiguous_roles, "none") != 0) {
        yvex_cli_out_writef(stdout, "ambiguous: %s\n", profile->ambiguous_roles);
    }
    yvex_cli_out_writef(stdout, "result: %s\n", profile->gate_result);
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next: %s\n", profile->next_required_row);
    yvex_cli_out_writef(stdout, "boundary: report-only; no artifact/runtime/generation\n");
}
    ...
    const yvex_tensor_mapping_gate_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "TENSOR MAPPING GATE\n\n");
    yvex_cli_out_writef(stdout, "%-15s  %-6s  %-8s  %-12s  %-10s  %7s  %5s  %-34s  %-30s  %s\n",
           "TARGET", "FAMILY", "GATE", "SOURCE_ROLES", "META_ROLES",
           "MISSING", "AMBIG", "TOP_BLOCKER", "STATUS", "NEXT");
    yvex_cli_out_writef(stdout, "%-15s  %-6s  %-8s  %2llu/12         %2llu/4       %7llu  %5llu  %-34s  %-30s  %s\n",
           profile->record->target_id,
           profile->spec->family_key,
           "v0.1.0",
           profile->missing_role.source_role_observed_count,
...
static void print_tensor_mapping_gate_audit(
    const yvex_tensor_mapping_gate_profile *profile)
{
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate: v0.1.0-tensor-mapping\n");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_result: %s\n", profile->gate_result);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_target_id: %s\n", profile->record->target_id);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_family: %s\n", profile->spec->family_key);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_model_class: %s\n", profile->spec->class_name);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_source_class: %s\n",
           profile->record->source_artifact_class);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_source_status: %s\n",
           tensor_mapping_gate_source_ready(profile) ? "present" : "missing");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_source_path: %s\n",
           profile->missing_role.source_path);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_source_path_source: %s\n",
           profile->missing_role.source_path_source);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_source_report_path: not-written\n");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_tensor_map_sidecar_path: not-written\n");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_output_head_map_sidecar_path: not-written\n");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_tokenizer_metadata_source_path: %s\n",
           profile->tokenizer.source_path);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_missing_role_report_sidecar_path: not-written\n");
    yvex_cli_out_writef(stdout, "map_source.0: source-tensor-metadata-inventory\n");
    yvex_cli_out_writef(stdout, "map_source.1: model-class-profile\n");
    yvex_cli_out_writef(stdout, "map_source.2: tensor-collection-inventory\n");
    yvex_cli_out_writef(stdout, "map_source.3: tensor-naming-map\n");
    yvex_cli_out_writef(stdout, "map_source.4: output-head-tensor-map\n");
    yvex_cli_out_writef(stdout, "map_source.5: tokenizer-metadata-map\n");
    yvex_cli_out_writef(stdout, "map_source.6: missing-role-blocker-report\n");
    yvex_cli_out_writef(stdout, "model_class_profile_status: %s\n", profile->model_class.status);
    yvex_cli_out_writef(stdout, "tensor_collection_status: %s\n", profile->tensor_collection.status);
    yvex_cli_out_writef(stdout, "tensor_naming_map_status: %s\n", profile->tensor_naming.status);
    yvex_cli_out_writef(stdout, "output_head_map_status: %s\n", profile->output_head.status);
    yvex_cli_out_writef(stdout, "tokenizer_metadata_map_status: %s\n", profile->tokenizer.status);
    yvex_cli_out_writef(stdout, "missing_role_report_status: %s\n", profile->missing_role.status);
    yvex_cli_out_writef(stdout, "expected_source_role_count: 12\n");
    yvex_cli_out_writef(stdout, "observed_source_role_count: %llu\n",
           profile->missing_role.source_role_observed_count);
    yvex_cli_out_writef(stdout, "missing_source_role_count: %llu\n",
           profile->missing_role.source_role_missing_count);
    yvex_cli_out_writef(stdout, "ambiguous_source_role_count: %llu\n",
           profile->missing_role.source_role_ambiguous_count);
    yvex_cli_out_writef(stdout, "expected_metadata_role_count: 4\n");
    yvex_cli_out_writef(stdout, "observed_metadata_role_count: %llu\n",
           profile->missing_role.metadata_observed_count);
    yvex_cli_out_writef(stdout, "missing_metadata_role_count: %llu\n",
           profile->missing_role.metadata_missing_count);
    yvex_cli_out_writef(stdout, "ambiguous_metadata_role_count: %llu\n",
           profile->missing_role.metadata_ambiguous_count);
    yvex_cli_out_writef(stdout, "missing_source_roles: %s\n", profile->missing_source_roles);
    yvex_cli_out_writef(stdout, "missing_metadata_roles: %s\n", profile->missing_metadata_roles);
    yvex_cli_out_writef(stdout, "missing_roles: %s\n", profile->missing_roles);
    yvex_cli_out_writef(stdout, "ambiguous_roles: %s\n", profile->ambiguous_roles);
    yvex_cli_out_writef(stdout, "downstream_blockers: artifact_contract=missing qtype_policy=missing runtime_descriptor=missing graph_consumer=missing backend_residency=missing logits_runtime=missing tokenizer_runtime=missing generation_runtime=missing eval_benchmark=missing\n");
    yvex_cli_out_writef(stdout, "artifact_contract_status: missing\n");
    yvex_cli_out_writef(stdout, "qtype_policy_status: missing\n");
    yvex_cli_out_writef(stdout, "runtime_descriptor_status: missing\n");
    yvex_cli_out_writef(stdout, "graph_consumer_status: missing\n");
    yvex_cli_out_writef(stdout, "backend_residency_status: missing\n");
    yvex_cli_out_writef(stdout, "logits_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "tokenizer_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "generation_runtime_status: missing\n");
    yvex_cli_out_writef(stdout, "eval_benchmark_status: missing\n");
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next_required_rows: %s\n", profile->next_required_row);
    yvex_cli_out_writef(stdout, "payload_bytes_read: false\n");
    yvex_cli_out_writef(stdout, "artifact_emitted: false\n");
    yvex_cli_out_writef(stdout, "runtime_descriptor_constructed: false\n");
    yvex_cli_out_writef(stdout, "graph_consumer_fed: false\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    ...
    const yvex_model_target_record *record)
{
    ...
    spec = find_model_class_profile_spec(record->target_id);
    if (!spec) return;
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_status: not-run\n");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate: v0.1.0-tensor-mapping\n");
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_family: %s\n", spec->family_key);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_target_id: %s\n", spec->target_id);
    yvex_cli_out_writef(stdout, "tensor_mapping_gate_next_required_row: %s\n",
...
static void print_qtype_policy_table(
    const yvex_qtype_policy_profile *profile)
{
    yvex_cli_out_writef(stdout, "QTYPE POLICY\n\n");
    yvex_cli_out_writef(stdout, "%-16s  %-6s  %-28s  %-32s  %-9s  %-14s  %-25s  %-30s  %s\n",
...
static void print_qtype_policy_audit(
    const yvex_qtype_policy_profile *profile)
{
    yvex_cli_out_writef(stdout, "report: qtype-policy\n");
    yvex_cli_out_writef(stdout, "status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "target_id: %s\n", profile->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->family);
    ...
    yvex_cli_out_writef(stdout, "source_metadata_status: %s\n",
           profile->gate.model_class.source_metadata_status
               ? profile->gate.model_class.source_metadata_status
               : "not-run");
    yvex_cli_out_writef(stdout, "source_dtype_profile_status: %s\n",
           profile->source_dtype_profile_status);
    yvex_cli_out_writef(stdout, "source_dtype_counts: %s\n", profile->dtype_counts);
    yvex_cli_out_writef(stdout, "source_tensor_count: %llu\n", profile->source_tensor_count);
    yvex_cli_out_writef(stdout, "source_declared_data_bytes: %llu\n",
           profile->source_declared_data_bytes);
    yvex_cli_out_writef(stdout, "mapping_gate_status: %s\n", profile->mapping_gate_status);
    yvex_cli_out_writef(stdout, "mapping_gate_report_path: not-written\n");
    yvex_cli_out_writef(stdout, "tensor_map_status: %s\n",
    ...
    yvex_cli_out_writef(stdout, "missing_role_report_status: %s\n",
           profile->gate.missing_role.status
               ? profile->gate.missing_role.status
               : "not-run");
    yvex_cli_out_writef(stdout, "qtype_policy_basis: %s\n", profile->policy_basis);
    yvex_cli_out_writef(stdout, "qtype_policy_status: %s\n", profile->qtype_policy_status);
    yvex_cli_out_writef(stdout, "preferred_qtype: %s\n", profile->preferred_qtype);
    yvex_cli_out_writef(stdout, "candidate_qtypes: %s\n", profile->candidate_qtypes);
    yvex_cli_out_writef(stdout, "refused_qtypes: %s\n", profile->refused_qtypes);
    yvex_cli_out_writef(stdout, "refusal_reasons: %s\n", profile->refusal_reasons);
    yvex_cli_out_writef(stdout, "per_role_qtype_status: %s\n", profile->per_role_qtype_status);
    yvex_cli_out_writef(stdout, "compute_support_status: %s\n", profile->compute_support_status);
    yvex_cli_out_writef(stdout, "calibration_status: %s\n", profile->calibration_status);
    yvex_cli_out_writef(stdout, "imatrix_status: %s\n", profile->imatrix_status);
    yvex_cli_out_writef(stdout, "artifact_emit_status: %s\n", profile->artifact_emit_status);
    yvex_cli_out_writef(stdout, "artifact_identity_status: missing\n");
    yvex_cli_out_writef(stdout, "materialization_status: unsupported\n");
    yvex_cli_out_writef(stdout, "runtime_descriptor_status: missing\n");
    yvex_cli_out_writef(stdout, "graph_consumer_status: missing\n");
    yvex_cli_out_writef(stdout, "backend_residency_status: missing\n");
    yvex_cli_out_writef(stdout, "downstream_blockers: per_role_qtype=deferred compute_refusal_matrix=deferred calibration_imatrix=deferred artifact_emit=missing artifact_identity=missing runtime_descriptor=missing graph_consumer=missing backend_residency=missing generation_runtime=missing eval_benchmark=missing\n");
    yvex_cli_out_writef(stdout, "next_required_rows: %s\n", profile->next_required_row);
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
...
{
    yvex_cli_out_writef(stdout, "boundary: qtype role report only; no quantization/GGUF/runtime/generation\n");
}

static void print_qtype_role_support_normal(
    const yvex_qtype_role_support_profile *profile)
{
    yvex_cli_out_writef(stdout, "qtype-role-support: %s\n", profile->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->family);
    yvex_cli_out_writef(stdout, "status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "source_dtype: %s\n", profile->source_dtype);
    yvex_cli_out_writef(stdout, "preferred_artifact_qtype: unresolved\n");
    yvex_cli_out_writef(stdout, "supported_roles: %llu\n", profile->supported_role_count);
    yvex_cli_out_writef(stdout, "blocked_roles: %llu\n", profile->blocked_role_count);
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next: %s\n", profile->next_required_row);
    ...
    const yvex_qtype_role_support_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "QTYPE ROLE SUPPORT\n\n");
    yvex_cli_out_writef(stdout, "%-32s  %-10s  %-14s  %-22s  %-8s  %-11s  %s\n",
    ...
    const yvex_qtype_role_support_profile *profile)
{
    ...
    yvex_cli_out_writef(stdout, "report: qtype-role-support\n");
    yvex_cli_out_writef(stdout, "status: %s\n", profile->status);
    yvex_cli_out_writef(stdout, "target_id: %s\n", profile->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", profile->family);
    yvex_cli_out_writef(stdout, "target_class: %s\n",
    ...
    yvex_cli_out_writef(stdout, "source_dtype: %s\n", profile->source_dtype);
    yvex_cli_out_writef(stdout, "source_tensor_count: %llu\n", profile->source_tensor_count);
    yvex_cli_out_writef(stdout, "role_count: %llu\n", profile->role_count);
    yvex_cli_out_writef(stdout, "supported_role_count: %llu\n", profile->supported_role_count);
    yvex_cli_out_writef(stdout, "blocked_role_count: %llu\n", profile->blocked_role_count);
    yvex_cli_out_writef(stdout, "evidence_basis: %s\n", profile->evidence_basis);
    yvex_cli_out_writef(stdout, "selected_slice_evidence_only: %s\n",
           profile->selected_slice_evidence_only ? "true" : "false");
    yvex_cli_out_writef(stdout, "full_family_artifact_status: %s\n",
           profile->full_family_artifact_status);
    for (i = 0; i < profile->role_count; ++i) {
        const yvex_qtype_role_support_entry *entry = &profile->roles[i];
        yvex_cli_out_writef(stdout, "role.%llu.target_id: %s\n", i, profile->target_id);
        yvex_cli_out_writef(stdout, "role.%llu.family: %s\n", i, profile->family);
        yvex_cli_out_writef(stdout, "role.%llu.role_name: %s\n", i, entry->role_name);
        yvex_cli_out_writef(stdout, "role.%llu.role_status: %s\n", i, entry->role_status);
        yvex_cli_out_writef(stdout, "role.%llu.source_dtype: %s\n", i, entry->source_dtype);
        yvex_cli_out_writef(stdout, "role.%llu.source_dtype_status: %s\n", i,
               entry->source_dtype_status);
        yvex_cli_out_writef(stdout, "role.%llu.source_tensor_count: %llu\n", i,
               entry->source_tensor_count);
        yvex_cli_out_writef(stdout, "role.%llu.preferred_artifact_qtype: %s\n", i,
               entry->preferred_artifact_qtype);
        yvex_cli_out_writef(stdout, "role.%llu.candidate_artifact_qtypes: %s\n", i,
               entry->candidate_artifact_qtypes);
        yvex_cli_out_writef(stdout, "role.%llu.refused_artifact_qtypes: %s\n", i,
               entry->refused_artifact_qtypes);
        yvex_cli_out_writef(stdout, "role.%llu.qtype_policy_status: %s\n", i,
               entry->qtype_policy_status);
        yvex_cli_out_writef(stdout, "role.%llu.storage_support_status: %s\n", i,
               entry->storage_support_status);
        yvex_cli_out_writef(stdout, "role.%llu.compute_support_status: %s\n", i,
               entry->compute_support_status);
        yvex_cli_out_writef(stdout, "role.%llu.calibration_required: %s\n", i,
               entry->calibration_required);
        yvex_cli_out_writef(stdout, "role.%llu.imatrix_required: %s\n", i,
               entry->imatrix_required);
        yvex_cli_out_writef(stdout, "role.%llu.artifact_emission_allowed: %s\n", i,
               entry->artifact_emission_allowed);
        yvex_cli_out_writef(stdout, "role.%llu.artifact_emission_blocker: %s\n", i,
               entry->artifact_emission_blocker);
    }
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", profile->top_blocker);
    yvex_cli_out_writef(stdout, "next_required_rows: %s\n", profile->next_required_row);
    yvex_cli_out_writef(stdout, "payload_bytes_read: false\n");
    yvex_cli_out_writef(stdout, "quantization_performed: false\n");
    yvex_cli_out_writef(stdout, "gguf_emitted: false\n");
    yvex_cli_out_writef(stdout, "materialization_status: unsupported\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
...
static void print_qtype_role_support_gate_normal(
    const yvex_qtype_role_support_gate_profile *gate)
{
    yvex_cli_out_writef(stdout, "qtype-role-support-gate: v0.1.0\n");
    yvex_cli_out_writef(stdout, "status: %s\n", gate->status);
    ...
    const yvex_qtype_role_support_gate_profile *gate)
{
    ...
    yvex_cli_out_writef(stdout, "report: qtype-role-support-gate\n");
    yvex_cli_out_writef(stdout, "status: %s\n", gate->status);
    yvex_cli_out_writef(stdout, "gate: v0.1.0\n");
    yvex_cli_out_writef(stdout, "family_count: %llu\n", gate->row_count);
    for (i = 0; i < gate->row_count; ++i) {
        const yvex_qtype_role_support_gate_row *row = &gate->rows[i];
        yvex_cli_out_writef(stdout, "family.%llu.family: %s\n", i, row->family);
        yvex_cli_out_writef(stdout, "family.%llu.target_id: %s\n", i, row->target_id);
        yvex_cli_out_writef(stdout, "family.%llu.status: %s\n", i, row->status);
        yvex_cli_out_writef(stdout, "family.%llu.role_count: %llu\n", i, row->role_count);
        yvex_cli_out_writef(stdout, "family.%llu.blocked_role_count: %llu\n", i,
               row->blocked_role_count);
        yvex_cli_out_writef(stdout, "family.%llu.top_blocker: %s\n", i, row->top_blocker);
        yvex_cli_out_writef(stdout, "family.%llu.next_required_rows: %s\n", i,
               row->next_required_row);
    }
    yvex_cli_out_writef(stdout, "top_blocker: %s\n", gate->top_blocker);
    yvex_cli_out_writef(stdout, "next_required_rows: %s\n", gate->next_required_row);
    ...
    const void *profile)
{
    ...
    yvex_cli_out_writef(stdout, "output-contract: %s\n", output_contract_report_name(report));
    yvex_cli_out_writef(stdout, "target: %s\n", target_id && target_id[0] ? target_id : "none");
    yvex_cli_out_writef(stdout, "mode: %s\n", model_target_output_mode_name(mode));
    yvex_cli_out_writef(stdout, "status: %s\n", status);
    yvex_cli_out_writef(stdout, "render_path: %s\n", output_contract_render_path_name(mode));
    yvex_cli_out_writef(stdout, "line_count: %llu\n", stats.line_count);
    if (mode == YVEX_MODEL_TARGET_OUTPUT_AUDIT) {
        yvex_cli_out_writef(stdout, "line_limit: unbounded\n");
    } else {
        yvex_cli_out_writef(stdout, "line_limit: %llu\n", limit);
    }
    yvex_cli_out_writef(stdout, "audit_prefix_hits: %llu\n", stats.audit_prefix_hits);
    yvex_cli_out_writef(stdout, "detail_dump: %s\n",
           stats.detail_dump_hits > 0 ? "present" : "suppressed");
    yvex_cli_out_writef(stdout, "table_only: %s\n", table_only);
    yvex_cli_out_writef(stdout, "audit_evidence: %s\n", audit_evidence);
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    yvex_cli_out_writef(stdout, "boundary: output-contract check only; no runtime/generation claim\n");
                                         ...
                                         const char *status)
{
    ...
    yvex_cli_out_writef(stdout, "status: %s\n", status);
    yvex_cli_out_writef(stdout, "render_path: not-run\n");
    yvex_cli_out_writef(stdout, "line_count: 0\n");
    yvex_cli_out_writef(stdout, "line_limit: not-run\n");
    yvex_cli_out_writef(stdout, "audit_prefix_hits: 0\n");
    yvex_cli_out_writef(stdout, "detail_dump: not-run\n");
    yvex_cli_out_writef(stdout, "table_only: not-applicable\n");
    yvex_cli_out_writef(stdout, "audit_evidence: not-run\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
                                                  ...
                                                  const yvex_model_target_record *record)
{
    ...
    yvex_cli_out_writef(stdout, "candidate.%lu.id: %s\n", index, record->target_id);
    yvex_cli_out_writef(stdout, "candidate.%lu.class: %s\n", index, target_decision_candidate_class(record));
    yvex_cli_out_writef(stdout, "candidate.%lu.family: %s\n", index, record->family);
    yvex_cli_out_writef(stdout, "candidate.%lu.model: %s\n", index, record->model);
    yvex_cli_out_writef(stdout, "candidate.%lu.source_class: %s\n", index, record->source_artifact_class);
    yvex_cli_out_writef(stdout, "candidate.%lu.artifact_class: %s\n", index, record->target_artifact_class);
    yvex_cli_out_writef(stdout, "candidate.%lu.status: %s\n", index, status);
    yvex_cli_out_writef(stdout, "candidate.%lu.eligible_for_v010: %s\n", index,
           strcmp(status, "eligible") == 0 ? "true" : "false");
    yvex_cli_out_writef(stdout, "candidate.%lu.reason: %s\n", index, target_decision_candidate_reason(record));
    yvex_cli_out_writef(stdout, "candidate.%lu.next: %s\n", index, target_decision_candidate_next(record));
}

static void print_target_decision_constant_tail(void)
{
    yvex_cli_out_writef(stdout, "release_critical_tracks: TRACK.TARGET,TRACK.SOURCE,TRACK.ARTIFACT,TRACK.INTEGRITY,TRACK.MODEL,TRACK.TENSOR,TRACK.RESIDENCY,TRACK.BACKEND,TRACK.GRAPH,TRACK.PREFILL,TRACK.KV,TRACK.DECODE,TRACK.LOGITS,TRACK.SAMPLING,TRACK.TOKENIZER,TRACK.GENERATION,TRACK.OPERATOR,TRACK.CI,TRACK.RELEASE\n");
    yvex_cli_out_writef(stdout, "blocked_tracks: TRACK.TARGET,TRACK.ARTIFACT,TRACK.MODEL,TRACK.TENSOR,TRACK.GRAPH,TRACK.PREFILL,TRACK.KV,TRACK.DECODE,TRACK.LOGITS,TRACK.SAMPLING,TRACK.GENERATION\n");
    yvex_cli_out_writef(stdout, "excluded_tracks: TRACK.SERVE,TRACK.EVAL,TRACK.BENCH,TRACK.SPEC\n");
    yvex_cli_out_writef(stdout, "post010_tracks: TRACK.SERVE,TRACK.BENCH,TRACK.SPEC,TRACK.POST010\n");
    yvex_cli_out_writef(stdout, "blocking_rows: V010.TARGET.2,V010.TARGET.3,V010.TARGET.4,V010.MAP.2,V010.ARTIFACT.EMIT.2,V010.FULLMODEL.6\n");
    yvex_cli_out_writef(stdout, "next_required_rows: V010.TARGET.2\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
                                              ...
                                              int include_next)
{
    ...
    if (!fact) return;
    yvex_cli_out_writef(stdout, "candidate_%lu_id: %s\n", index, fact->id);
    yvex_cli_out_writef(stdout, "candidate_%lu_class: %s\n", index, fact->class_name);
    yvex_cli_out_writef(stdout, "candidate_%lu_stage: %s\n", index, fact->stage);
    yvex_cli_out_writef(stdout, "candidate_%lu_eligibility: %s\n", index, fact->eligibility);
    yvex_cli_out_writef(stdout, "candidate_%lu_artifact_status: %s\n", index, fact->artifact_status);
    yvex_cli_out_writef(stdout, "candidate_%lu_source_status: %s\n", index, fact->source_status);
    yvex_cli_out_writef(stdout, "candidate_%lu_tensor_coverage_status: %s\n", index, fact->tensor_coverage_status);
    yvex_cli_out_writef(stdout, "candidate_%lu_runtime_path_status: %s\n", index, fact->runtime_path_status);
    yvex_cli_out_writef(stdout, "candidate_%lu_generation_status: %s\n", index, fact->generation_status);
    yvex_cli_out_writef(stdout, "candidate_%lu_benchmark_status: %s\n", index, fact->benchmark_status);
    yvex_cli_out_writef(stdout, "candidate_%lu_blocker_count: %u\n", index, fact->blocker_count);
    if (include_blockers) {
        for (i = 0; i < fact->blocker_count; ++i) {
            yvex_cli_out_writef(stdout, "candidate_%lu_blocker_%u: %s\n", index, i, fact->blockers[i]);
        }
    }
    if (include_next) {
        yvex_cli_out_writef(stdout, "candidate_%lu_next_required_rows: %s\n", index, fact->next_required_rows);
    }
}
                                       ...
                                       int include_next)
{
    ...
    yvex_cli_out_writef(stdout, "candidate_%lu_id: %s\n", index, alias);
    yvex_cli_out_writef(stdout, "candidate_%lu_class: registered-alias\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_stage: report-only\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_eligibility: candidate-incomplete\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_artifact_status: registered-artifact-not-inspected\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_source_status: unknown\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_tensor_coverage_status: %s\n", index,
           registered_candidate_tensor_status(entry));
    yvex_cli_out_writef(stdout, "candidate_%lu_runtime_path_status: unsupported\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_generation_status: unsupported-full-model\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_benchmark_status: not-measured\n", index);
    yvex_cli_out_writef(stdout, "candidate_%lu_blocker_count: 8\n", index);
    if (include_blockers) {
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_0: missing-source-inventory\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_1: missing-tensor-map\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_2: missing-required-tensor-coverage\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_3: missing-tokenizer-metadata\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_4: missing-output-head\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_5: missing-real-prefill\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_6: missing-real-decode\n", index);
        yvex_cli_out_writef(stdout, "candidate_%lu_blocker_7: missing-real-logits\n", index);
    }
    if (include_next) {
        yvex_cli_out_writef(stdout, "candidate_%lu_next_required_rows: V010.TARGET.3,V010.MAP.*,V010.FULLMODEL.*\n", index);
    }
}

static int print_model_target_candidate_missing(const char *release, const char *target)
{
    yvex_cli_out_writef(stdout, "model-target: candidate\n");
    yvex_cli_out_writef(stdout, "status: full-runtime-candidate-report-fail\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release && release[0] ? release : "v0.1.0");
    yvex_cli_out_writef(stdout, "target_requested: %s\n", target && target[0] ? target : "none");
    yvex_cli_out_writef(stdout, "decision_state: blocked-no-candidate\n");
    yvex_cli_out_writef(stdout, "selected_target_id: none\n");
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: missing\n");
    yvex_cli_out_writef(stdout, "candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "eligible_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "pressure_target_count: 0\n");
    yvex_cli_out_writef(stdout, "fixture_target_count: 0\n");
    yvex_cli_out_writef(stdout, "global_blocker: no eligible full-runtime candidate\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
...
{
    yvex_cli_out_writef(stdout, "model-target: candidate\n");
    yvex_cli_out_writef(stdout, "status: unsupported-release\n");
                                               ...
                                               int include_next)
{
    ...
    yvex_cli_out_writef(stdout, "model-target: candidate\n");
    yvex_cli_out_writef(stdout, "status: full-runtime-candidate-report\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "decision_state: blocked-no-candidate\n");
    yvex_cli_out_writef(stdout, "selected_target_id: none\n");
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: missing\n");
    yvex_cli_out_writef(stdout, "candidate_count: %lu\n", candidate_count);
    yvex_cli_out_writef(stdout, "eligible_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "pressure_target_count: %lu\n", pressure_count);
    yvex_cli_out_writef(stdout, "fixture_target_count: %lu\n", fixture_count);
    yvex_cli_out_writef(stdout, "registered_alias_count: %lu\n", target_id ? (target_entry ? 1ul : 0ul) : registry_count);
    if (include_pressure_targets) {
        yvex_cli_out_writef(stdout, "deepseek_pressure_status: selected-slice-pressure-only\n");
        yvex_cli_out_writef(stdout, "glm_pressure_status: source-storage-pressure-only\n");
        yvex_cli_out_writef(stdout, "qwen_metal_pressure_status: planned-portability-pressure-only\n");
    }
    yvex_cli_out_writef(stdout, "global_blocker: no eligible full-runtime candidate\n");
    if (include_next) {
        yvex_cli_out_writef(stdout, "next_required_rows: V010.TARGET.3\n");
    }
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
                                               ...
                                               const char *target_id)
{
    ...
    yvex_cli_out_writef(stdout, "report: model-target candidate\n");
    yvex_cli_out_writef(stdout, "status: blocked-no-candidate\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "selected: none\n");
    yvex_cli_out_writef(stdout, "candidates: 0 eligible / %lu known (%lu pressure, %lu fixture)\n",
           candidate_count, pressure_count, fixture_count);
    yvex_cli_out_writef(stdout, "top_blocker: no eligible full-runtime candidate\n");
    yvex_cli_out_writef(stdout, "next: %s\n", YVEX_MODEL_CLASS_NEXT_ROW);
    yvex_cli_out_writef(stdout, "boundary: report-only; generation unsupported; benchmark not measured\n");
    yvex_model_registry_close(registry);
    return 0;
}
...
static void print_dense_candidate_requirements(unsigned long index)
{
    ...
    for (i = 0; i < dense_candidate_required_role_count; ++i) {
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_required_role_%lu: %s\n",
                                       ...
                                       int include_next)
{
    ...
    if (!fact) return;
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_id: %s\n", index, fact->id);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_family: %s\n", index, fact->family);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_class: %s\n", index, fact->class_name);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_stage: %s\n", index, fact->stage);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_eligibility: %s\n", index, fact->eligibility);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_source_status: %s\n", index, fact->source_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_artifact_status: %s\n", index, fact->artifact_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_tensor_map_status: %s\n", index, fact->tensor_map_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_tensor_coverage_status: %s\n", index, fact->tensor_coverage_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_tokenizer_status: %s\n", index, fact->tokenizer_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_output_head_status: %s\n", index, fact->output_head_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_runtime_path_status: %s\n", index, fact->runtime_path_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_generation_status: %s\n", index, fact->generation_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_benchmark_status: %s\n", index, fact->benchmark_status);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_count: %u\n", index, fact->blocker_count);
    if (include_requirements) {
        print_dense_candidate_requirements(index);
    }
    if (include_blockers) {
        for (i = 0; i < fact->blocker_count; ++i) {
            yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_%u: %s\n", index, i, fact->blockers[i]);
        }
    }
    if (include_next) {
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_next_required_rows: %s\n", index, fact->next_required_rows);
    }
}
                                             ...
                                             int include_next)
{
    ...
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_id: %s\n", index, alias);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_family: registered\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_class: registered-alias\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_stage: report-only\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_eligibility: candidate-incomplete\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_source_status: unknown\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_artifact_status: registered-artifact-not-inspected\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_tensor_map_status: missing-tensor-map\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_tensor_coverage_status: %s\n", index,
           registered_candidate_tensor_status(entry));
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_tokenizer_status: unknown\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_output_head_status: unknown\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_runtime_path_status: unsupported\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_generation_status: unsupported-full-model\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_benchmark_status: not-measured\n", index);
    yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_count: 12\n", index);
    if (include_requirements) {
        print_dense_candidate_requirements(index);
    }
    if (include_blockers) {
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_0: missing-dense-source\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_1: missing-dense-artifact\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_2: missing-source-manifest\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_3: missing-native-inventory\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_4: missing-tensor-map\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_5: missing-required-tensor-coverage\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_6: missing-tokenizer-metadata\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_7: missing-output-head\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_8: missing-dense-mlp\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_9: missing-real-prefill\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_10: missing-real-decode\n", index);
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_blocker_11: missing-real-logits\n", index);
    }
    if (include_next) {
        yvex_cli_out_writef(stdout, "dense_candidate_%lu_next_required_rows: V010.TARGET.7,V010.MAP.*\n", index);
    }
}

static int print_model_target_dense_candidate_missing(const char *release, const char *target)
{
    yvex_cli_out_writef(stdout, "model-target: dense-candidate\n");
    yvex_cli_out_writef(stdout, "status: dense-candidate-report-fail\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release && release[0] ? release : "v0.1.0");
    yvex_cli_out_writef(stdout, "target_requested: %s\n", target && target[0] ? target : "none");
    yvex_cli_out_writef(stdout, "decision_state: dense-candidate-missing\n");
    yvex_cli_out_writef(stdout, "selected_dense_candidate_id: none\n");
    yvex_cli_out_writef(stdout, "dense_candidate_status: missing\n");
    yvex_cli_out_writef(stdout, "dense_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "eligible_dense_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "dense_pressure_target_count: 0\n");
    yvex_cli_out_writef(stdout, "fixture_target_count: 0\n");
    yvex_cli_out_writef(stdout, "global_blocker: no eligible dense full-runtime candidate\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
                                                     ...
                                                     int include_next)
{
    ...
    yvex_cli_out_writef(stdout, "model-target: dense-candidate\n");
    yvex_cli_out_writef(stdout, "status: dense-candidate-report\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "decision_state: %s\n", eligible_count ? "dense-candidate-found" : "dense-candidate-missing");
    yvex_cli_out_writef(stdout, "selected_dense_candidate_id: none\n");
    yvex_cli_out_writef(stdout, "dense_candidate_status: %s\n",
           target_id ? dense_candidate_status_for_target(target_fact, target_entry)
                     : (eligible_count ? "candidate-found" : "missing"));
    yvex_cli_out_writef(stdout, "dense_candidate_count: %lu\n", dense_candidate_count);
    yvex_cli_out_writef(stdout, "eligible_dense_candidate_count: %lu\n", eligible_count);
    yvex_cli_out_writef(stdout, "dense_pressure_target_count: %lu\n", dense_pressure_count);
    yvex_cli_out_writef(stdout, "fixture_target_count: %lu\n", fixture_count);
    yvex_cli_out_writef(stdout, "registered_alias_count: %lu\n", target_id ? (target_entry ? 1ul : 0ul) : registry_count);
    yvex_cli_out_writef(stdout, "global_blocker: no eligible dense full-runtime candidate\n");
    if (include_next) {
        yvex_cli_out_writef(stdout, "next_required_rows: V010.TARGET.7\n");
    }
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
                                                     ...
                                                     const char *target_id)
{
    ...
    yvex_cli_out_writef(stdout, "report: model-target dense-candidate\n");
    yvex_cli_out_writef(stdout, "status: %s\n", eligible_count ? "dense-candidate-found" : "dense-candidate-missing");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "selected: none\n");
    yvex_cli_out_writef(stdout, "candidates: %lu eligible / %lu known (%lu dense pressure)\n",
           eligible_count, dense_candidate_count, dense_pressure_count);
    yvex_cli_out_writef(stdout, "top_blocker: no selected dense full-runtime candidate\n");
    yvex_cli_out_writef(stdout, "next: %s\n", YVEX_MODEL_CLASS_NEXT_ROW);
    yvex_cli_out_writef(stdout, "boundary: report-only; generation unsupported; benchmark not measured\n");
    yvex_model_registry_close(registry);
    return 0;
                                       ...
                                       int include_blockers)
{
    ...
    if (!fact) return;
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_id: %s\n", index, fact->id);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_class: %s\n", index, fact->class_name);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_stage: %s\n", index, fact->stage);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_eligibility: %s\n", index, fact->eligibility);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_source_target_status: %s\n", index, fact->source_target_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_source_status: %s\n", index, fact->source_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_artifact_status: %s\n", index, fact->artifact_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_tensor_map_status: %s\n", index, fact->tensor_map_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_backend_status: %s\n", index, fact->backend_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_runtime_status: %s\n", index, fact->runtime_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_generation_status: %s\n", index, fact->generation_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_benchmark_status: %s\n", index, fact->benchmark_status);
    yvex_cli_out_writef(stdout, "qwen_candidate_%lu_blocker_count: %u\n", index, fact->blocker_count);
    if (include_blockers) {
        for (i = 0; i < fact->blocker_count; ++i) {
            yvex_cli_out_writef(stdout, "qwen_candidate_%lu_blocker_%u: %s\n", index, i, fact->blockers[i]);
        }
    }
}
...
{
    yvex_cli_out_writef(stdout, "model-target: qwen-metal\n");
    yvex_cli_out_writef(stdout, "status: qwen-metal-pressure-report-fail\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release && release[0] ? release : "v0.1.0");
    yvex_cli_out_writef(stdout, "target_requested: %s\n", target && target[0] ? target : "none");
    yvex_cli_out_writef(stdout, "lane_id: qwen-metal\n");
    yvex_cli_out_writef(stdout, "target_family: qwen\n");
    yvex_cli_out_writef(stdout, "target_class: source-model-candidate\n");
    yvex_cli_out_writef(stdout, "runtime_shape: dense-or-dense-like-candidate-pending-source-config\n");
    yvex_cli_out_writef(stdout, "hardware_lane: apple-silicon-metal\n");
    yvex_cli_out_writef(stdout, "backend_lane: metal-planned\n");
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: candidate-planned\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
                                                ...
                                                int include_next)
{
    ...
    yvex_cli_out_writef(stdout, "model-target: qwen-metal\n");
    yvex_cli_out_writef(stdout, "status: qwen-metal-pressure-report\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "lane_id: qwen-metal\n");
    yvex_cli_out_writef(stdout, "target_family: qwen\n");
    yvex_cli_out_writef(stdout, "target_class: source-model-candidate\n");
    yvex_cli_out_writef(stdout, "stage: report-only\n");
    yvex_cli_out_writef(stdout, "eligibility: pressure-target-only\n");
    yvex_cli_out_writef(stdout, "candidate_id: qwen3-8b\n");
    yvex_cli_out_writef(stdout, "candidate_stage: source-target-profiled\n");
    yvex_cli_out_writef(stdout, "candidate_eligibility: pressure-target-only\n");
    yvex_cli_out_writef(stdout, "source_target_status: profiled\n");
    yvex_cli_out_writef(stdout, "runtime_shape: dense-or-dense-like-candidate-pending-source-config\n");
    yvex_cli_out_writef(stdout, "hardware_lane: apple-silicon-metal\n");
    yvex_cli_out_writef(stdout, "backend_lane: metal-planned\n");
    yvex_cli_out_writef(stdout, "source_status: missing\n");
    yvex_cli_out_writef(stdout, "artifact_status: missing\n");
    yvex_cli_out_writef(stdout, "metal_backend_status: unsupported\n");
    yvex_cli_out_writef(stdout, "qwen_runtime_status: unsupported\n");
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: candidate-planned\n");
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: unsupported-full-model\n");
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    ...
    if (include_candidates || target_id) {
        if (target_fact) {
            yvex_cli_out_writef(stdout, "qwen_candidate_count: 1\n");
            print_qwen_metal_candidate(0, target_fact, include_blockers);
        } else {
            yvex_cli_out_writef(stdout, "qwen_candidate_count: %lu\n", qwen_metal_candidate_fact_count);
            for (i = 0; i < qwen_metal_candidate_fact_count; ++i) {
    ...
    if (include_hardware) {
        yvex_cli_out_writef(stdout, "hardware_profile_status: planned\n");
        yvex_cli_out_writef(stdout, "machine_profile_required: true\n");
        yvex_cli_out_writef(stdout, "unified_memory_report_required: true\n");
        yvex_cli_out_writef(stdout, "metal_device_report_required: true\n");
    }
    if (include_backend) {
        yvex_cli_out_writef(stdout, "metal_feasibility_status: missing\n");
        yvex_cli_out_writef(stdout, "metal_allocation_status: unsupported\n");
        yvex_cli_out_writef(stdout, "metal_graph_primitive_status: unsupported\n");
        yvex_cli_out_writef(stdout, "cuda_lane_independent: true\n");
    }
    if (include_source) {
        yvex_cli_out_writef(stdout, "source_family: qwen\n");
        yvex_cli_out_writef(stdout, "source_target_status: profiled\n");
        yvex_cli_out_writef(stdout, "source_manifest_status: missing\n");
        yvex_cli_out_writef(stdout, "native_tensor_inventory_status: missing\n");
        yvex_cli_out_writef(stdout, "source_config_status: missing\n");
        yvex_cli_out_writef(stdout, "model_class_profile_status: command-visible\n");
        yvex_cli_out_writef(stdout, "model_class_role_mapping_status: not-implemented\n");
    }
    if (include_blockers) {
        for (i = 0; i < qwen_metal_blocker_count; ++i) {
            yvex_cli_out_writef(stdout, "blocker_%lu: %s\n", i, qwen_metal_blockers[i]);
        }
    }
    if (include_next) {
        yvex_cli_out_writef(stdout, "next_required_rows: %s\n", YVEX_MODEL_CLASS_NEXT_ROW);
                                                ...
                                                const char *target_id)
{
    ...
    yvex_cli_out_writef(stdout, "report: model-target qwen-metal\n");
    yvex_cli_out_writef(stdout, "status: pressure-target-only\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "lane: qwen-metal / apple-silicon-metal\n");
    yvex_cli_out_writef(stdout, "target: qwen3-8b\n");
    yvex_cli_out_writef(stdout, "candidate: source-target-profiled pressure-target-only\n");
    yvex_cli_out_writef(stdout, "source_target: profiled\n");
    yvex_cli_out_writef(stdout, "source: missing\n");
    yvex_cli_out_writef(stdout, "backend: metal unsupported\n");
    yvex_cli_out_writef(stdout, "next: %s\n", YVEX_MODEL_CLASS_NEXT_ROW);
    yvex_cli_out_writef(stdout, "boundary: report-only; generation unsupported; benchmark not measured\n");
    return 0;
}
...
static int print_model_target_decision_unsupported_release(const char *release)
{
    ...
    yvex_cli_out_writef(stdout, "target_decision: %s\n", value);
    yvex_cli_out_writef(stdout, "status: unsupported-release\n");
    yvex_cli_out_writef(stdout, "release: %s\n", value);
    yvex_cli_out_writef(stdout, "decision_state: deferred\n");
    yvex_cli_out_writef(stdout, "selected_target_id: none\n");
    yvex_cli_out_writef(stdout, "selected_target_class: none\n");
    yvex_cli_out_writef(stdout, "selected_family: none\n");
    yvex_cli_out_writef(stdout, "selected_model: none\n");
    yvex_cli_out_writef(stdout, "selected_source_class: none\n");
    yvex_cli_out_writef(stdout, "selected_artifact_class: none\n");
    yvex_cli_out_writef(stdout, "selected_backend_policy: none\n");
    yvex_cli_out_writef(stdout, "selected_reason: unsupported release target decision vocabulary\n");
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: unknown\n");
    yvex_cli_out_writef(stdout, "candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "eligible_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "ineligible_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "selected_runtime_slice_eligible: false\n");
    yvex_cli_out_writef(stdout, "source_only_eligible: false\n");
    yvex_cli_out_writef(stdout, "external_reference_eligible: false\n");
    yvex_cli_out_writef(stdout, "fixture_only_eligible: false\n");
    yvex_cli_out_writef(stdout, "deepseek_pressure_status: unknown\n");
    yvex_cli_out_writef(stdout, "glm_pressure_status: unknown\n");
    yvex_cli_out_writef(stdout, "qwen_metal_pressure_status: unknown\n");
    yvex_cli_out_writef(stdout, "dense_candidate_status: unknown\n");
    yvex_cli_out_writef(stdout, "moe_candidate_status: unknown\n");
    yvex_cli_out_writef(stdout, "selected_candidate_tensor_coverage: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_artifact_status: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_integrity_status: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_backend_status: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_graph_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_prefill_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_kv_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_decode_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_logits_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_sampling_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_generation_status: unsupported\n");
                                                        ...
                                                        const char *candidate)
{
    ...
    yvex_cli_out_writef(stdout, "target_decision: %s\n", release_value);
    yvex_cli_out_writef(stdout, "status: missing-candidate\n");
    yvex_cli_out_writef(stdout, "release: %s\n", release_value);
    yvex_cli_out_writef(stdout, "candidate_requested: %s\n", candidate_value);
    yvex_cli_out_writef(stdout, "decision_state: blocked-no-candidate\n");
    yvex_cli_out_writef(stdout, "selected_target_id: none\n");
    yvex_cli_out_writef(stdout, "selected_target_class: none\n");
    yvex_cli_out_writef(stdout, "selected_family: none\n");
    ...
    yvex_cli_out_writef(stdout, "selected_artifact_class: none\n");
    yvex_cli_out_writef(stdout, "selected_backend_policy: none\n");
    yvex_cli_out_writef(stdout, "selected_reason: requested candidate is not registered\n");
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: missing\n");
    yvex_cli_out_writef(stdout, "candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "eligible_candidate_count: 0\n");
    yvex_cli_out_writef(stdout, "ineligible_candidate_count: 0\n");
    ...
    yvex_cli_out_writef(stdout, "qwen_metal_pressure_status: planned-portability-pressure-only\n");
    yvex_cli_out_writef(stdout, "dense_candidate_status: missing\n");
    yvex_cli_out_writef(stdout, "moe_candidate_status: blocked-missing-tensor-map\n");
    yvex_cli_out_writef(stdout, "selected_candidate_tensor_coverage: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_artifact_status: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_integrity_status: none\n");
    yvex_cli_out_writef(stdout, "selected_candidate_backend_status: none\n");
                                              ...
                                              const yvex_model_target_record *candidate_filter)
{
    ...
    yvex_cli_out_writef(stdout, "selected_target_id: %s\n", selected ? selected->target_id : "none");
    yvex_cli_out_writef(stdout, "selected_target_class: %s\n", selected ? target_decision_candidate_class(selected) : "none");
    yvex_cli_out_writef(stdout, "selected_family: %s\n", selected ? selected->family : "none");
    yvex_cli_out_writef(stdout, "selected_model: %s\n", selected ? selected->model : "none");
    yvex_cli_out_writef(stdout, "selected_source_class: %s\n", selected ? selected->source_artifact_class : "none");
    yvex_cli_out_writef(stdout, "selected_artifact_class: %s\n", selected ? selected->target_artifact_class : "none");
    yvex_cli_out_writef(stdout, "selected_backend_policy: %s\n", selected ? "cpu-cuda-capability-required" : "none");
    yvex_cli_out_writef(stdout, "selected_reason: %s\n", selected
    ...
    yvex_cli_out_writef(stdout, "full_runtime_candidate_status: %s\n", eligible_count ? "present" : "missing");
    yvex_cli_out_writef(stdout, "candidate_count: %lu\n", candidate_count);
    yvex_cli_out_writef(stdout, "eligible_candidate_count: %lu\n", eligible_count);
    yvex_cli_out_writef(stdout, "ineligible_candidate_count: %lu\n", ineligible_count);
    yvex_cli_out_writef(stdout, "selected_runtime_slice_eligible: false\n");
    yvex_cli_out_writef(stdout, "source_only_eligible: false\n");
    yvex_cli_out_writef(stdout, "external_reference_eligible: false\n");
    yvex_cli_out_writef(stdout, "fixture_only_eligible: false\n");
    ...
    yvex_cli_out_writef(stdout, "moe_candidate_status: %s\n", selected ? "target-dependent" : "blocked-missing-tensor-map");
    yvex_cli_out_writef(stdout, "selected_candidate_tensor_coverage: %s\n", selected ? "requires-report" : "none");
    yvex_cli_out_writef(stdout, "selected_candidate_artifact_status: %s\n", selected ? "requires-integrity-gate" : "none");
    yvex_cli_out_writef(stdout, "selected_candidate_integrity_status: %s\n", selected ? "requires-integrity-gate" : "none");
    yvex_cli_out_writef(stdout, "selected_candidate_backend_status: %s\n", selected ? "requires-backend-gate" : "none");
    yvex_cli_out_writef(stdout, "selected_candidate_graph_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_prefill_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_kv_status: unsupported\n");
    yvex_cli_out_writef(stdout, "selected_candidate_decode_status: unsupported\n");
                                              ...
                                              const yvex_model_target_record *candidate_filter)
{
    ...
    yvex_cli_out_writef(stdout, "report: target-decision\n");
    yvex_cli_out_writef(stdout, "status: %s\n", selected ? "target-decision-selected" : "target-decision-blocked");
    yvex_cli_out_writef(stdout, "release: %s\n", release);
    yvex_cli_out_writef(stdout, "selected: %s\n", selected ? selected->target_id : "none");
    yvex_cli_out_writef(stdout, "eligible: %lu / %lu candidates (%lu ineligible)\n",
...
{
    yvex_cli_out_writef(fp, "usage: yvex model-target classes\n");
    yvex_cli_out_writef(fp, "       yvex model-target list [--audit | --output normal|table|audit]\n");
    yvex_cli_out_writef(fp, "       yvex model-target candidate --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target dense-candidate --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target qwen-metal --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target decision --release v0.1.0 [options]\n");
    yvex_cli_out_writef(fp, "       yvex model-target class-profile TARGET [--models-root DIR] [--source DIR] [--audit | --output normal|table|audit]\n");
    yvex_cli_out_writef(fp, "       yvex model-target tensor-collection TARGET [--models-root DIR] [--source DIR] [--audit | --output normal|table|audit]\n");
    yvex_cli_out_writef(fp, "       yvex model-target missing-roles TARGET [--models-root DIR] [--source DIR] [--audit | --output normal|table|audit|json]\n");
    yvex_cli_out_writef(fp, "       yvex model-target tokenizer-map TARGET [--models-root DIR] [--source DIR] [--audit | --output normal|table|audit|json]\n");
    yvex_cli_out_writef(fp, "       yvex model-target tensor-map TARGET [--role output-head|tokenizer|missing-roles | --gate v0.1.0] [--models-root DIR] [--source DIR] [--audit | --output normal|table|audit] [--check-output-contract normal|table|audit]\n");
    yvex_cli_out_writef(fp, "       yvex model-target quant-policy TARGET [--models-root DIR] [--source DIR] [--role-support] [--audit | --output normal|table|audit] [--check-output-contract normal|table|audit]\n");
    yvex_cli_out_writef(fp, "       yvex model-target quant-policy --gate v0.1.0 [--models-root DIR] [--audit | --output normal|table|audit]\n");
    yvex_cli_out_writef(fp, "       yvex model-target inspect TARGET [--paths] [--models-root DIR] [--audit | --output normal|table|audit]\n");
}
...
void yvex_model_target_help(FILE *fp)
{
    ...
    yvex_cli_out_writef(fp, "--models-root DIR override configured operator model root for this command only\n");
    yvex_cli_out_writef(fp, "--audit | --output normal|table|audit\n");
    yvex_cli_out_writef(fp, "\nDecision report:\n");
    yvex_cli_out_writef(fp, "  yvex model-target decision --release v0.1.0 --include-candidates --include-blockers --include-next\n");
    yvex_cli_out_writef(fp, "  This command records the v0.1.0 target decision. It does not download models, emit artifacts, materialize tensors, execute graph work, run prefill, decode, logits, sampling, generation, evaluation, or benchmarks.\n");
    yvex_cli_out_writef(fp, "  Default output is compact. Use --audit or --output audit for full row-promotion fields.\n");
    yvex_cli_out_writef(fp, "\nCandidate report:\n");
    yvex_cli_out_writef(fp, "  yvex model-target candidate --release v0.1.0 --include-candidates --include-blockers --include-next\n");
    yvex_cli_out_writef(fp, "  The candidate report evaluates full-runtime target eligibility for a release. It does not select a ready model, download weights, emit artifacts, materialize tensors, execute runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "  Default output is compact. Use --audit or --output audit for candidate lists, blockers, and next-row evidence.\n");
    yvex_cli_out_writef(fp, "\nDense candidate report:\n");
    yvex_cli_out_writef(fp, "  yvex model-target dense-candidate --release v0.1.0 --include-candidates --include-requirements --include-blockers --include-next\n");
    yvex_cli_out_writef(fp, "  The dense-candidate report evaluates whether a dense model target can become the first v0.1.0 full-runtime candidate. It does not download weights, emit artifacts, materialize tensors, execute graph/runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "  Default output is compact. Use --audit or --output audit for requirements and blocker detail.\n");
    yvex_cli_out_writef(fp, "\nQwen/Metal pressure report:\n");
    yvex_cli_out_writef(fp, "  yvex model-target qwen-metal --release v0.1.0 --include-candidates --include-hardware --include-backend --include-source --include-blockers --include-next\n");
    yvex_cli_out_writef(fp, "  The Qwen/Metal pressure report records a planned reduced-scale Apple Silicon / Metal lane for future full-runtime work. It does not download weights, implement Metal, emit Qwen artifacts, materialize tensors, execute graph/runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "  Default output is compact. Use --audit or --output audit for hardware, backend, source, and blocker detail.\n");
    yvex_cli_out_writef(fp, "\nModel-class profile:\n");
    yvex_cli_out_writef(fp, "  yvex model-target class-profile qwen3-8b --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target class-profile gemma-4-12b-it --audit\n");
    yvex_cli_out_writef(fp, "  The class-profile report reads safetensors headers only and counts lexical tensor-name patterns. It does not map tensor roles, emit artifacts, materialize tensors, execute runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nTensor collection inventory:\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-collection qwen3-8b --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-collection gemma-4-12b-it --audit\n");
    yvex_cli_out_writef(fp, "  The tensor collection inventory reads safetensors headers only and groups lexical tensor candidates for the selected source target. It does not map runtime roles, emit artifacts, materialize tensors, execute runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nTensor naming map:\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map qwen3-8b --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map gemma-4-12b-it --audit\n");
    yvex_cli_out_writef(fp, "  The tensor naming map reads safetensors headers only and assigns native source tensor names to canonical YVEX role labels. It does not complete runtime role coverage, build runtime descriptors, emit artifacts, materialize tensors, feed graph consumers, execute the model, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nOutput-head tensor map:\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map qwen3-8b --role output-head --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map gemma-4-12b-it --role output-head --audit\n");
    yvex_cli_out_writef(fp, "  The output-head tensor map reads safetensors headers only and identifies output-head, final-norm, and embedding candidates. It does not compute logits, complete runtime descriptors, feed graph consumers, execute runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nTokenizer metadata map:\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map qwen3-8b --role tokenizer --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map gemma-4-12b-it --role tokenizer --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tokenizer-map qwen3-6-35b-a3b --models-root ~/lab/models --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tokenizer-map gemma-4-31b-it --models-root ~/lab/models --output table\n");
    yvex_cli_out_writef(fp, "  The tokenizer metadata map reads local sidecars only and reports tokenizer/config/special-token metadata candidates. It does not tokenize, detokenize, apply chat templates, stop on EOS, compute logits, execute runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nMissing-role blocker report:\n");
    yvex_cli_out_writef(fp, "  yvex model-target missing-roles qwen3-8b --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target missing-roles gemma-4-12b-it --audit\n");
    yvex_cli_out_writef(fp, "  The missing-role report aggregates header-derived tensor naming, output-head, tokenizer metadata, and planned artifact facts into the blocker list that prevents full GGUF emission. It does not load tensor payloads, emit GGUF, materialize tensors, execute graph/runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nTensor mapping gate:\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map qwen3-8b --gate v0.1.0 --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target tensor-map gemma-4-12b-it --gate v0.1.0 --audit\n");
    yvex_cli_out_writef(fp, "  The tensor mapping gate aggregates model-class, tensor-collection, tensor naming, output-head, tokenizer metadata, and missing-role reports. It can pass only into artifact/quantization planning; it does not emit artifacts, construct runtime descriptors, execute graph/runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nQtype policy report:\n");
    yvex_cli_out_writef(fp, "  yvex model-target quant-policy qwen3-8b --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target quant-policy gemma-4-12b-it --audit\n");
    yvex_cli_out_writef(fp, "  yvex model-target quant-policy qwen3-6-35b-a3b --role-support --models-root ~/lab/models\n");
    yvex_cli_out_writef(fp, "  yvex model-target quant-policy --gate v0.1.0 --models-root ~/lab/models --output table\n");
    yvex_cli_out_writef(fp, "  The qtype policy report consumes source/header/mapping evidence and existing YVEX qtype policy rows for artifact planning. It does not load tensor payloads, quantize tensors, emit GGUF, materialize tensors, construct runtime descriptors, execute graph/runtime paths, generate, evaluate, benchmark, or mark a release ready.\n");
    yvex_cli_out_writef(fp, "\nDefault output is compact. Use --audit for full diagnostic fields.\n");
    yvex_cli_out_writef(fp, "Model targets are pressure objects, not capability claims.\n");
    yvex_cli_out_writef(fp, "External GGUFs and external runners are reference evidence only.\n");
    yvex_cli_out_writef(fp, "Model-target path reporting does not read model payloads, create artifacts, register aliases, or claim runtime support.\n");
}
...
static void print_model_target_list(void)
{
    ...
    yvex_cli_out_writef(stdout, "status: model-target-list\n");
    for (i = 0; i < model_target_count; ++i) {
        const yvex_model_target_record *record = &model_targets[i];
        yvex_cli_out_writef(stdout, "target: %s\n", record->target_id);
        yvex_cli_out_writef(stdout, "family: %s\n", record->family);
        ...
        yvex_cli_out_writef(stdout, "source_artifact_status: %s\n",
               model_target_source_artifact_status(record));
        yvex_cli_out_writef(stdout, "target_artifact_class: %s\n", record->target_artifact_class);
        yvex_cli_out_writef(stdout, "target_artifact_status: %s\n",
               model_target_target_artifact_status(record));
        yvex_cli_out_writef(stdout, "source_artifact_origin: %s\n",
               model_target_source_artifact_origin(record));
        yvex_cli_out_writef(stdout, "target_artifact_origin: %s\n",
               model_target_target_artifact_origin(record));
        yvex_cli_out_writef(stdout, "source_provenance_status: %s\n",
               model_target_source_provenance_status(record));
        yvex_cli_out_writef(stdout, "source_origin: %s\n", model_target_source_origin(record));
        yvex_cli_out_writef(stdout, "source_authority: %s\n", model_target_source_authority(record));
        yvex_cli_out_writef(stdout, "source_revision_status: %s\n",
               model_target_source_revision_status(record));
        yvex_cli_out_writef(stdout, "source_identity_status: %s\n",
               model_target_source_identity_status(record));
        yvex_cli_out_writef(stdout, "source_hash_status: %s\n", model_target_source_hash_status(record));
        yvex_cli_out_writef(stdout, "source_verification_status: %s\n",
               model_target_source_verification_status(record));
        yvex_cli_out_writef(stdout, "native_inventory_status: %s\n",
               model_target_native_inventory_status(record));
        yvex_cli_out_writef(stdout, "native_tensor_count: 0\n");
        yvex_cli_out_writef(stdout, "native_safetensors_payload_loaded: false\n");
        yvex_cli_out_writef(stdout, "source_tensor_metadata_status: %s\n",
               model_target_source_tensor_metadata_status(record));
        yvex_cli_out_writef(stdout, "source_tensor_count: 0\n");
        yvex_cli_out_writef(stdout, "source_tensor_metadata_payload_loaded: false\n");
        yvex_cli_out_writef(stdout, "source_tensor_metadata_payload_bytes_read: 0\n");
        ...
        yvex_cli_out_writef(stdout, "runtime_shape: %s\n", model_target_runtime_shape(record));
        yvex_cli_out_writef(stdout, "backend_selection: %s\n", model_target_backend_selection(record));
...
static void print_model_target_list_table(void)
{
    ...
    yvex_cli_out_writef(stdout, "MODEL TARGETS  count=%lu\n\n", model_target_count);
    yvex_cli_out_writef(stdout, "%-43s  %-8s  %-40s  %-11s  %s\n",
           "TARGET",
           "FAMILY",
           "CLASS",
           "RUNTIME",
...
{
    yvex_cli_out_writef(stdout, "status: model-target\n");
    yvex_cli_out_writef(stdout, "target_id: %s\n", record->target_id);
    yvex_cli_out_writef(stdout, "family: %s\n", record->family);
    ...
    yvex_cli_out_writef(stdout, "source_artifact_class: %s\n", record->source_artifact_class);
    yvex_cli_out_writef(stdout, "source_artifact_status: %s\n", model_target_source_artifact_status(record));
    yvex_cli_out_writef(stdout, "source_artifact_format: %s\n", model_target_source_artifact_format(record));
    yvex_cli_out_writef(stdout, "source_artifact_origin: %s\n", model_target_source_artifact_origin(record));
    yvex_cli_out_writef(stdout, "source_artifact_authority: %s\n",
           model_target_source_artifact_authority(record));
    yvex_cli_out_writef(stdout, "source_sidecar_status: %s\n", model_target_source_sidecar_status(record));
    yvex_cli_out_writef(stdout, "source_tensor_container: %s\n", model_target_source_tensor_container(record));
    yvex_cli_out_writef(stdout, "source_tensor_payload_status: %s\n",
    ...
    yvex_cli_out_writef(stdout, "target_artifact_status: %s\n", model_target_target_artifact_status(record));
    yvex_cli_out_writef(stdout, "target_artifact_origin: %s\n", model_target_target_artifact_origin(record));
    yvex_cli_out_writef(stdout, "target_artifact_required: %s\n",
           model_target_target_artifact_required(record));
    yvex_cli_out_writef(stdout, "external_reference_status: %s\n",
           model_target_external_reference_status(record));
    yvex_cli_out_writef(stdout, "yvex_produced_artifact_status: %s\n",
           model_target_yvex_produced_artifact_status(record));
    yvex_cli_out_writef(stdout, "pressure_purpose: %s\n", record->pressure_purpose);
    yvex_cli_out_writef(stdout, "tensor_set: %s\n", record->tensor_set);
    yvex_cli_out_writef(stdout, "local_path_class: %s\n", record->local_path_class);
    yvex_cli_out_writef(stdout, "source_footprint_class: %s\n", record->source_footprint_class);
    yvex_cli_out_writef(stdout, "runtime_boundary: %s\n", record->runtime_boundary);
    yvex_cli_out_writef(stdout, "runtime_execution: %s\n", record->runtime_execution);
    yvex_cli_out_writef(stdout, "runtime_claim: unsupported\n");
    yvex_cli_out_writef(stdout, "generation: %s\n", record->generation);
    yvex_cli_out_writef(stdout, "benchmark_status: not-measured\n");
    yvex_cli_out_writef(stdout, "release_ready: false\n");
    yvex_cli_out_writef(stdout, "external_reference: %s\n", record->external_reference);
}
...
static void print_model_target_record_normal(const yvex_model_target_record *record)
{
    if (model_target_is_source_model_candidate(record)) {
        yvex_cli_out_writef(stdout, "target: %s\n", record->target_id);
        yvex_cli_out_writef(stdout, "family: %s  class=%s\n", record->family, record->target_class);
        yvex_cli_out_writef(stdout, "source: %s  status=%s\n",
               record->source_artifact_class,
               model_target_source_artifact_status(record));
        yvex_cli_out_writef(stdout, "artifact: %s  status=%s\n",
               record->target_artifact_class,
               model_target_target_artifact_status(record));
        yvex_cli_out_writef(stdout, "backend_selection: %s\n", model_target_backend_selection(record));
        yvex_cli_out_writef(stdout, "backend_pressure: %s\n", model_target_backend_pressure(record));
        yvex_cli_out_writef(stdout, "runtime: %s\n", record->runtime_execution);
        yvex_cli_out_writef(stdout, "generation: %s\n", record->generation);
        yvex_cli_out_writef(stdout, "next: %s\n", YVEX_MODEL_CLASS_NEXT_ROW);
        yvex_cli_out_writef(stdout, "boundary: target/source profile only; no source download/runtime/generation\n");
        yvex_cli_out_writef(stdout, "status: model-target\n");
        return;
    }
    if (strcmp(record->target_id, "glm-5.2-official-safetensors") == 0) {
        ...
        yvex_cli_out_writef(stdout, "runtime: %s\n", record->runtime_execution);
        yvex_cli_out_writef(stdout, "generation: %s\n", record->generation);
        yvex_cli_out_writef(stdout, "next: V010.SOURCE.8\n");
        yvex_cli_out_writef(stdout, "boundary: source/storage pressure only; no GLM runtime/generation\n");
        yvex_cli_out_writef(stdout, "status: model-target\n");
        return;
    }
    if (model_target_is_selected_slice(record)) {
        ...
        yvex_cli_out_writef(stdout, "generation: %s\n", record->generation);
        yvex_cli_out_writef(stdout, "boundary: selected-slice only; no full-runtime generation\n");
        yvex_cli_out_writef(stdout, "status: model-target\n");
        return;
    }
    ...
    yvex_cli_out_writef(stdout, "artifact: %s  status=%s\n",
           record->target_artifact_class,
           model_target_target_artifact_status(record));
    yvex_cli_out_writef(stdout, "runtime: %s generation=%s\n",
           record->runtime_execution,
           record->generation);
    yvex_cli_out_writef(stdout, "top_blocker: %s\n",
    ...
    yvex_cli_out_writef(stdout, "boundary: target report only, no runtime execution\n");
    yvex_cli_out_writef(stdout, "status: model-target\n");
}
...
{
    yvex_cli_out_writef(stdout, "%-24s  %-8s  %-8s  %8s  %s\n",
           "REPORT",
           "STATUS",
           "SELECTED",
           "ELIGIBLE",
           "NEXT");
    yvex_cli_out_writef(stdout, "%-24s  %-8s  %-8s  %8lu  %s\n",
                                             ...
                                             const char *family, const char *filename)
{
    ...
    if (!out || cap == 0 || !operator_paths || !family || !filename) {
        yvex_cli_out_writef(stderr, "model-target: artifact path fields are required\n");
        return 2;
    }
    n = snprintf(out, cap, "%s/%s/%s", operator_paths->gguf_root, family, filename);
    if (n < 0 || (size_t)n >= cap) {
        yvex_cli_out_writef(stderr, "model-target: artifact path is too long\n");
        return 2;
    }
    return 0;
}
                                    ...
                                    int audit_output)
{
    ...
    if (strcmp(record->family, "DeepSeek") == 0) {
        ...
        yvex_cli_out_writef(stderr, "model-target: no path mapping for family: %s\n", record->family);
        return 2;
    }
    ...
    if (strcmp(record->target_id, "deepseek4-v4-flash-selected-embed") == 0) {
        ...
        yvex_cli_out_writef(stderr, "model-target: no path mapping for target: %s\n", record->target_id);
        return 2;
    }
    ...
    if (!audit_output) {
        if (model_target_is_source_model_candidate(record)) {
            yvex_cli_out_writef(stdout, "target: %s\n", record->target_id);
            yvex_cli_out_writef(stdout, "source: %s  %s\n", source_exists ? "present" : "missing", source_path);
            yvex_cli_out_writef(stdout, "source_class: %s\n", source_class);
            yvex_cli_out_writef(stdout, "artifact: planned  %s\n", artifact_path);
            yvex_cli_out_writef(stdout, "artifact_class: %s\n", target_class);
            yvex_cli_out_writef(stdout, "backend_selection: %s\n", model_target_backend_selection(record));
            yvex_cli_out_writef(stdout, "backend_pressure: %s\n", model_target_backend_pressure(record));
            yvex_cli_out_writef(stdout, "reports: %s\n", report_dir);
            yvex_cli_out_writef(stdout, "registry: %s\n", registry_dir);
            yvex_cli_out_writef(stdout, "boundary: path report only, no runtime execution\n");
            yvex_cli_out_writef(stdout, "status: model-target-paths\n");
            return 0;
        }
        yvex_cli_out_writef(stdout, "target: %s\n", record->target_id);
        yvex_cli_out_writef(stdout, "models_root: %s\n", operator_paths.models_root);
        yvex_cli_out_writef(stdout, "source: %s exists=%s\n", source_path, source_exists ? "true" : "false");
        yvex_cli_out_writef(stdout, "source_class: %s\n", source_class);
        yvex_cli_out_writef(stdout, "artifact: %s exists=%s\n", artifact_path, artifact_exists ? "true" : "false");
        yvex_cli_out_writef(stdout, "artifact_class: %s\n", target_class);
        yvex_cli_out_writef(stdout, "registry_alias: %s\n", registry_alias);
        yvex_cli_out_writef(stdout, "boundary: path report only, no payload read or runtime execution\n");
        yvex_cli_out_writef(stdout, "status: model-target-paths\n");
    } else {
        ...
        yvex_cli_out_writef(stdout, "artifact_exists: %s\n", artifact_exists ? "true" : "false");
        yvex_cli_out_writef(stdout, "report_dir: %s\n", report_dir);
        yvex_cli_out_writef(stdout, "report_dir_exists: %s\n", report_exists ? "true" : "false");
        yvex_cli_out_writef(stdout, "reference_dir: %s\n", reference_dir);
        yvex_cli_out_writef(stdout, "reference_dir_exists: %s\n", reference_exists ? "true" : "false");
        yvex_cli_out_writef(stdout, "registry_dir: %s\n", registry_dir);
        yvex_cli_out_writef(stdout, "registry_dir_exists: %s\n", registry_exists ? "true" : "false");
        yvex_cli_out_writef(stdout, "registry_alias: %s\n", registry_alias);
        yvex_cli_out_writef(stdout, "source_artifact_class: %s\n", source_class);
        yvex_cli_out_writef(stdout, "source_artifact_status: %s\n",
...
int yvex_model_target_command(int argc, char **argv)
{
    ...
    if (strcmp(argv[2], "list") == 0) {
        ...
        for (i = 3; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target list: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target list: unsupported output mode: %s\n", argv[i]);
                    return 2;
                }
            } else {
                yvex_cli_out_writef(stderr, "model-target list: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
    ...
    if (strcmp(argv[2], "candidate") == 0) {
        ...
        for (i = 3; i < argc; ++i) {
            if (strcmp(argv[i], "--help") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target candidate: --release requires VERSION\n");
                    return 2;
                }
                release = argv[++i];
            } else if (strcmp(argv[i], "--target") == 0) {
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target candidate: --target requires TARGET\n");
                    return 2;
                }
                target_id = argv[++i];
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target candidate: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target candidate: unsupported output mode: %s\n", argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--json") == 0) {
                yvex_cli_out_writef(stderr, "model-target candidate: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target candidate: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        if (!release || release[0] == '\0') {
            yvex_cli_out_writef(stderr, "model-target candidate: --release is required\n");
            print_model_target_candidate_usage(stderr);
            return 2;
        }
    ...
    if (strcmp(argv[2], "dense-candidate") == 0) {
        ...
        for (i = 3; i < argc; ++i) {
            if (strcmp(argv[i], "--help") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target dense-candidate: --release requires VERSION\n");
                    return 2;
                }
                release = argv[++i];
            } else if (strcmp(argv[i], "--target") == 0) {
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target dense-candidate: --target requires TARGET\n");
                    return 2;
                }
                target_id = argv[++i];
            } else if (strcmp(argv[i], "--include-candidates") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target dense-candidate: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target dense-candidate: unsupported output mode: %s\n", argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--json") == 0) {
                yvex_cli_out_writef(stderr, "model-target dense-candidate: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target dense-candidate: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        if (!release || release[0] == '\0') {
            yvex_cli_out_writef(stderr, "model-target dense-candidate: --release is required\n");
            print_model_target_dense_candidate_usage(stderr);
            return 2;
        }
        if (strcmp(release, "v0.1.0") != 0) {
    ...
    if (strcmp(argv[2], "qwen-metal") == 0) {
        ...
        for (i = 3; i < argc; ++i) {
            if (strcmp(argv[i], "--help") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target qwen-metal: --release requires VERSION\n");
                    return 2;
                }
                release = argv[++i];
            } else if (strcmp(argv[i], "--target") == 0) {
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target qwen-metal: --target requires TARGET\n");
                    return 2;
                }
                target_id = argv[++i];
            } else if (strcmp(argv[i], "--include-candidates") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target qwen-metal: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target qwen-metal: unsupported output mode: %s\n", argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--json") == 0) {
                yvex_cli_out_writef(stderr, "model-target qwen-metal: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target qwen-metal: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        if (!release || release[0] == '\0') {
            yvex_cli_out_writef(stderr, "model-target qwen-metal: --release is required\n");
            print_model_target_qwen_metal_usage(stderr);
            return 2;
        }
        if (strcmp(release, "v0.1.0") != 0) {
    ...
    if (strcmp(argv[2], "decision") == 0) {
        ...
        for (i = 3; i < argc; ++i) {
            if (strcmp(argv[i], "--help") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target decision: --release requires VERSION\n");
                    return 2;
                }
                release = argv[++i];
            } else if (strcmp(argv[i], "--candidate") == 0) {
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target decision: --candidate requires TARGET\n");
                    return 2;
                }
                candidate_id = argv[++i];
            } else if (strcmp(argv[i], "--include-candidates") == 0 ||
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target decision: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target decision: unsupported output mode: %s\n", argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--json") == 0) {
                yvex_cli_out_writef(stderr, "model-target decision: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target decision: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        if (!release || release[0] == '\0') {
            yvex_cli_out_writef(stderr, "model-target decision: --release is required\n");
            print_model_target_decision_usage(stderr);
            return 2;
        }
        if (strcmp(release, "v0.1.0") != 0) {
    ...
    if (strcmp(argv[2], "class-profile") == 0) {
        ...
        if (argc < 4) {
            yvex_cli_out_writef(stderr, "model-target class-profile: requires TARGET\n");
            return 2;
        }
        target_id = argv[3];
        record = NULL;
        ...
        for (i = 4; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target class-profile: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target class-profile: unsupported output mode: %s\n",
                            argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--models-root") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target class-profile: --models-root requires DIR\n");
                    return 2;
                }
                models_root = argv[++i];
            } else if (strcmp(argv[i], "--source") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target class-profile: --source requires DIR\n");
                    return 2;
                }
                source = argv[++i];
            } else if (strcmp(argv[i], "--json") == 0) {
                yvex_cli_out_writef(stderr, "model-target class-profile: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target class-profile: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        ...
        if (!record || !spec) {
            yvex_cli_out_writef(stderr, "model-target class-profile: unsupported target: %s\n",
                    target_id && target_id[0] ? target_id : "none");
            return 2;
        }
    ...
    if (strcmp(argv[2], "tensor-collection") == 0) {
        ...
        output_mode = YVEX_MODEL_TARGET_OUTPUT_NORMAL;
        if (argc < 4) {
            yvex_cli_out_writef(stderr, "model-target tensor-collection: requires TARGET\n");
            return 2;
        }
        target_id = argv[3];
        record = NULL;
        ...
        for (i = 4; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target tensor-collection: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target tensor-collection: unsupported output mode: %s\n",
                            argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--models-root") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target tensor-collection: --models-root requires DIR\n");
                    return 2;
                }
                models_root = argv[++i];
            } else if (strcmp(argv[i], "--source") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target tensor-collection: --source requires DIR\n");
                    return 2;
                }
                source = argv[++i];
            } else if (strcmp(argv[i], "--json") == 0) {
                yvex_cli_out_writef(stderr, "model-target tensor-collection: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target tensor-collection: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        record = find_model_target(target_id);
        ...
        if (!record || !spec) {
            yvex_cli_out_writef(stderr, "model-target tensor-collection: unsupported target: %s\n",
                    target_id && target_id[0] ? target_id : "none");
            return 2;
        }
    ...
    if (strcmp(argv[2], "missing-roles") == 0) {
        ...
        output_mode = YVEX_MODEL_TARGET_OUTPUT_NORMAL;
        if (argc < 4) {
            yvex_cli_out_writef(stderr, "model-target missing-roles: requires TARGET\n");
            return 2;
        }
        target_id = argv[3];
        record = NULL;
        ...
        for (i = 4; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                const char *value;
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target missing-roles: --output requires normal|table|audit|json\n");
                    return 2;
                }
                value = argv[++i];
                if (strcmp(value, "json") == 0) {
                    ...
                    yvex_cli_out_writef(stderr, "model-target missing-roles: unsupported output mode: %s\n",
                            value);
                    return 2;
                }
            } else if (strcmp(argv[i], "--models-root") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target missing-roles: --models-root requires DIR\n");
                    return 2;
                }
                models_root = argv[++i];
            } else if (strcmp(argv[i], "--source") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target missing-roles: --source requires DIR\n");
                    return 2;
                }
                source = argv[++i];
            } else {
                yvex_cli_out_writef(stderr, "model-target missing-roles: unknown option: %s\n",
                        argv[i]);
                return 2;
            }
        }
        ...
        if (!record || !spec ||
            (strcmp(spec->family_key, "qwen") != 0 &&
             strcmp(spec->family_key, "gemma") != 0)) {
            yvex_cli_out_writef(stderr, "model-target missing-roles: unsupported target: %s\n",
                    target_id && target_id[0] ? target_id : "none");
            return 2;
        }
        rc = yvex_paths_default(&paths, &err);
        if (rc != YVEX_OK ||
            yvex_operator_paths_resolve(&paths, models_root, &operator_paths,
                                        &err) != YVEX_OK) {
            yvex_cli_out_writef(stderr, "model-target missing-roles: cannot resolve operator paths: %s\n",
    ...
    if (strcmp(argv[2], "tokenizer-map") == 0) {
        ...
        output_mode = YVEX_MODEL_TARGET_OUTPUT_NORMAL;
        if (argc < 4) {
            yvex_cli_out_writef(stderr, "model-target tokenizer-map: requires TARGET\n");
            return 2;
        }
        target_id = argv[3];
        record = NULL;
        ...
        for (i = 4; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                yvex_cli_out_writef(stderr, "model-target tokenizer-map: unknown option: %s\n",
                        argv[i]);
                return 2;
            }
        }
        ...
        if (!record || !spec ||
            (strcmp(spec->family_key, "qwen") != 0 &&
             strcmp(spec->family_key, "gemma") != 0)) {
            yvex_cli_out_writef(stderr, "model-target tokenizer-map: unsupported target: %s\n",
                    target_id && target_id[0] ? target_id : "none");
            return 2;
        }
    ...
    if (strcmp(argv[2], "tensor-map") == 0) {
        ...
        output_mode = YVEX_MODEL_TARGET_OUTPUT_NORMAL;
        if (argc < 4) {
            yvex_cli_out_writef(stderr, "model-target tensor-map: requires TARGET\n");
            return 2;
        }
        target_id = argv[3];
        record = NULL;
        ...
        for (i = 4; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: unsupported output mode: %s\n",
                            argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--models-root") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: --models-root requires DIR\n");
                    return 2;
                }
                models_root = argv[++i];
            } else if (strcmp(argv[i], "--source") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: --source requires DIR\n");
                    return 2;
                }
                source = argv[++i];
            } else if (strcmp(argv[i], "--role") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: --role requires output-head|tokenizer|missing-roles\n");
                    return 2;
                }
                role = argv[++i];
                if (strcmp(role, "output-head") != 0 &&
                    ...
                    yvex_cli_out_writef(stderr, "model-target tensor-map: unsupported role: %s\n",
                            role);
                    return 2;
                }
            } else if (strcmp(argv[i], "--gate") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: --gate requires v0.1.0\n");
                    return 2;
                }
                gate = argv[++i];
                if (strcmp(gate, "v0.1.0") != 0) {
                    yvex_cli_out_writef(stderr, "model-target tensor-map: unsupported release: %s\n",
                            gate);
                    return 2;
                }
                ...
                yvex_cli_out_writef(stderr, "model-target tensor-map: JSON output is unsupported; use --output normal|table|audit\n");
                return 2;
            } else {
                yvex_cli_out_writef(stderr, "model-target tensor-map: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        if (role && gate) {
            yvex_cli_out_writef(stderr, "model-target tensor-map: --gate cannot be combined with --role\n");
            return 2;
        }
        record = find_model_target(target_id);
        spec = find_model_class_profile_spec(target_id);
        ...
        if (!record || !spec ||
            ...
            yvex_cli_out_writef(stderr, "model-target tensor-map: unsupported target: %s\n",
                    target_id && target_id[0] ? target_id : "none");
            return 2;
        }
        if (gate) {
    ...
    if (strcmp(argv[2], "quant-policy") == 0) {
        ...
        for (i = start_arg; i < argc; ++i) {
            if (strcmp(argv[i], "--audit") == 0) {
                ...
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: --gate requires v0.1.0\n");
                    return 2;
                }
                gate = argv[++i];
                if (strcmp(gate, "v0.1.0") != 0) {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: unsupported gate: %s\n",
                            gate);
                    return 2;
                }
            } else if (strcmp(argv[i], "--output") == 0) {
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: --output requires normal|table|audit\n");
                    return 2;
                }
                ++i;
                if (strcmp(argv[i], "json") == 0) {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: JSON output is unsupported; use --output normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: unsupported output mode: %s\n",
                            argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--models-root") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: --models-root requires DIR\n");
                    return 2;
                }
                models_root = argv[++i];
            } else if (strcmp(argv[i], "--source") == 0) {
                if (i + 1 >= argc || argv[i + 1][0] == '\0') {
                    yvex_cli_out_writef(stderr, "model-target quant-policy: --source requires DIR\n");
                    return 2;
                }
                source = argv[++i];
            } else if (strcmp(argv[i], "--check-output-contract") == 0) {
                ...
                yvex_cli_out_writef(stderr, "model-target quant-policy: unknown option: %s\n", argv[i]);
                return 2;
            }
        }
        if (gate) {
            if (target_id) {
                yvex_cli_out_writef(stderr, "model-target quant-policy: --gate v0.1.0 is an aggregate report and does not take TARGET\n");
                return 2;
            }
            if (check_output_contract) {
                yvex_cli_out_writef(stderr, "model-target quant-policy: --check-output-contract is unsupported for --gate\n");
                return 2;
            }
        ...
        if (!target_id) {
            yvex_cli_out_writef(stderr, "model-target quant-policy: requires TARGET\n");
            return 2;
        }
        memset(&dynamic_target, 0, sizeof(dynamic_target));
        record = find_model_target(target_id);
        ...
        if (role_support) {
            if (check_output_contract) {
                yvex_cli_out_writef(stderr, "model-target quant-policy: --check-output-contract is unsupported for --role-support\n");
                return 2;
            }
    ...
    if (strcmp(argv[2], "inspect") == 0) {
        ...
        if (!record) {
            yvex_cli_out_writef(stderr, "model-target: unknown target: %s\n", argv[3]);
            return 2;
        }
        for (i = 4; i < argc; ++i) {
            if (strcmp(argv[i], "--paths") == 0) {
                ...
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target inspect: --output requires normal|table|audit\n");
                    return 2;
                }
                if (!parse_model_target_output_mode(argv[++i], &output_mode)) {
                    yvex_cli_out_writef(stderr, "model-target inspect: unsupported output mode: %s\n", argv[i]);
                    return 2;
                }
            } else if (strcmp(argv[i], "--models-root") == 0) {
                if (i + 1 >= argc) {
                    yvex_cli_out_writef(stderr, "model-target: --models-root requires DIR\n");
                    return 2;
                }
                models_root = argv[++i];
            } else {
                yvex_cli_out_writef(stderr, "model-target: unknown inspect option: %s\n", argv[i]);
                return 2;
            }
        }
        if (models_root && !want_paths) {
            yvex_cli_out_writef(stderr, "model-target: --models-root requires --paths\n");
            return 2;
        }
        if (want_paths) {
            if (output_mode == YVEX_MODEL_TARGET_OUTPUT_AUDIT) {
    ...
    yvex_cli_out_writef(stderr, "model-target: unknown subcommand: %s\n", argv[2]);
    yvex_cli_out_writef(stderr, "Try 'yvex help model-target' for usage.\n");
    return 2;
}
