...
static void sample_print_summary(const yvex_sampling_summary *summary,
                                 ...
                                 const char *status)
{
    yvex_cli_out_writef(stdout, "sample: token\n");
    yvex_cli_out_writef(stdout, "status: sample-token\n");
    ...
    yvex_cli_out_writef(stdout, "logits_invoked: %s\n", summary && summary->logits_invoked ? "true" : "false");
    ...
    yvex_cli_out_writef(stdout, "selected_logit: %.9g\n", summary ? summary->selected_logit : 0.0);
    yvex_cli_out_writef(stdout, "sample_checksum: %llu\n", summary ? summary->sample_checksum : 0ull);
    ...
    yvex_cli_out_writef(stdout, "bounded_sampling_ready: %s\n",
           summary && summary->bounded_sampling_ready ? "true" : "false");
    yvex_cli_out_writef(stdout, "real_vocab_sampling: false\n");
    yvex_cli_out_writef(stdout, "real_model_sampling: false\n");
...
int yvex_sample_command(int argc, char **argv)
{
    ...
    for (i = 2; i < argc; ++i) {
        if (strcmp(argv[i], "--model") == 0) {
            ...
            if (i + 1 >= argc || !sample_parse_strategy(argv[i + 1], &strategy)) {
                yvex_cli_out_writef(stderr, "yvex: --strategy supports only greedy\n");
                return 2;
            }
            i += 1;
            ...
            yvex_cli_out_writef(stderr, "yvex: unknown sample option: %s\n", argv[i]);
            yvex_cli_out_writef(stderr, "Try 'yvex help sample' for usage.\n");
            return 2;
        }
    }

    if (!model_arg || !backend_name || !tokens_text || !segment_name) {
        yvex_cli_out_writef(stderr, "usage: yvex sample --model FILE_OR_ALIAS --backend cpu|cuda --segment embedding-rmsnorm --tokens IDS [--layers N [--layer-hidden-dim N --layer-head-dim N --layer-ffn-dim N]] [--chunk-size N] [--position-start N] [--context-length N] [--attach-kv --kv-layers N --kv-heads N --kv-head-dim N --kv-capacity N] [--logits-count N] [--strategy greedy]\n");
        return 2;
    }
    ...
    if (strcmp(segment_name, "embedding-rmsnorm") != 0) {
        yvex_cli_out_writef(stderr, "yvex: unsupported sample segment: %s\n", segment_name);
        return 2;
    }
