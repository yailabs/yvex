...
int yvex_paths_print(const yvex_paths *paths, FILE *fp, yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "config: %s\n", paths->config_dir);
    yvex_cli_out_writef(fp, "cache: %s\n", paths->cache_dir);
    yvex_cli_out_writef(fp, "state: %s\n", paths->state_dir);
    yvex_cli_out_writef(fp, "data: %s\n", paths->data_dir);
    yvex_cli_out_writef(fp, "project: %s\n", paths->project_dir);
                                  ...
                                  yvex_error *err)
{
    ...
    if (yvex_cli_out_writef(fp, "models_root=%s\n", out->models_root) < 0) {
        yvex_error_setf(err, YVEX_ERR_IO, "operator_paths", "cannot write %s: %s",
                        out->config_path, strerror(errno));
        fclose(fp);
        return YVEX_ERR_IO;
                              ...
                              yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "models_root_source: %s\n", operator_paths->models_root_source);
    yvex_cli_out_writef(fp, "models_root: %s\n", operator_paths->models_root);
    yvex_cli_out_writef(fp, "hf_root: %s\n", operator_paths->hf_root);
    yvex_cli_out_writef(fp, "gguf_root: %s\n", operator_paths->gguf_root);
    yvex_cli_out_writef(fp, "reports_root: %s\n", operator_paths->reports_root);
    yvex_cli_out_writef(fp, "reference_root: %s\n", operator_paths->reference_root);
    yvex_cli_out_writef(fp, "registry_root: %s\n", operator_paths->registry_root);
    yvex_cli_out_writef(fp, "operator_config_path: %s\n", operator_paths->config_path);
    if (include_created) {
        yvex_cli_out_writef(fp, "created: %s\n", created ? "true" : "false");
    }
    yvex_error_clear(err);
    return YVEX_OK;
}
...
int yvex_run_dir_print(const yvex_run_dir *run, FILE *fp, yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "root: %s\n", run->root);
    yvex_cli_out_writef(fp, "command: %s\n", run->command_path);
    yvex_cli_out_writef(fp, "stdout: %s\n", run->stdout_path);
    yvex_cli_out_writef(fp, "stderr: %s\n", run->stderr_path);
    yvex_cli_out_writef(fp, "metrics: %s\n", run->metrics_path);
    yvex_cli_out_writef(fp, "trace: %s\n", run->trace_path);
    yvex_cli_out_writef(fp, "receipt: %s\n", run->receipt_path);
