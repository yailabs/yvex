/*
 * decode.c - decode diagnostic command surface.
 *
                                 ...
                                 const char *status)
{
    ...
    yvex_cli_out_writef(stdout, "decode: step\n");
    yvex_cli_out_writef(stdout, "status: decode-step\n");
    ...
    yvex_cli_out_writef(stdout, "real_model_decode: false\n");
    yvex_cli_out_writef(stdout, "full_model_decode_ready: false\n");
...
int yvex_decode_command(int arg_count, char **args)
{
    ...
    if (!model_arg || !backend_name || !tokens_text || !segment_name) {
        yvex_cli_out_writef(stderr, "usage: " "yvex decode --model FILE_OR_ALIAS --backend cpu|cuda --segment embedding-rmsnorm --tokens IDS [--layers N [--layer-hidden-dim N --layer-head-dim N --layer-ffn-dim N]] [--chunk-size N] [--position-start N] [--context-length N] [--attach-kv --kv-layers N --kv-heads N --kv-head-dim N --kv-capacity N]\n");
        return 2;
    }
...
{
    yvex_cli_out_writef(fp, "usage: " "yvex decode --model FILE_OR_ALIAS --backend cpu|cuda --segment embedding-rmsnorm --tokens IDS [--layers N [--layer-hidden-dim N --layer-head-dim N --layer-ffn-dim N]] [--chunk-size N] [--position-start N] [--context-length N] [--attach-kv --kv-layers N --kv-heads N --kv-head-dim N --kv-capacity N]\n\nDecode creates one bounded diagnostic decode-state step from implemented prefill/KV state. It does not produce logits, sample, generate, or claim full DeepSeek decode.\n");
}
