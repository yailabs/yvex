...
static void ij_write_string(FILE *fp, const char *s)
{
    ...
    while (*p) {
        ...
        else if (ch == '\r') yvex_cli_out_fputs("\\r", fp);
        else if (ch == '\t') yvex_cli_out_fputs("\\t", fp);
        else if (ch < 32) yvex_cli_out_writef(fp, "\\u%04x", (unsigned int)ch);
                                          ...
                                          yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "  \"schema\": \"yvex.imatrix_manifest.v1\",\n");
    yvex_cli_out_writef(fp, "  \"name\": "); ij_write_string(fp, manifest->name); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"architecture\": "); ij_write_string(fp, manifest->architecture); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"status\": "); ij_write_string(fp, yvex_imatrix_status_name(manifest->declared_status)); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"format\": "); ij_write_string(fp, yvex_imatrix_format_name(manifest->format)); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"source_manifest\": "); ij_write_string(fp, manifest->source_manifest_path); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"quant_policy\": "); ij_write_string(fp, manifest->quant_policy_path); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"imatrix\": {\n");
    yvex_cli_out_writef(fp, "    \"path\": "); ij_write_string(fp, manifest->imatrix_path); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "    \"external\": true\n");
    yvex_cli_out_writef(fp, "  },\n");
    yvex_cli_out_writef(fp, "  \"calibration\": {\n");
    yvex_cli_out_writef(fp, "    \"dataset\": "); ij_write_string(fp, manifest->calibration_dataset); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "    \"command\": "); ij_write_string(fp, manifest->calibration_command); yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "    \"producer\": "); ij_write_string(fp, manifest->producer); yvex_cli_out_writef(fp, "\n");
    yvex_cli_out_writef(fp, "  },\n");
    yvex_cli_out_writef(fp, "  \"coverage\": [\n");
    for (i = 0; i < manifest->coverage_count; ++i) {
        ...
        yvex_cli_out_writef(fp, ", \"selector\": ");
        ij_write_string(fp, row->selector);
        yvex_cli_out_writef(fp, ", \"purpose\": ");
                                      ...
                                      yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "{\n");
    yvex_cli_out_writef(fp, "  \"schema\": \"yvex.quant_policy.v1\",\n");
    yvex_cli_out_writef(fp, "  \"name\": ");
                                     ...
                                     const char *path)
{
    ...
    if (path) yvex_cli_out_writef(stdout, "policy: %s\n", path);
