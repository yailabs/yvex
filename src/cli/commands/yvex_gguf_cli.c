                                      ...
                                      const char *template_path)
{
    ...
    yvex_cli_out_writef(stdout, "gguf template: %s\n", mode);
...
static int command_source_manifest(int argc, char **argv)
{
    ...
    if (strcmp(argv[2], "inspect") == 0) {
        yvex_cli_out_writef(stderr, "yvex: source-manifest inspect is not implemented in open-weight intake\n");
        return 5;
    }
    if (strcmp(argv[2], "create") != 0) {
    ...
    if (!options.repo || !options.revision || !options.local_path || !out_path) {
        yvex_cli_out_writef(stderr, "yvex: --hf-repo, --revision, --local-path, and --out are required\n");
        return 2;
    }
