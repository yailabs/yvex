                            ...
                            yvex_error *err)
{
    ...
    yvex_cli_out_writef(fp, "{\n");
    yvex_cli_out_writef(fp, "  \"schema\": \"yvex.metrics.v1\",\n");
    yvex_cli_out_writef(fp, "  \"status\": \"accepted-only\",\n");
    write_counters(fp, &counters);
    yvex_cli_out_writef(fp, ",\n");
    yvex_cli_out_writef(fp, "  \"phases\": [\n");
    for (i = 0; i <= (unsigned long)YVEX_METRIC_PHASE_TOTAL; ++i) {
        ...
        yvex_cli_out_writef(fp, ", \"count\": %llu, \"total_ns\": %llu, \"last_ns\": %llu, \"min_ns\": %llu, \"max_ns\": %llu}",
                                     ...
                                     yvex_error *err)
{
    ...
    for (i = 0; i < argc; ++i) {
        ...
        yvex_cli_out_fputs(argv[i] ? argv[i] : "", fp);
    }
    fputc('\n', fp);
    fclose(fp);
                    ...
                    yvex_error *err)
{
    ...
    trace->seq += 1u;
    yvex_cli_out_writef(trace->fp, "{");
    yvex_cli_out_writef(trace->fp, "\"schema\": \"yvex.trace.v1\", ");
    yvex_cli_out_writef(trace->fp, "\"run_id\": ");
    yvex_json_write_string(trace->fp, trace->run_id);
    yvex_cli_out_writef(trace->fp, ", \"seq\": %llu, ", trace->seq);
    yvex_cli_out_writef(trace->fp, "\"event\": ");
    yvex_json_write_string(trace->fp, yvex_trace_event_kind_name(kind));
    yvex_cli_out_writef(trace->fp, ", \"name\": ");
    yvex_json_write_string(trace->fp, name ? name : "");
    yvex_cli_out_writef(trace->fp, ", \"status\": ");
    yvex_json_write_string(trace->fp, status ? status : "");
    yvex_cli_out_writef(trace->fp, ", \"message\": ");
    yvex_json_write_string(trace->fp, message ? message : "");
    yvex_cli_out_writef(trace->fp, ", \"ts_ns\": %llu", yvex_time_monotonic_ns());
    yvex_cli_out_writef(trace->fp, "}\n");
    fflush(trace->fp);
