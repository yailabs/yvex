...
void yvex_weight_mapping_print_shape(const unsigned long long *dims, unsigned int rank)
{
    ...
    if (!dims || rank == 0) {
        yvex_cli_out_writef(stdout, "unknown");
        return;
    }
    yvex_cli_out_writef(stdout, "[");
    for (i = 0; i < rank; ++i) {
        if (i) yvex_cli_out_writef(stdout, ",");
