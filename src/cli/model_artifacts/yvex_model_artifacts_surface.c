/*
 * yvex_model_artifacts_surface.c - historical model-artifacts CLI surfaces.
 *
 * Owner:
 *   src/cli/model_artifacts
 *
 * Owns:
 *   existing models/fullmodel/attention/context/moe/tensor-collection command
 *   surface behavior while model artifact command ownership is decomposed.
 *
 * Does not own:
 *   registry storage, model reference ownership, model gate algorithms,
 *   public command adapter symbols, artifact emission, runtime generation,
 *   eval, benchmark, or release decisions.
 *
 * Invariants:
 *   existing command syntax and output behavior remain stable during topology
 *   extraction. This file is CLI-only and must not enter libyvex.a.
 *
 * Boundary:
 *   model artifact command reports do not claim quantization, artifact
 *   emission, runtime generation, benchmark evidence, or release readiness.
 */
...
static char *model_artifacts_cli_strdup(const char *s)
{
    size_t len;
    char *out;
                                           ...
                                           int selected)
{
    ...
    yvex_cli_out_writef(stdout, "  producer: %s\n", entry->producer ? entry->producer : "");
    yvex_cli_out_writef(stdout, "  schema_version: %s\n", entry->schema_version ? entry->schema_version : "");
...
{
    if (!entry) return;
    yvex_cli_out_writef(stdout, "%-3s  %-44s  %-10s  %-16s  %7llu  %12llu  %s\n",
           selected ? "*" : "-",
