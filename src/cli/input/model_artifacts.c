...
static const char *const literal_lines_0[] = {
    "yvex: fullmodel requires report, materialization-plan, materialize, descriptor, or family-runtime",
    "usage: yvex fullmodel materialization-plan --model FILE_OR_ALIAS [--backend cpu|cuda] [--residency "
        "resident|host-staged|ssd-staged|hybrid] [--limit-tensors N]",
    "usage: yvex fullmodel materialize --model FILE_OR_ALIAS [--backend cpu|cuda] [--dry-run] [--plan-"
        "only] [--limit-bytes N]",
    "usage: yvex fullmodel descriptor --model FILE_OR_ALIAS [--backend cpu|cuda] [--target TARGET] [--"
        "format text] [--limit-tensors N]",
    "usage: yvex fullmodel family-runtime --model FILE_OR_ALIAS [--family auto|deepseek|glm|qwen] [--backend cpu|cuda]"
};

static const char *const literal_lines_1[] = {
    "usage: yvex fullmodel report --model FILE_OR_ALIAS [--backend cpu|cuda] [--target TARGET] [--limit-tensors N]",
    "usage: yvex fullmodel materialization-plan --model FILE_OR_ALIAS [--backend cpu|cuda] [--residency "
        "resident|host-staged|ssd-staged|hybrid] [--limit-tensors N]",
    "usage: yvex fullmodel materialize --model FILE_OR_ALIAS [--backend cpu|cuda] [--dry-run] [--plan-"
        "only] [--limit-bytes N]",
...
static const yvex_models_option_spec download_bound_options[] = {
    ...
    {"--revision", YVEX_MODELS_OPTION_TEXT,
    ...
    {"--token-env", YVEX_MODELS_OPTION_TEXT,
     offsetof(yvex_cli_models_download_options, token_env)},
    ...
    {"--logs", YVEX_MODELS_OPTION_FLAG,
     offsetof(yvex_cli_models_download_options, cleanup_logs)},
...
static const yvex_models_option_spec fullmodel_bound_options[] = {
    ...
    {"--registry", YVEX_MODELS_OPTION_TEXT,
     offsetof(yvex_cli_fullmodel_options, registry_path)},
    {"--family", YVEX_MODELS_OPTION_TEXT, offsetof(yvex_cli_fullmodel_options, family)},
    {"--residency", YVEX_MODELS_OPTION_TEXT,
     offsetof(yvex_cli_fullmodel_options, residency)},
    {"--require-role", YVEX_MODELS_OPTION_TEXT,
     offsetof(yvex_cli_fullmodel_options, require_role)},
    ...
    {"--report-dir", YVEX_MODELS_OPTION_TEXT,
     offsetof(yvex_cli_fullmodel_options, report_dir)},
    ...
    {"--include-kv", YVEX_MODELS_OPTION_FLAG,
     offsetof(yvex_cli_fullmodel_options, include_kv)},
    {"--include-logits", YVEX_MODELS_OPTION_FLAG,
     offsetof(yvex_cli_fullmodel_options, include_logits)},
    {"--include-moe", YVEX_MODELS_OPTION_FLAG,
     offsetof(yvex_cli_fullmodel_options, include_moe)},
    {"--include-output", YVEX_MODELS_OPTION_FLAG,
     offsetof(yvex_cli_fullmodel_options, include_output)},
    {"--output", YVEX_MODELS_OPTION_OUTPUT,
     offsetof(yvex_cli_fullmodel_options, output_mode)},
};
...
static int model_download_options_validate(yvex_cli_models_download_options *options)
{
    if (!options->target && !options->repo) {
        yvex_cli_out_writef(stderr,
                            "yvex: models download requires TARGET or --repo OWNER/NAME\n");
        return 2;
    }
    if (options->target && options->repo) {
        yvex_cli_out_writef(stderr,
    ...
    if (options->repo && !model_download_repo_valid(options->repo)) {
        yvex_cli_out_writef(stderr,
                            "yvex: models download --repo requires OWNER/NAME\n");
        return 2;
    }
    ...
    if (strcmp(options->provider, "github") != 0 && options->repo &&
        ...
        yvex_cli_out_writef(stderr,
                            "yvex: models download --repo requires --family deepseek|glm|qwen|gemma\n");
        return 2;
    }
    if (options->repo && !options->name) {
        options->name = options->asset_name
                            ? options->asset_name
                            : model_download_repo_basename(options->repo);
    }
    if (options->repo && !model_download_local_name_valid(options->name)) {
        yvex_cli_out_writef(stderr,
                            "yvex: models download --name is required and must be a local model name\n");
        return 2;
    }
    return 0;
}
                                              ...
                                              yvex_cli_models_download_options *options)
{
    ...
    for (i = start_index; i < arg_count; ++i) {
        ...
        int handled = 0;
        int rc = parse_models_bound_option("models download", arg_count, args, &i,
                                           options, download_bound_options,
                                           sizeof(download_bound_options) /
                                               sizeof(download_bound_options[0]),
                                           &handled);
        if (rc != 0) return rc;
        if (handled) continue;
        ...
        if (strcmp(args[i], "--source") == 0 ||
            ...
            rc = parse_models_value_option("models download", flag,
 ...
 * Failure: Typed refusal; outputs remain defined.
 * Boundary: No capability policy. */
static int fullmodel_options_begin(int arg_count,
                                   char **args,
                                   yvex_cli_fullmodel_options *options)
{
    ...
    if (arg_count >= 3 &&
        (strcmp(args[2], "--help") == 0 || strcmp(args[2], "-h") == 0)) {
        yvex_model_artifacts_surface_fullmodel_help(stdout);
        return 1;
    }
    if (arg_count < 3) {
        yvex_cli_out_lines(stderr, literal_lines_0, sizeof(literal_lines_0) / sizeof(literal_lines_0[0]));
        return 2;
    }
    if (strcmp(args[2], "report") == 0) {
        ...
        yvex_cli_out_lines(stderr, literal_lines_1, sizeof(literal_lines_1) / sizeof(literal_lines_1[0]));
        return 2;
    }
    return 0;
}
 ...
 * Failure: Typed refusal; outputs remain defined.
 * Boundary: No capability policy. */
int model_artifacts_fullmodel_options_parse(int arg_count, char **args, yvex_cli_fullmodel_options *options)
{
    int i;
    int begin_rc;
    ...
    for (i = 3; i < arg_count; ++i) {
        ...
        const char *flag = args[i];
        int handled = 0;
        int rc = parse_models_bound_option("fullmodel", arg_count, args, &i,
                                           options, fullmodel_bound_options,
                                           sizeof(fullmodel_bound_options) /
                                               sizeof(fullmodel_bound_options[0]),
                                           &handled);
        if (rc != 0) return rc;
        if (handled) {
            value = i > 3 ? args[i] : NULL;
            if (strcmp(flag, "--backend") == 0 &&
                strcmp(options->backend, "cpu") != 0 && strcmp(options->backend, "cuda") != 0) {
            ...
            if (strcmp(flag, "--residency") == 0 &&
                strcmp(options->residency, "resident") != 0 &&
                ...
                yvex_cli_out_writef(stderr,
                    "yvex: fullmodel --residency must be resident, host-staged, ssd-staged, hybrid, ssd-"
            ...
            if ((strcmp(flag, "--dry-run") == 0 || strcmp(flag, "--plan-only") == 0 ||
                 strcmp(flag, "--report-dir") == 0 ||
                 strcmp(flag, "--fail-after-phase") == 0) &&
            ...
            if ((strcmp(flag, "--include-blockers") == 0 ||
                 strcmp(flag, "--include-placement") == 0 ||
                 strcmp(flag, "--include-graph") == 0 ||
                 strcmp(flag, "--include-kv") == 0 ||
                 strcmp(flag, "--include-logits") == 0) &&
            ...
            if ((strcmp(flag, "--include-roles") == 0 ||
                 strcmp(flag, "--include-moe") == 0 ||
        ...
        if (strcmp(flag, "--limit-tensors") == 0) {
            ...
            if (!fullmodel_command_is_materialize(options)) {
                yvex_cli_out_writef(stderr,
                                    "yvex: fullmodel --limit-bytes is only valid with materialize\n");
                return 2;
            }
            if (!parse_positive_ull(value, &parsed) || parsed == 0ull) {
                yvex_cli_out_writef(stderr,
