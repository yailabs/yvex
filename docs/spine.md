...
Main v0.1.0 blocker:
  native safetensors inventory remains missing
```
...
| --- | --- | --- | --- | --- | --- |
| official source tensors | upstream source authority | report-only | partial | target records, source manifests, source artifact class fields, source shard count/footprint reports, and source provenance fields | native safetensors inventory |
...
Main blockers:
  ...
  manifest, native safetensors inventory, model class, tensor map, artifact,
  Metal backend, residency, graph/runtime, eval, and benchmark
...
Note:
  ...
  fields; native safetensors inventory remains next.
...
| --- | --- | --- | --- | --- | --- |
| 1 | TRACK.TARGET | GATE.SCOPE | complete / source-target-profiled | `yvex model-target decision --release v0.1.0`; `yvex model-target candidate --release v0.1.0`; `yvex model-target dense-candidate --release v0.1.0`; `yvex model-target qwen-metal --release v0.1.0`; `yvex model-target inspect qwen-metal-portability`; `yvex model-target inspect gemma-dense-portability`; `yvex source-manifest report --family qwen --release v0.1.0`; `yvex source-manifest report --family gemma --release v0.1.0`; native safetensors inventory missing | all v0.1.0 runtime |
...
benchmark_status: not-measured
release_ready: false
next_required_rows: V010.SOURCE.5
```
...
Active Next is now the native safetensors inventory lane:
...
report-only source provenance fields row. Qwen/Metal and Gemma still lack
native safetensors inventory, source manifest, native inventory, model-class
facts, tensor map, artifact, runtime path, generation path, eval path,
...
`V010.SOURCE.2`, `V010.SOURCE.3`, `V010.SOURCE.4`, and
...
Reason:
  ...
  V010.SOURCE.3, V010.SOURCE.4, and OWI.TARGETS.GEMMA.0 remain complete
  report-only rows; native safetensors inventory is now the next target/source
  blocker.
  `V010.SOURCE.5` is the next source-lane row after concrete Qwen and Gemma
