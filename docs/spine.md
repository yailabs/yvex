...
Main v0.1.0 blocker:
  source provenance fields remain missing
```
...
| --- | --- | --- | --- | --- | --- |
| official source tensors | upstream source authority | report-only | partial | target records, source manifests, source artifact class fields, and source shard count/footprint reports | provenance fields |
...
| TRACK.SOURCE | Source intake | official sources, manifests, native inventories | partial | source manifest/native inventory, Qwen/Gemma source pressure reports, Qwen/Gemma source target profiles, source artifact class fields, and source shard count/footprint reports | source provenance fields | active |
...
Main blockers:
  ...
  source provenance detail, source manifest, native inventory, model class, tensor
  map, artifact, Metal backend, residency, graph/runtime, eval, and benchmark
...
Note:
  ...
  shard count/footprint fields; source provenance fields remain next.
...
| --- | --- | --- | --- | --- | --- |
| 1 | TRACK.TARGET | GATE.SCOPE | complete / source-target-profiled | `yvex model-target decision --release v0.1.0`; `yvex model-target candidate --release v0.1.0`; `yvex model-target dense-candidate --release v0.1.0`; `yvex model-target qwen-metal --release v0.1.0`; `yvex model-target inspect qwen-metal-portability`; `yvex model-target inspect gemma-dense-portability`; `yvex source-manifest report --family qwen --release v0.1.0`; `yvex source-manifest report --family gemma --release v0.1.0`; source provenance fields missing | all v0.1.0 runtime |
...
benchmark_status: not-measured
release_ready: false
next_required_rows: V010.SOURCE.4
```
...
Active Next is now the source provenance lane:
...
report-only source shard count/footprint row. Qwen/Metal and Gemma still lack
source provenance detail, source manifest, native inventory, model-class facts,
tensor map, artifact, runtime path, generation path, eval path, benchmark path,
...
`V010.SOURCE.2`, `V010.SOURCE.3`, and `OWI.TARGETS.GEMMA.0` completed, Active
Next advances to:
...
Reason:
  ...
  V010.SOURCE.3, and OWI.TARGETS.GEMMA.0 remain complete report-only rows;
  source provenance fields are now the next target/source blocker.
  `V010.SOURCE.4` is the next source-lane row after concrete Qwen and Gemma
  source shard count/footprint reports.
```
