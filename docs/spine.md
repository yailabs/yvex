...
Current Qwen generation:
  unsupported

Current Gemma generation:
  unsupported
...
v0.1.0 supported generation families:
  DeepSeek
  Qwen
  Gemma

v0.1.0 artifact priority:
  produce generation-capable quantized YVEX artifacts for every supported family
  before claiming runtime generation.

Current generation state:
  unsupported for DeepSeek, Qwen, and Gemma.

Primary v0.1.0 supported families:
  DeepSeek
  Qwen
  Gemma

Secondary/post-v0.1.0 pressure lanes:
  GLM huge source/storage
  Qwen/Metal portability
  ROCm/Strix Halo portability
  speculative acceleration
  production serving hardening

Main v0.1.0 blocker:
  Multi-family generation-capable artifact chain is incomplete. DeepSeek, Qwen,
  and Gemma still require per-role dtype/qtype support, qtype compute/refusal
  matrix, calibration/imatrix decisions where required, quantized YVEX-produced
  artifact emission, artifact identity, materialization/residency, runtime
  descriptor readiness, graph/runtime generation path, eval, and benchmark.
```
...
| Qwen generation | unsupported |
| Gemma generation | unsupported |
...
| tensor mapping | map source/artifact tensor names to YVEX roles | report-only gate | partial | tensor-map, family-runtime reports, Qwen header-only tensor naming map, dense header-only tensor naming map, header-only output-head tensor mapping, sidecar tokenizer metadata mapping, missing-role blocker reporting, `yvex model-target tensor-map TARGET --gate v0.1.0`, and `yvex model-target quant-policy TARGET` | full runtime tensor role maps for every supported v0.1.0 family: DeepSeek, Qwen, Gemma |
| artifact production | produce controlled or selected GGUF | selected-slice | partial | controlled/selected GGUF emission | generation-capable quantized YVEX-produced artifacts for DeepSeek, Qwen, and Gemma |
| artifact identity/integrity | digest, range, shape/dtype, corruption refusal | implemented | yes | integrity reports and tests | identity and integrity gates for every supported-family quantized artifact |
| model-family mapping | family adapter facts and blockers | report-only | partial | `yvex fullmodel family-runtime` | multi-family runtime route: DeepSeek/MoE, Qwen, Gemma/dense |
| model class | dense/MoE/source-only/selected-slice routing | report-only | partial | fullmodel descriptor reports, `yvex moe report`, and Qwen/Gemma model-class profiles | supported-family runtime class acceptance for DeepSeek, Qwen, and Gemma |
| tensor collections | embedding/norm/attention/MLP/MoE/output/tokenizer roles | report-only | partial | fullmodel report, descriptor, `yvex tensor-collection report`, and Qwen/Gemma header-only tensor collection inventories | complete runtime tensor coverage for every supported family |
| storage/residency | placement, cache, staging, storage-stream planning | report-only | partial | materialization plans | materialization and residency plans for every supported-family artifact |
...
| real transformer graph | execute real model block/layers | planned | no | none; selected-slice and primitive CUDA evidence only | graph route over supported-family artifact tensors |
...
| real prefill | build runtime state from real transformer path | unsupported | no | none; CUDA primitives do not skip graph/runtime rows | real prefill over supported-family runtime artifacts |
| KV cache | K/V runtime state | diagnostic/report-only | partial | `yvex kv report`, diagnostic KV | real KV writes/reads over supported-family runtime artifacts |
| decode | advance runtime one token | diagnostic-runtime | partial | `yvex decode` | real decode over supported-family runtime artifacts |
| logits | output token scores | diagnostic-runtime | partial | `yvex logits` | real output-head logits for supported families |
| sampling | choose next token | diagnostic-runtime | partial | `yvex sample` | real vocabulary sampling for supported families |
| tokenizer/stop | tokenizer metadata, detokenization, EOS/stop policy | planned | partial | tokenizer diagnostics and sidecar metadata mapping | tokenizer-backed stop behavior |
| generation loop | repeated decode/logits/sample/append | diagnostic-runtime | partial | `yvex generate` | runtime generation over DeepSeek, Qwen, and Gemma |
| CLI generation | operator command over generation path | diagnostic-runtime | partial | `yvex generate` help and runbook | CLI generation over DeepSeek, Qwen, and Gemma |
| serving | daemon/provider generation surface | planned | no | `yvexd` status shell | runtime-backed provider generation |
| evaluation | correctness/capability evidence | planned | no | none | v0.1.0 eval smoke/regression over every supported family generation path |
| benchmark/profile | measured evidence | planned | no | none; no CUDA throughput claim | benchmark transcript over every supported family generation path |
...
| Model class | Runtime path | Current state | v0.1.0 role |
| --- | --- | --- | --- |
| DeepSeek / MoE | embedding -> norm -> attention -> router -> expert selection -> dispatch -> accumulation -> output head -> logits -> sampling -> generation | report-only / runtime planned | required supported family |
| Qwen | embedding -> norm -> attention -> MLP or family-specific block -> output head -> logits -> sampling -> generation | report-only / runtime planned | required supported family |
| Gemma / dense | embedding -> norm -> attention -> dense MLP -> output head -> logits -> sampling -> generation | report-only / runtime planned | required supported family |
| GLM source-only | source/intake/storage/model-class only | source/storage pressure lane | not a v0.1.0 supported generation family unless promoted |
| selected-runtime-slice | parser/materialization/partial graph/diagnostic runtime only | DeepSeek selected artifacts | bootstrap proof only; cannot close v0.1.0 |
| Metal/Qwen | backend portability lane | report-only pressure lane | post-v0.1.0 backend portability; Qwen generation may close v0.1.0 on CPU/CUDA or another implemented backend without Metal |
...
| Qwen tokenizer metadata mapping | report-only | yes | `yvex model-target tokenizer-map qwen3-8b --audit`; downloaded Qwen targets such as `qwen3-6-35b-a3b` write target-specific tokenizer-map sidecars | not tokenization, not detokenization, not chat-template runtime, not EOS/stop behavior, not logits, not artifact emission, not runtime descriptor construction, no Metal support, no generation, eval, benchmark, throughput, or release readiness |
| Qwen missing-role blocker report | report-only | yes | `yvex model-target missing-roles qwen3-8b --audit` and dynamic Qwen source targets | not tensor payload loading, not artifact contract completion, not runtime descriptor construction, not graph consumption, no Metal support, no logits computation, no tokenizer runtime, no generation, eval, benchmark, throughput, or release readiness |
| Qwen tensor mapping gate | report-only | yes | `yvex model-target tensor-map qwen3-8b --gate v0.1.0 --audit` | not tensor payload loading, not artifact emission, not artifact contract completion, not per-role qtype support completion, not runtime descriptor construction, not graph consumption, no Metal support, no logits computation, no tokenizer runtime, no generation, eval, benchmark, throughput, or release readiness |
| Qwen qtype policy report | report-only | yes | `yvex model-target quant-policy qwen3-8b --audit` | not tensor payload loading, not quantization, not GGUF emission, not per-role qtype support completion, not compute/refusal matrix completion, not calibration/imatrix completion, not artifact identity, not runtime descriptor construction, not graph consumption, no Metal support, no generation, eval, benchmark, throughput, or release readiness |
...
| Gemma tokenizer metadata mapping | report-only | yes | `yvex model-target tokenizer-map gemma-4-12b-it --audit`; downloaded Gemma targets such as `gemma-4-31b-it` write target-specific tokenizer-map sidecars | not tokenization, not detokenization, not chat-template runtime, not EOS/stop behavior, not logits, not artifact emission, not runtime descriptor construction, no Gemma/CUDA runtime, no generation, eval, benchmark, throughput, or release readiness |
| Gemma missing-role blocker report | report-only | yes | `yvex model-target missing-roles gemma-4-12b-it --audit` and dynamic Gemma source targets | not tensor payload loading, not artifact contract completion, not runtime descriptor construction, not graph consumption, no Gemma/CUDA runtime, no logits computation, no tokenizer runtime, no generation, eval, benchmark, throughput, or release readiness |
| Gemma tensor mapping gate | report-only | yes | `yvex model-target tensor-map gemma-4-12b-it --gate v0.1.0 --audit` | not tensor payload loading, not artifact emission, not artifact contract completion, not per-role qtype support completion, not runtime descriptor construction, not graph consumption, no Gemma/CUDA runtime, no logits computation, no tokenizer runtime, no generation, eval, benchmark, throughput, or release readiness |
| Gemma qtype policy report | report-only | yes | `yvex model-target quant-policy gemma-4-12b-it --audit` | not tensor payload loading, not quantization, not GGUF emission, not per-role qtype support completion, not compute/refusal matrix completion, not calibration/imatrix completion, not artifact identity, not runtime descriptor construction, not graph consumption, no Gemma/CUDA runtime, no generation, eval, benchmark, throughput, or release readiness |
...
| Dynamic Qwen/Gemma source role coverage | report-only | yes | dynamic Qwen/Gemma tensor-map, output-head, missing-role, and prepare dry-run tests map language-model embedding/norm/attention/MLP, Qwen linear-attention/MoE/shared-expert roles, and Gemma tied-head candidates from header/config evidence | not tensor payload loading, tokenizer runtime, qtype support, GGUF emission, materialization, graph execution, runtime readiness, generation, eval, benchmark, throughput, or release readiness |
...
candidate facts, Qwen/Metal pressure target facts, and Qwen source pressure
...
Qwen source pressure report
Qwen/Gemma tokenizer metadata map
```
...
| supported-generation-family set | DeepSeek, Qwen, Gemma | blocked until all supported families have generation-capable quantized YVEX artifacts and runtime generation proof | no current full-runtime/generation claim |

GLM remains source/storage pressure until a future row promotes it into the
supported-generation-family set.
...
Current evidence:
  ...
  `yvex model-target inspect qwen3-8b`, and
...
Current evidence:
  ...
  `yvex model-target tensor-map qwen3-8b --gate v0.1.0 --audit`, and
  `yvex model-target tensor-map gemma-4-12b-it --gate v0.1.0 --audit`; source reports expose
...
Note:
  ...
  `OWI.TARGETS.GEMMA.0` is the Gemma source-target profile row. `V010.SOURCE.1`
  completed multi-family source family/profile fields; `V010.SOURCE.2`
...
Family-specific map row policy:

```text
V010.MAP.5:
  canonical Qwen tensor naming map row.

V010.MAP.1:
  canonical dense tensor naming map row. Gemma may close the first dense-family
  mapping path here unless a future spine row explicitly creates a dedicated
  Gemma map row.
```

Do not use unregistered family-specific tensor-map aliases as implementation
row IDs unless a future spine row explicitly creates those aliases.

Tensor mapping begins only after tensor collection inventory. It turns native
...
Family-specific collection rows:

```text
TENSOR.COLLECTION.QWEN.0:
  maps to V010.TENSOR.1, V010.TENSOR.5-8, V010.TENSOR.11-12,
  V010.TENSOR.18-19, and V010.TENSOR.21-23 as a Qwen
  header-only tensor collection inventory.

TENSOR.COLLECTION.GEMMA.0:
  maps to V010.TENSOR.1, V010.TENSOR.5-8, V010.TENSOR.11-12,
  V010.TENSOR.18-19, and V010.TENSOR.21-23 as a Gemma
  header-only tensor collection inventory.
```

These rows group header-derived source tensor names into structural collection
candidates. They do not map canonical YVEX runtime roles.
...
`V010.CLI.18` is complete as the diagnostic demotion pass: report-heavy model,
...
Current target assignments:
  ...
  Qwen small/medium official source tensors: planned source-model-candidate
...
OPERATOR.PATHS.0 command surface:
  ...
  yvex paths resolve --family gemma --kind gguf
  yvex paths resolve --family gemma --kind reports
  yvex paths resolve --family gemma --kind reference
  yvex paths resolve --family gemma --kind registry
...
| --- | --- | --- | --- | --- | --- |
| 1 | TRACK.TARGET | GATE.SCOPE | complete / source-target-profiled | `yvex model-target decision --release v0.1.0`; `yvex model-target candidate --release v0.1.0`; `yvex model-target dense-candidate --release v0.1.0`; `yvex model-target qwen-metal --release v0.1.0`; `yvex model-target inspect qwen3-8b`; `yvex model-target inspect gemma-4-12b-it`; `yvex source-manifest report --family qwen --release v0.1.0`; `yvex source-manifest report --family gemma --release v0.1.0`; header-only source tensor metadata inventory | all v0.1.0 runtime |
...
`V010.CLI.18` demotes diagnostic-heavy report output from broader normal CLI
surfaces. Models inspect/verify/check, integrity report, fullmodel reports,
attention/context/KV/MoE reports, and MoE tensor-collection reports now default
to compact normal output where touched; `--audit` and `--output audit` preserve
the detailed diagnostic evidence and unsupported runtime/generation/benchmark
boundaries required for row promotion. This is an operator-output improvement
...
target_family: qwen
target_class: source-model-candidate
...
pressure lane, Gemma is command-visible as a source target profile, and tiny
fixtures remain fixture-only.
...
`yvex source-manifest report --family qwen --release v0.1.0` reports Qwen
source pressure, source path/config/tokenizer/safetensors/manifest/native
inventory status, blockers, and next rows without download, artifact emission,
materialization, tensor payload loading, graph/runtime execution, Qwen
...
Qwen source target profile is command-visible as `qwen3-8b`.
...
Qwen and Gemma source family/profile fields are now visible in source pressure
reports and model-target inspect/path surfaces.

Completed target/source lane row:

```text
OWI.TARGETS.GEMMA.0 - Gemma source target profile
```

Gemma source target profile is command-visible as `gemma-4-12b-it`.
`model-target list`, `model-target inspect`, path reports, and Gemma source
pressure reports expose family, target class, source path convention, future
artifact class, pressure purpose, unsupported runtime/generation boundaries,
and source-manifest Gemma reports without download, artifact emission,
materialization, graph/runtime execution, Gemma generation, evaluation,
benchmark, throughput, or release-ready claim.
...
`V010.SOURCE.6` adds a source tensor metadata inventory to Qwen and Gemma
source pressure reports. It derives tensor names, shard file placement, dtype,
rank, shape, element counts, declared byte spans, largest tensor metadata,
dtype/rank distributions, and lexical name-pattern summaries from safetensors
headers only. It does not read tensor payload bytes, hash files, map tensors to
runtime roles, infer Qwen/Gemma model classes, emit artifacts, materialize
...
small catalog or direct `--repo` request to a provider CLI download, records
...
Completed source/account lane row:

```text
V010.SOURCE.7B / ACCOUNTS.PROVIDER.0 - local provider account lane
```

`V010.SOURCE.7B / ACCOUNTS.PROVIDER.0` adds `yvex accounts` as the local
provider-account boundary for Hugging Face and GitHub. It discovers provider
CLIs, reports status, invokes provider-owned login flows, writes only a
non-secret `accounts.local.json` observation cache, and lets `models download`
preflight provider auth with `--auth auto|required|never` before source
acquisition. It does not store raw tokens, implement OAuth, install provider
tools, use MCP/YAI account state, verify source identity, hash payloads, create
...
blocked. `models prepare TARGET --dry-run` now keeps normal output compact for
blocked downloaded source targets, while `--audit` preserves expected artifact
path, artifact plan/emission/identity status, blocker count, and top blocker.
This is artifact discovery and preflight UX only: no source tensor payload
loading, no payload hashing, no GGUF emission, no quantization, no
materialization, no graph execution, no runtime readiness, no generation, no
eval, no benchmark, no throughput, and no release-ready claim.
...
`MODEL.TARGET.IDENTITY.0` replaces backend- or pressure-specific source target
IDs with backend-neutral model/source target IDs. `qwen3-8b` and
`gemma-4-12b-it` are now the command-visible source targets; backend selection
and backend pressure are reported as separate fields. Old target IDs are
...
`MODEL.CLASS.GEMMA.0` adds `yvex model-target class-profile
gemma-4-12b-it` as a header-metadata-only source profile over existing
Gemma source evidence. It exposes family, target, model-class status,
dense runtime-shape candidate, lexical tensor pattern counters, evidence
basis, backend pressure as a separate deferred field, and explicit
no-role-map/no-runtime/no-generation/no-benchmark boundaries without tensor
payload loading, tensor role mapping, model execution, artifact emission,
generation, eval, throughput, or release-ready claim.

Completed tensor-collection row:

```text
TENSOR.COLLECTION.QWEN.0 - Qwen tensor collection inventory
```

`TENSOR.COLLECTION.QWEN.0` adds `yvex model-target tensor-collection
qwen3-8b` as a header-only tensor collection inventory over Qwen source
metadata. It groups lexical embedding, attention Q/K/V/O, MLP gate/up/down,
normalization, output-head, MoE, tokenizer-sidecar, and KV-runtime-state
candidate facts from safetensors headers only. It does not load tensor payloads,
map runtime roles, build runtime descriptors, feed graph consumers, emit
artifacts, materialize tensors, execute runtime paths, support Metal, generate,
evaluate, benchmark, claim throughput, or mark a release ready.

Completed tensor-collection row:

```text
TENSOR.COLLECTION.GEMMA.0 - Gemma tensor collection inventory
```

`TENSOR.COLLECTION.GEMMA.0` adds `yvex model-target tensor-collection
gemma-4-12b-it` as a header-only tensor collection inventory over Gemma source
metadata. It groups lexical embedding, attention Q/K/V/O, MLP gate/up/down,
normalization, output-head, MoE, tokenizer-sidecar, and KV-runtime-state
candidate facts from safetensors headers only. It does not load tensor payloads,
map runtime roles, build runtime descriptors, feed graph consumers, emit
artifacts, materialize tensors, execute runtime paths, run CUDA/Metal model
runtime, generate, evaluate, benchmark, claim throughput, or mark a release
ready.

Completed tensor-map row:
...
`V010.MAP.5` adds `yvex model-target tensor-map qwen3-8b` as a
header-derived Qwen tensor naming map. It assigns native source tensor names to
canonical YVEX role labels for embedding, attention Q/K/V/O, MLP gate/up/down,
normalization, output head, and MoE-observed candidates while preserving
lexical-and-header-only validation, incomplete runtime role coverage, and
explicit no-artifact/no-runtime-descriptor/no-graph/no-generation/no-benchmark
boundaries. It does not load tensor payloads, emit artifacts, materialize
tensors, attach backend residency, feed graph consumers, run Qwen or Metal
runtime paths, evaluate, benchmark, claim throughput, or mark a release ready.

`V010.MAP.1` adds a dense tensor naming map using `gemma-4-12b-it`
as the first concrete dense target, assigning header-derived native source
tensor names to canonical YVEX dense role labels for embedding, attention
Q/K/V/O, MLP gate/up/down, normalization, output head, and sidecar candidates
while preserving lexical-and-header-only validation, incomplete runtime role
coverage, and explicit no-artifact/no-runtime-descriptor/no-graph/no-generation
/no-benchmark boundaries without tensor payload loading, artifact emission,
...
`V010.MAP.6` adds focused output-head mapping for Qwen and Gemma source
targets. It identifies output-head, final-norm, and embedding candidates from
source metadata, reports missing/ambiguous/profiled output-head state, and
checks the vocab/output-head shape relation where available. It does not load
payloads, compute logits, produce final hidden state, emit artifacts, build
runtime descriptors, feed graph consumers, generate, evaluate, benchmark,
claim throughput, or mark a release ready.
...
`V010.MAP.7` adds `yvex model-target tokenizer-map TARGET` as the primary
sidecar tokenizer metadata map for Qwen and Gemma source targets, including
downloaded dynamic targets. It reports tokenizer/config/special-token/generation
sidecar presence, bounded metadata parse status, tokenizer backend, vocab and
merges evidence, special token IDs/text, additional special-token count,
chat-template presence, and vocab/output-head relation, and writes
`reports/<family>/<target>.tokenizer-map.json` for dynamic targets. It does not
tokenize, detokenize, execute chat templates, implement EOS/stop behavior,
compute logits, emit artifacts, build runtime descriptors, feed graph consumers,
generate, evaluate, benchmark, claim throughput, or mark a release ready.
...
`V010.MAP.8` adds `yvex model-target missing-roles TARGET` as the compact
missing-role blocker report for Qwen and Gemma source targets, including
downloaded dynamic targets. It aggregates header-derived tensor naming,
output-head mapping, tokenizer metadata, planned artifact, and sidecar evidence
into normal/table/audit/json output with the current top blocker. It preserves
header-and-sidecar-metadata-only and report-only boundaries without tensor
payload loading, artifact contract completion, runtime descriptor construction,
graph consumption, backend residency, runtime execution, logits computation,
tokenizer runtime, generation, eval, benchmark, throughput, or release-ready
claim.
...
`V010.MAP.9` adds `yvex model-target tensor-map TARGET --gate v0.1.0` for
Qwen and Gemma source targets. It aggregates model-class, tensor collection,
tensor naming, output-head, tokenizer metadata, and missing-role evidence into
a compact mapping-stage gate with normal, table, and audit output. A passing
gate moves only to artifact/quantization planning and keeps qtype policy,
artifact emission, runtime descriptors, backend residency, graph consumers,
prefill/decode/logits/tokenizer/sampling/generation, eval, benchmark,
throughput, and release readiness unsupported.

The tensor mapping gate handed off to the qtype policy report:

```text
V010.QUANT.0 - qtype policy report
```

Completed qtype policy row:

```text
V010.QUANT.0 - qtype policy report
```

`V010.QUANT.0` adds `yvex model-target quant-policy TARGET` for Qwen and
Gemma source targets. It consumes source header evidence and the report-only
tensor mapping gate, reports source dtype profile, existing YVEX qtype policy
basis, preferred and candidate artifact-planning qtypes, refused qtypes,
calibration/imatrix deferrals, compute-support deferral, artifact-emission
blockers, and downstream runtime blockers. It does not load tensor payloads,
quantize tensors, emit GGUF, complete per-role qtype support, complete
compute/refusal matrix, complete calibration/imatrix policy, create artifact
identity, materialize tensors, construct runtime descriptors, attach backend
residency, feed graph consumers, execute prefill/decode/logits/tokenizer/
...
report-only source artifact class row, `V010.SOURCE.3` is complete as a
...
and Gemma now have report-only model-class, collection, naming-map,
output-head-map, tokenizer-metadata-map, missing-role blocker, tensor mapping
gate, and qtype policy facts, but still lack per-role dtype/qtype support,
artifacts, runtime path, generation path, eval path, benchmark path, throughput
...
| MODELS.SOURCE.ROLEMAP.COVERAGE.0 | complete | source | Dynamic Qwen/Gemma source tensor role coverage | dynamic downloaded Qwen/Gemma source targets map language-model embedding, norm, attention, MLP, Qwen linear-attention, Qwen MoE router/expert/shared-expert, Gemma layer-scalar, separate output-head, and config-proven tied output-head candidates from safetensors headers and bounded config evidence; missing-role normal/table/audit/json reports agree on role groups and prepare dry-runs move past tensor-map blockers when required groups are present, without tensor payload loading, tokenizer runtime, qtype support, GGUF emission, materialization, graph execution, runtime readiness, generation, eval, benchmark, throughput, or release-ready claim |
...
| MODEL.CLASS.GEMMA.0 | complete | model | Gemma model-class profile | Gemma model-class profile is command-visible as a header-metadata-only source profile for `gemma-4-12b-it`, exposing family, target, model-class status, dense runtime-shape candidate, lexical tensor pattern counters, evidence basis, backend pressure as a separate deferred field, and explicit no-role-map/no-runtime/no-generation/no-benchmark boundaries without tensor payload loading, tensor role mapping, model execution, artifact emission, generation, eval, throughput, or release-ready claim |
| TENSOR.COLLECTION.QWEN.0 | complete | tensor-collection | Qwen tensor collection inventory | `yvex model-target tensor-collection qwen3-8b` profiles Qwen source tensor collection candidates from safetensors headers only, grouping embedding, attention Q/K/V/O, MLP gate/up/down, normalization, output-head, MoE, tokenizer-sidecar, and KV runtime-state boundaries without tensor payload loading, tensor role mapping, artifact emission, runtime descriptors, graph consumption, Metal support, generation, eval, benchmark, throughput, or release-ready claim |
...
| V010.MAP.7 | complete | tensor-map | Tokenizer metadata mapping | `yvex model-target tokenizer-map TARGET` reports sidecar tokenizer/config/special-token/generation metadata presence, bounded parse status, tokenizer backend/class/model type, vocab and merges evidence, special token IDs/text, additional special-token count, chat-template presence, and vocab/output-head relation for static and downloaded Qwen/Gemma source targets, writing target-specific tokenizer-map sidecars for dynamic targets without tokenization, detokenization, chat-template runtime, EOS/stop behavior, logits, artifact emission, runtime descriptors, graph consumption, generation, eval, benchmark, throughput, or release-ready claim |
| V010.MAP.8 | complete | tensor-map | Missing-role blocker report | `yvex model-target missing-roles TARGET` reports compact normal/table/json and audit missing-role blockers for static and downloaded Qwen/Gemma source targets, combining header-derived tensor naming, output-head mapping, tokenizer metadata, planned artifact, and sidecar evidence into current top blockers while preserving header-and-sidecar-metadata-only/report-only boundaries without tensor payload loading, artifact contract completion, runtime descriptor construction, graph consumption, backend residency, runtime execution, logits computation, tokenizer runtime, generation, eval, benchmark, throughput, or release-ready claim |
| V010.MAP.9 | complete | tensor-map | v0.1.0 tensor mapping gate | `yvex model-target tensor-map qwen3-8b --gate v0.1.0` and `yvex model-target tensor-map gemma-4-12b-it --gate v0.1.0` aggregate model-class, tensor collection, tensor naming, output-head, tokenizer metadata, and missing-role evidence into a report-only mapping gate with normal/table/audit output, deterministic blocker statuses, and `next: V010.QUANT.0` only when mapping evidence is coherent enough for artifact/quant planning, without tensor payload loading, artifact emission, per-role qtype support completion, runtime descriptors, backend residency, graph consumption, prefill/decode/logits/tokenizer/sampling/generation, eval, benchmark, throughput, or release-ready claim |
| V010.QUANT.0 | complete | quant | qtype policy report | `yvex model-target quant-policy qwen3-8b` and `yvex model-target quant-policy gemma-4-12b-it` consume source/header/mapping evidence and existing YVEX qtype support rows to report source dtype profile, policy basis, preferred/candidate/refused qtypes, calibration/imatrix deferrals, compute-support deferral, artifact-emission blockers, and downstream runtime blockers with normal/table/audit output and `next: V010.QUANT.1`, without tensor payload loading, quantization, GGUF emission, per-role qtype completion, compute/refusal matrix completion, calibration/imatrix completion, artifact identity, materialization, runtime descriptors, backend residency, graph consumption, generation, eval, benchmark, throughput, or release-ready claim |
...
| `yvex model-target tensor-map` / `tokenizer-map` / `missing-roles` | report-only | header-derived Qwen/Gemma tensor naming maps, focused output-head/final-norm/embedding candidate mapping, target-specific sidecar tokenizer metadata mapping, missing-role blocker reporting, and the v0.1.0 mapping gate | tensor payload loading, per-role qtype support completion, tokenization, detokenization, chat-template runtime, EOS/stop behavior, artifact emission, runtime descriptor construction, graph consumption, logits computation, generation, release readiness | TRACK.TENSOR |
| `yvex model-target quant-policy` | report-only | Qwen/Gemma source dtype profile and artifact-planning qtype policy over existing YVEX qtype support rows | tensor payload loading, quantization, GGUF emission, per-role qtype completion, compute/refusal matrix completion, calibration/imatrix policy, artifact identity, runtime descriptors, graph consumption, generation, release readiness | TRACK.TENSOR / TRACK.ARTIFACT |
...
Target/source planning now advances to `V010.SOURCE.2` after `V010.SOURCE.1`
and `OWI.TARGETS.GEMMA.0` because Qwen and Gemma source family/profile fields
and concrete source target profiles are command-visible, but source artifact
class detail, manifest, native inventory, model-class profile, and tensor map
remain missing.
...
| `yvex_accounts.c` | source | BLOCK 1/8 | local provider account boundary | provider CLI discovery, login/status/ensure, non-secret account observation cache, source download preflight | `tests/cli/accounts.sh`, `tests/cli/models.sh` | low | keep |
...
| `yvex_accounts.c` | provider account boundary | accounts command | provider CLI discovery/status/login/ensure, non-secret local account observations | account observation structs | provider process cleanup | ACCOUNTS.PROVIDER.0 | no hosted auth, no token storage, no MCP/YAI |
...
`MODELS.SOURCE.ROLEMAP.COVERAGE.0`, `TENSOR.COLLECTION.QWEN.0`,
`TENSOR.COLLECTION.GEMMA.0`, `V010.MAP.5`,
...
| Output-head tensor mapping command-visible while tokenizer metadata mapping is missing | V010.MAP.7 |
| Dense tensor naming map command-visible while output-head tensor mapping is missing | V010.MAP.6 |
| Qwen tensor naming map command-visible while dense tensor naming map is missing | V010.MAP.1 |
| Qwen and Gemma tensor collection inventories complete while Qwen tensor naming map is missing | V010.MAP.5 |
| Qwen tensor collection inventory complete while Gemma tensor collection inventory is missing | TENSOR.COLLECTION.GEMMA.0 |
| Backend-neutral source target identity plus Qwen/Gemma model-class profiles complete while Gemma tensor collection is missing | TENSOR.COLLECTION.GEMMA.0 |
| Source download/account lanes complete and Qwen tensor collection is complete while Gemma tensor collection is missing | TENSOR.COLLECTION.GEMMA.0 |
...
| Qwen/Gemma source shard count/footprint fields complete and source provenance fields are missing | V010.SOURCE.4 |
| Qwen/Gemma source artifact class fields complete and source shard count/footprint fields are missing | V010.SOURCE.3 |
| Qwen/Gemma source family/profile fields complete and source artifact class fields are missing | V010.SOURCE.2 |
...
| Qwen/Metal pressure report complete and Qwen source pressure facts are missing | V010.SOURCE.9 |
...
Reason:
  ...
  header-only output-head tensor mapping and sidecar tokenizer metadata maps.
  Missing-role blocker reporting, the v0.1.0 tensor mapping gate, and
  report-only qtype policy are now command-visible, so artifact planning can
  move only to per-role dtype/qtype support while artifact emission, runtime
  descriptor, graph, logits, tokenizer runtime, generation, eval, and benchmark
  work remain blocked downstream. The first normal operator
  ...
  map handoff over existing Qwen/dense/output-head map surfaces.
  MODELS.SOURCE.ROLEMAP.COVERAGE.0 is complete as the dynamic Qwen/Gemma
  header-only role coverage repair that maps Qwen linear-attention, MoE, and
  shared-expert names plus Gemma language-model and tied-head candidates while
  leaving tokenizer runtime, qtype support, artifact emission, and runtime
  execution blocked. MODEL.CLASS.QWEN.0 and
  ...
  output-head tensor mapping. V010.MAP.7 is complete as sidecar tokenizer
  metadata mapping. V010.MAP.8 is complete as missing-role blocker reporting.
  V010.MAP.9 is complete as the report-only tensor mapping gate. V010.QUANT.0
  is complete as the report-only qtype policy report. V010.QUANT.1 is the next
  blocker before artifact, logits, and runtime-path work.
```
...
| tensor collection inventory | header-derived structural grouping of candidate tensor collections; not canonical tensor role mapping, runtime descriptor completion, graph consumption, backend residency, model execution, generation, eval, or benchmark |
...
Qwen-Metal planned target facts:
  ...
  pressure class: planned source-model-candidate
