...
| Field | Current value |
| --- | --- |
| Completed rows | `V010.DOCS.REFOUNDATION.0`, `V010.PROJECT.RECOVERY.1` |
| Partial rows | `V010.PROJECT.RECOVERY.0` |
| Completed proof stage | documentation/project-control reconstruction only |
...
| `V010.PROJECT.RECOVERY.0` / partial | `PROJECT.md`, this file, direct project-control consumers, structural docs guards | The first recovery restored doctrine and a short milestone map but still omitted the complete track/wave ledger and demoted Qwen/Gemma/common engineering scope. | Retain the useful single-authority and claim-boundary work while recording that the operational project map remained incomplete. | The row remains traceable as partial and cannot be cited as complete project-ledger recovery. | Depends on docs refoundation; next `V010.PROJECT.RECOVERY.1`. |
| `V010.PROJECT.RECOVERY.1` / complete | `PROJECT.md`, `AGENTS.md`, `tests/test_project_ledger.sh`, direct project-control consumers | The current authority did not let maintainers or agents locate every historical wave, its truthful rank, family scope, state, or conclusive consumer. | Recover 631 historical IDs across the original 24 tracks, add the conclusive milestone system, restore Qwen/Gemma/common scope, and preserve DeepSeek as the sole v0.1 release gate. | The ledger has 665 unique canonical IDs, calculated state and per-track counts, one milestone Active Next, stable rank/state vocabulary, no shadow spine, and no runtime promotion. | Depends on `V010.PROJECT.RECOVERY.0`; next `V010.DOCS.ARCHITECTURE.0`. |
| `V010.DOCS.ARCHITECTURE.0` / active | Canonical documentation taxonomy and guards, with `PROJECT.md` retaining project state | Current canonical docs mix architecture, operator, API, evidence, and historical ownership; the verified vLLM, SGLang/DS4, ggml/llama.cpp, TensorRT-LLM/NVIDIA, and DeepSeek reference baseline is not mapped to YVEX owners. | Standardize document ownership, reference/paper taxonomy, architecture descriptions, current DS4 terminology, and migration rules without changing runtime capability. | One non-duplicated documentation system maps external reference concepts to YVEX owners, preserves claim truth, passes permanent guards, and leaves implementation gates unsupported. | Depends on complete ledger recovery; next `V010.REBASE.DEEPSEEK.0`. |
...
| `V010.MODEL.ARCH.IR.0` / blocked | `src/model/target/` architecture/profile owners and typed headers | No execution-complete typed DeepSeek-V4-Flash architecture specification exists. | Represent layers, attention and position rules, KV geometry, MoE routing/expert topology, norms, output head, tokenizer requirements, and validation blockers as typed facts. | Exact config evidence builds one deterministic typed specification; missing, inconsistent, unknown, and unsupported fields refuse precisely. | Depends on CUDA fail-closed truth; next `V010.TENSOR.COVERAGE.DEEPSEEK.0`. |
| `V010.TENSOR.COVERAGE.DEEPSEEK.0` / blocked | `src/model/target/` tensor collection and missing-role owners | Existing Qwen, Gemma, dense, MoE, and bounded DeepSeek inventories do not define the exact execution-required DeepSeek tensor set. | Derive every required global, layer, attention, position, MoE, norm, tokenizer, and output role from the architecture IR without discarding multi-family evidence. | Required-role construction is deterministic; missing, duplicate, ambiguous, unexpected-required, shape, and expert-index cases refuse precisely. | Depends on architecture IR; next `V010.MAP.GGUF.DEEPSEEK.0`. |
| `V010.MAP.GGUF.DEEPSEEK.0` / blocked | `src/model/target/` role-map owners plus `src/gguf/yvex_gguf_name_map.c` and `yvex_gguf_layout_map.c` | Source tensor names are not completely mapped to runtime roles and GGUF names/layout for the exact architecture. | Map every required global, per-layer, attention, position, MoE, expert, norm, tokenizer, and output role with shape and layout constraints. | The complete header inventory maps exactly once to required roles; missing, duplicate, ambiguous, extra-required, shape, and expert-index failures are tested. | Depends on complete tensor coverage; next `V010.SOURCE.PAYLOAD.STREAM.0`. |
...
The complete track/wave disposition now lives only in `PROJECT.md`. Its
permanent parser guard verifies all 24 original tracks, 631 recovered IDs,
34 explicit new IDs, state totals, stable rank vocabulary, one Active Next,
and the Qwen/Gemma/common family evidence anchors. This repair file does not
...
| Fixture logits and sampling | `src/generation/yvex_logits.c`, `yvex_sampling.c`, `yvex_sampling_report.c`, sample/logits CLI surfaces and tests | Retain numeric selection cases only as internal test fixtures; replace public behavior with final-norm/output-head vocabulary logits and sampling. | `V010.RUNTIME.DEEPSEEK.LOGITS.0` and `V010.RUNTIME.SAMPLING.0`; sampled tokens derive from full model vocabulary logits. |
