...
If the binaries are missing, run `make` or `make check`. `./yvex` and
`./yvexd` are generated build products, not tracked source files.
...
`./yvexd`. They are generated compiled products at the repository root.
