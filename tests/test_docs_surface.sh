...
grep -nF "Active Next: \`V010.QUANT.2 - qtype compute/refusal matrix\`" docs/spine.md >/dev/null || {
  echo "spine must set Active Next to the qtype compute/refusal matrix" >&2
  exit 1
}
...
grep -nF "Active Next: \`V010.QUANT.2 - qtype compute/refusal matrix\`" docs/spine.md >/dev/null || {
  echo "spine must keep the active implementation row on qtype compute/refusal matrix" >&2
  exit 1
}
...
grep -nF "| V010.GGUF.QTYPE.ABI.0 | complete/report-only + fixture-proof | GGUF qtype byte geometry and refusal ABI. |" docs/spine.md >/dev/null || {
  echo "spine must preserve V010.GGUF.QTYPE.ABI.0 as complete qtype ABI row" >&2
  exit 1
}

grep -nF "| V010.QUANT.2 | active | qtype compute/refusal matrix. |" docs/spine.md >/dev/null || {
  echo "spine must keep V010.QUANT.2 active after qtype ABI" >&2
  exit 1
}
