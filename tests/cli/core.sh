...
contains "$OUT_DIR/info.out" "status: selected tensor materialization, engine weight attachment, fixture graph execution, real selected graph segments, standalone RoPE, attention, and matmul ops, explicit token input boundary, prefill state foundation, minimal KV binding, and minimal KV ownership"
...
contains "$OUT_DIR/info.out" "graph: partial planning, deterministic fixture execution, selected embedding partial execution, selected embedding RMSNorm segment execution, standalone RoPE position op, standalone F32 attention primitive, and standalone F32 matmul/projection primitive implemented"
contains "$OUT_DIR/info.out" "planner: estimate-only implemented"
contains "$OUT_DIR/info.out" "backend: CPU reference implemented"
contains "$OUT_DIR/info.out" "backend_cuda: tensor movement plus F32/F16 embed, RMSNorm, RoPE position op, F32 attention primitive, and F32 matmul/projection primitive implemented when CUDA is available"
