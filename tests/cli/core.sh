...
contains "$OUT_DIR/info.out" "status: selected tensor materialization, engine weight attachment, fixture graph execution, real selected graph segments, standalone RoPE position op, explicit token input boundary, prefill state foundation, minimal KV binding, and minimal KV ownership"
...
contains "$OUT_DIR/info.out" "graph: partial planning, deterministic fixture execution, selected embedding partial execution, selected embedding RMSNorm segment execution, and standalone RoPE position op implemented"
contains "$OUT_DIR/info.out" "planner: estimate-only implemented"
contains "$OUT_DIR/info.out" "backend: CPU reference implemented"
contains "$OUT_DIR/info.out" "backend_cuda: tensor movement plus F32/F16 embed, RMSNorm, and RoPE position op implemented when CUDA is available"
