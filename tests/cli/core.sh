...
contains "$OUT_DIR/info.out" "status: selected tensor materialization, engine weight attachment, fixture graph execution, real selected graph segments, standalone RoPE and attention ops, explicit token input boundary, prefill state foundation, minimal KV binding, and minimal KV ownership"
...
contains "$OUT_DIR/info.out" "graph: partial planning, deterministic fixture execution, selected embedding partial execution, selected embedding RMSNorm segment execution, standalone RoPE position op, and standalone F32 attention primitive implemented"
contains "$OUT_DIR/info.out" "planner: estimate-only implemented"
contains "$OUT_DIR/info.out" "backend: CPU reference implemented"
contains "$OUT_DIR/info.out" "backend_cuda: tensor movement plus F32/F16 embed, RMSNorm, RoPE position op, and F32 attention primitive implemented when CUDA is available"
