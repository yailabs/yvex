...
grep 'next: V010.SOURCE.6' "$ROOT/model-target-decision-normal.out"
...
matches "$ROOT/model-target-decision-table.out" '^target-decision[[:space:]]{2,}blocked[[:space:]]{2,}none[[:space:]]{2,}0[[:space:]]{2,}V010\.SOURCE\.6$'
...
grep 'candidate.3.next: source tensor metadata inventory' "$ROOT/model-target-decision.out"
...
grep 'candidate.4.next: source tensor metadata inventory' "$ROOT/model-target-decision.out"
