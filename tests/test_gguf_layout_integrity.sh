...
grep -nF 'yvex_gguf_layout_validate(artifact, gguf' \
  src/artifact/yvex_artifact_integrity.c >/dev/null
grep -nF 'yvex_gguf_layout_validate(artifact, gguf' \
  src/gguf/yvex_gguf_report.c >/dev/null
grep -nF 'yvex_gguf_range_fact_from_layout' src/gguf/yvex_gguf_range_map.c >/dev/null
grep -nF 'yvex_artifact_integrity_validate' src/graph/yvex_graph_guard.c >/dev/null
grep -nF 'yvex_artifact_integrity_validate' src/runtime/yvex_runtime.c >/dev/null
grep -nF 'yvex_artifact_integrity_validate' \
  src/model/artifacts/yvex_model_artifact_gate.c >/dev/null
grep -nF 'yvex_gguf_layout_validate(artifact, gguf' \
  src/model/yvex_model.c >/dev/null
...
if grep -nF 'strstr' src/gguf/yvex_gguf_layout_integrity.c \
  src/artifact/yvex_artifact_integrity.c >/dev/null; then
  echo 'GGUF layout guard: semantic policy depends on error strings' >&2
  exit 1
fi
if grep -nE '\b(qsort|heapsort|mergesort)\s*\(' \
  src/gguf/yvex_gguf_layout_integrity.c >/dev/null; then
