...
| Active engineering boundary | `FINITE.DECISION.PRODUCER.0` — make the admitted finite-decision engine a consumer-safe, local, out-of-process typed producer without exposing Laya token/template/marker mechanics. |
...
| Next decision point | Qualify one exact bounded question/frontier-to-result contract through model-owned input construction and the existing resident-host transport. After this producer seam, reconsider Architecture Spectrum A02–A11 by falsification value, not numeric order; CUDA speed, broader checkpoint/head breadth, calibration, YAI authority, Program N, A11 execution and GB10 remain separately unselected. |
...
| 🟡 PARTIAL | Real foundation; genericity, breadth, portability, performance or evidence incomplete. | 44 |
...
| Native finite-decision model computation | 🟡 PARTIAL | One exact Laya typed-decisions checkpoint executes a bounded token-domain `choice` input through source-verified F16 Safetensors, compiler-owned bidirectional physical SSA, authenticated tensor binding, CPU engine generation and an independently resident persistent-host engine. The typed C producer returns exact candidate-row raw logits and an uncalibrated relative distribution without autoregressive generation; one upstream PyTorch comparison qualifies the declared numerical scope. This is distinct from token-likelihood Decision Readout. | First qualify a consumer-safe local producer: exact model-owned tokenizer/input policy must construct candidate markers inside YVEX, and a separate process must submit a bounded typed question/frontier and receive the identity-bound result through the resident host. Wider model/input/head classes, portability and useful low-latency residency remain separate. | R / C / P / O / S / Q | [Laya first scope](docs/model-families/laya.md); [finite-decision C contract](docs/contracts/c-api.md) |
...
| 22 | `PROJECT_CONTROL.POST.RUNTIME.EXECUTION.CONSOLIDATION.0` | COMPLETE | R / O / S / Q | Project-control selection only; no capability promotion | The common substrate and its remaining evidence limits were reconciled; the first non-autoregressive finite-decision model class is selected as a distinct execution pressure. Existing token-likelihood Decision Readout remains separate. | `DECISION.READOUT.QWEN.BREADTH.0`; `MAINTENANCE.RUNTIME.EXECUTION.CONSOLIDATION.0` |
...
| 25 | `FINITE.DECISION.PRODUCER.0` | ACTIVE | R / C / O / S / X / Q | Consumer-safe local finite-decision producer at one exact model/input scope if qualified; no automatic maturity promotion | One separate local process sends a bounded semantic-neutral typed question and finite candidate frontier to the persistent host and receives exact candidate IDs, raw scores, explicitly uncalibrated relative distribution, model/binding/tokenizer/engine generation and execution evidence. YVEX constructs and authenticates tokenization, input template, type and marker positions from admitted source/compiler policy; the client supplies none of those Laya-private facts. Reuse the resident engine/lifecycle and local transport rather than another daemon or generation wrapper. Independent upstream input-and-score controls, malformed/stale/refusal, cancellation/disconnect and host-only-decision-engine evidence close the scope. No CUDA speed, calibration, YAI ABI, semantic decision authority or general checkpoint breadth is claimed. | `PROJECT_CONTROL.POST.SYSTEM.MODEL.LAYA.RECONCILIATION.0` |
| 26 | `V010.RUNTIME.DEEPSEEK.GB10.OPTIMIZATION.0` | PARTIAL | Q / S / R | Warm execution performance | Measured bottleneck-driven improvement without numerical/lifecycle regression. | Explicit resumption and controlled workload |
| 27 | `V010.EVAL.DEEPSEEK.0` | BLOCKED | Q | Model behavior evidence | Repeatable quality, tokenizer, long-context and refusal evaluation. | `V010.RUNTIME.DEEPSEEK.GB10.OPTIMIZATION.0` |
| 28 | `V010.BENCH.DEEPSEEK.0` | NOT MEASURED | Q | Full-model performance | Identity-bound latency, throughput, memory and reliability evidence. | `V010.EVAL.DEEPSEEK.0` |
| 29 | `V010.RELEASE.0` | BLOCKED | Q | Release | All version-specific gates close together. | Benchmark and remaining release obligations |

Active Next: FINITE.DECISION.PRODUCER.0
...
Text-engine context is now selected at explicit load time rather than fixed by
the registry profile. Local protocol v22 carries an optional positive context
override to the same engine, common capacity and backend admission owners used
for the default; server options schema v5 binds the context-aware loader
callback. The host does not resize a loaded alias. A tiny-model control proves
per-generation selection and a separate admitted Mamba CPU engine opened at
32,768 tokens with an unchanged 4,096-token registry default; a request beyond
its compiled maximum refused without retiring the host. This is generic load
semantics, not DeepSeek 32k fit, long-prompt execution, a performance claim or
resolution of the open DeepSeek numerical gate. An already-running v21 host
needs a controlled restart before it can accept v22 clients; the active user
engine was not interrupted for this control.
...
`MAINTENANCE.RUNTIME.EXECUTION.CONSOLIDATION.0` and its project-control
reconciliation and `SYSTEM.MODEL.LAYA.0` are COMPLETE at their bounded scopes.
The latter adds one exact CPU/token-domain non-autoregressive finite-decision
model class with an independent upstream raw-logit control; it does not add
native text construction, general decision-model breadth, CUDA execution,
calibration or a remote/YAI producer. Its first CPU forward is roughly 37 s,
so continuously resident low-latency usefulness is not established.
`PROJECT_CONTROL.POST.SYSTEM.MODEL.LAYA.RECONCILIATION.0` is COMPLETE. The
remaining consumer-safety gap is one producer seam, not Laya execution math:
the public C request still requires caller-supplied token IDs/type and marker
positions, while its host scoring function requires an in-process server
pointer. The existing local protocol exposes finite-decision engine lifecycle
but not scoring. `FINITE.DECISION.PRODUCER.0` is the sole ACTIVE boundary and
must close native source-authored input construction and separate-process
local scoring together; a wire endpoint alone would export Laya-private input
mechanics, while an input builder alone would leave the resident host
unreachable. The ByteLevel BPE tokenizer also declares NFC normalization and
special-token post-processing, which require exact admission rather than an
assumed match to current decoder tokenizer paths. This is a computational
producer scope, not YAI integration or a public remote-management service.
The observed roughly 35–53 s CPU forwards characterize cost but do not block
computational correctness or establish useful System-1 latency; CUDA remains
optional future performance/portability pressure. After this seam is earned,
project control may return to Spectrum selection without automatically
requiring another Laya-specific wave.
...
| --- | --- | --- | --- |
| Now | Make the qualified finite-decision computation a local, consumer-safe typed producer | R / C / O / S / X / Q | One separate process submits a bounded question/frontier without Laya token/template/marker knowledge and receives an exact identity-bound computational result from the resident host; independent input/score and refusal evidence close the scope. |
...
| `FINITE.DECISION.PRODUCER.0`, ACTIVE | R / C / O / S / X / Q | Exact admitted input policy and separate-process local typed scoring on the resident host, without caller Laya mechanics | One computational producer at the selected scope; no Axx, YAI, calibration or useful-latency promotion by selection | No release gate change |
